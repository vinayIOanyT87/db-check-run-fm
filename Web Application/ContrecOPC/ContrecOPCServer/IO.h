...
public:
	...
	CWinThread*				m_pTimerThread;			// Scan Thread
	...
	BOOL						m_bMessageTimeout;
	BOOL						m_bMessageInProgress;
	int						m_iTimeRemaining;
	int						m_iResetTimeValue;
	BOOL						m_bResetTimeChanged;
	int						m_iArm1Density;
	int						m_iArm2Density;
	int						m_iArm3Density;
	int						m_iArm4Density;
	int						m_StoredTransactionNumber;
	int						m_StoredEntryNumber;
	CString					m_CSForceChange;
	CString					m_CSLastTouchKeyData;
	...
	unsigned char	CalculateLCRC(int,unsigned char *);
	BOOL				formatContrecPrompt(CStringA oString,char * MessageString,int *iReturnedLength);
	BOOL				StatusIsValid(char chTestByte3,char chTestByte4);
	...
	static UINT TimerThread(LPVOID lpDeviceManager);
	void CheckTimer();
	void Scan();
	...
	VARENUM ContrecDataType(short Code);
};
