	...
	public class TCPCommDTN : TCPCommBase
	{
		...
		protected override bool WriteSpecificConnectionString()
		{
			...
			try
			{
				...
				{
					String message = base.LoginName + "," + base.LoginPassword + "\r";
				...
				{
					base.ExceptionString=PIDXConstants.ERR_MSG_030;
				}
			}
		...
		public override bool SendTransaction()
		{
			...
			try
			{
				...
				if (typeof(CreditAuthorizationRecordDTN) == base.PidxRecord.GetType())
				{
					if (base.OpenConnection())
					{
						CreditAuthorizationRecordDTN ca = (CreditAuthorizationRecordDTN)base.PidxRecord;
						...
						while (retries > 0 && false == result)
						{
							if (base.WriteStream(ca.GetDataRecord() + '\r'))
							{
								if (-1 != base.ResponseString.IndexOf("AUTH"))
								{
									...
									int responseindex = base.ResponseString.IndexOf("AUTH");
									string actualresponse = base.ResponseString.Substring( responseindex );
								...
								else if (-1 != base.ResponseString.IndexOf("DENY"))
								{
									...
									int responseindex = base.ResponseString.IndexOf("DENY");
									string actualresponse = base.ResponseString.Substring( responseindex );
				...
				else if (typeof(BOLCBRecordDTN) == base.PidxRecord.GetType())
				{
					if (base.OpenConnection())
					{
						BOLCBRecordDTN cb = (BOLCBRecordDTN)base.PidxRecord;
