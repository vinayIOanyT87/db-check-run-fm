...
HRESULT COPCServer::FinalConstruct()
{
	...
	try
	{
		return g_OsdpControllerManager.AddServer(this);
	}
	catch (_com_error& e)
	{
...
void COPCServer::FinalRelease() 
{
   ...
   m_groupMap.RemoveAll();
	...
	g_OsdpControllerManager.RemoveServer(this);
}
...
HRESULT COPCServer::DoQueryNumProperties(
                              ...
                              LPVOID    * ppVoid)
{
   CTag*	pTag = g_OsdpControllerManager.FindTag( szItemID );
   if( pTag 
	&& pTag->m_bLeaf)
   {
      *ppVoid = (LPVOID) pTag;
                              ...
                              LPCWSTR           szString)
{
   ...
   if( dwBrowseDirection == OPC_BROWSE_UP )
	...
	{
		m_pCurrentTag=g_OsdpControllerManager.m_pRoot;
                              ...
                              LPWSTR *    szItemID)
{
   ...
   CString name( szItemDataID );
	...
	if(name.GetLength())
	{
		...
		CTag* pTag=g_OsdpControllerManager.FindTag(path.GetBuffer(0));
...
//*******************************************************************
// Note: No protection agains an OsdpController being deleted during
