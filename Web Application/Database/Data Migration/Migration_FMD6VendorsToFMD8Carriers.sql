...
/****** Object:  StoredProcedure [dbo].[Migration_FMD6VendorsToFMD8Carriers]    Script Date: 03/15/2010 11:39:45 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Migration_FMD6VendorsToFMD8Carriers]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].Migration_FMD6VendorsToFMD8Carriers
GO


CREATE PROCEDURE [dbo].Migration_FMD6VendorsToFMD8Carriers
 /*=============================================
 Author:			Eric Simmons
 Create date:		3/15/2010
 Description:		Migrating FuelsManager Defense 6.0 Vendors to FuelsManager 8.0 Carriers
...
EXEC Migration_FMD6VendorsToFMD8Carriers 2, null
...
*/	
	...
	Insert Into #tmpTableVendors
	Select Distinct ts.SiteID8 + '_' + ta.Vendor, ts.SiteID8, ta.Vendor,-1,-2 
	from AccountingDB6.dbo.t_Acct_Tx5 ta JOIN #TMPSITES ts ON ta.SiteIndex = ts.siteIndex6  where 
	ISNULL(ta.Vendor,'') <> '' AND
	ts.SiteID8 + '_' + ta.Vendor not in
	(Select keyField from #tmpTableVendors)

	--Get Distinct "Owners" from t_Acct_Tx12
	Insert Into #tmpTableVendors
	Select Distinct ts.SiteID8 + '_' + ta.Vendor, ts.SiteID8, ta.Vendor,-1,-2 
	from AccountingDB6.dbo.t_Acct_Tx12 ta JOIN #TMPSITES ts ON ta.SiteIndex = ts.siteIndex6  where 
	ts.SiteIndex8 = ta.SiteIndex AND
	ISNULL(ta.Vendor,'') <> '' AND
	ts.SiteID8 + '_' + ta.Vendor not in
	(Select keyField from #tmpTableVendors)
	...
	if(@IsBaseDB <> 2)
		...
		--Get Distinct "Consumers" from tbl_CommonRequest_BillingInfo.Vendor
		Insert Into #tmpTableVendors
		Select Distinct S.SiteID8 + '_' + tac.Vendor, S.SiteID8, tac.Vendor,-1,-2 
		from AviationDB6.dbo.tbl_CommonRequest_BillingInfo tac, #TMPSITES S where 
		ISNULL(tac.Vendor,'') <> '' AND
		S.SiteID8 + '_' + tac.Vendor not in
		(Select keyField from #tmpTableVendors)
		
		END

	Update #tmpTableVendors
...
*/
	...
	'','','','','','',@date,
	'Varec',@date,'Varec'
	from #tmpTableVendors t where CompanyID not in
	...
	--Assign companies to Carrier To Role
	Insert Into [ConsolidatedDB].[dbo].[tblCompanyRoleMap]
	Select CompanyIndex8,5,@date,'Varec',SiteIndex8
	from #tmpTableVendors
	where CONVERT(nvarchar(20),SiteIndex8) + '_' + CONVERT(nvarchar(20),CompanyIndex8) not in
	(Select CONVERT(nvarchar(20),SiteIndex) + '_' + CONVERT(nvarchar(20),[CompanyIndex]) from [ConsolidatedDB].[dbo].[tblCompanyRoleMap] where [Role] = 5)
	and SiteIndex8 > -2
	
	drop table #tmpTableVendors;
	drop table #tmpsites
