...
/****** Object:  StoredProcedure [dbo].[Migrate_FMD6DETERMINEFieldsToFMD8LineItems]    Script Date: 03/15/2010 11:39:45 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Migrate_FMD6DETERMINEFieldsToFMD8LineItems]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Migrate_FMD6DETERMINEFieldsToFMD8LineItems]
GO

CREATE PROCEDURE [dbo].[Migrate_FMD6DETERMINEFieldsToFMD8LineItems]
 /*=============================================
 Author:			Eric Simmons
...
EXEC Migrate_FMD6DETERMINEFieldsToFMD8LineItems 2,null
...
Set @aliasName6 = 'DETERMINE'
Set @aliasName8 = 'Determine'
...
( Select isnull(COUNT(TransactionID),0) from AccountingDB6.dbo.t_Acct_Tx1 ta where ta.Alias = @aliasName6 AND ta.SiteIndex = S6.siteIndex) > 0
...
BEGIN TRANSACTION
	*/
	...
	/* MeterStart */NULL,
	/* MeterStop */NULL,
	/* GrossQuantity */ case ta.IncreaseFlag when 0 then -1 * (abs(isnull(ta.GrossVolume,0)) / 1000.0) else abs(ISNULL(ta.GrossVolume,0)) / 1000.0 END,
	...
	/* CLIN */@blank,
	/* NetQuantity */ case ta.IncreaseFlag when 0 then -1 * (abs(isnull(ta.GrossVolume,0)) / 1000.0) else abs(ISNULL(ta.GrossVolume,0)) / 1000.0 END,
