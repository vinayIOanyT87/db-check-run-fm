...
/****** Object:  StoredProcedure [dbo].[Migrate_FMD6REQUESTTransactionToFMD8Transactions]    Script Date: 03/15/2010 11:39:45 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Migrate_FMD6REQUESTTransactionToFMD8Transactions]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Migrate_FMD6REQUESTTransactionToFMD8Transactions]
GO

CREATE PROCEDURE [dbo].[Migrate_FMD6REQUESTTransactionToFMD8Transactions]
 /*=============================================
 Author:			Eric Simmons
...
EXEC Migrate_FMD6REQUESTTransactionToFMD8Transactions 2, null
...
*/
	...
	/* ShipToID */ ta.Manager,
	/* ShipToCode */ (Select top 1 Code from tblCompanies where ta.Manager = ID),
	/* ShipToIndex */ (Select top 1 CompanyIndex from tblCompanies where ta.Manager = ID),
	/* SupplierID */ ta.Supplier,
	/* SupplierCode */ (Select top 1 Code from tblCompanies where ta.Supplier = ID),
	/* SupplierIndex */ (Select top 1 CompanyIndex from tblCompanies where ta.Supplier = ID),
