...
/****** Object:  StoredProcedure [dbo].[Migrate_FMD6SHIPMENTCONTRACTWithShippingDocumentFieldsToFMD8LineItems]    Script Date: 03/15/2010 11:39:45 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Migrate_FMD6SHIPMENTCONTRACTWithShippingDocumentFieldsToFMD8LineItems]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Migrate_FMD6SHIPMENTCONTRACTWithShippingDocumentFieldsToFMD8LineItems]
GO


CREATE PROCEDURE [dbo].[Migrate_FMD6SHIPMENTCONTRACTWithShippingDocumentFieldsToFMD8LineItems]
 /*=============================================
 Author:			Eric Simmons
...
EXEC Migrate_FMD6SHIPMENTCONTRACTWithShippingDocumentFieldsToFMD8LineItems 2,NULL
...
( Select isnull(COUNT(TransactionID),0) from AccountingDB6.dbo.t_Acct_Tx5 ta WHERE
	...
	ta.TransactionID in (Select TransactionID from AccountingDB6.dbo.t_Acct_FuelLoad)) > 0
...
*/	
	...
	(
		...
		[FuelLoadIndex] [bigint] NULL
	)

	Insert Into #LineItemIDTable
	(TransIndex,FuelLoadIndex)
	Select tt.TransIndex,taf.FuelLoadIndex
	from AccountingDB6.dbo.t_Acct_Tx5 ta
	JOIN #TMPSITES s ON s.SiteIndex6 = ta.SiteIndex
	LEFT JOIN AccountingDB6.dbo.t_Acct_FuelLoad taf on taf.TransactionID = ta.TransactionID
	LEFT JOIN ConsolidatedDB.dbo.tblTransactions tt
	ON tt.TransID = ta.TransactionID
	where 
	--ta.SiteIndex = @siteIndex6 and 
	tt.TransIndex is not null and 
	ta.Alias = @aliasName6 	and 
	isnull(ta.UserData12,'') <> '' and
	ta.TransactionID in (Select TransactionID from AccountingDB6.dbo.t_Acct_FuelLoad)
	...
	--P6.SiteIndex = @SiteIndex6 AND M.SiteIndex = @SiteIndex8 AND 
	P6.ProductIndex  <> -1
	...
	/* SequenceID */lit.FuelLoadIndex,
	/* MeterStart */NULL,
	/* MeterStop */NULL,
	/* GrossQuantity */ abs(isnull(taf.GrossQuantity,0)) * -1.0/ 1000.0, -- CHK 4/9/2010 avoid integer math
	/* Temperature */ taf.ObservedTemperature,
	/* Vcf */ NULL,
	/* Density */ taf.ObservedDensity * 1000.0, -- CHK 4/9/2010 change to 8.0 standard units
	...
	/* CLIN */ta.UserData12,
	/* NetQuantity */ abs(isnull(taf.NetQuantity,0)) * -1.0 / 1000.0, -- CHK 4/9/2010 avoid integer math
	/* ContractNumber */@blank,
	/* DestinationRegistrationID */ taf.TruckNumber,
	/* DestinationSerialNumber */ @blank,
	/* DestinationEquipmentType */ (Select top 1 EqTypeName from tblEquipment te,tblEquipmentTypes tet where te.EqTypeIndex = tet.EqTypeIndex and te.ID = taf.TruckNumber ),
	/* DestinationEquipmentModel */ (Select top 1 Model from tblEquipment where ID = taf.TruckNumber),
	/* DestinationCompanyEquipmentID */ @blank,
	/* DestinationEquipmentIndex */ (Select top 1 [Index] from tblEquipment where ID = taf.TruckNumber),
	...
	/* BatchNumber */ta.UserData15,  -- CHK 4/9/2010
	...
	from AccountingDB6.dbo.t_Acct_Tx5 ta
	JOIN #TMPSITES s ON s.SiteIndex6 = ta.SiteIndex
	LEFT JOIN ConsolidatedDB.dbo.tblTransactions tt ON tt.TransID = ta.TransactionID
	LEFT JOIN #LineItemIDTable lit on lit.TransIndex = tt.TransIndex 
	LEFT JOIN AccountingDB6.dbo.t_Acct_FuelLoad taf on taf.FuelLoadIndex = lit.FuelLoadIndex
	LEFT JOIN (SELECT DISTINCT ProductID, ProductID6, ProductIndex8 FROM #TMP_PRODUCT_INDEX_MAP) tp on tp.ProductID6 = ta.ProductID
	where 
	--ta.SiteIndex = @siteIndex6 and 
	tt.TransIndex is not null and ta.Alias = @aliasName6 and 
	ta.TransactionID = taf.TransactionID and lit.TransIndex = tt.TransIndex and isnull(ta.UserData12,'') <> '' and
	ta.TransactionID in (Select TransactionID from AccountingDB6.dbo.t_Acct_FuelLoad)
