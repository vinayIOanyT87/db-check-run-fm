USE [ConsolidatedDB]
GO
/****** Object:  StoredProcedure [dbo].[Migration_AccountingDB6RegradeTo8Transactions]    ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].Migration_AccountingDB6RegradeTo8Transactions 

 /*=============================================
 Author:			A. Coker
 Create date:		3/12/2010
Description:		Migrating AccountingDB6.0 Regrade to ConsolidatedDB8.0 Transactions
...
EXEC Migration_AccountingDB6RegradeTo8Transactions 2, null
...
SELECT DISTINCT P8.ProductID, P8.ProductCode, P8.ProductIndex AS ProductIndex8, 
		P6.ProductID AS ProductID6, P8.ProductType, P6.[UnitOfIssue]
...
SELECT 
	...
	,CASE C.[SubtypeCode3]
		WHEN 'A' THEN 'Additive'
		WHEN 'D' THEN 'Downgrade'
		WHEN 'R' THEN 'Regrade'
		ELSE 'None'
		END																	AS UserData4
	,C.[UserData5]															AS UserData6
  ...
  FROM [AccountingDB6].[dbo].[t_Acct_Tx15] C 
	JOIN #TMPSITES s ON s.SiteIndex6 = C.SiteIndex
  LEFT JOIN #TMP_PRODUCT_INDEX_MAP P ON C.FromProductID= P.ProductID6				
...
SELECT 
  ...
  FROM [AccountingDB6].[dbo].[t_Acct_Tx15] C 
	JOIN #TMPSITES s ON s.SiteIndex6 = C.SiteIndex
  LEFT JOIN #TMP_PRODUCT_INDEX_MAP P ON C.ToProductID= P.ProductID6				
  WHERE C.DeleteFlag = 0 --AND C.SiteIndex=@SiteIndex6 
  AND C.Alias = 'REGRADE'
  AND transactiondate BETWEEN '1900-01-01' AND '2079-06-06'
...
--select * from #TMP_TRANSACTIONLINEITEMS
