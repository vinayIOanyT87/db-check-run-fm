...
/****** Object:  StoredProcedure [dbo].[Migrate_FMD6SHIPMENTCONTRACTWithoutShippingDocumentFieldsToFMD8LineItems]    Script Date: 03/15/2010 11:39:45 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Migrate_FMD6SHIPMENTCONTRACTWithoutShippingDocumentFieldsToFMD8LineItems]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Migrate_FMD6SHIPMENTCONTRACTWithoutShippingDocumentFieldsToFMD8LineItems]
GO


CREATE PROCEDURE [dbo].[Migrate_FMD6SHIPMENTCONTRACTWithoutShippingDocumentFieldsToFMD8LineItems]
 /*=============================================
 Author:			Eric Simmons
...
EXEC Migrate_FMD6SHIPMENTCONTRACTWithoutShippingDocumentFieldsToFMD8LineItems 2, null
...
--	M.SiteIndex = @SiteIndex8 AND 
	...
	LEFT JOIN (SELECT DISTINCT ProductID, ProductIndex8, ProductID6 FROM #TMP_PRODUCT_INDEX_MAP) tp on tp.ProductID6 = ta.ProductID
	where 
	--ta.SiteIndex = @siteIndex6 and 
	tt.TransIndex is not null and 
	ta.Alias = @aliasName6 and 
