...
/****** Object:  StoredProcedure [dbo].[Migrate_FMD6DEFUELieldsToFMD8LineItems]    Script Date: 03/15/2010 11:39:45 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Migrate_FMD6DEFUELFieldsToFMD8LineItems]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Migrate_FMD6DEFUELFieldsToFMD8LineItems]
GO

CREATE PROCEDURE [dbo].[Migrate_FMD6DEFUELFieldsToFMD8LineItems]
 /*=============================================
 Author:			Eric Simmons
...
EXEC Migrate_FMD6DEFUELFieldsToFMD8LineItems 2, null
...
BEGIN TRANSACTION
	...
	*/
	...
	/* MeterStart */NULL,
	/* MeterStop */NULL,
	/* GrossQuantity */ abs(isnull(ta.GrossVolume,0)) / 1000.0,
	...
	/* CLIN */@blank,
	/* NetQuantity */ abs(isnull(ta.GrossVolume,0)) / 1000.0,
	...
	ON tt.TransID = ta.TransactionID
	LEFT JOIN (SELECT DISTINCT ProductID6, ProductIndex8, ProductID FROM #TMP_PRODUCT_INDEX_MAP) tp on tp.ProductID6 = ta.ProductID
	where 
