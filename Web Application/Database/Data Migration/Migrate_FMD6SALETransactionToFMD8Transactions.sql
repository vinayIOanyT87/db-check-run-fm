...
/****** Object:  StoredProcedure [dbo].[Migrate_FMD6SALETransactionToFMD8Transactions]    Script Date: 03/15/2010 11:39:45 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Migrate_FMD6SALETransactionToFMD8Transactions]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[Migrate_FMD6SALETransactionToFMD8Transactions]
GO

CREATE PROCEDURE [dbo].[Migrate_FMD6SALETransactionToFMD8Transactions]
 /*=============================================
 Author:			Eric Simmons
...
EXEC Migrate_FMD6SALETransactionToFMD8Transactions2 2, null 
...
*/
	...
	SELECT M.SiteIndex,PersonIndex, PersonID FROM [ConsolidatedDB].[dbo].[tblPersonnel] P 
	...
	from AccountingDB6.dbo.t_Acct_Tx5 ta WHERE
	ta.Alias = @aliasName6 
	...
	/* SubType */ @blank, 
	/* Site */ ta.Manager,
	/* SiteIndex */ s.SiteIndex8,--@siteIndex8,
	...
	/* CarrierID */ ta.Vendor,
	/* CarrierCode */ (Select top 1 Code from tblCompanies where ta.Vendor = ID),
	/* CarrierIndex */ (Select top 1CompanyIndex from tblCompanies where ta.Vendor = ID),
	...
	/* DestinationEquipmentModel1 */ CASE ISNULL(ta.UserData18,'')
	...
	/* DestinationEquipmentIndex1 */ (Select top 1 [Index] from tblEquipment where ID = ta.ReceivingEqID),
	...
	/* SourceRegistrationID1 */ CASE UPPER(SUBSTRING(isnull(ta.TransactionID,''),1,2)) 
								WHEN 'DP' THEN cl.VEHICLE_ID
								ELSE ta.EquipmentID END,
	/* SourceSerialNumber1 */ @blank,
	/* SourceEquipmentType1 */ CASE UPPER(SUBSTRING(isnull(ta.TransactionID,''),1,2)) 
	...
	/* SourceEquipmentModel1 */ CASE UPPER(SUBSTRING(isnull(ta.TransactionID,''),1,2)) 
	...
	/* SourceEquipmentIndex1 */ CASE UPPER(SUBSTRING(isnull(ta.TransactionID,''),1,2)) 
	...
	ta.TransactionDate between '1900/1/1' and '2079/06/06 23:59:59'
	AND tvt.TransID = ta.TransactionID
