CREATE TABLE [dbo].[tblScullys] (
    [Index] INT IDENTITY (1, 1) NOT NULL,
    [ID] NVARCHAR (50) NOT NULL,
    [PortIndex] INT NOT NULL,
    [DeviceID] INT NOT NULL,
    CONSTRAINT [PK_tblScullys] PRIMARY KEY CLUSTERED ([Index] ASC),
    CONSTRAINT [FK_tblScullys_tblPorts] FOREIGN KEY ([PortIndex]) REFERENCES [dbo].[tblPorts] ([Index]),
    CONSTRAINT [IX_tblScullys] UNIQUE NONCLUSTERED ([ID] ASC)
);

GO
-- TRIGGER [dbo].[trg_Audit_upd_tblScullys] 
CREATE TRIGGER [trg_Audit_upd_tblScullys] ON [dbo].[tblScullys] 
AFTER 
UPDATE 
AS BEGIN 
SET 
...
IF [FuelsManagerDB].[fmaudit].[udf_DisableTriggerByAuditRule]('dbo', 'tblScullys', 'D')= 1 RETURN DECLARE @_AuditEventType CHAR(1), 
...
INSERT INTO [Fuelsmanagerdb].[fmaudit].[ScullyOPC_tblScullys]
  (
[Index]
,[ID]
,[PortIndex]
...
/****** Object:  Trigger [dbo].[trg_Audit_ins_tblScullys] ******/

CREATE TRIGGER [dbo].[trg_Audit_ins_tblScullys] ON [dbo].[tblScullys] 
AFTER INSERT AS 
BEGIN 
SET 
  NOCOUNT ON;
-- Verifies whether the trigger is active based on configuration and Audit
-- rules which are resolved by the User Defined Function [fmaudit].[udf_DisableTriggerByAuditRule]
IF [Fuelsmanagerdb].[fmaudit].[udf_DisableTriggerByAuditRule]('dbo', 'tblScullys', 'D')= 1 -- Notes: Verify this setting
...
--TRIGGER [dbo].[trg_Audit_del_tblScullys]
CREATE TRIGGER [dbo].[trg_Audit_del_tblScullys] ON [dbo].[tblScullys] 
AFTER 
  DELETE AS BEGIN 
SET 
  NOCOUNT ON;
