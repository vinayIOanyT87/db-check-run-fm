CREATE FUNCTION [dbo].[udf_ConvertFromSIUnits]
(@Value FLOAT, @Units INT, @RoundFactor INT)
RETURNS FLOAT
WITH SCHEMABINDING
AS
BEGIN 
...
-- Centigrade (SI)
			when 1 then @Value
-- Ferenheit
			when 2 then @Value*1.8+32
-- Kelvin
			when 3 then @Value-273.15
-- Rankine
			when 4 then (@Value+273.15)*1.8
...
/*
	This resolves an issue with rounding in SQL
	See http://stackoverflow.com/questions/9526386/rounding-issue-with-sql-server-and-real-datatype for more details
*/
	if(@Result < 0)
    BEGIN
        Set @Result = @Result - 0.000000001
    END
    else
    BEGIN
        Set @Result = @Result + 0.000000001 
	...
	return Round(@Result,@RoundFactor) 
end