/*
	DROP PROCEDURE [Staging].[usp_UpdateConjoinTransactionsForSubLineItems]

	EXEC [staging].[usp_UpdateConjoinTransactionsForSubLineItems] 0, 0

	EXEC [staging].[usp_UpdateConjoinTransactionsForSubLineItems] 200000, 400000
	
*/
CREATE PROCEDURE [staging].[usp_UpdateConjoinTransactionsForSubLineItems]
(
	@startSKey  int,
	@endSKey int
)
...
BEGIN
  ------------------------------------------------------------------------------------------------------
  -- Stored procedure: [staging].[usp_UpdateConjoinTransactionsForSubLineItems]
  ...
  --          FactTransaction table into the staging.tblTransactionSubLineItems table.
  -- Notes:
  ...
  --   from the Merge operation in the SubLineItem Loading operations, allows us to only update the Conjoin fields without touching the 
  ...
  BEGIN CATCH
    ...
    + 'Procedure Name: [staging].[usp_UpdateConjoinTransactionsForSubLineItems]' + CHAR(13) + CHAR(10)
