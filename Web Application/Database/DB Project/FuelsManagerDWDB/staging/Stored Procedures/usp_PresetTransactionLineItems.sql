/*
    DROP PROCEDURE [Staging].[usp_PresetTransactionLineItems]

	EXEC [staging].[usp_PresetTransactionLineItems]
	
*/
CREATE PROCEDURE [staging].[usp_PresetTransactionLineItems]
AS
BEGIN
  ------------------------------------------------------------------------------------------------------
  ...
  -- Purpose: For table staging.tblTransactionLineItems, set all the extra fields that were added to the table and that are not populated 
  --          from the corresponding tblTransactionLineItems OLTP table.
  -- Notes:
  ...
  --    load those fields for each record of staging.tblTransactionLineItems. 
