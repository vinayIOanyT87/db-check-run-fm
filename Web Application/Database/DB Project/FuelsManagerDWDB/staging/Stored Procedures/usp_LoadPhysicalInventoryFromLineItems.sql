/*
	DROP PROCEDURE [staging].[usp_LoadPhysicalInventoryFromLineItems]

	EXEC [staging].[usp_LoadPhysicalInventoryFromLineItems] 0, 0

	EXEC [staging].[usp_LoadPhysicalInventoryFromLineItems] 0, 200000

	EXEC [staging].[usp_LoadPhysicalInventoryFromLineItems] 200000, 0
	
*/
CREATE PROCEDURE [staging].[usp_LoadPhysicalInventoryFromLineItems]
(
	@startSKey  int,
	@endSKey int
)
...
BEGIN
  ------------------------------------------------------------------------------------------------------
  -- Stored procedure: [staging].[usp_LoadPhysicalInventoryFromLineItems]
  -- Author: Hansraj Bapoo
  -- Version/Date: 1.0.003 / 2012-07-13 14:21:10.4470770 -04:00
  -- Purpose: Loads Physical Inventory records from the LineItem records from staging into the FactPhysicalInventorySnapshot table in the OLAP database.
  -- Notes:
  ...
  BEGIN TRY
	...
	SELECT
		src.SKey, src.[TransactionLineItemKey]
	...
	(
		...
		WHERE b.LineItemLastSKey = a.RecordSKey
	)
	...
	ON a.RecordKey = src.TransactionLineItemKey	
	WHERE src.TransactionLineItemKey IS NOT NULL
	AND src.IgnoreRecord = 0
	AND a.IsNewRecord = 1
  ...
  BEGIN CATCH
    ...
    + 'Procedure Name: [staging].[usp_LoadPhysicalInventoryFromLineItems]' + CHAR(13) + CHAR(10)
