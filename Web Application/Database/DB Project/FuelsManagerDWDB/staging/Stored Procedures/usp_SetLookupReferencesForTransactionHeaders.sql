/*
    DROP PROCEDURE [Staging].[usp_SetLookupReferencesForTransactionHeaders]

	EXEC [staging].[usp_SetLookupReferencesForTransactionHeaders]
	
*/
CREATE PROCEDURE [staging].[usp_SetLookupReferencesForTransactionHeaders]
AS
BEGIN
  ------------------------------------------------------------------------------------------------------
  -- Stored procedure: [staging].[usp_SetLookupReferencesForTransactionHeaders]
  -- Author: Hansraj Bapoo
  -- Version/Date: 1.0.003 / 2012-07-13 14:21:10.4470770 -04:00
  -- Purpose: Sets, in staging, the value of the corresponding name field for each lookup index field in tblTransactions. 
  ...
  BEGIN TRY
    ...
    -- TransactionHeader Lookup references		
    UPDATE a
    SET a.OriginApplicationName = b.LookupName
    FROM staging.tblTransactions a
    INNER JOIN lookup.tblLookup b
      ON b.LookupIndex = a.OriginApplicationIndex
    WHERE b.LookupType = 'TransactionOrigin'
    ...
    (
        (
            ...
            WHERE 
            (
                OriginApplicationIndex IS NOT NULL AND OriginApplicationName IS NULL
            )
            OR 
            (
    ...
    BEGIN
      RAISERROR ('Failure to resolve TransactionHeader-Lookup references', 16, 1);
      RETURN;
    END
  ...
  BEGIN CATCH
    ...
    + 'Procedure Name: [staging].[usp_SetLookupReferencesForTransactionHeaders]' + CHAR(13) + CHAR(10)
