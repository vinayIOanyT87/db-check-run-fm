/*
  DROP PROCEDURE [Staging].[usp_SetLookupReferencesForLineItems]

	EXEC [staging].[usp_SetLookupReferencesForLineItems]
	
*/
CREATE PROCEDURE [staging].[usp_SetLookupReferencesForLineItems]
AS
BEGIN
  ------------------------------------------------------------------------------------------------------
  -- Stored procedure: [staging].[usp_SetLookupReferencesForLineItems]
  ...
  BEGIN TRY
    ...
    BEGIN
      RAISERROR ('Failure to resolve TransactionLineItem-Lookup references', 16, 1);
      RETURN;
    END
  ...
  BEGIN CATCH
    ...
    + 'Procedure Name: [staging].[usp_SetLookupReferencesForLineItems]' + CHAR(13) + CHAR(10)
