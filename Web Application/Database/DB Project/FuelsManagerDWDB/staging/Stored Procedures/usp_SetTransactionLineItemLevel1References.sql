/*
	DROP PROCEDURE [Staging].[usp_SetTransactionLineItemLevel1References]

	EXEC [staging].[usp_SetTransactionLineItemLevel1References]
	
*/
CREATE PROCEDURE [staging].[usp_SetTransactionLineItemLevel1References]
(
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [staging].[usp_SetTransactionLineItemLevel1References]
	-- Author: Hansraj Bapoo
	-- Version/Date: 1.0.003 / 2012-07-13 14:21:10.4470770 -04:00
	-- Purpose: Sets, in staging, the TransactionLineItem fields that references Level1 tables.
	-- Notes:
	...
	-- 3. TransactionLineItems references to those entity types that support Record Versioning are not set right away after the entity 
	--    types are loaded (e.g. they are not set in usp_SetLevel1References), because TransactionLineItems do not come readily with 
	...
	--    the Record Versioning references. Therefore this Stored Procedure can only be executed after the TransactionLineItem site 
	...
	BEGIN TRY
		...
		-- TransactionLineItem Equipment references			
		UPDATE a 
		SET a.DestinationCompartmentSKey = x.RecordVersionSKey
		FROM staging.tblTransactionLineItems a
		INNER JOIN 
		(
			SELECT b.TransactionLineItemKey, MAX(c.RecordVersionSKey) RecordVersionSKey 
			FROM staging.tblTransactionLineItems b
			INNER JOIN map.tblEquipmentToSiteRecordVersion c
			ON c.EquipmentKey = b.DestinationCompartmentKey
			...
			GROUP BY b.TransactionLineItemKey
		) x
		ON x.TransactionLineItemKey = a.TransactionLineItemKey
		WHERE a.IgnoreRecord = 0

		UPDATE a 
		SET a.DestinationCompartmentSKey = b.RecordVersionSKey
		FROM staging.tblTransactionLineItems a
		...
		ON b.EquipmentKey = a.DestinationCompartmentKey
		AND b.SiteSKey = a.SiteSKey
		WHERE a.IgnoreRecord = 0
		AND a.DestinationCompartmentSKey IS NULL
		AND 
		(
		...
		ON b.AKey = a.DestinationCompartmentKey
		WHERE a.IgnoreRecord = 0
		AND a.DestinationCompartmentSKey IS NULL
		AND NOT EXISTS
		(
		...
		UPDATE a 
		SET a.DestinationEquipmentSKey = x.RecordVersionSKey
		FROM staging.tblTransactionLineItems a
		INNER JOIN 
		(
			SELECT b.TransactionLineItemKey, MAX(c.RecordVersionSKey) RecordVersionSKey 
			FROM staging.tblTransactionLineItems b
			INNER JOIN map.tblEquipmentToSiteRecordVersion c
			ON c.EquipmentKey = b.DestinationEquipmentKey
			AND c.SiteSKey = b.SiteSKey
			WHERE b.IgnoreRecord = 0
			AND b.TransactionInventoryDate BETWEEN CONVERT(DATE, c.StartDate) AND ISNULL(c.EndDate, @openEndedDate)
			GROUP BY b.TransactionLineItemKey
		...
		UPDATE a 
		SET a.DestinationEquipmentSKey = b.RecordVersionSKey
		FROM staging.tblTransactionLineItems a
		INNER JOIN (SELECT EquipmentKey, SiteSKey, MAX(RecordVersionSKey) RecordVersionSKey FROM map.tblEquipmentToSiteRecordVersion GROUP BY EquipmentKey, SiteSKey) b
		ON b.EquipmentKey = a.DestinationEquipmentKey
		AND b.SiteSKey = a.SiteSKey
		WHERE a.IgnoreRecord = 0
		...
		FROM staging.tblTransactionLineItems a
		INNER JOIN dbo.DimEquipment b
		ON b.AKey = a.DestinationEquipmentKey
		WHERE a.IgnoreRecord = 0
		AND a.DestinationEquipmentSKey IS NULL
		AND NOT EXISTS
		(
		...
		UPDATE a 
		SET a.SourceCompartmentSKey = x.RecordVersionSKey
		FROM staging.tblTransactionLineItems a
		INNER JOIN 
		(
			SELECT b.TransactionLineItemKey, MAX(c.RecordVersionSKey) RecordVersionSKey 
			FROM staging.tblTransactionLineItems b
			INNER JOIN map.tblEquipmentToSiteRecordVersion c
			ON c.EquipmentKey = b.SourceCompartmentKey
			AND c.SiteSKey = b.SiteSKey
			WHERE b.IgnoreRecord = 0
			AND b.TransactionInventoryDate BETWEEN CONVERT(DATE, c.StartDate) AND ISNULL(c.EndDate, @openEndedDate)
			GROUP BY b.TransactionLineItemKey
		...
		UPDATE a 
		SET a.SourceCompartmentSKey = b.RecordVersionSKey
		FROM staging.tblTransactionLineItems a
		INNER JOIN (SELECT EquipmentKey, SiteSKey, MAX(RecordVersionSKey) RecordVersionSKey FROM map.tblEquipmentToSiteRecordVersion GROUP BY EquipmentKey, SiteSKey) b
		ON b.EquipmentKey = a.SourceCompartmentKey
		AND b.SiteSKey = a.SiteSKey
		WHERE a.IgnoreRecord = 0
		AND a.SourceCompartmentSKey IS NULL
		AND 
		(
			(@IgnoreDateMismatch = 1)
			OR
		...
		FROM staging.tblTransactionLineItems a
		INNER JOIN dbo.DimEquipment b
		ON b.AKey = a.SourceCompartmentKey
		WHERE a.IgnoreRecord = 0
		AND a.SourceCompartmentSKey IS NULL
		AND NOT EXISTS
		(
		...
		UPDATE a 
		SET a.SourceEquipmentSKey = x.RecordVersionSKey
		FROM staging.tblTransactionLineItems a
		INNER JOIN 
		(
			SELECT b.TransactionLineItemKey, MAX(c.RecordVersionSKey) RecordVersionSKey 
			FROM staging.tblTransactionLineItems b
			INNER JOIN map.tblEquipmentToSiteRecordVersion c
			ON c.EquipmentKey = b.SourceEquipmentKey
			AND c.SiteSKey = b.SiteSKey
			WHERE b.IgnoreRecord = 0
			AND b.TransactionInventoryDate BETWEEN CONVERT(DATE, c.StartDate) AND ISNULL(c.EndDate, @openEndedDate)
			GROUP BY b.TransactionLineItemKey
		...
		UPDATE a 
		SET a.SourceEquipmentSKey = b.RecordVersionSKey
		FROM staging.tblTransactionLineItems a
		INNER JOIN (SELECT EquipmentKey, SiteSKey, MAX(RecordVersionSKey) RecordVersionSKey FROM map.tblEquipmentToSiteRecordVersion GROUP BY EquipmentKey, SiteSKey) b
		ON b.EquipmentKey = a.SourceEquipmentKey
		AND b.SiteSKey = a.SiteSKey
		WHERE a.IgnoreRecord = 0
		...
		FROM staging.tblTransactionLineItems a
		INNER JOIN dbo.DimEquipment b
		ON b.AKey = a.SourceEquipmentKey
		WHERE a.IgnoreRecord = 0
		AND a.SourceEquipmentSKey IS NULL
		AND NOT EXISTS
		(
		...
		UPDATE a
		SET a.LoadingLocationStationSKey = b.SKey
		FROM staging.tblTransactionLineItems a
		INNER JOIN dbo.DimStation b
			ON b.AKey = a.LoadingLocationStationKey
		WHERE a.IgnoreRecord = 0
		AND a.LoadingLocationStationSKey IS NULL


		-- TransactionLineItem LoadArm references			
		UPDATE a
		...
		-- TransactionLineItem Tank references			
		UPDATE a
		...
		IF 
		(
			(SELECT COUNT(*) FROM staging.tblTransactionLineItems WHERE DestinationCompartmentKey IS NOT NULL AND DestinationCompartmentSKey IS NULL AND IgnoreRecord = 0) > 0
			OR (SELECT COUNT(*) FROM staging.tblTransactionLineItems WHERE DestinationEquipmentKey IS NOT NULL AND DestinationEquipmentSKey IS NULL AND IgnoreRecord = 0) > 0
			OR (SELECT COUNT(*) FROM staging.tblTransactionLineItems WHERE SourceCompartmentKey IS NOT NULL AND SourceCompartmentSKey IS NULL AND IgnoreRecord = 0) > 0
			OR (SELECT COUNT(*) FROM staging.tblTransactionLineItems WHERE SourceEquipmentKey IS NOT NULL AND SourceEquipmentSKey IS NULL AND IgnoreRecord = 0) > 0
		)
		BEGIN
			RAISERROR('Failure to resolve TransactionLineItem-to-Equipment references',16,1); 
			RETURN;
		END

		IF 
		(
			(SELECT COUNT(*) FROM staging.tblTransactionLineItems WHERE ProductKey IS NOT NULL AND ProductSKey IS NULL AND IgnoreRecord = 0) > 0
		)
		BEGIN
			RAISERROR('Failure to resolve TransactionLineItem-to-Product references',16,1); 
			RETURN;
		END

		IF 
		(
			(SELECT COUNT(*) FROM staging.tblTransactionLineItems WHERE LoadingLocationStationKey IS NOT NULL AND LoadingLocationStationSKey IS NULL AND IgnoreRecord = 0) > 0
		)
		BEGIN
			RAISERROR('Failure to resolve TransactionLineItem-to-Station references',16,1); 
			RETURN;
		END
		
		IF 
		(
			(SELECT COUNT(*) FROM staging.tblTransactionLineItems WHERE LoadArmKey IS NOT NULL AND LoadArmSKey IS NULL AND IgnoreRecord = 0) > 0
		)
		BEGIN
			RAISERROR('Failure to resolve TransactionLineItem-to-LoadArm references',16,1); 
			RETURN;
		END

		IF 
		(
			(SELECT COUNT(*) FROM staging.tblTransactionLineItems WHERE StorageLocationTankKey IS NOT NULL AND StorageLocationTankSKey IS NULL AND IgnoreRecord = 0) > 0
		)
		BEGIN
			RAISERROR('Failure to resolve TransactionLineItem-to-StorageTank references',16,1); 
			RETURN;
		END
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: [staging].[usp_SetTransactionLineItemLevel1References]' + CHAR(13)+CHAR(10)                  
