/*
	DROP PROCEDURE [Staging].[usp_SetTransactionLineItemConjoinReferences]

	EXEC [staging].[usp_SetTransactionLineItemConjoinReferences] 0, 0

	EXEC [staging].[usp_SetTransactionLineItemConjoinReferences] 0, 20000

	EXEC [staging].[usp_SetTransactionLineItemConjoinReferences] 20000, 0
	
*/
CREATE PROCEDURE [staging].[usp_SetTransactionLineItemConjoinReferences]
(
	@startSKey  int,
	@endSKey int
)
...
BEGIN
  ------------------------------------------------------------------------------------------------------
  -- Stored procedure: [staging].[usp_SetTransactionLineItemConjoinReferences]
  -- Author: Hansraj Bapoo
  -- Version/Date: 1.0.003 / 2012-07-13 14:21:10.4470770 -04:00
  -- Purpose: Resolves and sets the Conjoin fields for each Conjoin Transaction LineItem record.
  -- Notes:
  -- 1. @startSKey: TransactionLineItemSKey from which to filter the records to be updated. Leave as 0 to ignore this filter.
  -- 2. @endSKey: TransactionLineItemSKey to which to filter the records to be updated. Leave as 0 to ignore this filter.
  ...
  BEGIN TRY
    ...
    BEGIN
      RAISERROR ('Failure to find matching conjoin Transaction LineItem pair', 16, 1);
      RETURN;
    END
