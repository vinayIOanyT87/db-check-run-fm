/*
	...
	EXEC [staging].[usp_LoadTransactionSubLineItems] 0, 0

	EXEC [staging].[usp_LoadTransactionSubLineItems] 0, 200000

	EXEC [staging].[usp_LoadTransactionSubLineItems] 200000, 0
...
BEGIN
  ...
  -- Purpose: Loads SubLineItem records from staging into the FactTransaction table in the OLAP database.
  -- Notes:
  ...
  -- 4. The Transaction SubLineItem Table is a Level 3 table. Level 3 tables are those tables that have a foreign key dependency to a level 2 table.
  ...
  -- 9. SubLineItem deletions in the OLTP database, whether soft deletions (DeleteFlag = 1) or physical deletions are translated in the 
  --    OLAP database as soft deletions.
  -- 10. SubLineItems are not covered by the Conjoin Transaction processing since they are not involved in Conjoin Transactions.
  ------------------------------------------------------------------------------------------------------
  SET NOCOUNT ON;
  BEGIN TRY
	...
	(
		...
		WHERE tgt.TransactionSubLineItemKey = src.RecordKey
	)
	...
	SELECT 
		...
		0 [ConjoinProductSKey],
		ISNULL(src.[Density], 0),
		ISNULL(src.[LineItemDestinationEquipmentSKey], 0),
		...
		0 [NetVolumeIndicator],
		ISNULL(src.[ProductSKey], 0),
		ISNULL(src.[SequenceID], 0),
		ISNULL(src.[LineItemSourceEquipmentSKey], 0),
		ISNULL(src.[LineItemLoadingLocationStationSKey], 0),
