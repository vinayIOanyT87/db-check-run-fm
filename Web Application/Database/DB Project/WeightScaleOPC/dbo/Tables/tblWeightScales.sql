CREATE TABLE [dbo].[tblWeightScales] (
    [Index] INT IDENTITY (1, 1) NOT NULL,
    [ID] NVARCHAR (50) NOT NULL,
    [Type] INT NOT NULL,
    [PortIndex] INT NOT NULL,
    [DeviceID]  INT NULL,
    CONSTRAINT [PK_tblWeightScales] PRIMARY KEY CLUSTERED ([Index] ASC),
    CONSTRAINT [FK_tblWeightScales_PortIndex] FOREIGN KEY ([PortIndex]) REFERENCES [dbo].[tblPorts] ([Index]),
    CONSTRAINT [FK_tblWeightScales_tblPorts] FOREIGN KEY ([PortIndex]) REFERENCES [dbo].[tblPorts] ([Index]),
    CONSTRAINT [IX_tblWeightScales] UNIQUE NONCLUSTERED ([ID] ASC)
);
GO


--  TRIGGER [dbo].[trg_Audit_del_tblWeightScales]
CREATE TRIGGER [dbo].[trg_Audit_del_tblWeightScales] ON [dbo].[tblWeightScales] 
AFTER 
  DELETE AS BEGIN 
SET 
  NOCOUNT ON;
...
IF [Fuelsmanagerdb].[fmaudit].[udf_DisableTriggerByAuditRule]('dbo', 'tblWeightScales', 'D')= 1 -- Notes: Verify this setting
RETURN 
...
SET   @_UserId = SUSER_NAME() 
  ...
  INSERT INTO [Fuelsmanagerdb].[fmaudit].[WeightScaleOPC_tblWeightScales]
  (
[Index],
[ID],
[Type],
...
deleted d 
 ...
 --  TRIGGER [dbo].[trg_Audit_ins_tblWeightScales]
CREATE TRIGGER [dbo].[trg_Audit_ins_tblWeightScales] ON [dbo].[tblWeightScales] 
AFTER INSERT AS 
BEGIN 
SET 
  NOCOUNT ON;
...
-- TRIGGER [dbo].[trg_Audit_upd_tblWeightScales] 
CREATE TRIGGER [dbo].[trg_Audit_upd_tblWeightScales] ON [dbo].[tblWeightScales] 
AFTER 
UPDATE 
AS BEGIN 
SET 
...
-- Verifies whether the trigger is active based on configuration and Audit
-- rules which are resolved by the User Defined Function [fmaudit].[udf_DisableTriggerByAuditRule]
IF [FuelsManagerDB].[fmaudit].[udf_DisableTriggerByAuditRule]('dbo', 'tblWeightScales', 'D')= 1 RETURN DECLARE @_AuditEventType CHAR(1), 
@_AuditEventSequence TINYINT, 
@_AuditSessionGUID UNIQUEIDENTIFIER, 
@_AuditSessionTokenID UNIQUEIDENTIFIER, 
@_AuditSiteGUID UNIQUEIDENTIFIER, 
