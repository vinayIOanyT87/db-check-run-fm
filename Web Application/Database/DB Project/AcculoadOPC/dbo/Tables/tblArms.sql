CREATE TABLE [dbo].[tblArms] (
    [Index]         INT     IDENTITY (1, 1) NOT NULL,
    [AcculoadIndex] INT     NOT NULL,
    [Number]        TINYINT NOT NULL,
    [Address]       TINYINT NOT NULL,
    [Type]          INT     NOT NULL,
    [Products]      TINYINT NOT NULL,
    CONSTRAINT [PK_tblArms] PRIMARY KEY CLUSTERED ([Index] ASC),
    CONSTRAINT [FK_tblArms_tblAcculoads] FOREIGN KEY ([AcculoadIndex]) REFERENCES [dbo].[tblAcculoads] ([Index])
);

GO


/****** Object:  Trigger [dbo].[trg_Audit_ins_tblArms]   ******/

CREATE TRIGGER [trg_Audit_del_tblArms] ON [dbo].[tblArms] 
AFTER 
  DELETE AS BEGIN 
SET 
  NOCOUNT ON;
...
IF [Fuelsmanagerdb].[fmaudit].[udf_DisableTriggerByAuditRule]('dbo', 'tblArms', 'D')= 1 -- Notes: Verify this setting
RETURN 
...
SET   @_UserId = SUSER_NAME() 
  ...
  INSERT INTO [Fuelsmanagerdb].[fmaudit].[AcculoadOPC_tblArms]
  (
    [Index], 
	[AcculoadIndex], 
	[Number], 
...
CREATE TRIGGER [dbo].[trg_Audit_ins_tblArms] ON [dbo].[tblArms] 
AFTER INSERT AS 
BEGIN 
SET 
  NOCOUNT ON;
...
/****** Object:  Trigger [dbo].[trg_Audit_upd_tblArms]   ******/

CREATE TRIGGER [dbo].[trg_Audit_upd_tblArms] ON [dbo].[tblArms] 
AFTER 
UPDATE 
AS BEGIN 
SET 
...
-- Verifies whether the trigger is active based on configuration and Audit
-- rules which are resolved by the User Defined Function [fmaudit].[udf_DisableTriggerByAuditRule]
IF [FuelsManagerDB].[fmaudit].[udf_DisableTriggerByAuditRule]('dbo', 'tblArms', 'D')= 1 RETURN DECLARE @_AuditEventType CHAR(1), 
@_AuditEventSequence TINYINT, 
@_AuditSessionGUID UNIQUEIDENTIFIER, 
@_AuditSessionTokenID UNIQUEIDENTIFIER, 
@_AuditSiteGUID UNIQUEIDENTIFIER, 
