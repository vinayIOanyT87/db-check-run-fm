CREATE PROCEDURE [rpt].[usp_Nspa_OverAllocationReport_FuelCard]
(
...
IF @ViewingSiteID <> 'NSPA' RAISERROR ( 'Can only be run at NSPA site group', 16, 1 )
...
SELECT	f.ID,
		...
		a.AllocationLimit,
		a.QuantityDelivered
FROM	[rpt].[udf_Nspa_OverAllocation_Transaction](@StartDate, @EndDate) a INNER JOIN tblFuelCards f ON f.FuelCardGuid = a.FuelCardGuid
...
SELECT	f.ID,
		...
		a.AllocationLimit,
		a.QuantityDelivered
FROM	[rpt].[udf_Nspa_OverAllocation_Monthly](@StartDate, @EndDate) a INNER JOIN tblFuelCards f ON f.FuelCardGuid = a.FuelCardGuid
...
SELECT	f.ID,
		...
		a.AllocationLimit,
		a.QuantityDelivered
FROM	[rpt].[udf_Nspa_OverAllocation_Daily](@StartDate, @EndDate) a INNER JOIN tblFuelCards f ON f.FuelCardGuid = a.FuelCardGuid
...
SELECT	CardNumber,
		...
		BillTo,
		AllocationType,
		dbo.udf_ConvertFromSIUnits(SUM(ISNULL(AllocationLimit,0)), @VolumeUnits, @VolumeDecimalPlaces) AS AllocationLimit,
