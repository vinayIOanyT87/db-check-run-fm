-----------------------------------------------------------------------------
-- This stored procedure will rebuild indexes and update their statistics
-----------------------------------------------------------------------------
CREATE  PROCEDURE [maint].[usp_ReindexDatabase_UpdateStats]
AS
...
DECLARE @sqledition varchar(max)
DECLARE @sqledition1 varchar(max)
SET @sqledition =(SELECT CAST(SERVERPROPERTY('Edition') AS VARCHAR(30)))
SET @sqledition1 =( SELECT SUBSTRING(@sqledition,1,3))
...
BEGIN 
   ...
   BEGIN   
     ...
     SELECT
        ...
        CASE WHEN pst.avg_fragmentation_in_percent > 40 THEN 
        ''ALTER INDEX [''+idx.Name+''] ON [''+DB_NAME()+''].[''+SCHEMA_NAME (tbl.schema_id)+''].[''+tbl.name+''] REBUILD WITH (FILLFACTOR = '+@indexFillFactor+', SORT_IN_TEMPDB = '+@sortInTempdb+', STATISTICS_NORECOMPUTE = OFF,MAXDOP = 4);''
        WHEN pst.avg_fragmentation_in_percent > 15 AND pst.avg_fragmentation_in_percent <= 40 THEN 
   ...
   BEGIN
    ...
    INSERT INTO #tmpFragmentedIndexes (
     ...
     SELECT
       ...
       WAIT_AT_LOW_PRIORITY ( MAX_DURATION = 2 MINUTES, ABORT_AFTER_WAIT = SELF )),FILLFACTOR = '+@indexFillFactor+', SORT_IN_TEMPDB = '+@sortInTempdb+', STATISTICS_NORECOMPUTE = OFF);''
        WHEN pst.avg_fragmentation_in_percent > 15 AND pst.avg_fragmentation_in_percent <= 40 THEN 
        ''ALTER INDEX [''+idx.Name+''] ON [''+DB_NAME()+''].[''+SCHEMA_NAME (tbl.schema_id)+''].[''+tbl.name+''] REORGANIZE;''     
        ELSE
        NULL
     ...
     WHERE pst.index_id != 0  
      ...
      AND pst.avg_fragmentation_in_percent >= '+ @fragmentationThreshold + '
	  	AND page_count > 500
	  AND tbl.name <> ''tblAuditHandler'' ';  -- we cannot reindex the cluster index of the tblaudithandler in online because it has nvarchar(max) columns. This table should not change so it's not a problem
