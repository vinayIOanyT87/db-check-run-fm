CREATE FUNCTION [dbo].[udf_GetAssignmentTreeForEquipmentListForSite](
@sync_context_site_guid uniqueidentifier
)
RETURNS @tblEntityAssignmentTree TABLE
(
	[EquipmentToSiteGuid] [uniqueidentifier]
	,[TrackedEquipmentToSiteGuid] [uniqueidentifier]
...
BEGIN
	...
	-- Find any entity assigment that pertains to the specified sync_context_site_guid.  We're not looking for the specific EquipmentGuid that will be used, we're just interested in the
	-- entity assignment records.  If the AssignedFromSiteGuid equals the SiteGuid (AssignedTo) then the record is a self owned entity mapping so the EquipmentGuid is the real entity.
	INSERT INTO @entityAssignmentList
		SELECT [map].[tblEntityEquipmentToSite].[EquipmentToSiteGuid] 'EntityToSiteGuid'
				,CASE WHEN [map].[tblEntityEquipmentToSite].[AssignedFromSiteGuid] = [map].[tblEntityEquipmentToSite].[SiteGuid] THEN [map].[tblEntityEquipmentToSite].[EquipmentGuid] ELSE NULL END 'EntityGuid'
				,[map].[tblEntityEquipmentToSite].[EquipmentGuid] 'MasterRecordGuid'
				,[map].[tblEntityEquipmentToSite].[AssignedFromSiteGuid]
				,[map].[tblEntityEquipmentToSite].[SiteGuid] 'AssignedToSiteGuid'
	...
	INSERT INTO @tblEntityAssignmentTree SELECT EntityToSiteGuid, EntityToSiteGuid 'TrackedEquipmentToSiteGuid', IncludeChangeTrackingFlag FROM @entityAssignmentList WHERE AssignedToSiteGuid = AssignedFromSiteGuid;
	...
	BEGIN
		...
		BEGIN
			...
			INSERT INTO @tblEntityAssignmentTree SELECT @EntityToSiteGuid, @EntityToSiteGuid 'TrackedEquipmentToSiteGuid', @IncludeChangeTrackingFlag;
			...
			WHILE ((SELECT COUNT(*) FROM map.tblEntityEquipmentToSite WHERE EquipmentGuid = @MasterRecGuid AND SiteGuid = @targetSiteGuid) > 0)
			BEGIN
				SELECT @newEntityToSiteGuid = EquipmentToSiteGuid, @newTargetSiteGuid = AssignedFromSiteGuid FROM map.tblEntityEquipmentToSite WHERE EquipmentGuid = @MasterRecGuid AND SiteGuid = @targetSiteGuid
				...
				BEGIN
					...
					INSERT INTO @tblEntityAssignmentTree SELECT @newEntityToSiteGuid, @EntityToSiteGuid 'TrackedEquipmentToSiteGuid', @IncludeChangeTrackingFlag
