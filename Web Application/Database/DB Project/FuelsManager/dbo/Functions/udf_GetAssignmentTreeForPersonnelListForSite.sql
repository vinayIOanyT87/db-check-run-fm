CREATE FUNCTION [dbo].[udf_GetAssignmentTreeForPersonnelListForSite](
@sync_context_site_guid uniqueidentifier
)
RETURNS @tblEntityAssignmentTree TABLE
(
	[PersonnelToSiteGuid] [uniqueidentifier]
	,[TrackedPersonnelToSiteGuid] [uniqueidentifier]
...
BEGIN
	...
	-- Find any entity assigment that pertains to the specified sync_context_site_guid.  We're not looking for the specific PersonnelGuid that will be used, we're just interested in the
	-- entity assignment records.  If the AssignedFromSiteGuid equals the SiteGuid (AssignedTo) then the record is a self owned entity mapping so the PersonnelGuid is the real entity.
	INSERT INTO @entityAssignmentList
		SELECT [map].[tblEntityPersonnelToSite].[PersonnelToSiteGuid] 'EntityToSiteGuid'
				,CASE WHEN [map].[tblEntityPersonnelToSite].[AssignedFromSiteGuid] = [map].[tblEntityPersonnelToSite].[SiteGuid] THEN [map].[tblEntityPersonnelToSite].[PersonnelGuid] ELSE NULL END 'EntityGuid'
				,[map].[tblEntityPersonnelToSite].[PersonnelGuid] 'MasterRecordGuid'
				,[map].[tblEntityPersonnelToSite].[AssignedFromSiteGuid]
				,[map].[tblEntityPersonnelToSite].[SiteGuid] 'AssignedToSiteGuid'
	...
	INSERT INTO @tblEntityAssignmentTree SELECT EntityToSiteGuid, EntityToSiteGuid 'TrackedPersonnelToSiteGuid', IncludeChangeTrackingFlag FROM @entityAssignmentList WHERE AssignedToSiteGuid = AssignedFromSiteGuid;
	...
	BEGIN
		...
		BEGIN
			...
			INSERT INTO @tblEntityAssignmentTree SELECT @EntityToSiteGuid, @EntityToSiteGuid 'TrackedPersonnelToSiteGuid', @IncludeChangeTrackingFlag;
			...
			WHILE ((SELECT COUNT(*) FROM map.tblEntityPersonnelToSite WHERE PersonnelGuid = @MasterRecGuid AND SiteGuid = @targetSiteGuid) > 0)
			BEGIN
				SELECT @newEntityToSiteGuid = PersonnelToSiteGuid, @newTargetSiteGuid = AssignedFromSiteGuid FROM map.tblEntityPersonnelToSite WHERE PersonnelGuid = @MasterRecGuid AND SiteGuid = @targetSiteGuid
				...
				BEGIN
					...
					INSERT INTO @tblEntityAssignmentTree SELECT @newEntityToSiteGuid, @EntityToSiteGuid 'TrackedPersonnelToSiteGuid', @IncludeChangeTrackingFlag
