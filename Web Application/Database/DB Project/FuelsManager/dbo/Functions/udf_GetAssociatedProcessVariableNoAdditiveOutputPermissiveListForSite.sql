...
CREATE FUNCTION [dbo].[udf_GetAssociatedProcessVariableNoAdditiveOutputPermissiveListForSite](
	@sync_context_site_guid uniqueidentifier
)
RETURNS @tblProcessVariableNoAdditiveOutputPermissiveList TABLE
(
...
BEGIN
	...
	; WITH ProcessVariableNoAdditiveOutputPermissive_CTE ([ProcessVariableLoadArmGuid],[LoadArmGuid],[OPCConnectionGuid],[StationGuid],[OwnerSiteGuid])
	AS
	(
		SELECT [dbo].[tblProcessVariableNoAdditiveOutputPermissive].[ProcessVariableLoadArmGuid],[dbo].[tblProcessVariableNoAdditiveOutputPermissive].[LoadArmGuid],[dbo].[tblProcessVariableNoAdditiveOutputPermissive].[OPCConnectionGuid],data1.[StationGuid],data1.[OwnerSiteGuid]
			FROM [dbo].[tblProcessVariableNoAdditiveOutputPermissive]
				INNER JOIN (SELECT LoadArmGuid, StationGuid, OwnerSiteGuid FROM @tblLoadArmList) data1
					ON [dbo].[tblProcessVariableNoAdditiveOutputPermissive].[LoadArmGuid] = data1.[LoadArmGuid]
	)
	INSERT INTO @tblProcessVariableNoAdditiveOutputPermissiveList SELECT [ProcessVariableLoadArmGuid],[LoadArmGuid],[OPCConnectionGuid],[StationGuid],[OwnerSiteGuid] FROM ProcessVariableNoAdditiveOutputPermissive_CTE
