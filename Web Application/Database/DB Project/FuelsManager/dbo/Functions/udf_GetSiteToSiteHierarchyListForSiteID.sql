...
/* {CheckPoint: CREATING FUNCTION: udf_GetSiteToSiteHierarchyListForSiteID } */
CREATE FUNCTION [dbo].[udf_GetSiteToSiteHierarchyListForSiteID](
...
BEGIN
	...
	SELECT @SiteGuid = SiteGuid
			...
			, @DisableSyncTransferFlag = DisableSyncTransferFlag FROM dbo.tblSites WHERE ID = @SiteID;
	...
	BEGIN
		INSERT INTO @tblSiteTreeList (SiteGuid, SiteID, EnablePeriodicSyncFlag, PeriodicSyncIntervalMinutes, DisableSyncTransferFlag, Level, NodeType, SiteGroupFlag) VALUES ('00000000-0000-0000-0000-000000000001', 'SiteAdmin', 0, 0, 0, 0, 'Root', 1)
		...
		BEGIN
			...
			SELECT  CAST(@SiteGuid AS uniqueidentifier) AS SiteGuid
					...
					,CAST(@DisableSyncTransferFlag AS bit) AS DisableSyncTransferFlag
			END
		...
		BEGIN
			...
			SELECT  ChildSiteGuid AS SiteGuid
					...
					,CAST(parentSite.DisableSyncTransferFlag AS BIT) AS DisableSyncTransferFlag
		...
		BEGIN
			...
			BEGIN
				...
				SELECT  ParentSiteGuid AS SiteGuid
						...
						,CAST(childSite.DisableSyncTransferFlag AS bit) AS DisableSyncTransferFlag
					FROM map.tblSiteToSite map
							LEFT OUTER JOIN dbo.tblSites childSite
								ON map.ChildSiteGuid = childSite.SiteGuid
							LEFT OUTER JOIN dbo.tblSites parentSite
	...
	BEGIN
		...
		SELECT  CAST(NULL AS uniqueidentifier) AS SiteGuid
				...
				,CAST(NULL AS bit) AS DisableSyncTransferFlag
	...
	; WITH FlatSiteMap AS (
		SELECT ChildSiteGuid
				...
				FROM (SELECT s.SiteGuid AS SiteGuid
							...
							,s.DisableSyncTransferFlag AS DisableSyncTransferFlag
							FROM dbo.tblSites s
	...
	,RecursiveAncestors_CTE AS (
		...
		SELECT
			...
			,CAST((CASE WHEN grandParent.ParentSiteGuid IS NULL THEN grandParent.DisableSyncTransferFlag ELSE grandParent.DisableSyncTransferFlag END) AS bit) AS DisableSyncTransferFlag
	...
	,RecursiveDescendants_CTE AS (
		...
		SELECT
			...
			,CAST((CASE WHEN grandChild.ChildSiteGuid IS NULL THEN grandChild.DisableSyncTransferFlag ELSE grandChild.DisableSyncTransferFlag END) AS bit) AS DisableSyncTransferFlag
	...
	, SiteTree_CTE AS (
		...
		SELECT RelatedSiteGuid AS SiteGuid
				...
				,EnablePeriodicSyncFlag AS EnablePeriodicSyncFlag
				,PeriodicSyncIntervalMinutes AS PeriodicSyncIntervalMinutes
				,DisableSyncTransferFlag AS DisableSyncTransferFlag
				FROM RecursiveAncestors_CTE
		...
		SELECT cte.RelatedSiteGuid AS SiteGuid
				...
				,cte.DisableSyncTransferFlag AS DisableSyncTransferFlag
