CREATE FUNCTION [dbo].[udf_GetAssignmentTreeForTransactionAliasListForSite](
@sync_context_site_guid uniqueidentifier
)
RETURNS @tblEntityAssignmentTree TABLE
(
	[TransactionAliasToSiteGuid] [uniqueidentifier]
	,[TrackedTransactionAliasToSiteGuid] [uniqueidentifier]
...
BEGIN
	...
	-- Find any entity assigment that pertains to the specified sync_context_site_guid.  We're not looking for the specific TransactionAliasGuid that will be used, we're just interested in the
	-- entity assignment records.  If the AssignedFromSiteGuid equals the SiteGuid (AssignedTo) then the record is a self owned entity mapping so the TransactionAliasGuid is the real entity.
	INSERT INTO @entityAssignmentList
		SELECT [map].[tblEntityTransactionAliasToSite].[TransactionAliasToSiteGuid] 'EntityToSiteGuid'
				,CASE WHEN [map].[tblEntityTransactionAliasToSite].[AssignedFromSiteGuid] = [map].[tblEntityTransactionAliasToSite].[SiteGuid] THEN [map].[tblEntityTransactionAliasToSite].[TransactionAliasGuid] ELSE NULL END 'EntityGuid'
				,[map].[tblEntityTransactionAliasToSite].[TransactionAliasGuid] 'MasterRecordGuid'
				,[map].[tblEntityTransactionAliasToSite].[AssignedFromSiteGuid]
				,[map].[tblEntityTransactionAliasToSite].[SiteGuid] 'AssignedToSiteGuid'
	...
	INSERT INTO @tblEntityAssignmentTree SELECT EntityToSiteGuid, EntityToSiteGuid 'TrackedTransactionAliasToSiteGuid', IncludeChangeTrackingFlag FROM @entityAssignmentList WHERE AssignedToSiteGuid = AssignedFromSiteGuid;
	...
	BEGIN
		...
		BEGIN
			...
			INSERT INTO @tblEntityAssignmentTree SELECT @EntityToSiteGuid, @EntityToSiteGuid 'TrackedTransactionAliasToSiteGuid', @IncludeChangeTrackingFlag;
			...
			WHILE ((SELECT COUNT(*) FROM map.tblEntityTransactionAliasToSite WHERE TransactionAliasGuid = @MasterRecGuid AND SiteGuid = @targetSiteGuid) > 0)
			BEGIN
				SELECT @newEntityToSiteGuid = TransactionAliasToSiteGuid, @newTargetSiteGuid = AssignedFromSiteGuid FROM map.tblEntityTransactionAliasToSite WHERE TransactionAliasGuid = @MasterRecGuid AND SiteGuid = @targetSiteGuid
				...
				BEGIN
					...
					INSERT INTO @tblEntityAssignmentTree SELECT @newEntityToSiteGuid, @EntityToSiteGuid 'TrackedTransactionAliasToSiteGuid', @IncludeChangeTrackingFlag
