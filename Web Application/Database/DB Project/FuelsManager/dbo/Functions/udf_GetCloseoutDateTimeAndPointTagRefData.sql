CREATE FUNCTION dbo.udf_GetCloseoutDateTimeAndPointTagRefData(
@SiteGuid UNIQUEIDENTIFIER, 
@Date DATE)
RETURNS @Closeout TABLE 
(
    -- Columns returned by the function
	 CloseoutDateTime DATETIMEOFFSET NOT NULL,
	 PointTagRefData XML NOT NULL
)
AS
BEGIN
	DECLARE @CloseoutDateTime DATETIMEOFFSET 
	DECLARE @CloseoutTime TIME = NULL
	DECLARE @PointTagRefDataXML XML = NULL
	...
	ELSE
	BEGIN
		SELECT @CloseoutTime=CloseoutTime, @PointTagRefDataXML=PointTagRefDataAsXML FROM tblSiteCloseoutTime  WHERE SiteGuid=@SiteGuid AND  @DateTimeOffset BETWEEN EffectiveDate AND ExpirationDate
	END
	...
	IF @PointTagRefDataXML IS NULL
	BEGIN
		SET @PointTagRefDataXML = rpt.udf_GetPointTagRefDataAsXML(@SiteGuid)
	END
	...
	INSERT @Closeout
   SELECT @CloseoutDateTime, @PointTagRefDataXML
	RETURN
END