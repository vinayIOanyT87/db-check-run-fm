CREATE FUNCTION [dbo].[udf_GetAssignmentTreeForProductListForSite](
@sync_context_site_guid uniqueidentifier
)
RETURNS @tblEntityAssignmentTree TABLE
(
	[ProductToSiteGuid] [uniqueidentifier]
	,[TrackedProductToSiteGuid] [uniqueidentifier]
...
BEGIN
	...
	-- Find any entity assigment that pertains to the specified sync_context_site_guid.  We're not looking for the specific ProductGuid that will be used, we're just interested in the
	-- entity assignment records.  If the AssignedFromSiteGuid equals the SiteGuid (AssignedTo) then the record is a self owned entity mapping so the ProductGuid is the real entity.
	INSERT INTO @entityAssignmentList
		SELECT [map].[tblEntityProductToSite].[ProductToSiteGuid] 'EntityToSiteGuid'
				,CASE WHEN [map].[tblEntityProductToSite].[AssignedFromSiteGuid] = [map].[tblEntityProductToSite].[SiteGuid] THEN [map].[tblEntityProductToSite].[ProductGuid] ELSE NULL END 'EntityGuid'
				,[map].[tblEntityProductToSite].[ProductGuid] 'MasterRecordGuid'
				,[map].[tblEntityProductToSite].[AssignedFromSiteGuid]
				,[map].[tblEntityProductToSite].[SiteGuid] 'AssignedToSiteGuid'
	...
	INSERT INTO @tblEntityAssignmentTree SELECT EntityToSiteGuid, EntityToSiteGuid 'TrackedProductToSiteGuid', IncludeChangeTrackingFlag FROM @entityAssignmentList WHERE AssignedToSiteGuid = AssignedFromSiteGuid;
	...
	BEGIN
		...
		BEGIN
			...
			INSERT INTO @tblEntityAssignmentTree SELECT @EntityToSiteGuid, @EntityToSiteGuid 'TrackedProductToSiteGuid', @IncludeChangeTrackingFlag;
			...
			WHILE ((SELECT COUNT(*) FROM map.tblEntityProductToSite WHERE ProductGuid = @MasterRecGuid AND SiteGuid = @targetSiteGuid) > 0)
			BEGIN
				SELECT @newEntityToSiteGuid = ProductToSiteGuid, @newTargetSiteGuid = AssignedFromSiteGuid FROM map.tblEntityProductToSite WHERE ProductGuid = @MasterRecGuid AND SiteGuid = @targetSiteGuid
				...
				BEGIN
					...
					INSERT INTO @tblEntityAssignmentTree SELECT @newEntityToSiteGuid, @EntityToSiteGuid 'TrackedProductToSiteGuid', @IncludeChangeTrackingFlag
