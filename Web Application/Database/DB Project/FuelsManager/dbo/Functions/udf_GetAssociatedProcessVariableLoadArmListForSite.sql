...
CREATE FUNCTION [dbo].[udf_GetAssociatedProcessVariableLoadArmListForSite](
	@sync_context_site_guid uniqueidentifier
)
RETURNS @tblProcessVariableLoadArmList TABLE
(
...
BEGIN
	...
	; WITH ProcessVariableLoadArm_CTE ([ProcessVariableLoadArmGuid],[LoadArmGuid],[OPCConnectionGuid],[StationGuid],[OwnerSiteGuid])
	AS
	(
		SELECT [dbo].[tblProcessVariableLoadArm].[ProcessVariableLoadArmGuid],[dbo].[tblProcessVariableLoadArm].[LoadArmGuid],[dbo].[tblProcessVariableLoadArm].[OPCConnectionGuid],data1.[StationGuid],data1.[OwnerSiteGuid]
			FROM [dbo].[tblProcessVariableLoadArm]
				INNER JOIN (SELECT LoadArmGuid, StationGuid, OwnerSiteGuid FROM @tblLoadArmList) data1
					ON [dbo].[tblProcessVariableLoadArm].[LoadArmGuid] = data1.[LoadArmGuid]
	)
	INSERT INTO @tblProcessVariableLoadArmList SELECT [ProcessVariableLoadArmGuid],[LoadArmGuid],[OPCConnectionGuid],[StationGuid],[OwnerSiteGuid] FROM ProcessVariableLoadArm_CTE
