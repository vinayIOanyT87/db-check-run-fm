CREATE PROCEDURE [dbo].[gsp_QualityTagsUpdateByPK]
(
	...
	,	@NullOverrideSeverity BIT=0 
	,	@NullOverrideActive BIT=0 
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_QualityTagsUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:29.7124184 -05:00
	-- Purpose: Update table [dbo].[tblQualityTags]
	-- Notes:
	-- 1. @QualityTagGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblQualityTags] WHERE QualityTagGuid=@QualityTagGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblQualityTags] SET
			[Name]=(CASE ISNULL(@NullOverrideName,0) WHEN 1 THEN @Name ELSE ISNULL(@Name,[Name]) END)
		,	[Severity]=(CASE ISNULL(@NullOverrideSeverity,0) WHEN 1 THEN @Severity ELSE ISNULL(@Severity,[Severity]) END)
		,	[Active]=(CASE ISNULL(@NullOverrideActive,0) WHEN 1 THEN @Active ELSE ISNULL(@Active,[Active]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_QualityTagsUpdateByPK' + CHAR(13)+CHAR(10)                  
