CREATE PROCEDURE [dbo].[gsp_GatesUpdateByPK]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_GatesUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:29.5233995 -05:00
	-- Purpose: Update table [dbo].[tblGates]
	-- Notes:
	-- 1. @GateGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblGates] WHERE GateGuid=@GateGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblGates] SET
		...
		,	[ConcourseID]=(CASE ISNULL(@NullOverrideConcourseID,0) WHEN 1 THEN @ConcourseID ELSE ISNULL(@ConcourseID,[ConcourseID]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_GatesUpdateByPK' + CHAR(13)+CHAR(10)                  
