CREATE PROCEDURE [dbo].[gsp_SiteAdminDeleteBySiteGuid](@SiteGuid uniqueidentifier,@DetachOnly BIT=NULL,@SwapToGuid uniqueidentifier=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblSiteAdmin] WHERE [SiteGuid] = @SiteGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
				 RAISERROR('Attempted to delete a stale version of SiteAdmin.',18,1); 
				 RETURN; 
			END 
		END 
		--
		...
		BEGIN
			...
			ELSE
			BEGIN
				DELETE [dbo].[tblSiteAdmin] WHERE [SiteGuid] = @SiteGuid; 
			END
		END
	END TRY
	BEGIN CATCH
