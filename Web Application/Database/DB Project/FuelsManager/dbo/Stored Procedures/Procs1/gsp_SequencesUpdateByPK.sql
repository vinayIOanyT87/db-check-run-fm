CREATE PROCEDURE [dbo].[gsp_SequencesUpdateByPK]
(
		@SequenceGuid uniqueidentifier
	,	@SequenceKey nvarchar(30)=NULL
	,	@SequenceValue bigint=NULL
	,	@_RowVersion timestamp=NULL OUTPUT
	,	@NullOverrideSequenceKey BIT=0 
	,	@NullOverrideSequenceValue BIT=0 
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_SequencesUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:29.7444216 -05:00
	-- Purpose: Update table [dbo].[tblSequences]
	-- Notes:
	-- 1. @SequenceGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblSequences] WHERE SequenceGuid=@SequenceGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblSequences] SET
			[SequenceKey]=(CASE ISNULL(@NullOverrideSequenceKey,0) WHEN 1 THEN @SequenceKey ELSE ISNULL(@SequenceKey,[SequenceKey]) END)
		,	[SequenceValue]=(CASE ISNULL(@NullOverrideSequenceValue,0) WHEN 1 THEN @SequenceValue ELSE ISNULL(@SequenceValue,[SequenceValue]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_SequencesUpdateByPK' + CHAR(13)+CHAR(10)                  
