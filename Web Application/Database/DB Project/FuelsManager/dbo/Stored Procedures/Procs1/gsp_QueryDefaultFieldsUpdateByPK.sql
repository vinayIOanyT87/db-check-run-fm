CREATE PROCEDURE [dbo].[gsp_QueryDefaultFieldsUpdateByPK]
(
		@QueryDefaultFieldGuid uniqueidentifier
	...
	,	@NullOverrideTopic BIT=0 
	,	@NullOverrideFieldName BIT=0 
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_QueryDefaultFieldsUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:29.7144186 -05:00
	-- Purpose: Update table [dbo].[tblQueryDefaultFields]
	-- Notes:
	-- 1. @QueryDefaultFieldGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblQueryDefaultFields] WHERE QueryDefaultFieldGuid=@QueryDefaultFieldGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblQueryDefaultFields] SET
			[Topic]=(CASE ISNULL(@NullOverrideTopic,0) WHEN 1 THEN @Topic ELSE ISNULL(@Topic,[Topic]) END)
		,	[FieldName]=(CASE ISNULL(@NullOverrideFieldName,0) WHEN 1 THEN @FieldName ELSE ISNULL(@FieldName,[FieldName]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_QueryDefaultFieldsUpdateByPK' + CHAR(13)+CHAR(10)                  
