CREATE PROCEDURE [dbo].[gsp_UserDataFieldEquipmentDeleteByTransactionAliasGuid](@TransactionAliasGuid uniqueidentifier,@DetachOnly BIT=NULL,@SwapToGuid uniqueidentifier=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblUserDataFieldEquipment] WHERE [TransactionAliasGuid] = @TransactionAliasGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		BEGIN
			...
			ELSE
			BEGIN
				DELETE [dbo].[tblUserDataFieldEquipment] WHERE [TransactionAliasGuid] = @TransactionAliasGuid; 
			END
		END
	END TRY
	BEGIN CATCH
