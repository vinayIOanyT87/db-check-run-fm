CREATE PROCEDURE [dbo].[gsp_CompanyCrossReferenceUpdateByPK]
(
	...
	,	@NullOverrideCompanyCrossReferenceGuid BIT=0 
	,	@NullOverrideReferenceTypeIndex BIT=0 
	,	@NullOverrideUpdatedDate BIT=0 
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_CompanyCrossReferenceUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:29.4483920 -05:00
	-- Purpose: Update table [dbo].[tblCompanyCrossReference]
	-- Notes:
	-- 1. @ReferenceName and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblCompanyCrossReference] WHERE ReferenceName=@ReferenceName AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblCompanyCrossReference] SET
			[CompanyCrossReferenceGuid]=(CASE ISNULL(@NullOverrideCompanyCrossReferenceGuid,0) WHEN 1 THEN @CompanyCrossReferenceGuid ELSE ISNULL(@CompanyCrossReferenceGuid,[CompanyCrossReferenceGuid]) END)
		,	[ReferenceTypeIndex]=(CASE ISNULL(@NullOverrideReferenceTypeIndex,0) WHEN 1 THEN @ReferenceTypeIndex ELSE ISNULL(@ReferenceTypeIndex,[ReferenceTypeIndex]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_CompanyCrossReferenceUpdateByPK' + CHAR(13)+CHAR(10)                  
