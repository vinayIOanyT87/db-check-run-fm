CREATE PROCEDURE [dbo].[gsp_IATAUpdateByPK]
(
	...
	,	@NullOverrideIATAID BIT=0 
	,	@NullOverrideName BIT=0 
	,	@NullOverrideCountryID BIT=0 
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_IATAUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:29.5384010 -05:00
	-- Purpose: Update table [dbo].[tblIATA]
	-- Notes:
	-- 1. @IATAGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblIATA] WHERE IATAGuid=@IATAGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblIATA] SET
			[IATAID]=(CASE ISNULL(@NullOverrideIATAID,0) WHEN 1 THEN @IATAID ELSE ISNULL(@IATAID,[IATAID]) END)
		,	[Name]=(CASE ISNULL(@NullOverrideName,0) WHEN 1 THEN @Name ELSE ISNULL(@Name,[Name]) END)
		,	[CountryID]=(CASE ISNULL(@NullOverrideCountryID,0) WHEN 1 THEN @CountryID ELSE ISNULL(@CountryID,[CountryID]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_IATAUpdateByPK' + CHAR(13)+CHAR(10)                  
