CREATE PROCEDURE [dbo].[gsp_GSTDeleteByRowGuid](@GSTGuid UNIQUEIDENTIFIER,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblGST] WHERE [GSTGuid] = @GSTGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
				 RAISERROR('Attempted to delete a stale version of GST.',18,1); 
				 RETURN; 
			END 
		END 
		DELETE [dbo].[tblGST] WHERE [GSTGuid] = @GSTGuid; 
	END TRY
	BEGIN CATCH
