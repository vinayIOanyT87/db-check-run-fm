CREATE PROCEDURE [dbo].[gsp_QueryDefaultsUpdateByPK]
(
	...
	,	@NullOverrideHeader BIT=0 
	,	@NullOverrideFooter BIT=0 
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_QueryDefaultsUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:29.7164188 -05:00
	-- Purpose: Update table [dbo].[tblQueryDefaults]
	-- Notes:
	-- 1. @QueryDefaultGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblQueryDefaults] WHERE QueryDefaultGuid=@QueryDefaultGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblQueryDefaults] SET
			[Header]=(CASE ISNULL(@NullOverrideHeader,0) WHEN 1 THEN @Header ELSE ISNULL(@Header,[Header]) END)
		,	[Footer]=(CASE ISNULL(@NullOverrideFooter,0) WHEN 1 THEN @Footer ELSE ISNULL(@Footer,[Footer]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_QueryDefaultsUpdateByPK' + CHAR(13)+CHAR(10)                  
