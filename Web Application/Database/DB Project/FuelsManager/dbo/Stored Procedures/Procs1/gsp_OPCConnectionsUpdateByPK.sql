CREATE PROCEDURE [dbo].[gsp_OPCConnectionsUpdateByPK]
(
		@OPCConnectionGuid uniqueidentifier
	,	@URL nvarchar(100)=NULL
	,	@ProgID nvarchar(50)=NULL
	,	@_RowVersion timestamp=NULL OUTPUT
	,	@NullOverrideURL BIT=0 
	,	@NullOverrideProgID BIT=0 
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_OPCConnectionsUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:29.6014073 -05:00
	-- Purpose: Update table [dbo].[tblOPCConnections]
	-- Notes:
	-- 1. @OPCConnectionGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblOPCConnections] WHERE OPCConnectionGuid=@OPCConnectionGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblOPCConnections] SET
			[URL]=(CASE ISNULL(@NullOverrideURL,0) WHEN 1 THEN @URL ELSE ISNULL(@URL,[URL]) END)
		,	[ProgID]=(CASE ISNULL(@NullOverrideProgID,0) WHEN 1 THEN @ProgID ELSE ISNULL(@ProgID,[ProgID]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_OPCConnectionsUpdateByPK' + CHAR(13)+CHAR(10)                  
