CREATE PROCEDURE [dbo].[gsp_PointTagDeleteByRowGuid](@PointTagGuid UNIQUEIDENTIFIER,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblPointTag] WHERE [PointTagGuid] = @PointTagGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
				 RAISERROR('Attempted to delete a stale version of PointTag.',18,1); 
				 RETURN; 
			END 
		END 
		DELETE [dbo].[tblPointTag] WHERE [PointTagGuid] = @PointTagGuid; 
	END TRY
	BEGIN CATCH
