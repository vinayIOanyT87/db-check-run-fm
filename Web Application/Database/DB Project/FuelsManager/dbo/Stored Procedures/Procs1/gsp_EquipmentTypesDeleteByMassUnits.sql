CREATE PROCEDURE [dbo].[gsp_EquipmentTypesDeleteByMassUnits](@MassUnits int,@DetachOnly BIT=NULL,@SwapToIndex int=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblEquipmentTypes] WHERE [MassUnits] = @MassUnits AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		END 
		--
		-- REPLACE MassUnits PER @SwapToGuid IF @SwapToGuid IS NOT NULL
		--
		IF NOT @SwapToIndex IS NULL
		BEGIN
			UPDATE [dbo].[tblEquipmentTypes]
			SET [MassUnits]=@SwapToIndex
			WHERE [MassUnits] = @MassUnits;
		END
		ELSE
		BEGIN
			...
			ELSE
			BEGIN
				UPDATE [dbo].[tblEquipmentTypes]SET [DeleteFlag]=1  WHERE [MassUnits] = @MassUnits; 
			END
		END
	END TRY
	BEGIN CATCH
