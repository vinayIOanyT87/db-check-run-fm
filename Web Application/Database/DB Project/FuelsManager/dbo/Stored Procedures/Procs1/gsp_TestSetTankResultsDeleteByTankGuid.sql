CREATE PROCEDURE [dbo].[gsp_TestSetTankResultsDeleteByTankGuid](@TankGuid uniqueidentifier,@DetachOnly BIT=NULL,@SwapToGuid uniqueidentifier=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblTestSetTankResults] WHERE [TankGuid] = @TankGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		BEGIN
			...
			ELSE
			BEGIN
				UPDATE [dbo].[tblTestSetTankResults]SET [DeleteFlag]=1  WHERE [TankGuid] = @TankGuid; 
			END
		END
	END TRY
	BEGIN CATCH
