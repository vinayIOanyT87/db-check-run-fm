CREATE PROCEDURE [dbo].[gsp_DataDictionariesUpdateByPK]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_DataDictionariesUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:29.4623934 -05:00
	-- Purpose: Update table [dbo].[tblDataDictionaries]
	-- Notes:
	-- 1. @DataDictionaryGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblDataDictionaries] WHERE DataDictionaryGuid=@DataDictionaryGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblDataDictionaries] SET
			[Key]=(CASE ISNULL(@NullOverrideKey,0) WHEN 1 THEN @Key ELSE ISNULL(@Key,[Key]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_DataDictionariesUpdateByPK' + CHAR(13)+CHAR(10)                  
