CREATE PROCEDURE [dbo].[gsp_MeterDeleteByRowGuid](@MeterGuid UNIQUEIDENTIFIER,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblMeter] WHERE [MeterGuid] = @MeterGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
				 RAISERROR('Attempted to delete a stale version of Meter.',18,1); 
				 RETURN; 
			END 
		END 
		DELETE [dbo].[tblMeter] WHERE [MeterGuid] = @MeterGuid; 
	END TRY
	BEGIN CATCH
