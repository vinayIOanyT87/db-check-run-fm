CREATE PROCEDURE [dbo].[gsp_TankQualityTagLogDeleteByRowGuid](@TankQualityTagLogGuid UNIQUEIDENTIFIER,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblTankQualityTagLog] WHERE [TankQualityTagLogGuid] = @TankQualityTagLogGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
				 RAISERROR('Attempted to delete a stale version of TankQualityTagLog.',18,1); 
				 RETURN; 
			END 
		END 
		UPDATE [dbo].[tblTankQualityTagLog]SET [DeleteFlag]=1  WHERE [TankQualityTagLogGuid] = @TankQualityTagLogGuid; 
	END TRY
	BEGIN CATCH
