CREATE PROCEDURE [dbo].[gsp_ProcessVariablePresetInjectorDeleteByOPCConnectionGuid](@OPCConnectionGuid uniqueidentifier,@DetachOnly BIT=NULL,@SwapToGuid uniqueidentifier=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblProcessVariablePresetInjector] WHERE [OPCConnectionGuid] = @OPCConnectionGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		BEGIN
			...
			ELSE
			BEGIN
				DELETE [dbo].[tblProcessVariablePresetInjector] WHERE [OPCConnectionGuid] = @OPCConnectionGuid; 
			END
		END
	END TRY
	BEGIN CATCH
