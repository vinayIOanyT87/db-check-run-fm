CREATE PROCEDURE [dbo].[gsp_StandingOffersDeleteByRowGuid](@StandingOfferGuid UNIQUEIDENTIFIER,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblStandingOffers] WHERE [StandingOfferGuid] = @StandingOfferGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
				 RAISERROR('Attempted to delete a stale version of StandingOffers.',18,1); 
				 RETURN; 
			END 
		END 
		DELETE [dbo].[tblStandingOffers] WHERE [StandingOfferGuid] = @StandingOfferGuid; 
	END TRY
	BEGIN CATCH
