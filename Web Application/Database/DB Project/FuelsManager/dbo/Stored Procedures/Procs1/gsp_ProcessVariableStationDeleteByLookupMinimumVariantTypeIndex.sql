CREATE PROCEDURE [dbo].[gsp_ProcessVariableStationDeleteByLookupMinimumVariantTypeIndex](@LookupMinimumVariantTypeIndex int,@DetachOnly BIT=NULL,@SwapToIndex int=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblProcessVariableStation] WHERE [LookupMinimumVariantTypeIndex] = @LookupMinimumVariantTypeIndex AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		BEGIN
			...
			ELSE
			BEGIN
				DELETE [dbo].[tblProcessVariableStation] WHERE [LookupMinimumVariantTypeIndex] = @LookupMinimumVariantTypeIndex; 
			END
		END
	END TRY
	BEGIN CATCH
