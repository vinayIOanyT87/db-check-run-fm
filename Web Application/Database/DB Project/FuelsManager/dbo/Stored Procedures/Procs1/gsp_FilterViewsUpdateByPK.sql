CREATE PROCEDURE [dbo].[gsp_FilterViewsUpdateByPK]
(
	...
	,	@NullOverrideLookupFilterFieldIndex BIT=0 
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_FilterViewsUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:29.5183990 -05:00
	-- Purpose: Update table [dbo].[tblFilterViews]
	-- Notes:
	-- 1. @FilterViewGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblFilterViews] WHERE FilterViewGuid=@FilterViewGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblFilterViews] SET
		...
		,	[LookupFilterFieldIndex]=(CASE ISNULL(@NullOverrideLookupFilterFieldIndex,0) WHEN 1 THEN @LookupFilterFieldIndex ELSE ISNULL(@LookupFilterFieldIndex,[LookupFilterFieldIndex]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_FilterViewsUpdateByPK' + CHAR(13)+CHAR(10)                  
