CREATE PROCEDURE [dbo].[gsp_ExportTransportModeMappingUpdateByPK]
(
		@FMATransportMode nvarchar(100)
	,	@ExportTransportModeMappingGuid uniqueidentifier=NULL
	,	@FuelPlusTransportMode nvarchar(100)=NULL
	,	@UpdatedDate datetimeoffset(7)=NULL
	,	@UpdatedBy udtUserID=NULL
	,	@_RowVersion timestamp=NULL OUTPUT
	,	@NullOverrideExportTransportModeMappingGuid BIT=0 
	,	@NullOverrideFuelPlusTransportMode BIT=0 
	,	@NullOverrideUpdatedDate BIT=0 
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_ExportTransportModeMappingUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:29.5163988 -05:00
	-- Purpose: Update table [dbo].[tblExportTransportModeMapping]
	-- Notes:
	-- 1. @FMATransportMode and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblExportTransportModeMapping] WHERE FMATransportMode=@FMATransportMode AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblExportTransportModeMapping] SET
			[ExportTransportModeMappingGuid]=(CASE ISNULL(@NullOverrideExportTransportModeMappingGuid,0) WHEN 1 THEN @ExportTransportModeMappingGuid ELSE ISNULL(@ExportTransportModeMappingGuid,[ExportTransportModeMappingGuid]) END)
		,	[FuelPlusTransportMode]=(CASE ISNULL(@NullOverrideFuelPlusTransportMode,0) WHEN 1 THEN @FuelPlusTransportMode ELSE ISNULL(@FuelPlusTransportMode,[FuelPlusTransportMode]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_ExportTransportModeMappingUpdateByPK' + CHAR(13)+CHAR(10)                  
