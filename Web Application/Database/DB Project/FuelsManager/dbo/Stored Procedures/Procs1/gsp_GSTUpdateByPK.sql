CREATE PROCEDURE [dbo].[gsp_GSTUpdateByPK]
(
		@GSTGuid uniqueidentifier
	...
	,	@NullOverrideGSTCode BIT=0 
	,	@NullOverrideGSTValue BIT=0 
	,	@NullOverrideGSTDate BIT=0 
	,	@NullOverrideUpdatedDate BIT=0 
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_GSTUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:29.5324004 -05:00
	-- Purpose: Update table [dbo].[tblGST]
	-- Notes:
	-- 1. @GSTGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblGST] WHERE GSTGuid=@GSTGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblGST] SET
			[GSTCode]=(CASE ISNULL(@NullOverrideGSTCode,0) WHEN 1 THEN @GSTCode ELSE ISNULL(@GSTCode,[GSTCode]) END)
		,	[GSTValue]=(CASE ISNULL(@NullOverrideGSTValue,0) WHEN 1 THEN @GSTValue ELSE ISNULL(@GSTValue,[GSTValue]) END)
		,	[GSTDate]=(CASE ISNULL(@NullOverrideGSTDate,0) WHEN 1 THEN @GSTDate ELSE ISNULL(@GSTDate,[GSTDate]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_GSTUpdateByPK' + CHAR(13)+CHAR(10)                  
