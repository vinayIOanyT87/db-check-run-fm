CREATE PROCEDURE [dbo].[gsp_ConfigurationSettingUpdateByPK]
(
	...
	,	@NullOverrideKeyType BIT=0 
	,	@NullOverrideSettingKey BIT=0 
	,	@NullOverrideSettingValue BIT=0 
	,	@NullOverrideUpdatedDate BIT=0 
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_ConfigurationSettingUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:29.4503922 -05:00
	-- Purpose: Update table [dbo].[tblConfigurationSetting]
	-- Notes:
	-- 1. @ConfigurationSettingGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblConfigurationSetting] WHERE ConfigurationSettingGuid=@ConfigurationSettingGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblConfigurationSetting] SET
			[KeyType]=(CASE ISNULL(@NullOverrideKeyType,0) WHEN 1 THEN @KeyType ELSE ISNULL(@KeyType,[KeyType]) END)
		,	[SettingKey]=(CASE ISNULL(@NullOverrideSettingKey,0) WHEN 1 THEN @SettingKey ELSE ISNULL(@SettingKey,[SettingKey]) END)
		,	[SettingValue]=(CASE ISNULL(@NullOverrideSettingValue,0) WHEN 1 THEN @SettingValue ELSE ISNULL(@SettingValue,[SettingValue]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_ConfigurationSettingUpdateByPK' + CHAR(13)+CHAR(10)                  
