CREATE PROCEDURE [dbo].[gsp_SRMAdaptorUpdateByPK]
(
	...
	,	@NullOverrideSRMAdaptorName BIT=0 
	,	@NullOverrideIsEnabled BIT=0 
	,	@NullOverrideCustomWebApplicationPage BIT=0 
	,	@NullOverrideUpdatedDate BIT=0 
	,	@NullOverrideDuplicateCount BIT=0 
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_SRMAdaptorUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:29.7834255 -05:00
	-- Purpose: Update table [dbo].[tblSRMAdaptor]
	-- Notes:
	-- 1. @SRMAdaptorGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblSRMAdaptor] WHERE SRMAdaptorGuid=@SRMAdaptorGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblSRMAdaptor] SET
			[SRMAdaptorName]=(CASE ISNULL(@NullOverrideSRMAdaptorName,0) WHEN 1 THEN @SRMAdaptorName ELSE ISNULL(@SRMAdaptorName,[SRMAdaptorName]) END)
		,	[IsEnabled]=(CASE ISNULL(@NullOverrideIsEnabled,0) WHEN 1 THEN @IsEnabled ELSE ISNULL(@IsEnabled,[IsEnabled]) END)
		,	[CustomWebApplicationPage]=(CASE ISNULL(@NullOverrideCustomWebApplicationPage,0) WHEN 1 THEN @CustomWebApplicationPage ELSE ISNULL(@CustomWebApplicationPage,[CustomWebApplicationPage]) END)
		,	[UpdatedDate]=ISNULL(@UpdatedDate,SYSDATETIMEOFFSET())
		,	[UpdatedBy]= ISNULL(@UpdatedBy,SUSER_SNAME())
		,	[DuplicateCount]=(CASE ISNULL(@NullOverrideDuplicateCount,0) WHEN 1 THEN @DuplicateCount ELSE ISNULL(@DuplicateCount,[DuplicateCount]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_SRMAdaptorUpdateByPK' + CHAR(13)+CHAR(10)                  
