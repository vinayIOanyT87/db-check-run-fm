CREATE PROCEDURE [dbo].[gsp_ProductsDeleteByVolumeUnitIndex](@VolumeUnitIndex int,@DetachOnly BIT=NULL,@SwapToIndex int=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblProducts] WHERE [VolumeUnitIndex] = @VolumeUnitIndex AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		BEGIN
			...
			ELSE
			BEGIN
				DELETE [dbo].[tblProducts] WHERE [VolumeUnitIndex] = @VolumeUnitIndex; 
			END
		END
	END TRY
	BEGIN CATCH
