CREATE PROCEDURE [dbo].[gsp_TransactionsDeleteByRowGuid](@TransactionGuid UNIQUEIDENTIFIER,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblTransactions] WHERE [TransactionGuid] = @TransactionGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
				 RAISERROR('Attempted to delete a stale version of Transactions.',18,1); 
				 RETURN; 
			END 
		END 
		UPDATE [dbo].[tblTransactions]SET [DeleteFlag]=1  WHERE [TransactionGuid] = @TransactionGuid; 
	END TRY
	BEGIN CATCH
