CREATE PROCEDURE [dbo].[gsp_UserDataFieldFuelCardDeleteByRowGuid](@UserDataFieldFuelCardGuid UNIQUEIDENTIFIER,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblUserDataFieldFuelCard] WHERE [UserDataFieldFuelCardGuid] = @UserDataFieldFuelCardGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
				 RAISERROR('Attempted to delete a stale version of UserDataFieldFuelCard.',18,1); 
				 RETURN; 
			END 
		END 
		DELETE [dbo].[tblUserDataFieldFuelCard] WHERE [UserDataFieldFuelCardGuid] = @UserDataFieldFuelCardGuid; 
	END TRY
	BEGIN CATCH
