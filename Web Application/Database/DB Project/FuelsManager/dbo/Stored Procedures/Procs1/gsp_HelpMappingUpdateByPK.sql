CREATE PROCEDURE [dbo].[gsp_HelpMappingUpdateByPK]
(
		@HelpMappingGuid uniqueidentifier
	,	@HelpContextKey nvarchar(250)=NULL
	,	@HelpPage nvarchar(250)=NULL
	,	@UpdatedDate datetimeoffset(7)=NULL
	,	@UpdatedBy udtUserID=NULL
	,	@_RowVersion timestamp=NULL OUTPUT
	,	@NullOverrideHelpContextKey BIT=0 
	,	@NullOverrideHelpPage BIT=0 
	,	@NullOverrideUpdatedDate BIT=0 
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_HelpMappingUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:29.5344006 -05:00
	-- Purpose: Update table [dbo].[tblHelpMapping]
	-- Notes:
	-- 1. @HelpMappingGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblHelpMapping] WHERE HelpMappingGuid=@HelpMappingGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblHelpMapping] SET
			[HelpContextKey]=(CASE ISNULL(@NullOverrideHelpContextKey,0) WHEN 1 THEN @HelpContextKey ELSE ISNULL(@HelpContextKey,[HelpContextKey]) END)
		,	[HelpPage]=(CASE ISNULL(@NullOverrideHelpPage,0) WHEN 1 THEN @HelpPage ELSE ISNULL(@HelpPage,[HelpPage]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_HelpMappingUpdateByPK' + CHAR(13)+CHAR(10)                  
