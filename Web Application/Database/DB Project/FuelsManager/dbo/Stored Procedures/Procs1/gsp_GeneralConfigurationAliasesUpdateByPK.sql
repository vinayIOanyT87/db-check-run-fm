CREATE PROCEDURE [dbo].[gsp_GeneralConfigurationAliasesUpdateByPK]
(
	...
	,	@NullOverrideGeneralConfigurationGuid BIT=0 
	,	@NullOverrideTransactionAliasGuid BIT=0 
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_GeneralConfigurationAliasesUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:29.5284000 -05:00
	-- Purpose: Update table [dbo].[tblGeneralConfigurationAliases]
	-- Notes:
	-- 1. @GeneralConfigurationAliasGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblGeneralConfigurationAliases] WHERE GeneralConfigurationAliasGuid=@GeneralConfigurationAliasGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblGeneralConfigurationAliases] SET
		...
		,	[GeneralConfigurationGuid]=(CASE ISNULL(@NullOverrideGeneralConfigurationGuid,0) WHEN 1 THEN @GeneralConfigurationGuid ELSE ISNULL(@GeneralConfigurationGuid,[GeneralConfigurationGuid]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_GeneralConfigurationAliasesUpdateByPK' + CHAR(13)+CHAR(10)                  
