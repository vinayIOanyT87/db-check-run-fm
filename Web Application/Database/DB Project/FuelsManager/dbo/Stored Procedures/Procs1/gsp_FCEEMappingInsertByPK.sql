CREATE PROCEDURE [dbo].[gsp_FCEEMappingInsertByPK]
(
	@FCEEMappingGuid uniqueidentifier=NULL OUTPUT,
	@FCEDeviceGuid uniqueidentifier=NULL,
	@MsgType int=NULL,
	@Index int=NULL,
	@Device int=NULL,
	@TagSelection int=NULL,
	@PointGuid uniqueidentifier=NULL,
	@CreatedDate datetimeoffset(7)=NULL,
	@CreatedBy udtUserID=NULL,
	@UpdatedDate datetimeoffset(7)=NULL,
	@UpdatedBy udtUserID=NULL,
	@_RowVersion timestamp=NULL OUTPUT,
	@PointID nvarchar(35)=NULL,
	@ID nvarchar(35)=NULL,
	@SiteGuid uniqueidentifier 
)
AS
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_FCEEMappingInsertByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.001 / 2014-02-05 15:58:32.0032767 -05:00
	-- Purpose: Insert into table [dbo].[tblFCEEMapping]
	-- Notes:
	------------------------------------------------------------------------------------------------------

SET NOCOUNT ON;
BEGIN TRY
	IF (@FCEEMappingGuid IS NULL)
	Begin
		SET @FCEEMappingGuid=NEWID();
	END
	SET @CreatedDate=ISNULL(@CreatedDate,sysdatetimeoffset())

	INSERT INTO [dbo].[tblFCEEMapping]
	(
		[FCEEMappingGuid]
	,	[FCEDeviceGuid]
	,	[MsgType]
	,	[Index]
	,	[Device]
	,	[TagSelection]
	,	[PointGuid]
	,	[CreatedDate]
	,	[CreatedBy]
	,	[UpdatedDate]
	,	[UpdatedBy]
	)
	VALUES
	(
		@FCEEMappingGuid
	,	@FCEDeviceGuid
	,	@MsgType
	,	@Index
	,	@Device
	,	@TagSelection
	,	@PointGuid
	,	@CreatedDate
	,	@CreatedBy
	,	@UpdatedDate
	,	@UpdatedBy
	)

		SELECT @_RowVersion=_RowVersion        
		FROM [dbo].[tblFCEEMapping]           
		WHERE FCEEMappingGuid=@FCEEMappingGuid;
	END TRY
	BEGIN CATCH        
		DECLARE	@_ErrMessage NVARCHAR(2048)      
				, @_ErrNumber INT           
				, @_ErrProcName NVARCHAR(126)           
				, @_ErrLineNumber INT;            
		SET @_ErrMessage = ERROR_MESSAGE();        
		SET @_ErrNumber = ERROR_NUMBER();        
		SET @_ErrProcName= ERROR_PROCEDURE();        
		SET @_ErrLineNumber = ERROR_LINE();            
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						+ 'Number: ' + CAST(@_ErrNumber AS VARCHAR(20)) + CHAR(13)+CHAR(10)                 
						+ 'Procedure Name: gsp_FCEEMappingInsertByPK' + CHAR(13)+CHAR(10)                  
						+ 'Line Number: ' + ISNULL(CAST(@_ErrLineNumber AS VARCHAR(20)),'') + CHAR(13)+CHAR(10);         
		RAISERROR(@_ErrMessage,18,1);      
	END CATCH    
END     