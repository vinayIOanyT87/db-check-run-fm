CREATE PROCEDURE [dbo].[gsp_ReportGroupsUpdateByPK]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_ReportGroupsUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:29.7264198 -05:00
	-- Purpose: Update table [dbo].[tblReportGroups]
	-- Notes:
	-- 1. @ReportGroupGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblReportGroups] WHERE ReportGroupGuid=@ReportGroupGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblReportGroups] SET
			[GroupName]=(CASE ISNULL(@NullOverrideGroupName,0) WHEN 1 THEN @GroupName ELSE ISNULL(@GroupName,[GroupName]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_ReportGroupsUpdateByPK' + CHAR(13)+CHAR(10)                  
