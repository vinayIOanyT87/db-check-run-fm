CREATE PROCEDURE [dbo].[gsp_SettingsUpdateByPK]
(
		@SettingKey nvarchar(50)
	,	@SettingID int=NULL
	,	@SettingValue nvarchar(128)=NULL
	,	@_RowVersion timestamp=NULL OUTPUT
	,	@NullOverrideSettingID BIT=0 
	,	@NullOverrideSettingValue BIT=0 
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_SettingsUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:29.7484220 -05:00
	-- Purpose: Update table [dbo].[tblSettings]
	-- Notes:
	-- 1. @SettingKey and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblSettings] WHERE SettingKey=@SettingKey AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblSettings] SET
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_SettingsUpdateByPK' + CHAR(13)+CHAR(10)                  
