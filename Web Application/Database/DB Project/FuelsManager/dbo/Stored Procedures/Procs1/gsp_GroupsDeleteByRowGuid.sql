CREATE PROCEDURE [dbo].[gsp_GroupsDeleteByRowGuid](@GroupGuid UNIQUEIDENTIFIER,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblGroups] WHERE [GroupGuid] = @GroupGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
				 RAISERROR('Attempted to delete a stale version of Groups.',18,1); 
				 RETURN; 
			END 
		END 
		DELETE [dbo].[tblGroups] WHERE [GroupGuid] = @GroupGuid; 
	END TRY
	BEGIN CATCH
