CREATE PROCEDURE [dbo].[gsp_AutoDistributionReasonCodesUpdateByPK]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_AutoDistributionReasonCodesUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:29.4143886 -05:00
	-- Purpose: Update table [dbo].[tblAutoDistributionReasonCodes]
	-- Notes:
	-- 1. @AutoDistributionReasonCodeGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblAutoDistributionReasonCodes] WHERE AutoDistributionReasonCodeGuid=@AutoDistributionReasonCodeGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblAutoDistributionReasonCodes] SET
			[SiteGuid]=(CASE ISNULL(@NullOverrideSiteGuid,0) WHEN 1 THEN @SiteGuid ELSE ISNULL(@SiteGuid,[SiteGuid]) END)
		,	[ReasonCode]=(CASE ISNULL(@NullOverrideReasonCode,0) WHEN 1 THEN @ReasonCode ELSE ISNULL(@ReasonCode,[ReasonCode]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_AutoDistributionReasonCodesUpdateByPK' + CHAR(13)+CHAR(10)                  
