CREATE PROCEDURE [dbo].[gsp_EquipmentDeleteByEquipmentTypeGuid](@EquipmentTypeGuid uniqueidentifier,@DetachOnly BIT=NULL,@SwapToGuid uniqueidentifier=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblEquipment] WHERE [EquipmentTypeGuid] = @EquipmentTypeGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		BEGIN
			...
			ELSE
			BEGIN
				DELETE [dbo].[tblEquipment] WHERE [EquipmentTypeGuid] = @EquipmentTypeGuid; 
			END
		END
	END TRY
	BEGIN CATCH
