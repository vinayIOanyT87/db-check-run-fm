CREATE PROCEDURE [dbo].[gsp_TransactionsDeleteByTransactionAliasGuid](@TransactionAliasGuid uniqueidentifier,@DetachOnly BIT=NULL,@SwapToGuid uniqueidentifier=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblTransactions] WHERE [TransactionAliasGuid] = @TransactionAliasGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		BEGIN
			...
			ELSE
			BEGIN
				UPDATE [dbo].[tblTransactions]SET [DeleteFlag]=1  WHERE [TransactionAliasGuid] = @TransactionAliasGuid; 
			END
		END
	END TRY
	BEGIN CATCH
