CREATE PROCEDURE [dbo].[gsp_EquipmentDeleteByCompanyGuid](@CompanyGuid uniqueidentifier,@DetachOnly BIT=NULL,@SwapToGuid uniqueidentifier=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblEquipment] WHERE [CompanyGuid] = @CompanyGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		BEGIN
			...
			ELSE
			BEGIN
				DELETE [dbo].[tblEquipment] WHERE [CompanyGuid] = @CompanyGuid; 
			END
		END
	END TRY
	BEGIN CATCH
