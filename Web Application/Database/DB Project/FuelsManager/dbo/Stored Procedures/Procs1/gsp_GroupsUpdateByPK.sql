CREATE PROCEDURE [dbo].[gsp_GroupsUpdateByPK]
(
	...
	,	@NullOverrideGroupID BIT=0 
	,	@NullOverrideGroupDescription BIT=0 
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_GroupsUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:29.5304002 -05:00
	-- Purpose: Update table [dbo].[tblGroups]
	-- Notes:
	-- 1. @GroupGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblGroups] WHERE GroupGuid=@GroupGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblGroups] SET
			[GroupID]=(CASE ISNULL(@NullOverrideGroupID,0) WHEN 1 THEN @GroupID ELSE ISNULL(@GroupID,[GroupID]) END)
		,	[GroupDescription]=(CASE ISNULL(@NullOverrideGroupDescription,0) WHEN 1 THEN @GroupDescription ELSE ISNULL(@GroupDescription,[GroupDescription]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_GroupsUpdateByPK' + CHAR(13)+CHAR(10)                  
