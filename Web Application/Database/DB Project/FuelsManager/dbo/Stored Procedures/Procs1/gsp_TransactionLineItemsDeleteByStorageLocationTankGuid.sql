CREATE PROCEDURE [dbo].[gsp_TransactionLineItemsDeleteByStorageLocationTankGuid](@StorageLocationTankGuid uniqueidentifier,@DetachOnly BIT=NULL,@SwapToGuid uniqueidentifier=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblTransactionLineItems] WHERE [StorageLocationTankGuid] = @StorageLocationTankGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		BEGIN
			...
			ELSE
			BEGIN
				UPDATE [dbo].[tblTransactionLineItems]SET [DeleteFlag]=1  WHERE [StorageLocationTankGuid] = @StorageLocationTankGuid; 
			END
		END
	END TRY
	BEGIN CATCH
