CREATE PROCEDURE [dbo].[gsp_DrawingsUpdateByPK]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_DrawingsUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-12-26 08:11:57.2297415 -05:00
	-- Purpose: Update table [dbo].[tblDrawings]
	-- Notes:
	-- 1. @DrawingGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblDrawings] WHERE DrawingGuid=@DrawingGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblDrawings] SET
		...
		,	[Image]=(CASE ISNULL(@NullOverrideImage,0) WHEN 1 THEN @Image ELSE ISNULL(@Image,[Image]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_DrawingsUpdateByPK' + CHAR(13)+CHAR(10)                  
