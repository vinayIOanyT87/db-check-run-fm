CREATE PROCEDURE [dbo].[gsp_MessageLogUpdateByPK]
(
	...
	,	@NullOverrideMessageGuid BIT=0 
	,	@NullOverridePersonnelGuid BIT=0 
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_MessageLogUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:29.5644036 -05:00
	-- Purpose: Update table [dbo].[tblMessageLog]
	-- Notes:
	-- 1. @MessageLogGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblMessageLog] WHERE MessageLogGuid=@MessageLogGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblMessageLog] SET
			[CompanyGuid]=(CASE ISNULL(@NullOverrideCompanyGuid,0) WHEN 1 THEN @CompanyGuid ELSE ISNULL(@CompanyGuid,[CompanyGuid]) END)
		,	[MessageGuid]=(CASE ISNULL(@NullOverrideMessageGuid,0) WHEN 1 THEN @MessageGuid ELSE ISNULL(@MessageGuid,[MessageGuid]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_MessageLogUpdateByPK' + CHAR(13)+CHAR(10)                  
