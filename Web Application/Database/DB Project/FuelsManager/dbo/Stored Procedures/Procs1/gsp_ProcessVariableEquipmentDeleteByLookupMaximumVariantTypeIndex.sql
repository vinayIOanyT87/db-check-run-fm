CREATE PROCEDURE [dbo].[gsp_ProcessVariableEquipmentDeleteByLookupMaximumVariantTypeIndex](@LookupMaximumVariantTypeIndex int,@DetachOnly BIT=NULL,@SwapToIndex int=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblProcessVariableEquipment] WHERE [LookupMaximumVariantTypeIndex] = @LookupMaximumVariantTypeIndex AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		BEGIN
			...
			ELSE
			BEGIN
				DELETE [dbo].[tblProcessVariableEquipment] WHERE [LookupMaximumVariantTypeIndex] = @LookupMaximumVariantTypeIndex; 
			END
		END
	END TRY
	BEGIN CATCH
