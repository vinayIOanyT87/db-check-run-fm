CREATE PROCEDURE [dbo].[gsp_ExciseUpdateByPK]
(
	...
	,	@NullOverrideExciseRate BIT=0 
	,	@NullOverrideExciseCode BIT=0 
	,	@NullOverrideExciseDate BIT=0 
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_ExciseUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:29.5033975 -05:00
	-- Purpose: Update table [dbo].[tblExcise]
	-- Notes:
	-- 1. @ExciseGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblExcise] WHERE ExciseGuid=@ExciseGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblExcise] SET
			[ExciseRate]=(CASE ISNULL(@NullOverrideExciseRate,0) WHEN 1 THEN @ExciseRate ELSE ISNULL(@ExciseRate,[ExciseRate]) END)
		,	[ExciseCode]=(CASE ISNULL(@NullOverrideExciseCode,0) WHEN 1 THEN @ExciseCode ELSE ISNULL(@ExciseCode,[ExciseCode]) END)
		,	[ExciseDate]=(CASE ISNULL(@NullOverrideExciseDate,0) WHEN 1 THEN @ExciseDate ELSE ISNULL(@ExciseDate,[ExciseDate]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_ExciseUpdateByPK' + CHAR(13)+CHAR(10)                  
