CREATE PROCEDURE [dbo].[gsp_AllocationsDeleteByCompanyBillToToShipperGuid](@CompanyBillToToShipperGuid uniqueidentifier,@DetachOnly BIT=NULL,@SwapToGuid uniqueidentifier=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblAllocations] WHERE [CompanyBillToToShipperGuid] = @CompanyBillToToShipperGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		BEGIN
			...
			ELSE
			BEGIN
				DELETE [dbo].[tblAllocations] WHERE [CompanyBillToToShipperGuid] = @CompanyBillToToShipperGuid; 
			END
		END
	END TRY
	BEGIN CATCH
