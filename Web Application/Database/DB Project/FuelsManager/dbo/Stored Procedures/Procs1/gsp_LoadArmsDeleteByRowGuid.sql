CREATE PROCEDURE [dbo].[gsp_LoadArmsDeleteByRowGuid](@LoadArmGuid UNIQUEIDENTIFIER,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblLoadArms] WHERE [LoadArmGuid] = @LoadArmGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
				 RAISERROR('Attempted to delete a stale version of LoadArms.',18,1); 
				 RETURN; 
			END 
		END 
		DELETE [dbo].[tblLoadArms] WHERE [LoadArmGuid] = @LoadArmGuid; 
	END TRY
	BEGIN CATCH
