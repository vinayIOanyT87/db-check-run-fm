CREATE PROCEDURE [dbo].[gsp_StandardImportConfigUpdateByPK]
(
		@StandardImportConfigGuid uniqueidentifier
	,	@Name nvarchar(50)=NULL
	,	@KeyName nvarchar(50)=NULL
	,	@KeyValue nvarchar(50)=NULL
	,	@_RowVersion timestamp=NULL OUTPUT
	,	@NullOverrideName BIT=0 
	,	@NullOverrideKeyName BIT=0 
	,	@NullOverrideKeyValue BIT=0 
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_StandardImportConfigUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:29.7994271 -05:00
	-- Purpose: Update table [dbo].[tblStandardImportConfig]
	-- Notes:
	-- 1. @StandardImportConfigGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblStandardImportConfig] WHERE StandardImportConfigGuid=@StandardImportConfigGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblStandardImportConfig] SET
			[Name]=(CASE ISNULL(@NullOverrideName,0) WHEN 1 THEN @Name ELSE ISNULL(@Name,[Name]) END)
		,	[KeyName]=(CASE ISNULL(@NullOverrideKeyName,0) WHEN 1 THEN @KeyName ELSE ISNULL(@KeyName,[KeyName]) END)
		,	[KeyValue]=(CASE ISNULL(@NullOverrideKeyValue,0) WHEN 1 THEN @KeyValue ELSE ISNULL(@KeyValue,[KeyValue]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_StandardImportConfigUpdateByPK' + CHAR(13)+CHAR(10)                  
