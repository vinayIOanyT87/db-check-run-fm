CREATE PROCEDURE [dbo].[gsp_TanksDeleteByRowGuid](@TankGuid UNIQUEIDENTIFIER,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblTanks] WHERE [TankGuid] = @TankGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
				 RAISERROR('Attempted to delete a stale version of Tanks.',18,1); 
				 RETURN; 
			END 
		END 
		DELETE [dbo].[tblTanks] WHERE [TankGuid] = @TankGuid; 
	END TRY
	BEGIN CATCH
