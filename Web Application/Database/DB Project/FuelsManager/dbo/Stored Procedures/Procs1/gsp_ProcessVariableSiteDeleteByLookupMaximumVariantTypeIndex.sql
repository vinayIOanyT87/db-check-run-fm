CREATE PROCEDURE [dbo].[gsp_ProcessVariableSiteDeleteByLookupMaximumVariantTypeIndex](@LookupMaximumVariantTypeIndex int,@DetachOnly BIT=NULL,@SwapToIndex int=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblProcessVariableSite] WHERE [LookupMaximumVariantTypeIndex] = @LookupMaximumVariantTypeIndex AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		BEGIN
			...
			ELSE
			BEGIN
				DELETE [dbo].[tblProcessVariableSite] WHERE [LookupMaximumVariantTypeIndex] = @LookupMaximumVariantTypeIndex; 
			END
		END
	END TRY
	BEGIN CATCH
