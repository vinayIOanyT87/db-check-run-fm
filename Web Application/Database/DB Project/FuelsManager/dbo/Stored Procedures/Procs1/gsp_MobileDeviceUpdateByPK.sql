CREATE PROCEDURE [dbo].[gsp_MobileDeviceUpdateByPK]
(
	...
	,	@NullOverrideMobileDeviceId BIT=0 
	,	@NullOverrideDescription BIT=0 
	,	@NullOverrideMobileDeviceType BIT=0 
	,	@NullOverrideUpdatedDate BIT=0 
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_MobileDeviceUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:29.5734045 -05:00
	-- Purpose: Update table [dbo].[tblMobileDevice]
	-- Notes:
	-- 1. @MobileDeviceGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblMobileDevice] WHERE MobileDeviceGuid=@MobileDeviceGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblMobileDevice] SET
			[SiteGuid]=(CASE ISNULL(@NullOverrideSiteGuid,0) WHEN 1 THEN @SiteGuid ELSE ISNULL(@SiteGuid,[SiteGuid]) END)
		,	[MobileDeviceId]=(CASE ISNULL(@NullOverrideMobileDeviceId,0) WHEN 1 THEN @MobileDeviceId ELSE ISNULL(@MobileDeviceId,[MobileDeviceId]) END)
		,	[Description]=(CASE ISNULL(@NullOverrideDescription,0) WHEN 1 THEN @Description ELSE ISNULL(@Description,[Description]) END)
		,	[MobileDeviceType]=(CASE ISNULL(@NullOverrideMobileDeviceType,0) WHEN 1 THEN @MobileDeviceType ELSE ISNULL(@MobileDeviceType,[MobileDeviceType]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_MobileDeviceUpdateByPK' + CHAR(13)+CHAR(10)                  
