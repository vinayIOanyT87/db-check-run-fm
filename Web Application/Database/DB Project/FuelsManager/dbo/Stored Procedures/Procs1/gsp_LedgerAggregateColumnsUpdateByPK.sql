CREATE PROCEDURE [dbo].[gsp_LedgerAggregateColumnsUpdateByPK]
(
	...
	,	@NullOverrideCustomFunctionName BIT=0 
	,	@NullOverrideSiteGuid BIT=0 
	,	@NullOverrideLookupAggregateFieldIndex BIT=0 
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_LedgerAggregateColumnsUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:29.5474019 -05:00
	-- Purpose: Update table [dbo].[tblLedgerAggregateColumns]
	-- Notes:
	-- 1. @LedgerAggregateColumnGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblLedgerAggregateColumns] WHERE LedgerAggregateColumnGuid=@LedgerAggregateColumnGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblLedgerAggregateColumns] SET
		...
		,	[CustomFunctionName]=(CASE ISNULL(@NullOverrideCustomFunctionName,0) WHEN 1 THEN @CustomFunctionName ELSE ISNULL(@CustomFunctionName,[CustomFunctionName]) END)
		,	[SiteGuid]=(CASE ISNULL(@NullOverrideSiteGuid,0) WHEN 1 THEN @SiteGuid ELSE ISNULL(@SiteGuid,[SiteGuid]) END)
		,	[LookupAggregateFieldIndex]=(CASE ISNULL(@NullOverrideLookupAggregateFieldIndex,0) WHEN 1 THEN @LookupAggregateFieldIndex ELSE ISNULL(@LookupAggregateFieldIndex,[LookupAggregateFieldIndex]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_LedgerAggregateColumnsUpdateByPK' + CHAR(13)+CHAR(10)                  
