CREATE PROCEDURE [dbo].[gsp_MarkupDeleteByRowGuid](@MarkupGuid UNIQUEIDENTIFIER,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblMarkup] WHERE [MarkupGuid] = @MarkupGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
				 RAISERROR('Attempted to delete a stale version of Markup.',18,1); 
				 RETURN; 
			END 
		END 
		DELETE [dbo].[tblMarkup] WHERE [MarkupGuid] = @MarkupGuid; 
	END TRY
	BEGIN CATCH
