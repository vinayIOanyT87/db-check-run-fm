CREATE PROCEDURE [dbo].[gsp_TanksDeleteByLookupVesselTypeIndex](@LookupVesselTypeIndex int,@DetachOnly BIT=NULL,@SwapToIndex int=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblTanks] WHERE [LookupVesselTypeIndex] = @LookupVesselTypeIndex AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		BEGIN
			...
			ELSE
			BEGIN
				DELETE [dbo].[tblTanks] WHERE [LookupVesselTypeIndex] = @LookupVesselTypeIndex; 
			END
		END
	END TRY
	BEGIN CATCH
