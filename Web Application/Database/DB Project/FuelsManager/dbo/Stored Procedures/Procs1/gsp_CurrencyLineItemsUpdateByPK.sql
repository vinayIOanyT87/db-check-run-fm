CREATE PROCEDURE [dbo].[gsp_CurrencyLineItemsUpdateByPK]
(
	...
	,	@NullOverrideDate BIT=0 
	,	@NullOverrideRate BIT=0 
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_CurrencyLineItemsUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:29.4563928 -05:00
	-- Purpose: Update table [dbo].[tblCurrencyLineItems]
	-- Notes:
	-- 1. @CurrencyLineItemGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblCurrencyLineItems] WHERE CurrencyLineItemGuid=@CurrencyLineItemGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblCurrencyLineItems] SET
			[Date]=(CASE ISNULL(@NullOverrideDate,0) WHEN 1 THEN @Date ELSE ISNULL(@Date,[Date]) END)
		,	[Rate]=(CASE ISNULL(@NullOverrideRate,0) WHEN 1 THEN @Rate ELSE ISNULL(@Rate,[Rate]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_CurrencyLineItemsUpdateByPK' + CHAR(13)+CHAR(10)                  
