CREATE PROCEDURE [dbo].[gsp_EquipmentTypesDeleteByProductGuid](@ProductGuid uniqueidentifier,@DetachOnly BIT=NULL,@SwapToGuid uniqueidentifier=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblEquipmentTypes] WHERE [ProductGuid] = @ProductGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		BEGIN
			...
			ELSE
			BEGIN
				UPDATE [dbo].[tblEquipmentTypes]SET [DeleteFlag]=1  WHERE [ProductGuid] = @ProductGuid; 
			END
		END
	END TRY
	BEGIN CATCH
