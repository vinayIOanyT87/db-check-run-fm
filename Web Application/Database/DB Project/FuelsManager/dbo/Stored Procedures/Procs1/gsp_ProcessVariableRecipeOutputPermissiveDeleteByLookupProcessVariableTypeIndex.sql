CREATE PROCEDURE [dbo].[gsp_ProcessVariableRecipeOutputPermissiveDeleteByLookupProcessVariableTypeIndex](@LookupProcessVariableTypeIndex int,@DetachOnly BIT=NULL,@SwapToIndex int=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblProcessVariableRecipeOutputPermissive] WHERE [LookupProcessVariableTypeIndex] = @LookupProcessVariableTypeIndex AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		BEGIN
			...
			ELSE
			BEGIN
				DELETE [dbo].[tblProcessVariableRecipeOutputPermissive] WHERE [LookupProcessVariableTypeIndex] = @LookupProcessVariableTypeIndex; 
			END
		END
	END TRY
	BEGIN CATCH
