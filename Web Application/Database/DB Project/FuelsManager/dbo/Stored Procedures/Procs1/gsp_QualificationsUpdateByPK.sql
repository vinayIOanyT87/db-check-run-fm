CREATE PROCEDURE [dbo].[gsp_QualificationsUpdateByPK]
(
	...
	,	@NullOverrideReoccurrence BIT=0 
	,	@NullOverrideUpdatedDate BIT=0 
	,	@NullOverrideSiteGuid BIT=0 
	,	@NullOverrideLookupQualificationTypeIndex BIT=0 
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_QualificationsUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:29.7104182 -05:00
	-- Purpose: Update table [dbo].[tblQualifications]
	-- Notes:
	-- 1. @QualificationGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblQualifications] WHERE QualificationGuid=@QualificationGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblQualifications] SET
		...
		,	[Reoccurrence]=(CASE ISNULL(@NullOverrideReoccurrence,0) WHEN 1 THEN @Reoccurrence ELSE ISNULL(@Reoccurrence,[Reoccurrence]) END)
		...
		,	[LookupQualificationTypeIndex]=(CASE ISNULL(@NullOverrideLookupQualificationTypeIndex,0) WHEN 1 THEN @LookupQualificationTypeIndex ELSE ISNULL(@LookupQualificationTypeIndex,[LookupQualificationTypeIndex]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_QualificationsUpdateByPK' + CHAR(13)+CHAR(10)                  
