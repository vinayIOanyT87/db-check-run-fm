CREATE PROCEDURE [dbo].[gsp_QueryStorageUpdateByPK]
(
		@QueryStorageGuid uniqueidentifier
	...
	,	@NullOverrideQueryDescription BIT=0 
	,	@NullOverrideQueryXML BIT=0 
	,	@NullOverrideUpdatedDate BIT=0 
	,	@NullOverrideSiteGuid BIT=0 
	,	@NullOverrideOwnerUserGuid BIT=0 
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_QueryStorageUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:29.7184190 -05:00
	-- Purpose: Update table [dbo].[tblQueryStorage]
	-- Notes:
	-- 1. @QueryStorageGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblQueryStorage] WHERE QueryStorageGuid=@QueryStorageGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblQueryStorage] SET
			[QueryName]=(CASE ISNULL(@NullOverrideQueryName,0) WHEN 1 THEN @QueryName ELSE ISNULL(@QueryName,[QueryName]) END)
		,	[QueryDescription]=(CASE ISNULL(@NullOverrideQueryDescription,0) WHEN 1 THEN @QueryDescription ELSE ISNULL(@QueryDescription,[QueryDescription]) END)
		,	[QueryXML]=(CASE ISNULL(@NullOverrideQueryXML,0) WHEN 1 THEN @QueryXML ELSE ISNULL(@QueryXML,[QueryXML]) END)
		...
		,	[OwnerUserGuid]=(CASE ISNULL(@NullOverrideOwnerUserGuid,0) WHEN 1 THEN @OwnerUserGuid ELSE ISNULL(@OwnerUserGuid,[OwnerUserGuid]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_QueryStorageUpdateByPK' + CHAR(13)+CHAR(10)                  
