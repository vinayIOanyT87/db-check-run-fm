CREATE PROCEDURE [dbo].[gsp_OpcUaSessionUpdateByPK]
(
	...
	,	@NullOverrideSerializedSession BIT=0 
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[[gsp_OpcUaSessionUpdateByPK]] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2015-08-11 07:47:30.4022475 -05:00
	-- Purpose: Update table [dbo].[tblOpcUaSession]
	-- Notes:
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblOpcUaSession] WHERE SessionGuid=@SessionGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblOpcUaSession] SET
			[SerializedSession]=(CASE ISNULL(@NullOverrideSerializedSession,0) WHEN 1 THEN @SerializedSession ELSE ISNULL(@SerializedSession,[SerializedSession]) END)
		,	[UpdatedDate]=ISNULL(@UpdatedDate,SYSDATETIMEOFFSET())
		,	[UpdatedBy]= ISNULL(@UpdatedBy,SUSER_SNAME())
		WHERE	SessionGuid=@SessionGuid;
 
		SELECT @_RowVersion=_RowVersion        
		FROM [dbo].[tblOpcUaSession]           
		WHERE SessionGuid=@SessionGuid;
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_OpcUaSessionUpdateByPK' + CHAR(13)+CHAR(10)                  
