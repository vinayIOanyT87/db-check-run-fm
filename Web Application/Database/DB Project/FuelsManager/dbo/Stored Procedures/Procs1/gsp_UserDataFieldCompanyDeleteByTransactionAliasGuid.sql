CREATE PROCEDURE [dbo].[gsp_UserDataFieldCompanyDeleteByTransactionAliasGuid](@TransactionAliasGuid uniqueidentifier,@DetachOnly BIT=NULL,@SwapToGuid uniqueidentifier=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblUserDataFieldCompany] WHERE [TransactionAliasGuid] = @TransactionAliasGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		BEGIN
			...
			ELSE
			BEGIN
				DELETE [dbo].[tblUserDataFieldCompany] WHERE [TransactionAliasGuid] = @TransactionAliasGuid; 
			END
		END
	END TRY
	BEGIN CATCH
