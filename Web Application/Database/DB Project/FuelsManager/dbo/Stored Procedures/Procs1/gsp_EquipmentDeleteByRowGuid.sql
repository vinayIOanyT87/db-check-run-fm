CREATE PROCEDURE [dbo].[gsp_EquipmentDeleteByRowGuid](@EquipmentGuid UNIQUEIDENTIFIER,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblEquipment] WHERE [EquipmentGuid] = @EquipmentGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
				 RAISERROR('Attempted to delete a stale version of Equipment.',18,1); 
				 RETURN; 
			END 
		END 
		DELETE [dbo].[tblEquipment] WHERE [EquipmentGuid] = @EquipmentGuid; 
	END TRY
	BEGIN CATCH
