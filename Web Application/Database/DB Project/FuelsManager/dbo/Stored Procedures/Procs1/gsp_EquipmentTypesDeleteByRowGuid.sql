CREATE PROCEDURE [dbo].[gsp_EquipmentTypesDeleteByRowGuid](@EquipmentTypeGuid UNIQUEIDENTIFIER,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblEquipmentTypes] WHERE [EquipmentTypeGuid] = @EquipmentTypeGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
				 RAISERROR('Attempted to delete a stale version of EquipmentTypes.',18,1); 
				 RETURN; 
			END 
		END 
		UPDATE [dbo].[tblEquipmentTypes]SET [DeleteFlag]=1  WHERE [EquipmentTypeGuid] = @EquipmentTypeGuid; 
	END TRY
	BEGIN CATCH
