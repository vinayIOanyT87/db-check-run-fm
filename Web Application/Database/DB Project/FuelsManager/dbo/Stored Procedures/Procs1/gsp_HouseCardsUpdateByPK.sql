CREATE PROCEDURE [dbo].[gsp_HouseCardsUpdateByPK]
(
	...
	,	@NullOverrideDriverPersonnelGuid BIT=0 
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_HouseCardsUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:29.5364008 -05:00
	-- Purpose: Update table [dbo].[tblHouseCards]
	-- Notes:
	-- 1. @HouseCardGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblHouseCards] WHERE HouseCardGuid=@HouseCardGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblHouseCards] SET
		...
		,	[DriverPersonnelGuid]=(CASE ISNULL(@NullOverrideDriverPersonnelGuid,0) WHEN 1 THEN @DriverPersonnelGuid ELSE ISNULL(@DriverPersonnelGuid,[DriverPersonnelGuid]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_HouseCardsUpdateByPK' + CHAR(13)+CHAR(10)                  
