CREATE PROCEDURE [dbo].[gsp_EquipmentDeleteByFuelCardGuid](@FuelCardGuid uniqueidentifier,@DetachOnly BIT=NULL,@SwapToGuid uniqueidentifier=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblEquipment] WHERE [FuelCardGuid] = @FuelCardGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		BEGIN
			...
			ELSE
			BEGIN
				DELETE [dbo].[tblEquipment] WHERE [FuelCardGuid] = @FuelCardGuid; 
			END
		END
	END TRY
	BEGIN CATCH
