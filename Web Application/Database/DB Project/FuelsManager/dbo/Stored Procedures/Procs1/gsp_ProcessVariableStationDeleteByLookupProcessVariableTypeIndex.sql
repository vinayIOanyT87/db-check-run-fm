CREATE PROCEDURE [dbo].[gsp_ProcessVariableStationDeleteByLookupProcessVariableTypeIndex](@LookupProcessVariableTypeIndex int,@DetachOnly BIT=NULL,@SwapToIndex int=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblProcessVariableStation] WHERE [LookupProcessVariableTypeIndex] = @LookupProcessVariableTypeIndex AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		BEGIN
			...
			ELSE
			BEGIN
				DELETE [dbo].[tblProcessVariableStation] WHERE [LookupProcessVariableTypeIndex] = @LookupProcessVariableTypeIndex; 
			END
		END
	END TRY
	BEGIN CATCH
