CREATE PROCEDURE [dbo].[gsp_GatesDeleteByRowGuid](@GateGuid UNIQUEIDENTIFIER,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblGates] WHERE [GateGuid] = @GateGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
				 RAISERROR('Attempted to delete a stale version of Gates.',18,1); 
				 RETURN; 
			END 
		END 
		DELETE [dbo].[tblGates] WHERE [GateGuid] = @GateGuid; 
	END TRY
	BEGIN CATCH
