CREATE PROCEDURE [dbo].[gsp_TankMaintenanceLogDeleteByLookupVesselTypeIndex](@LookupVesselTypeIndex int,@DetachOnly BIT=NULL,@SwapToIndex int=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblTankMaintenanceLog] WHERE [LookupVesselTypeIndex] = @LookupVesselTypeIndex AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		BEGIN
			...
			ELSE
			BEGIN
				DELETE [dbo].[tblTankMaintenanceLog] WHERE [LookupVesselTypeIndex] = @LookupVesselTypeIndex; 
			END
		END
	END TRY
	BEGIN CATCH
