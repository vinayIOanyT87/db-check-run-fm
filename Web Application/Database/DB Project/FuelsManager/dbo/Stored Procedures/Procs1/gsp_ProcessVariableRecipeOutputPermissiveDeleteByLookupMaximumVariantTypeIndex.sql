CREATE PROCEDURE [dbo].[gsp_ProcessVariableRecipeOutputPermissiveDeleteByLookupMaximumVariantTypeIndex](@LookupMaximumVariantTypeIndex int,@DetachOnly BIT=NULL,@SwapToIndex int=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblProcessVariableRecipeOutputPermissive] WHERE [LookupMaximumVariantTypeIndex] = @LookupMaximumVariantTypeIndex AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		BEGIN
			...
			ELSE
			BEGIN
				DELETE [dbo].[tblProcessVariableRecipeOutputPermissive] WHERE [LookupMaximumVariantTypeIndex] = @LookupMaximumVariantTypeIndex; 
			END
		END
	END TRY
	BEGIN CATCH
