CREATE PROCEDURE [dbo].[gsp_TransactionLineItemsDeleteByCurrencyGuid](@CurrencyGuid uniqueidentifier,@DetachOnly BIT=NULL,@SwapToGuid uniqueidentifier=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblTransactionLineItems] WHERE [CurrencyGuid] = @CurrencyGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		BEGIN
			...
			ELSE
			BEGIN
				UPDATE [dbo].[tblTransactionLineItems]SET [DeleteFlag]=1  WHERE [CurrencyGuid] = @CurrencyGuid; 
			END
		END
	END TRY
	BEGIN CATCH
