CREATE PROCEDURE [dbo].[gsp_SitesDeleteByRowGuid](@SiteGuid UNIQUEIDENTIFIER,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblSites] WHERE [SiteGuid] = @SiteGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
				 RAISERROR('Attempted to delete a stale version of Sites.',18,1); 
				 RETURN; 
			END 
		END 
		DELETE [dbo].[tblSites] WHERE [SiteGuid] = @SiteGuid; 
	END TRY
	BEGIN CATCH
