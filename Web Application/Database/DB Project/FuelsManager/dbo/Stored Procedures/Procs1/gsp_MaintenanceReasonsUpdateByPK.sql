CREATE PROCEDURE [dbo].[gsp_MaintenanceReasonsUpdateByPK]
(
	...
	,	@NullOverrideDeletedFlag BIT=0 
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_MaintenanceReasonsUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:29.5564028 -05:00
	-- Purpose: Update table [dbo].[tblMaintenanceReasons]
	-- Notes:
	-- 1. @MaintenanceReasonGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblMaintenanceReasons] WHERE MaintenanceReasonGuid=@MaintenanceReasonGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblMaintenanceReasons] SET
		...
		,	[DeletedFlag]=(CASE ISNULL(@NullOverrideDeletedFlag,0) WHEN 1 THEN @DeletedFlag ELSE ISNULL(@DeletedFlag,[DeletedFlag]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_MaintenanceReasonsUpdateByPK' + CHAR(13)+CHAR(10)                  
