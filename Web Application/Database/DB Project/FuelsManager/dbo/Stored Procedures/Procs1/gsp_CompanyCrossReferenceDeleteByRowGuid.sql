CREATE PROCEDURE [dbo].[gsp_CompanyCrossReferenceDeleteByRowGuid](@KeyName UNIQUEIDENTIFIER,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblCompanyCrossReference] WHERE [KeyName] = @KeyName AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
				 RAISERROR('Attempted to delete a stale version of CompanyCrossReference.',18,1); 
				 RETURN; 
			END 
		END 
		DELETE [dbo].[tblCompanyCrossReference] WHERE [KeyName] = @KeyName; 
	END TRY
	BEGIN CATCH
