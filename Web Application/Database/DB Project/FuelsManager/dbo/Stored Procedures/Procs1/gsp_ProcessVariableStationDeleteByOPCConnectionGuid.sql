CREATE PROCEDURE [dbo].[gsp_ProcessVariableStationDeleteByOPCConnectionGuid](@OPCConnectionGuid uniqueidentifier,@DetachOnly BIT=NULL,@SwapToGuid uniqueidentifier=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblProcessVariableStation] WHERE [OPCConnectionGuid] = @OPCConnectionGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		BEGIN
			...
			ELSE
			BEGIN
				DELETE [dbo].[tblProcessVariableStation] WHERE [OPCConnectionGuid] = @OPCConnectionGuid; 
			END
		END
	END TRY
	BEGIN CATCH
