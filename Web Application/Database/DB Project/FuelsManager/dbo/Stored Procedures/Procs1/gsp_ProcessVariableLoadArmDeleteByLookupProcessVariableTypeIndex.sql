CREATE PROCEDURE [dbo].[gsp_ProcessVariableLoadArmDeleteByLookupProcessVariableTypeIndex](@LookupProcessVariableTypeIndex int,@DetachOnly BIT=NULL,@SwapToIndex int=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblProcessVariableLoadArm] WHERE [LookupProcessVariableTypeIndex] = @LookupProcessVariableTypeIndex AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		BEGIN
			...
			ELSE
			BEGIN
				DELETE [dbo].[tblProcessVariableLoadArm] WHERE [LookupProcessVariableTypeIndex] = @LookupProcessVariableTypeIndex; 
			END
		END
	END TRY
	BEGIN CATCH
