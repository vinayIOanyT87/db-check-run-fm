CREATE PROCEDURE [dbo].[gsp_EmailGroupsUpdateByPK]
(
	...
	,	@NullOverrideAlwaysEnabled BIT=0 
	,	@NullOverrideStartTime BIT=0 
	,	@NullOverrideEndTime BIT=0 
	,	@NullOverrideCategoriesAndPriorities BIT=0 
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_EmailGroupsUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:29.4753947 -05:00
	-- Purpose: Update table [dbo].[tblEmailGroups]
	-- Notes:
	-- 1. @EmailGroupGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblEmailGroups] WHERE EmailGroupGuid=@EmailGroupGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblEmailGroups] SET
			[ID]=(CASE ISNULL(@NullOverrideID,0) WHEN 1 THEN @ID ELSE ISNULL(@ID,[ID]) END)
		,	[AlwaysEnabled]=(CASE ISNULL(@NullOverrideAlwaysEnabled,0) WHEN 1 THEN @AlwaysEnabled ELSE ISNULL(@AlwaysEnabled,[AlwaysEnabled]) END)
		,	[StartTime]=(CASE ISNULL(@NullOverrideStartTime,0) WHEN 1 THEN @StartTime ELSE ISNULL(@StartTime,[StartTime]) END)
		,	[EndTime]=(CASE ISNULL(@NullOverrideEndTime,0) WHEN 1 THEN @EndTime ELSE ISNULL(@EndTime,[EndTime]) END)
		,	[CategoriesAndPriorities]=(CASE ISNULL(@NullOverrideCategoriesAndPriorities,0) WHEN 1 THEN @CategoriesAndPriorities ELSE ISNULL(@CategoriesAndPriorities,[CategoriesAndPriorities]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_EmailGroupsUpdateByPK' + CHAR(13)+CHAR(10)                  
