CREATE PROCEDURE [dbo].[gsp_NotesDeleteByRowGuid](@NoteGuid UNIQUEIDENTIFIER,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblNotes] WHERE [NoteGuid] = @NoteGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
				 RAISERROR('Attempted to delete a stale version of Notes.',18,1); 
				 RETURN; 
			END 
		END 
		DELETE [dbo].[tblNotes] WHERE [NoteGuid] = @NoteGuid; 
	END TRY
	BEGIN CATCH
