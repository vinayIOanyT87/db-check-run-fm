CREATE PROCEDURE [dbo].[gsp_TransactionNotesUpdateByPK]
(
	...
	,	@NullOverrideAdditionalInformation BIT=0 
	,	@NullOverrideTransactionGuid BIT=0 
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_TransactionNotesUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:29.8804352 -05:00
	-- Purpose: Update table [dbo].[tblTransactionNotes]
	-- Notes:
	-- 1. @TransactionNoteGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblTransactionNotes] WHERE TransactionNoteGuid=@TransactionNoteGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblTransactionNotes] SET
		...
		,	[AdditionalInformation]=(CASE ISNULL(@NullOverrideAdditionalInformation,0) WHEN 1 THEN @AdditionalInformation ELSE ISNULL(@AdditionalInformation,[AdditionalInformation]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_TransactionNotesUpdateByPK' + CHAR(13)+CHAR(10)                  
