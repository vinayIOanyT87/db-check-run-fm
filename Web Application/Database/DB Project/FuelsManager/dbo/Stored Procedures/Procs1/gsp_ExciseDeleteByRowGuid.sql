CREATE PROCEDURE [dbo].[gsp_ExciseDeleteByRowGuid](@ExciseGuid UNIQUEIDENTIFIER,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblExcise] WHERE [ExciseGuid] = @ExciseGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
				 RAISERROR('Attempted to delete a stale version of Excise.',18,1); 
				 RETURN; 
			END 
		END 
		DELETE [dbo].[tblExcise] WHERE [ExciseGuid] = @ExciseGuid; 
	END TRY
	BEGIN CATCH
