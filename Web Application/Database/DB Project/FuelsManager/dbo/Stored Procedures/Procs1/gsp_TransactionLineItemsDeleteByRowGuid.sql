CREATE PROCEDURE [dbo].[gsp_TransactionLineItemsDeleteByRowGuid](@TransactionLineItemGuid UNIQUEIDENTIFIER,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblTransactionLineItems] WHERE [TransactionLineItemGuid] = @TransactionLineItemGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
				 RAISERROR('Attempted to delete a stale version of TransactionLineItems.',18,1); 
				 RETURN; 
			END 
		END 
		UPDATE [dbo].[tblTransactionLineItems]SET [DeleteFlag]=1  WHERE [TransactionLineItemGuid] = @TransactionLineItemGuid; 
	END TRY
	BEGIN CATCH
