CREATE PROCEDURE [dbo].[gsp_UserDataFieldFuelCardDeleteByUserGroupGuid](@UserGroupGuid uniqueidentifier,@DetachOnly BIT=NULL,@SwapToGuid uniqueidentifier=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblUserDataFieldFuelCard] WHERE [UserGroupGuid] = @UserGroupGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		BEGIN
			...
			ELSE
			BEGIN
				DELETE [dbo].[tblUserDataFieldFuelCard] WHERE [UserGroupGuid] = @UserGroupGuid; 
			END
		END
	END TRY
	BEGIN CATCH
