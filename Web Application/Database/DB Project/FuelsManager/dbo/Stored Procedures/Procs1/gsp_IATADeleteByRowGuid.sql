CREATE PROCEDURE [dbo].[gsp_IATADeleteByRowGuid](@IATAGuid UNIQUEIDENTIFIER,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblIATA] WHERE [IATAGuid] = @IATAGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
				 RAISERROR('Attempted to delete a stale version of IATA.',18,1); 
				 RETURN; 
			END 
		END 
		DELETE [dbo].[tblIATA] WHERE [IATAGuid] = @IATAGuid; 
	END TRY
	BEGIN CATCH
