CREATE PROCEDURE [dbo].[gsp_SchedulePersonnelAccessDeleteByPersonnelGuid](@PersonnelGuid uniqueidentifier,@DetachOnly BIT=NULL,@SwapToGuid uniqueidentifier=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblSchedulePersonnelAccess] WHERE [PersonnelGuid] = @PersonnelGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		BEGIN
			...
			ELSE
			BEGIN
				DELETE [dbo].[tblSchedulePersonnelAccess] WHERE [PersonnelGuid] = @PersonnelGuid; 
			END
		END
	END TRY
	BEGIN CATCH
