CREATE PROCEDURE [dbo].[gsp_UserDataListValueFuelCardUpdateByPK]
(
	...
	,	@NullOverrideUserDataFieldFuelCardGuid BIT=0 
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_UserDataListValueFuelCardUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:29.9514423 -05:00
	-- Purpose: Update table [dbo].[tblUserDataListValueFuelCard]
	-- Notes:
	-- 1. @UserDataListValueFuelCardGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblUserDataListValueFuelCard] WHERE UserDataListValueFuelCardGuid=@UserDataListValueFuelCardGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblUserDataListValueFuelCard] SET
			[UserDataFieldFuelCardGuid]=(CASE ISNULL(@NullOverrideUserDataFieldFuelCardGuid,0) WHEN 1 THEN @UserDataFieldFuelCardGuid ELSE ISNULL(@UserDataFieldFuelCardGuid,[UserDataFieldFuelCardGuid]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_UserDataListValueFuelCardUpdateByPK' + CHAR(13)+CHAR(10)                  
