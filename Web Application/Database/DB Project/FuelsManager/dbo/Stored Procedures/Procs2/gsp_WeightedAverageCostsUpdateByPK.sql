CREATE PROCEDURE [dbo].[gsp_WeightedAverageCostsUpdateByPK]
(
	...
	,	@NullOverrideWacValue BIT=0 
	,	@NullOverrideIsManualOverride BIT=0 
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_WeightedAverageCostsUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:29.9714443 -05:00
	-- Purpose: Update table [dbo].[tblWeightedAverageCosts]
	-- Notes:
	-- 1. @WeightedAverageCostGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [dbo].[tblWeightedAverageCosts] WHERE WeightedAverageCostGuid=@WeightedAverageCostGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [dbo].[tblWeightedAverageCosts] SET
			[WacValue]=(CASE ISNULL(@NullOverrideWacValue,0) WHEN 1 THEN @WacValue ELSE ISNULL(@WacValue,[WacValue]) END)
		,	[IsManualOverride]=(CASE ISNULL(@NullOverrideIsManualOverride,0) WHEN 1 THEN @IsManualOverride ELSE ISNULL(@IsManualOverride,[IsManualOverride]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_WeightedAverageCostsUpdateByPK' + CHAR(13)+CHAR(10)                  
