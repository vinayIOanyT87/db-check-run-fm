CREATE PROCEDURE [dbo].[gsp_UsersDeleteByRowGuid](@UserGuid UNIQUEIDENTIFIER,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [dbo].[tblUsers] WHERE [UserGuid] = @UserGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
				 RAISERROR('Attempted to delete a stale version of Users.',18,1); 
				 RETURN; 
			END 
		END 
		DELETE [dbo].[tblUsers] WHERE [UserGuid] = @UserGuid; 
	END TRY
	BEGIN CATCH
