...
/*
	EXEC [dbo].[usp_GetEquipmentsByProduct] NULL, NULL
	EXEC [dbo].[usp_GetEquipmentsByProduct] NULL, '291C57D1-06CF-49B3-A732-47F37A1267A0'

*/


CREATE PROCEDURE [dbo].[usp_GetEquipmentsByProduct]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored Procedure: [dbo].[usp_GetEquipmentsByProduct] 
	-- Author: Hansraj Bapoo
	-- Version/Date: 1.0.003 / 2012-07-13 14:21:10.4470770 -04:00
	-- Purpose: Retrieve Equipment records for a given target site/sitegroup, by Product
	-- Notes:
	-- 1. @TargetSiteGuid: Limit results to Equipments that have been assigned to this site/sitegroup only
	-- 2. @ProductGuid: Limit the results to Equipments that have a ProductGuid value that correspond to the @ProductGuid value
	-- 3. This stored procedure replaces the EquipmentClass.EnumerateByProductSQL inline SQL.
	...
	BEGIN TRY	
		...
		WHERE 
		(			
			((@ProductGuid = @emptyGuid) AND (b.ProductGuid IS NULL))
			-- Return equipment records without an assigned product even if a product is specified in the input parameters
			OR ((@ProductGuid <> @emptyGuid) AND ((b.ProductGuid = @ProductGuid) OR (b.ProductGuid IS NULL)))
	...
	BEGIN CATCH  
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: [dbo].usp_GetEquipmentsByProduct' + CHAR(13)+CHAR(10)                  
