...
CREATE PROCEDURE [dbo].[usp_SummaryJournalNet]
...
BEGIN
	...
	UPDATE #OwnerList
		SET [PriorCloseoutDate] = b.[CloseoutDate],
			[PriorNetVolume] = b.[NetBookInventory]
	...
	SELECT t.AliasName, t.LookupTransTypeIndex, t.OwnerID,
			 IsNull(SUM(dbo.udf_ConvertFromSIUnits(l.NetQuantity, @VolumeUnits, @VolumeDecimalPlaces)), 0.0) AS NetQuantity,
			 t.IsPrior
	...
	-- Update Begin Inventory with PriorNetVolume from dbo.tblOwnerClosout
	UPDATE #SummaryJournal 
		SET [Begin Inventory] = IsNull((SELECT SUM(dbo.udf_ConvertFromSIUnits(PriorNetVolume, @VolumeUnits, @VolumeDecimalPlaces))
	...
	UPDATE #SummaryJournal SET [Begin Inventory] = [Begin Inventory] +
		IsNull((SELECT SUM(IsNull(NetQuantity, 0))
	...
	BEGIN
		IF(@LookupTransTypeIndex = 5 OR @LookupTransTypeIndex = 6)
			SELECT @UpdateString = 'UPDATE #SummaryJournal SET ' + @AliasColumnName + ' = -NetQuantity,
			[Book Inventory] = [Book Inventory] + NetQuantity'
