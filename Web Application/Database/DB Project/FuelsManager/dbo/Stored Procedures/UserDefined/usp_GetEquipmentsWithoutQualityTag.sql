...
/*
	EXEC [dbo].[usp_GetEquipmentsWithoutQualityTag] NULL
	EXEC [dbo].[usp_GetEquipmentsWithoutQualityTag] 'F4761A16-AB2F-41EE-B6FA-D17658DF2602'
	
*/

CREATE PROCEDURE [dbo].[usp_GetEquipmentsWithoutQualityTag]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored Procedure: [map].[usp_GetEquipmentsWithoutQualityTag] 
	-- Author: Hansraj Bapoo
	-- Version/Date: 1.0.003 / 2012-07-13 14:21:10.4470770 -04:00
	-- Purpose: Retrieve Equipment records for a given target site/sitegroup, that do not have corresponding Quality Tag records.
	-- Notes:
	-- 1. @TargetSiteGuid: Guid of the site/sitegroup for which to limit the search.
	-- 3. This stored procedure replaces the EquipmentClass.EnumerateManagedEquipmentWithoutQualityTagSQL inline SQL.
	...
	BEGIN TRY	
		...
		WHERE b.ManagedEquipmentFlag = 1
		AND NOT EXISTS
		(
			SELECT * FROM tblEquipmentQualityTagLog 
			WHERE EquipmentGuid = b.EquipmentGuid 
			AND RemovedDate IS NULL 
			AND TaggedDate = (SELECT MAX(TaggedDate) FROM tblEquipmentQualityTagLog WHERE EquipmentGuid = b.EquipmentGuid)
		)
		AND
		(
	...
	BEGIN CATCH  
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: [dbo].usp_GetEquipmentsWithoutQualityTag' + CHAR(13)+CHAR(10)                  
