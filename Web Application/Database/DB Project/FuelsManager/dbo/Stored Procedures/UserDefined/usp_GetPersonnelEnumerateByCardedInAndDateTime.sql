/*
	DROP PROCEDURE [dbo].[usp_GetPersonnelEnumerateByCardedInAndDateTime]

	EXEC [dbo].[usp_GetPersonnelEnumerateByCardedInAndDateTime] 'AD74B677-F294-4BF8-8861-30D6B424ADC6', '8/15/2018'

*/
CREATE PROCEDURE [dbo].[usp_GetPersonnelEnumerateByCardedInAndDateTime]
	@TargetSiteGuid uniqueidentifier, 
	@TimeOutDate datetimeoffset
AS
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored Procedure: [dbo].[usp_GetPersonnelEnumerateByCardedInAndDateTime] 
	-- Author: Shawn Marlin
	-- Version/Date: 1.0.003 / 2013-04-02 07:54:10.4470770 -10:00
	-- Purpose: Retrieve the Personnel records that have been assigned to a given Site/SiteGroup and who have card activity before a given time
	-- Notes:
	-- 1. @TargetSiteGuid: Limit results to Personnel that have been assigned to this site/sitegroup only
	-- 2. @TimeOutDate: The date/time 
	-- 3. This stored procedure replaces the PersonnelClass.EnumerateSQL inline SQL.
	...
	BEGIN TRY
		BEGIN	
			...
			WHERE b.LastActivityDate < @TimeOutDate
			AND b.CardedIn = 1
