...
CREATE PROCEDURE [dbo].[usp_TransactionLineItemsAndUserDataGet]
(
...
BEGIN
	...
	LEFT OUTER JOIN tblTransactionLineItemUserData WITH(NOLOCK) ON tblTransactionLineItems.TransactionLineItemGuid = tblTransactionLineItemUserData.TransactionLineItemGuid 
	LEFT OUTER JOIN (tblProducts p WITH(NOLOCK) INNER JOIN [erv].[udf_GetProductRecordVersions](@SiteGuid) rp ON p.ProductGuid = rp.ProductGuid) ON tblTransactionLineItems.ProductGuid = rp.MasterRecordGuid 
	WHERE tblTransactionLineItems.TransactionGuid = @TransactionGuid
