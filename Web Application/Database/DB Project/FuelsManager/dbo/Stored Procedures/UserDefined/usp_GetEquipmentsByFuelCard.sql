...
/*
	EXEC [dbo].[usp_GetEquipmentsByFuelCard] NULL, NULL
	EXEC [dbo].[usp_GetEquipmentsByFuelCard] NULL, 'BAC33B72-00AC-4326-AD53-0086D78773C1'

*/


CREATE PROCEDURE [dbo].[usp_GetEquipmentsByFuelCard]
(
	@TargetSiteGuid uniqueidentifier, @FuelCardGuid uniqueidentifier, @HideHiddenEquipmentRecords BIT = 0
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored Procedure: [dbo].[usp_GetEquipmentsByFuelCard] 
	-- Author: Hansraj Bapoo
	-- Version/Date: 1.0.003 / 2012-07-13 14:21:10.4470770 -04:00
	-- Purpose: Retrieve Equipment records for a given target site/sitegroup, by FuelCard
	-- Notes:
	-- 1. @TargetSiteGuid: Limit results to Equipments that have been assigned to this site/sitegroup only
	-- 2. @FuelCardGuid: Limit the results to Equipments that have a FuelCardGuid value that correspond to the @FuelCardGuid value
	-- 3. @HideHiddenEquipmentRecords: If true (1), only equipment records with a NULL hiddenDate will be returned
	-- 4. This stored procedure replaces the EquipmentClass.EnumerateByFuelCardSQL inline SQL.
	...
	BEGIN TRY	
		...
		WHERE 
		(			
			((@FuelCardGuid = @emptyGuid) AND (b.FuelCardGuid IS NULL))
			OR ((@FuelCardGuid <> @emptyGuid) AND (b.FuelCardGuid = @FuelCardGuid))
			OR (@FuelCardGuid IS NULL)
		)
		AND
		(
	...
	BEGIN CATCH  
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: [dbo].usp_GetEquipmentsByFuelCard' + CHAR(13)+CHAR(10)                  
