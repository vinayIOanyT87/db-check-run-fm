...
CREATE PROCEDURE [dbo].[usp_GetEquipmentsBySource]
(
	@TargetSiteGuid uniqueidentifier, @HydrantCartType int
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored Procedure: [map].[usp_GetEquipmentsBySource] 
	-- Author: Hansraj Bapoo
	-- Version/Date: 1.0.004 / 2013-04-9
	-- Purpose: Retrieve Equipment records for a given target site/sitegroup for equipments of any type and for which the SecondaryStorageFlag is set or for HydrantCart only equipments for which the ManagedEquipmentFlag is set.
	-- Notes:
	-- 1. @TargetSiteGuid: Guid of the site/sitegroup for which to limit the search.
	-- 2. @HydrantCartTypeIndex: Index of the HydrantCart EquipmentType.
	-- 3. This stored procedure replaces the EquipmentClass.EnumerateBySourceSQL inline SQL.
	...
	BEGIN TRY	
		...
		CASE (SELECT COUNT(*) FROM tblPersonnel WHERE tblPersonnel.AssignedEquipmentGuid = a.EquipmentGuid) WHEN 0 THEN CAST(0 AS bit) ELSE CAST(1 AS bit) END AS IsAssignedToPersonnel,
		...
		WHERE (b.SecondaryStorageFlag=1 OR (b.ManagedEquipmentFlag = 1 AND d.LookupEquipmentTypeIndex = @HydrantCartType))
		AND
		(
	...
	BEGIN CATCH  
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10) 
						...
						+ 'Procedure Name: [dbo].usp_GetEquipmentsBySource' + CHAR(13)+CHAR(10) 
