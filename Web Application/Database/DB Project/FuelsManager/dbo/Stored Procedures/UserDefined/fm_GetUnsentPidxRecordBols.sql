...
-- =============================================
CREATE PROCEDURE [dbo].[fm_GetUnsentPidxRecordBols] 
	@SiteGuid int,
	@SendTime datetime
AS
BEGIN
	...
	WHERE p.SentFlag = 0 
		...
		AND ((p.UpdatedDate < dateadd(n,-5,@SendTime) AND (t.LookupTransactionStatusIndexs = 0 OR t.LookupTransactionStatusIndex = 11)) 
		OR (p.UpdatedDate < dateadd(n,-50,@SendTime) AND (t.TransactionStatus = 7))) 
		...
		AND NOT EXISTS (SELECT i.TransactionLineItemGuid FROM tblTransactionLineItems i WHERE i.TransactionLineItemGuid = p.TransactionGuid AND
						(((i.Density IS NULL OR i.Density < 1.0 OR dbo.ConvertFromSIUnits(i.Density,@DensityUnitIndex,0) = 0.0) AND
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[fm_GetUnsentPidxRecordBols] TO [FMDUserRole]
    AS [dbo];
