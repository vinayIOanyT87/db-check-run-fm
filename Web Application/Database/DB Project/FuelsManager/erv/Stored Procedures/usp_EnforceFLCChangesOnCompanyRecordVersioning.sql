...
/*
	...
	EXEC [erv].[usp_EnforceFLCChangesOnCompanyRecordVersioning] '23E3CCEC-2CCF-4653-A497-29FD15FAFCD4', '00000000-0000-0000-0000-000000000001', 'Administrator', 'ON_TO_OFF'

*/


CREATE PROCEDURE [erv].[usp_EnforceFLCChangesOnCompanyRecordVersioning]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored Procedure: [erv].[usp_EnforceFLCChangesOnCompanyRecordVersioning] 
	...
	BEGIN TRY	
		...
		BEGIN		
			...
			BEGIN
				...
				EXEC [erv].[usp_DeleteCompanyChildRecordVersionBySegment] @EntitySegmentTemplateGuid, @runningSiteGroupGuid
				UPDATE @tblSiteHierarchy SET Processed = 1 WHERE SiteGuid = @runningSiteGroupGuid
			END					
		END
		--If the FLC changes at the originating sitegroup caused FCM-RecordVersioning to be turned ON for the originating node, then create new child record versions at all the children nodes for the given Entity Segment Template and Filter value, for all Company entity assignement assigned to the children nodes and for which a child record version is missing, irrespective of the AssignedFrom sitegroup.
		ELSE IF (@RecordVersioningStatusChange = 'OFF_TO_ON')
		BEGIN					
			WHILE ((SELECT Count(*) From @tblSiteHierarchy Where Processed = 0) > 0)
			BEGIN
				SELECT TOP 1 @runningSiteGroupGuid = SiteGuid FROM @tblSiteHierarchy WHERE Processed = 0 ORDER BY HierarchyLevel, SiteGuid
				EXEC [erv].[usp_CreateCompanyChildRecordVersionBySegment] @EntitySegmentTemplateGuid, @runningSiteGroupGuid, @createdDate, @UserId
				UPDATE @tblSiteHierarchy SET Processed = 1 WHERE SiteGuid = @runningSiteGroupGuid
			END
		END
		--If the FLC changes at the originating sitegroup caused FCM-RecordVersioning to remain ON for the originating node, then re-run record versioning propagation down all the the children nodes for the given Entity Segment Template and Filter value, for all Company entity assignement assigned to the children nodes and for which a child record version exists, only for mappings with an AssignedFrom sitegroup that corresponds to the sitegroup where the FLC configuration changes were initiated or from one of its children sitegroups.
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					EXEC [erv].[usp_DeleteCompanyChildRecordVersionBySegment] @EntitySegmentTemplateGuid, @runningSiteGroupGuid, 1
				END
				ELSE
				BEGIN			
					EXEC [erv].[usp_PropagateCompanyRecordVersionBySegment] @EntitySegmentTemplateGuid, @runningSiteGroupGuid
				END
				UPDATE @tblSiteHierarchy SET Processed = 1 WHERE SiteGuid = @runningSiteGroupGuid
			END
		END
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: [erv].usp_EnforceFLCChangesOnCompanyRecordVersioning' + CHAR(13)+CHAR(10)                  
