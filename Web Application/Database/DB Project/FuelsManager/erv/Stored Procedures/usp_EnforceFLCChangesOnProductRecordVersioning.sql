...
/*
	...
	EXEC [erv].[usp_EnforceFLCChangesOnProductRecordVersioning] '23E3CCEC-2CCF-4653-A497-29FD15FAFCD4', '00000000-0000-0000-0000-000000000001', 'Administrator', 'ON_TO_OFF'

	EXEC [erv].[usp_EnforceFLCChangesOnProductRecordVersioning] '23E3CCEC-2CCF-4653-A497-29FD15FAFCD4', '6F38FF9E-D815-4E5B-B6B6-E6EAC0B1B76B', 'Administrator', 'ON_TO_ON'

	EXEC [erv].[usp_EnforceFLCChangesOnProductRecordVersioning] '23E3CCEC-2CCF-4653-A497-29FD15FAFCD4', 'F4761A16-AB2F-41EE-B6FA-D17658DF2602', 'Administrator', 'ON_TO_OFF'

*/



CREATE PROCEDURE [erv].[usp_EnforceFLCChangesOnProductRecordVersioning]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored Procedure: [erv].[usp_EnforceFLCChangesOnProductRecordVersioning] 
	-- Author: Hansraj Bapoo
	-- Version/Date: 1.0.003 / 2012-07-13 14:21:10.4470770 -04:00
	-- Purpose: Enforce the effect of the latest Product Field Level Control configuration changes onto Record Versioning. 
	...
	BEGIN TRY	
		...
		BEGIN		
			...
			BEGIN
				...
				EXEC [erv].[usp_DeleteProductChildRecordVersionBySegment] @EntitySegmentTemplateGuid, @runningSiteGroupGuid
				UPDATE @tblSiteHierarchy SET Processed = 1 WHERE SiteGuid = @runningSiteGroupGuid
			END					
		END
		--If the FLC changes at the originating sitegroup caused FCM-RecordVersioning to be turned ON for the originating node, then create new child record versions at all the children nodes for the given Entity Segment Template and Filter value, for all Product entity assignement assigned to the children nodes and for which a child record version is missing, irrespective of the AssignedFrom sitegroup.
		ELSE IF (@RecordVersioningStatusChange = 'OFF_TO_ON')
		BEGIN					
			WHILE ((SELECT Count(*) From @tblSiteHierarchy Where Processed = 0) > 0)
			BEGIN
				SELECT TOP 1 @runningSiteGroupGuid = SiteGuid FROM @tblSiteHierarchy WHERE Processed = 0 ORDER BY HierarchyLevel, SiteGuid
				EXEC [erv].[usp_CreateProductChildRecordVersionBySegment] @EntitySegmentTemplateGuid, @runningSiteGroupGuid, @createdDate, @UserId
				UPDATE @tblSiteHierarchy SET Processed = 1 WHERE SiteGuid = @runningSiteGroupGuid
			END
		END
		--If the FLC changes at the originating sitegroup caused FCM-RecordVersioning to remain ON for the originating node, then re-run record versioning propagation down all the the children nodes for the given Entity Segment Template and Filter value, for all Product entity assignement assigned to the children nodes and for which a child record version exists, only for mappings with an AssignedFrom sitegroup that corresponds to the sitegroup where the FLC configuration changes were initiated or from one of its children sitegroups.
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					EXEC [erv].[usp_DeleteProductChildRecordVersionBySegment] @EntitySegmentTemplateGuid, @runningSiteGroupGuid, 1
				END
				ELSE
				BEGIN
					EXEC [erv].[usp_PropagateProductRecordVersionBySegment] @EntitySegmentTemplateGuid, @runningSiteGroupGuid
				END
				UPDATE @tblSiteHierarchy SET Processed = 1 WHERE SiteGuid = @runningSiteGroupGuid
			END
		END
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: [erv].usp_EnforceFLCChangesOnProductRecordVersioning' + CHAR(13)+CHAR(10)                  
