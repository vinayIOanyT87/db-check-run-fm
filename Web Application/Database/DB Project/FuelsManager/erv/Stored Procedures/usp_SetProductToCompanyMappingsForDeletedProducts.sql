...
CREATE PROCEDURE [erv].[usp_SetProductToCompanyMappingsForDeletedProducts]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [erv].[usp_SetProductToCompanyMappingsForDeletedProducts] 
	-- Author: Hansraj Bapoo
	-- Version/Date: 1.0.003 / 2012-07-13 14:21:10.4470770 -04:00
	-- Purpose: Delete, Add, or Update the Product-To-Company mappings to the support the deletions of a set of Product child record versions as a result of FLC configuration changes.	
	-- Notes:
	-- 1. @tblTargetProducts: Table containing the Product record versions whose deletion impact on the Product-to-Company mappings need to be addressed.
	-- 2. This procedure addresses the Shared Mappings needs of the Product-to-Company mappings when a Product record version is deleted.
	...
	BEGIN TRY
		...
		BEGIN TRY
			...
			--For each Product that is mapped to a Company record version owned by the same site/sitegroup as the Product owner site or by a lower site/sitegroup, retrieve the details of the Company record version that is a parent to the Company record version that is tied in the mapping.
			INSERT INTO erv.tblTempProdToCompanyForParentCompany
			(ProductGuid, ProductMasterRecordGuid, TargetSiteGuid, CompanyGuid, CompanyMasterRecordGuid, CompanyParentSiteGuid, ParentCompanyGuid, ProductParentSiteGuid, IsMasterRecordCompany, CompanyOwnsRecordAtAssignedFromSitegroup, Processed, _CallingReferenceGuid)
			SELECT a.EntityGuid, a.MasterRecordGuid, a.SiteGuid, b.AssignedToCompanyGuid, c._MasterRecordGuid, e.SiteGuid, e.CompanyGuid, g.AssignedFromSiteGuid, 0, 1, 0, @callingRef1Guid
			FROM @tblTargetProducts a
			INNER JOIN map.tblProductToCompany b
			ON b.ProductGuid = a.EntityGuid
			INNER JOIN tblCompanies c
			ON c.CompanyGuid = b.AssignedToCompanyGuid -- This covers both Companies that are owned by the same site/sitegroup as the target Products, and those Companies that are owned by a lower site/sitegroup.
			...
			--Retrieve the first available parent Product record version applicable for all the Product records captured in erv.tblTempProdToCompanyForParentProduct, starting from the ProductParentSiteGuid.
			...
			FROM erv.tblTempProdToCompanyForParentCompany a
			...
			-- Retrieve the first available Product record applicable for the Product Parent Sitegroup. Note: Unlike with the Parent Company record, the ProductGuidForParentCompany does not have to be owned by the parent sitegroup. It can be owned by any sitegroup further up the site hierarchy. 
			UPDATE a 
			SET a.ProductGuidForParentCompany = b.EntityGuid
			FROM erv.tblTempProdToCompanyForParentCompany a
			...
			--Mark all Companies that have a master record at either the target (AssignedTo) sitegroup of the Product or lower, as a MasterRecordCompany
			UPDATE a 
			...
			-- Retrieve the Forward Control Mode of the Company field that is used to control the map.tblProductToCompany from the Company side
			UPDATE a 
			SET a.ShipToAuthorizedProductsFCM = b.ForwardControlMode
			FROM erv.tblTempProdToCompanyForParentCompany a
			...
			AND b.TargetField = 'ShipToAuthorizedProducts'
			AND a.CompanyOwnsRecordAtAssignedFromSitegroup = 1
			AND a._CallingReferenceGuid = @callingRef1Guid

			UPDATE erv.tblTempProdToCompanyForParentCompany
			SET ShipToAuthorizedProductsFCM = 'ParentSpecific'
			WHERE _CallingReferenceGuid = @callingRef1Guid
			AND ShipToAuthorizedProductsFCM IS NULL
			AND IsMasterRecordCompany <> 1
			AND CompanyOwnsRecordAtAssignedFromSitegroup = 1

			--Capture Product mappings with Companies which do not have a record that is owned by their AssignedFrom sitegroup
			INSERT INTO erv.tblTempProdToCompanyForParentCompany
			(ProductGuid, ProductMasterRecordGuid, TargetSiteGuid, CompanyGuid, CompanyMasterRecordGuid, IsMasterRecordCompany, CompanyOwnsRecordAtAssignedFromSitegroup, Processed, _CallingReferenceGuid)
			SELECT a.EntityGuid, a.MasterRecordGuid, a.SiteGuid, b.AssignedToCompanyGuid, c._MasterRecordGuid, 0, 0, 0, @callingRef1Guid
			FROM @tblTargetProducts a
			INNER JOIN map.tblProductToCompany b
			ON b.ProductGuid = a.EntityGuid
			INNER JOIN tblCompanies c
			ON c.CompanyGuid = b.AssignedToCompanyGuid
			INNER JOIN map.tblEntityCompanyToSite d
			ON d.CompanyGuid = c._MasterRecordGuid
			...
			AND NOT EXISTS 
			(
				SELECT * FROM erv.tblTempProdToCompanyForParentCompany f
				WHERE f.ProductGuid = a.EntityGuid
				AND f.CompanyGuid = b.AssignedToCompanyGuid
				AND f._CallingReferenceGuid = @callingRef1Guid
			)

			--Capture Product mappings with Companies which are not even mapped to the target site (This can happen as a result of Record Versioning cloning. All Mappings are cloned, irrespective of whether the associated/opposite entity is mapped to the target site or not.)
			INSERT INTO erv.tblTempProdToCompanyForParentCompany
			(ProductGuid, ProductMasterRecordGuid, TargetSiteGuid, CompanyGuid, CompanyMasterRecordGuid, IsMasterRecordCompany, CompanyOwnsRecordAtAssignedFromSitegroup, Processed, _CallingReferenceGuid)
			SELECT a.EntityGuid, a.MasterRecordGuid, a.SiteGuid, b.AssignedToCompanyGuid, c._MasterRecordGuid, 0, 0, 0, @callingRef1Guid
			FROM @tblTargetProducts a
			...
			AND NOT EXISTS 
			(
				SELECT * FROM erv.tblTempProdToCompanyForParentCompany e
				WHERE e.ProductGuid = a.EntityGuid
				AND e.CompanyGuid = b.AssignedToCompanyGuid
				AND e._CallingReferenceGuid = @callingRef1Guid
			)


			--Delete all the mappings owned by the target Product child record versions if the FMC of the Company.ShipToAuthorizedProducts is 'ParentSpecific', i.e. if the corresponding Company record in the mapping is not allowed to have its own version of the Product-to-Company mappings.
			DELETE a 
			FROM map.tblProductToCompany a
			INNER JOIN erv.tblTempProdToCompanyForParentCompany b
			ON b.ProductGuid = a.ProductGuid
			AND b.CompanyGuid = a.AssignedToCompanyGuid
			WHERE b._CallingReferenceGuid = @callingRef1Guid
			AND ((b.ShipToAuthorizedProductsFCM = 'ParentSpecific') OR (CompanyOwnsRecordAtAssignedFromSitegroup = 0))
			AND b.IsMasterRecordCompany <> 1

		
			-- If a Company in the mappings owned by the target Product child record versions has a Parent Company record which itself has a mapping with the Parent Product record, then clone that parent mapping for the child Company record version associated with the target Product child record version.
			...
			SELECT a.ProductGuid, b.CompanyGuid, a.Sequence, a.BlendPercentage, a.AdditiveRate, a.Ratio, a.AdditiveCycleVolume, a.Tolerance, a.PresetNumber, a.AdditiveProfileGuid, a.TankGuid, 
			...
			FROM map.tblProductToCompany a 
			INNER JOIN erv.tblTempProdToCompanyForParentCompany b
			ON b.ParentCompanyGuid = a.AssignedToCompanyGuid
			AND b.ProductGuidForParentCompany = a.ProductGuid
			WHERE b._CallingReferenceGuid = @callingRef1Guid
			AND b.CompanyOwnsRecordAtAssignedFromSitegroup = 1
			AND b.ShipToAuthorizedProductsFCM = 'ParentSpecific'
			AND b.IsMasterRecordCompany <> 1
			AND b.ProductGuidForParentCompany IS NOT NULL
			AND NOT EXISTS
			(
				SELECT * FROM map.tblProductToCompany c
				WHERE c.ProductGuid = a.ProductGuid
				AND c.AssignedToCompanyGuid = b.CompanyGuid
			)

			-- If the corresponding Company record in the target Product child record version mapping is allowed to have its own version of the Product-to-Company mappings, then do not delete that mapping, but simply modify it to point to the Parent Product Guid, instead of the target Product child record version (that is marked for deletion).
			UPDATE a 
			SET a.ProductGuid = b.ProductGuidForParentCompany, a.UpdatedDate = GETDATE()
			FROM map.tblProductToCompany a
			INNER JOIN erv.tblTempProdToCompanyForParentCompany b
			...
			AND
			(
				(b.ShipToAuthorizedProductsFCM = 'VersionSpecific')
				OR 
			...
			DELETE erv.tblTempProdToCompanyForParentCompany WHERE _CallingReferenceGuid = @callingRef1Guid
			...
			-- Retrieve the Forward Control Mode of the Company field that is used to control the map.tblProductToUnavailableInventoryCompany from the Company side
			UPDATE a 
			...
			ON c.EntitySegmentTemplateGuid = b.EntitySegmentTemplateGuid
			WHERE c.EntityTypeId = 'Company'
			AND b.TargetField = 'UnavailableInventories'
			AND a.CompanyOwnsRecordAtAssignedFromSitegroup = 1
			AND a._CallingReferenceGuid = @callingRef1Guid
			...
			--Capture the Product mappings with Companies which do not have a record that is owned by their AssignedFrom sitegroup
			INSERT INTO erv.tblTempProdToCompanyForParentCompany
			(ProductGuid, ProductMasterRecordGuid, TargetSiteGuid, CompanyGuid, CompanyMasterRecordGuid, IsMasterRecordCompany, CompanyOwnsRecordAtAssignedFromSitegroup, Processed, _CallingReferenceGuid)
			SELECT a.EntityGuid, a.MasterRecordGuid, a.SiteGuid, b.AssignedToCompanyGuid, c._MasterRecordGuid, 0, 0, 0, @callingRef1Guid
			FROM @tblTargetProducts a
			...
			--Delete all the mappings owned by the target Product child record versions if the FMC of the Company.UnavailableInventoriesFCM is 'ParentSpecific', i.e. if the corresponding Company record in the mapping is not allowed to have its own version of the Product-to-Company mappings.
			DELETE a 
			...
			AND b.CompanyGuid = a.AssignedToCompanyGuid
			WHERE b._CallingReferenceGuid = @callingRef1Guid
			AND ((b.UnavailableInventoriesFCM = 'ParentSpecific') OR (CompanyOwnsRecordAtAssignedFromSitegroup = 0))
			AND b.IsMasterRecordCompany <> 1
			...
			-- Retrieve the Forward Control Mode of the Company field that is used to control the map.tblProductToSupplierProductCompany from the Company side
			UPDATE a 
			...
			--Delete all the mappings owned by the target Product child record versions if the FMC of the Company.SupplierAuthorizedProductsFCM is 'ParentSpecific', i.e. if the corresponding Company record in the mapping is not allowed to have its own version of the Product-to-Company mappings.
			DELETE a 
			...
			AND b.CompanyGuid = a.AssignedToCompanyGuid
			WHERE b._CallingReferenceGuid = @callingRef1Guid
			AND ((b.SupplierAuthorizedProductsFCM = 'ParentSpecific') OR (CompanyOwnsRecordAtAssignedFromSitegroup = 0))
			AND b.IsMasterRecordCompany <> 1
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: [erv].usp_SetProductToCompanyMappingsForDeletedProducts' + CHAR(13)+CHAR(10)                  
