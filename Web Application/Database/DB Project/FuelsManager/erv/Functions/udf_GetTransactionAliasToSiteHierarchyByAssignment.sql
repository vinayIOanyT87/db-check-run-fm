...
/*
	SELECT * FROM [erv].[udf_GetTransactionAliasToSiteHierarchyByAssignment] ( '0DC68ACA-11AD-4F43-AD2B-87609738C453', '00000000-0000-0000-0000-000000000001', NULL) ORDER BY HierarchyLevel, SiteId	
	SELECT * FROM [erv].[udf_GetTransactionAliasToSiteHierarchyByAssignment] ( '0DC68ACA-11AD-4F43-AD2B-87609738C453', '00000000-0000-0000-0000-000000000001', 'F4761A16-AB2F-41EE-B6FA-D17658DF2602') ORDER BY HierarchyLevel, SiteId	
	SELECT * FROM [erv].[udf_GetTransactionAliasToSiteHierarchyByAssignment] ( '0DC68ACA-11AD-4F43-AD2B-87609738C453', NULL, NULL) ORDER BY HierarchyLevel, SiteId	

*/

CREATE FUNCTION [erv].[udf_GetTransactionAliasToSiteHierarchyByAssignment]
(
...
BEGIN
	/*
	------------------------------------------------------------------------------------------------------
	-- Function: [erv].[udf_GetTransactionAliasToSiteHierarchyByAssignment] 
	-- Author: Hansraj Bapoo
	-- Version/Date: 1.0.003 / 2012-07-13 14:21:10.4470770 -04:00
	-- Purpose: Retrieves the TransactionAliasToSite assignment hierarchy below a given TransactionAlias assignment. 	
	-- Notes:
	-- 1. @EntityMasterRecordGuid: Guid (MasterRecordGuid) of the TransactionAlias record for which the mapping hierarchy is to be retrieved.
	...
	*/
	...
	BEGIN
		SELECT @AssignedFromSiteGuid = SiteGuid FROM tblTransactionAliases WHERE TransactionAliasGuid = _MasterRecordGuid AND TransactionAliasGuid = @EntityMasterRecordGuid
		SET @AssignedToSiteGuid = @AssignedFromSiteGuid
	END;

	IF (@AssignedFromSiteGuid IS NULL)
	BEGIN
		SELECT @AssignedFromSiteGuid = AssignedFromSiteGuid FROM map.tblEntityTransactionAliasToSite WHERE TransactionAliasGuid = @EntityMasterRecordGuid AND SiteGuid = @AssignedToSiteGuid
	END;
	...
	(
		...
		SELECT TransactionAliasToSiteGuid, AssignedFromSiteGuid, SiteGuid, 0
		...
		-- Recursive Call
		UNION ALL
		SELECT a.TransactionAliasToSiteGuid, a.AssignedFromSiteGuid, a.SiteGuid, lvl + 1
		FROM map.tblEntityTransactionAliasToSite a 
		INNER JOIN AssignmentHierarchy b
		ON a.TransactionAliasGuid = @EntityMasterRecordGuid
