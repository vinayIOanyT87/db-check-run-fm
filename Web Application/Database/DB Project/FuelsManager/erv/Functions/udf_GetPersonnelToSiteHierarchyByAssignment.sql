...
/*
	SELECT * FROM [erv].[udf_GetPersonnelToSiteHierarchyByAssignment] ( '80B08634-D356-4569-B9A2-CD36DF955BD0', '00000000-0000-0000-0000-000000000001', NULL) ORDER BY HierarchyLevel, SiteId	
	SELECT * FROM [erv].[udf_GetPersonnelToSiteHierarchyByAssignment] ( 'B4E4B396-1366-4BEA-BDD6-D08F35863E87', '00000000-0000-0000-0000-000000000001', 'F4761A16-AB2F-41EE-B6FA-D17658DF2602') ORDER BY HierarchyLevel, SiteId	
	SELECT * FROM [erv].[udf_GetPersonnelToSiteHierarchyByAssignment] ( 'B4E4B396-1366-4BEA-BDD6-D08F35863E87', NULL, NULL) ORDER BY HierarchyLevel, SiteId	

*/

CREATE FUNCTION [erv].[udf_GetPersonnelToSiteHierarchyByAssignment]
(
...
BEGIN
	/*
	------------------------------------------------------------------------------------------------------
	-- Function: [erv].[udf_GetPersonnelToSiteHierarchyByAssignment] 
	-- Author: Shawn Marlin
	-- Version/Date: 1.0.003 / 2013-03-29 10:00:10.4470770 -10:00
	-- Purpose: Retrieves the PersonnelToSite assignment hierarchy below a given Personnel assignment. 	
	-- Notes:
	-- 1. @EntityMasterRecordGuid: Guid (MasterRecordGuid) of the Personnel record for which the mapping hierarchy is to be retrieved.
	...
	*/
	...
	BEGIN
		SELECT @AssignedFromSiteGuid = SiteGuid FROM tblPersonnel WHERE PersonnelGuid = _MasterRecordGuid AND PersonnelGuid = @EntityMasterRecordGuid
		SET @AssignedToSiteGuid = @AssignedFromSiteGuid
	END;

	IF (@AssignedFromSiteGuid IS NULL)
	BEGIN
		SELECT @AssignedFromSiteGuid = AssignedFromSiteGuid FROM map.tblEntityPersonnelToSite WHERE PersonnelGuid = @EntityMasterRecordGuid AND SiteGuid = @AssignedToSiteGuid
	END;
	...
	(
		...
		SELECT PersonnelToSiteGuid, AssignedFromSiteGuid, SiteGuid, 0
		...
		-- Recursive Call
		UNION ALL
		SELECT a.PersonnelToSiteGuid, a.AssignedFromSiteGuid, a.SiteGuid, lvl + 1
		FROM map.tblEntityPersonnelToSite a 
		INNER JOIN AssignmentHierarchy b
		ON a.PersonnelGuid = @EntityMasterRecordGuid
