...
/*
	SELECT * FROM [erv].[udf_GetEquipmentToSiteHierarchyByAssignment] ( '1BB8C558-5277-47A5-90AE-2461BBD1EFF7', '00000000-0000-0000-0000-000000000001', NULL) ORDER BY HierarchyLevel, SiteId	
	SELECT * FROM [erv].[udf_GetEquipmentToSiteHierarchyByAssignment] ( '1BB8C558-5277-47A5-90AE-2461BBD1EFF7', '00000000-0000-0000-0000-000000000001', 'F4761A16-AB2F-41EE-B6FA-D17658DF2602') ORDER BY HierarchyLevel, SiteId	
	SELECT * FROM [erv].[udf_GetEquipmentToSiteHierarchyByAssignment] ( '1BB8C558-5277-47A5-90AE-2461BBD1EFF7', NULL, NULL) ORDER BY HierarchyLevel, SiteId	

*/

CREATE FUNCTION [erv].[udf_GetEquipmentToSiteHierarchyByAssignment]
(
...
BEGIN
	/*
	------------------------------------------------------------------------------------------------------
	-- Function: [erv].[udf_GetEquipmentToSiteHierarchyByAssignment] 
	-- Author: Hansraj Bapoo
	-- Version/Date: 1.0.003 / 2012-07-13 14:21:10.4470770 -04:00
	-- Purpose: Retrieves the EquipmentToSite assignment hierarchy below a given equipment assignment. 	
	-- Notes:
	-- 1. @EntityMasterRecordGuid: Guid (MasterRecordGuid) of the Equipment record for which the mapping hierarchy is to be retrieved.
	...
	*/
	...
	BEGIN
		SELECT @AssignedFromSiteGuid = SiteGuid FROM tblEquipment WHERE EquipmentGuid = _MasterRecordGuid AND EquipmentGuid = @EntityMasterRecordGuid
		SET @AssignedToSiteGuid = @AssignedFromSiteGuid
	END;

	IF (@AssignedFromSiteGuid IS NULL)
	BEGIN
		SELECT @AssignedFromSiteGuid = AssignedFromSiteGuid FROM map.tblEntityEquipmentToSite WHERE EquipmentGuid = @EntityMasterRecordGuid AND SiteGuid = @AssignedToSiteGuid
	END;
	...
	(
		...
		SELECT EquipmentToSiteGuid, AssignedFromSiteGuid, SiteGuid, 0
		...
		-- Recursive Call
		UNION ALL
		SELECT a.EquipmentToSiteGuid, a.AssignedFromSiteGuid, a.SiteGuid, lvl + 1
		FROM map.tblEntityEquipmentToSite a 
		INNER JOIN AssignmentHierarchy b
		ON a.EquipmentGuid = @EntityMasterRecordGuid
