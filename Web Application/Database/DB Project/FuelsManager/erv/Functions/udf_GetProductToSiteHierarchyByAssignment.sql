...
/*
	SELECT * FROM [erv].[udf_GetProductToSiteHierarchyByAssignment] ( '80B08634-D356-4569-B9A2-CD36DF955BD0', '00000000-0000-0000-0000-000000000001', NULL) ORDER BY HierarchyLevel, SiteId	
	SELECT * FROM [erv].[udf_GetProductToSiteHierarchyByAssignment] ( 'B4E4B396-1366-4BEA-BDD6-D08F35863E87', '00000000-0000-0000-0000-000000000001', 'F4761A16-AB2F-41EE-B6FA-D17658DF2602') ORDER BY HierarchyLevel, SiteId	
	SELECT * FROM [erv].[udf_GetProductToSiteHierarchyByAssignment] ( 'B4E4B396-1366-4BEA-BDD6-D08F35863E87', NULL, NULL) ORDER BY HierarchyLevel, SiteId	

	SELECT * FROM [erv].[udf_GetProductToSiteHierarchyByAssignment] ( '80B08634-D356-4569-B9A2-CD36DF955BD0', NULL, '00000000-0000-0000-0000-000000000001') ORDER BY HierarchyLevel, SiteId	
	SELECT * FROM [erv].[udf_GetProductToSiteHierarchyByAssignment] ( '80B08634-D356-4569-B9A2-CD36DF955BD0', '00000000-0000-0000-0000-000000000001', NULL) ORDER BY HierarchyLevel, SiteId	
	SELECT * FROM [erv].[udf_GetProductToSiteHierarchyByAssignment] ( '80B08634-D356-4569-B9A2-CD36DF955BD0', '00000000-0000-0000-0000-000000000001', '00000000-0000-0000-0000-000000000001') ORDER BY HierarchyLevel, SiteId	
	SELECT * FROM [erv].[udf_GetProductToSiteHierarchyByAssignment] ( '80B08634-D356-4569-B9A2-CD36DF955BD0', NULL, 'F4761A16-AB2F-41EE-B6FA-D17658DF2602') ORDER BY HierarchyLevel, SiteId	
	SELECT * FROM [erv].[udf_GetProductToSiteHierarchyByAssignment] ( '80B08634-D356-4569-B9A2-CD36DF955BD0', NULL, '46426312-E408-4AF8-85FD-338B622B32BF') ORDER BY HierarchyLevel, SiteId	


*/

CREATE FUNCTION [erv].[udf_GetProductToSiteHierarchyByAssignment]
(
...
BEGIN
	/*
	------------------------------------------------------------------------------------------------------
	-- Function: [erv].[udf_GetProductToSiteHierarchyByAssignment] 
	-- Author: Hansraj Bapoo
	-- Version/Date: 1.0.003 / 2012-07-13 14:21:10.4470770 -04:00
	-- Purpose: Retrieves the ProductToSite assignment hierarchy below a given Product assignment. 	
	-- Notes:
	-- 1. @EntityMasterRecordGuid: Guid (MasterRecordGuid) of the Product record for which the mapping hierarchy is to be retrieved.
	...
	*/
	...
	BEGIN
		SELECT @AssignedFromSiteGuid = SiteGuid FROM tblProducts WHERE ProductGuid = _MasterRecordGuid AND ProductGuid = @EntityMasterRecordGuid
		SET @AssignedToSiteGuid = @AssignedFromSiteGuid
	END;

	IF (@AssignedFromSiteGuid IS NULL)
	BEGIN
		SELECT @AssignedFromSiteGuid = AssignedFromSiteGuid FROM map.tblEntityProductToSite WHERE ProductGuid = @EntityMasterRecordGuid AND SiteGuid = @AssignedToSiteGuid
	END;
	...
	(
		...
		SELECT ProductToSiteGuid, AssignedFromSiteGuid, SiteGuid, 0
		...
		-- Recursive Call
		UNION ALL
		SELECT a.ProductToSiteGuid, a.AssignedFromSiteGuid, a.SiteGuid, lvl + 1
		FROM map.tblEntityProductToSite a 
		INNER JOIN AssignmentHierarchy b
		ON a.ProductGuid = @EntityMasterRecordGuid
