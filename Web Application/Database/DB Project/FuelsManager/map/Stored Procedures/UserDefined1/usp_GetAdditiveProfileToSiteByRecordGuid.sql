...
/*
	EXEC [map].[usp_GetAdditiveProfileToSiteByRecordGuid] 'C0DA2E5F-711C-4245-B4DA-7085831674F1', NULL
	EXEC [map].[usp_GetAdditiveProfileToSiteByRecordGuid] '05C7D1E0-745D-488D-AF05-00DF1A70D05D', 'F4761A16-AB2F-41EE-B6FA-D17658DF2602', 1
*/

CREATE PROCEDURE [map].[usp_GetAdditiveProfileToSiteByRecordGuid]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_GetAdditiveProfileToSiteByRecordGuid]
	-- Author: Hansraj Bapoo
	-- Version/Date: 1.0.003 / 2012-07-13 14:21:10.4470770 -04:00
	-- Purpose: Retrieves the AdditiveProfileToSite assignments for a given AdditiveProfile RecordGuid to a given site/sitegroup, and optionally to children site/sitegroup of the given sitegroup as well. 	
	-- Notes:
	...
	-- 7. This stored procedure replaces the EntityTositeMapClass.SelectSQL() inline SQL for the case where entityType is ADDITIVE_PROFILE and bInTransaction is false.
	------------------------------------------------------------------------------------------------------
	SET NOCOUNT ON;
	BEGIN TRY
		...
		BEGIN
			SELECT @AssignedToSiteGuid = SiteGuid FROM tblAdditiveProfiles
		...
		SELECT a.AdditiveProfileToSiteGuid MappingGuid, a.AdditiveProfileGuid EntityRecordGuid, d.Id EntityId, a.SiteGuid AssignedToSiteGuid, b.SiteGroupId AssignedToSiteId, b.HierarchyLevel, a.CreatedDate, a.CreatedBy, a.UpdatedDate, a.UpdatedBy, a._RowVersion, a.AssignedFromSiteGuid , c.ID AssignedFromSiteId 
		...
		INNER JOIN tblAdditiveProfiles d
		ON d.AdditiveProfileGuid = a.AdditiveProfileGuid
		WHERE a.AdditiveProfileGuid = @EntityRecordGuid
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: map.usp_GetAdditiveProfileToSiteByRecordGuid' + CHAR(13)+CHAR(10)                  
