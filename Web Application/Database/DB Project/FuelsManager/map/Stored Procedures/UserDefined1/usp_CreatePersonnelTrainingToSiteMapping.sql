/*
	DROP PROCEDURE [map].[usp_CreatePersonnelTrainingToSiteMapping]

*/   

CREATE PROCEDURE [map].[usp_CreatePersonnelTrainingToSiteMapping]
(		
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_CreatePersonnelTrainingToSiteMapping]
	-- Author: Vivian Kiarie /  Hansraj Bapoo
	-- Version/Date: 1.0.002 / 2013-12-30 14:44:52.5194399 -05:00
	-- Purpose: Insert into table [map].[tblEntityPersonnelTrainingToSite]
		-- Notes:
	...
	BEGIN TRY
		...
		INSERT INTO [map].[tblEntityPersonnelTrainingToSite]
		(	
		  [QualificationGuid]
		  ,	[SiteGuid]
		  ,	[CreatedDate]
	...
	-- Cascading Assignments do not apply to the base entity assignment (assignment of the entity record with its owner site guid).
		...
		OPEN TableCursor 
			...
			BEGIN 
				...
				WHERE NOT EXISTS
				(
					SELECT * FROM [map].[tblEntityPersonnelTrainingToSite]
	...
	BEGIN CATCH
		...
		BEGIN
			...
			+ 'Procedure Name: usp_CreatePersonnelTrainingToSiteMapping' + CHAR(13)+CHAR(10)
