/*
	DROP PROCEDURE [map].[usp_CreateAutoDistributionReasonCodeToSiteMapping]

*/

CREATE PROCEDURE [map].[usp_CreateAutoDistributionReasonCodeToSiteMapping]
(
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_CreateAutoDistributionReasonCodeToSiteMapping]
	-- Author: Vivian Kiarie /  Hansraj Bapoo
	-- Version/Date: 1.0.002 / 2013-12-30 14:44:52.4864333 -05:00
	-- Purpose: Insert into table [map].[tblEntityAutoDistributionReasonCodeToSite]
	-- Notes:
	...
	BEGIN TRY
		...
		INSERT INTO [map].[tblEntityAutoDistributionReasonCodeToSite]
		(
		...
		OPEN TableCursor 
			...
			BEGIN 
				...
				WHERE NOT EXISTS
				(
					SELECT * FROM [map].[tblEntityAutoDistributionReasonCodeToSite]
	...
	BEGIN CATCH
		...
		BEGIN
			...
			+ 'Procedure Name: usp_CreateAutoDistributionReasonCodeToSiteMapping' + CHAR(13)+CHAR(10)
