/*
	DROP PROCEDURE [map].[usp_CreateProcessVariableMessageToSiteMapping]

*/ 

CREATE PROCEDURE [map].[usp_CreateProcessVariableMessageToSiteMapping]
(		
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_CreateProcessVariableMessageToSiteMapping]
	-- Author: Vivian Kiarie /  Hansraj Bapoo
	-- Version/Date: 1.0.002 / 2013-12-30 14:44:52.5204401 -05:00
	-- Purpose: Insert into table [map].[tblEntityProcessVariableMessageToSite]
		-- Notes:
	...
	BEGIN TRY
		...
		INSERT INTO [map].[tblEntityProcessVariableMessageToSite]
		(		
	...
	-- Cascading Assignments do not apply to the base entity assignment (assignment of the entity record with its owner site guid).
		...
		OPEN TableCursor 
			...
			BEGIN 
				...
				WHERE NOT EXISTS
				(
					SELECT * FROM [map].[tblEntityProcessVariableMessageToSite]
	...
	BEGIN CATCH
		...
		BEGIN
			...
			+ 'Procedure Name: usp_CreateProcessVariableMessageToSiteMapping' + CHAR(13)+CHAR(10)
