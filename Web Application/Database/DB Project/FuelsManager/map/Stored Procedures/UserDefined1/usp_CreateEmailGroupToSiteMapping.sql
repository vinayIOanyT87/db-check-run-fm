/*
	DROP PROCEDURE [map].[usp_CreateEmailGroupToSiteMapping]

*/ 

CREATE PROCEDURE [map].[usp_CreateEmailGroupToSiteMapping]
(
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_CreateEmailGroupToSiteMapping]
	-- Author: Vivian Kiarie /  Hansraj Bapoo
	-- Version/Date: 1.0.002 / 2013-12-30 14:44:52.5014363 -05:00
	-- Purpose: Insert into table [map].[tblEntityEmailGroupToSite]
		-- Notes:
	...
	BEGIN TRY
		...
		INSERT INTO [map].[tblEntityEmailGroupToSite]
		(
		  [EmailGroupGuid]
		  ,	[SiteGuid]
		  ,	[CreatedDate]
	...
	-- Cascading Assignments do not apply to the base entity assignment (assignment of the entity record with its owner site guid).
		...
		OPEN TableCursor 
			...
			BEGIN 
				...
				WHERE NOT EXISTS
				(
					SELECT * FROM [map].[tblEntityEmailGroupToSite]
	...
	BEGIN CATCH
		...
		BEGIN
			...
			+ 'Procedure Name: usp_CreateEmailGroupToSiteMapping' + CHAR(13)+CHAR(10)
