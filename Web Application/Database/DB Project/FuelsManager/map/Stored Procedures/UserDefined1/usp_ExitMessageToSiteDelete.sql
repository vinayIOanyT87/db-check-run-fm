...
/*
	EXEC [map].[usp_ExitMessageToSiteDelete] 'B4E4B396-1366-4BEA-BDD6-D08F35863E87', 'AEBA18E3-E97B-479E-8B2D-0BCD69C1C421'
	EXEC [map].[usp_ExitMessageToSiteDelete] 'B4E4B396-1366-4BEA-BDD6-D08F35863E87', NULL
	EXEC [map].[usp_ExitMessageToSiteDelete] 'B4E4B396-1366-4BEA-BDD6-D08F35863E87', NULL, 1

*/
CREATE PROCEDURE [map].[usp_ExitMessageToSiteDelete]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_ExitMessageToSiteDelete]
	-- Author: Hansraj Bapoo
	-- Version/Date: 1.0.003 / 2012-07-13 14:21:10.4470770 -04:00
	-- Purpose: Deletes a ExitMessageToSite mapping entry.
	-- Notes:
	-- 1. @EntityRecordGuid: Guid of the ExitMessage record for which the mapping is to be deleted.
	-- 2. @AssignedToSiteGuid: Guid of the AssignedTo site/sitegroup for which the mapping is to be deleted. 
	--    If the @AssignedToSiteGuid parameter is null, then all the ExitMessage to Site mappings for the entity record are deleted.
	...
	-- 5. This operation also deletes all the other ExitMessageToSite assignments that have been made possible by the given assignment (Cascading entity assignment deletion).
	...
	BEGIN TRY
		...
		SELECT MappingGuid, AssignedFromSiteGuid, SiteGuid, HierarchyLevel FROM [map].[udf_GetExitMessageToSiteHierarchyByAssignment] (@EntityRecordGuid, NULL, @AssignedToSiteGuid)
		...
		BEGIN
			INSERT INTO @tblEntityToSiteHierarchy
			(MappingGuid, AssignedFromSiteGuid, AssignedToSiteGuid, HierarchyLevel)
			SELECT b.ExitMessageToSiteGuid, a.SiteGuid, a.SiteGuid, -1 FROM dbo.tblApplicationString a
			INNER JOIN map.tblEntityExitMessageToSite b
		...
		DELETE a FROM map.tblEntityExitMessageToSite a
		INNER JOIN @tblEntityToSiteHierarchy b 
		ON b.MappingGuid = a.ExitMessageToSiteGuid
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: map.usp_ExitMessageToSiteDelete' + CHAR(13)+CHAR(10)                  
