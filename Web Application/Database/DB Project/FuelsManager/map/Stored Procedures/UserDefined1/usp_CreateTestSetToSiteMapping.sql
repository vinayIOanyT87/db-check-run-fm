/*
	DROP PROCEDURE [map].[usp_CreateTestSetToSiteMapping]

*/  

CREATE PROCEDURE [map].[usp_CreateTestSetToSiteMapping]
(
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_CreateTestSetToSiteMapping]
	-- Author: Vivian Kiarie /  Hansraj Bapoo
	-- Version/Date: 1.0.002 / 2013-12-30 14:44:52.5294419 -05:00
	-- Purpose: Insert into table [map].[tblEntityTestSetToSite]
		-- Notes:
	...
	BEGIN TRY
		...
		BEGIN
		  ...
		  INSERT INTO [map].[tblEntityTestSetToSite]
		  (
		...
		OPEN TableCursor 
			...
			BEGIN 
				...
				WHERE NOT EXISTS
				(
					SELECT * FROM [map].[tblEntityTestSetToSite]
	...
	BEGIN CATCH
		...
		BEGIN
			...
			+ 'Procedure Name: usp_CreateTestSetToSiteMapping' + CHAR(13)+CHAR(10)
