/*
	DROP PROCEDURE [map].[usp_CreateCompanyGroupToSiteMapping]

*/ 
CREATE PROCEDURE [map].[usp_CreateCompanyGroupToSiteMapping]
(
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_CreateCompanyGroupToSiteMapping]
	-- Author: Vivian Kiarie /  Hansraj Bapoo
	-- Version/Date: 1.0.002 / 2013-12-30 14:44:52.4894339 -05:00
	-- Purpose: Insert into table [map].[tblEntityCompanyGroupToSite]
		-- Notes:
	...
	BEGIN TRY
		...
		INSERT INTO [map].[tblEntityCompanyGroupToSite]
		(
		...
		OPEN TableCursor 
			...
			BEGIN 
				...
				WHERE NOT EXISTS
				(
					SELECT * FROM [map].[tblEntityCompanyGroupToSite]
	...
	BEGIN CATCH
		...
		BEGIN
			...
			+ 'Procedure Name: usp_CreateCompanyGroupToSiteMapping' + CHAR(13)+CHAR(10)
