...
/*
	...
	SET @dt = GETDATE()
	--EXEC [map].[usp_CreatePersonnelToSiteMapping] '886AA683-C97D-461C-AFB6-AD9A4579E51D', '00000000-0000-0000-0000-000000000001', 'F4761A16-AB2F-41EE-B6FA-D17658DF2602', @dt, 'HB'
	--EXEC [map].[usp_CreatePersonnelToSiteMapping] 'b44649ad-877a-4a41-93b1-9b0e048be377', 'f4761a16-ab2f-41ee-b6fa-d17658df2602', '46426312-e408-4af8-85fd-338b622b32bf', @dt, 'HB'

	--EXEC [map].[usp_CreatePersonnelToSiteMapping] '5dd680be-f07c-42f6-858e-6c908f1bb87b', '00000000-0000-0000-0000-000000000001', '00000000-0000-0000-0000-000000000001', @dt, 'HB'

	--EXEC [map].[usp_CreatePersonnelToSiteMapping] '1bb8c558-5277-47a5-90ae-2461bbd1eff7', '46426312-e408-4af8-85fd-338b622b32bf', 'aeba18e3-e97b-479e-8b2d-0bcd69c1c421', @dt, 'HB'

	EXEC [map].[usp_CreatePersonnelToSiteMapping] '9cd18f89-0799-4e96-8b28-f59ebdbd90cd', '00000000-0000-0000-0000-000000000001', '00000000-0000-0000-0000-000000000001', @dt, 'HB'

*/


CREATE PROCEDURE [map].[usp_CreatePersonnelToSiteMapping]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored Procedure: [map].[usp_CreatePersonnelToSiteMapping] 
	-- Author: Shawn Marlin
	-- Version/Date: 1.0.003 / 2013-03-29 8:21:10.4470770 -10:00
	-- Purpose: Creates a new Personnel To Site mapping record, and creates a child record version as necessary.
	-- Notes:
	...
	-- 5. A child record version is only created following the assignment if Record Versioning is verified to be On for the newly created entity assignment/s.
	------------------------------------------------------------------------------------------------------
	BEGIN TRY	
		DECLARE @EntityMasterRecGuid uniqueidentifier
		SELECT @EntityMasterRecGuid = _MasterRecordGuid FROM tblPersonnel
		...
		IF (@@TRANCOUNT = 0)   
        BEGIN  
            BEGIN TRANSACTION --MapPersonnel
            SET @BeginTran = 1   
		END  
		...
		BEGIN
			...
			(PersonnelGuid, SiteGuid, AssignedFromSiteGuid, CreatedDate, CreatedBy)
		...
		OPEN TableCursor 
			...
			BEGIN 
				...
				EXEC [erv].[usp_IsRecordVersioningOnForEntity] 'Personnel', @EntityMasterRecGuid, @AssignedFromSiteGuid, @IsRecVerOn OUTPUT
				IF ((@IsRecVerOn IS NOT NULL) AND (@IsRecVerOn = 1))
				BEGIN
					DECLARE @parentEntityGuid uniqueidentifier
					SELECT @parentEntityGuid = [erv].[udf_GetFirstParentRecordVersionGuid] ('Personnel', @EntityMasterRecGuid, @parentSiteGuid)
					IF (@parentEntityGuid IS NULL)
					BEGIN
						RAISERROR('Cannot locate the parent record version for the assignment.',16,1); 
						RETURN;
					END
					EXEC [erv].[usp_CreatePersonnelChildRecordVersion] @parentEntityGuid, @childSiteGuid, @CreatedDate, @CreatedBy
				END
		...
		COMMIT TRANSACTION --MapPersonnel		
	END TRY
	BEGIN CATCH  
		IF ((@@TRANCOUNT > 0) AND (XACT_STATE() <> 0) AND (@BeginTran = 1))
				ROLLBACK TRANSACTION --MapPersonnel
		...
		BEGIN
			...
			SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
							...
							+ 'Procedure Name: [map].usp_CreatePersonnelToSiteMapping' + CHAR(13)+CHAR(10)                  
