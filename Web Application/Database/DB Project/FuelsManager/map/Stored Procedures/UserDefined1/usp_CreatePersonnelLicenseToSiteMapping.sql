/*
	DROP PROCEDURE [map].[usp_CreatePersonnelLicenseToSiteMapping]

*/  

CREATE PROCEDURE [map].[usp_CreatePersonnelLicenseToSiteMapping]
(
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_CreatePersonnelLicenseToSiteMapping]
	...
	-- Cascading Assignments do not apply to the base entity assignment (assignment of the entity record with its owner site guid).
		...
		OPEN TableCursor 
			...
			BEGIN 
				...
				WHERE NOT EXISTS
				(
					SELECT * FROM [map].[tblEntityPersonnelLicenseToSite]
	...
	BEGIN CATCH
		...
		BEGIN
			...
			+ 'Procedure Name: usp_CreatePersonnelLicenseToSiteMapping' + CHAR(13)+CHAR(10)
