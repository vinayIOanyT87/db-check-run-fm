/*
	DROP PROCEDURE [map].[usp_CreateExitMessageToSiteMapping]

*/  
CREATE PROCEDURE [map].[usp_CreateExitMessageToSiteMapping]
(
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_CreateExitMessageToSiteMapping]
	-- Author: Vivian Kiarie /  Hansraj Bapoo
	-- Version/Date: 1.0.002 / 2013-12-30 14:44:52.5074375 -05:00
	-- Purpose: Insert into table [map].[tblEntityExitMessageToSite]
	-- Notes:
	...
	BEGIN TRY
		...
		INSERT INTO [map].[tblEntityExitMessageToSite]
		(
		...
		OPEN TableCursor 
			...
			BEGIN 
				...
				WHERE NOT EXISTS
				(
					SELECT * FROM [map].[tblEntityExitMessageToSite]
	...
	BEGIN CATCH
		...
		BEGIN
			...
			+ 'Procedure Name: usp_CreateExitMessageToSiteMapping' + CHAR(13)+CHAR(10)
