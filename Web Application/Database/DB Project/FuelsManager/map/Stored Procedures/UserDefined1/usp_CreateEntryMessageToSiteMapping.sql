/*
	DROP PROCEDURE [map].[usp_CreateEntryMessageToSiteMapping]

*/
CREATE PROCEDURE [map].[usp_CreateEntryMessageToSiteMapping]
(
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_CreateEntryMessageToSiteMapping]
	-- Author: Vivian Kiarie /  Hansraj Bapoo
	-- Version/Date: 1.0.002 / 2013-12-30 14:44:52.5024365 -05:00
	-- Purpose: Insert into table [map].[tblEntityEntryMessageToSite]
		-- Notes:
	...
	BEGIN TRY
		...
		INSERT INTO [map].[tblEntityEntryMessageToSite]
		(
		...
		OPEN TableCursor 
			...
			BEGIN 
				...
				WHERE NOT EXISTS
				(
					SELECT * FROM [map].[tblEntityEntryMessageToSite]
	...
	BEGIN CATCH
		...
		BEGIN
			...
			+ 'Procedure Name: usp_CreateEntryMessageToSiteMapping' + CHAR(13)+CHAR(10)
