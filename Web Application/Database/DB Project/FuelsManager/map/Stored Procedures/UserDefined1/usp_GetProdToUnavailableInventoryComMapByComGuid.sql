/*
	DROP PROCEDURE [map].[usp_GetProdToUnavailableInventoryComMapByComGuid]

	EXEC [map].[usp_GetProdToUnavailableInventoryComMapByComGuid] '38B24944-C577-4193-B9FC-554205CB39D7', '00000000-0000-0000-0000-000000000001'


*/

CREATE PROCEDURE [map].[usp_GetProdToUnavailableInventoryComMapByComGuid]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored Procedure: [map].[usp_GetProdToUnavailableInventoryComMapByComGuid] 
	-- Author: Hansraj Bapoo
	-- Version/Date: 1.0.003 / 2013-04-04 14:21:10.4470770 -10:00
	-- Purpose: Retrieves the map.tblProductToUnavailableInventoryCompany mappings for a given CompanyGuid and Site/Sitegroup.
	-- Notes:
	...
	-- 3. @HideHiddenProducts: If true, only products that have a NULL inactive date will be shown.
	-- 4. This stored procedure replaces the ProductMapClass.ProductMapClass.EnumerateByAssignedToGuidAndTypeSQL() inline SQL, for the case where Type = PRODUCT_MAP_TYPE.UNAVAILABLE_INVENTORY_COMPANY_MAP and where bInTransaction is false.
	...
	BEGIN TRY	
		...
		DECLARE @targetRecGuid uniqueidentifier
		...
		SELECT @targetRecGuid = [erv].[udf_GetFirstParentRecordVersionGuid] ('Company', @masterRecGuid, @TargetSiteGuid)

		SELECT @firstAvailableOwnerSite = a.SiteGuid
		FROM tblCompanies a
		WHERE CompanyGuid = @targetRecGuid
		...
		BEGIN		
			...
			(
				...
				INNER JOIN map.tblProductToUnavailableInventoryCompany c
		...
		WHERE a.AssignedToCompanyGuid = @targetRecGuid
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: [map].usp_GetProdToUnavailableInventoryComMapByComGuid' + CHAR(13)+CHAR(10)                  
