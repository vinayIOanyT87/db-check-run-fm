/*
	DROP PROCEDURE [map].[usp_CreateDispatchConfigurationToSiteMapping]

*/ 
CREATE PROCEDURE [map].[usp_CreateDispatchConfigurationToSiteMapping]
(
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_CreateDispatchConfigurationToSiteMapping]
	-- Author: Vivian Kiarie /  Hansraj Bapoo
	-- Version/Date: 1.0.002 / 2013-12-30 14:44:52.4944349 -05:00
	-- Purpose: Insert into table [map].[tblEntityDispatchConfigurationToSite]
		-- Notes:
	...
	BEGIN TRY
		...
		OPEN TableCursor 
			...
			BEGIN 
				...
				WHERE NOT EXISTS
				(
					 SELECT * FROM [map].[tblEntityDispatchConfigurationToSite]
	...
	BEGIN CATCH
		...
		BEGIN
			...
			+ 'Procedure Name: usp_CreateDispatchConfigurationToSiteMapping' + CHAR(13)+CHAR(10)
