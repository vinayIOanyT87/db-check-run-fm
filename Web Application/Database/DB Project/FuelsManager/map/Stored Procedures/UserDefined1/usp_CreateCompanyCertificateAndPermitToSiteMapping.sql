/*
	DROP PROCEDURE [map].[usp_CreateCompanyCertificateAndPermitToSiteMapping]

*/

CREATE PROCEDURE [map].[usp_CreateCompanyCertificateAndPermitToSiteMapping]
(
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_CreateCompanyCertificateAndPermitToSiteMapping]
	-- Author: Vivian Kiarie /  Hansraj Bapoo
	-- Version/Date: 1.0.002 / 2013-12-30 14:44:52.4884337 -05:00
	-- Purpose: Insert into table [map].[tblEntityCompanyCertificateAndPermitToSite]
		-- Notes:
	...
	BEGIN TRY
		...
		INSERT INTO [map].[tblEntityCompanyCertificateAndPermitToSite]
		(
		  [QualificationGuid]
		  ,	[SiteGuid]
		  ,	[CreatedDate]
		...
		OPEN TableCursor 
			...
			BEGIN 
				...
				WHERE NOT EXISTS
				(
					SELECT * FROM [map].[tblEntityCompanyCertificateAndPermitToSite]
	...
	BEGIN CATCH
		...
		BEGIN
			...
			+ 'Procedure Name: usp_CreateCompanyCertificateAndPermitToSiteMapping' + CHAR(13)+CHAR(10)
