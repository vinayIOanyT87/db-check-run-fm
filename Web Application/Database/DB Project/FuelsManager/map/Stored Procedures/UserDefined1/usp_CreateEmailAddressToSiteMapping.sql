/*
	DROP PROCEDURE [map].[usp_CreateEmailAddressToSiteMapping]

*/ 
CREATE PROCEDURE [map].[usp_CreateEmailAddressToSiteMapping]
(
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_CreateEmailAddressToSiteMapping]
	-- Author: Vivian Kiarie /  Hansraj Bapoo
	-- Version/Date: 1.0.002 / 2013-12-30 14:44:52.5004361 -05:00
	-- Purpose: Insert into table [map].[tblEntityEmailAddressToSite]
		-- Notes:
	...
	BEGIN TRY
		...
		INSERT INTO [map].[tblEntityEmailAddressToSite]
		(
		...
		OPEN TableCursor 
			...
			BEGIN 
				...
				WHERE NOT EXISTS
				(
					SELECT * FROM [map].[tblEntityEmailAddressToSite]
	...
	BEGIN CATCH
		...
		BEGIN
			...
			+ 'Procedure Name: usp_CreateEmailAddressToSiteMapping' + CHAR(13)+CHAR(10)
