/*
	DROP PROCEDURE [map].[usp_CreateProductGroupToSiteMapping]

*/  

CREATE PROCEDURE [map].[usp_CreateProductGroupToSiteMapping]
(	
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_CreateProductGroupToSiteMapping]
	-- Author: Vivian Kiarie /  Hansraj Bapoo
	-- Version/Date: 1.0.002 / 2013-12-30 14:44:52.5224405 -05:00
	-- Purpose: Insert into table [map].[tblEntityProductGroupToSite]
		-- Notes:
	...
	BEGIN TRY
		...
		INSERT INTO [map].[tblEntityProductGroupToSite]
		(		
		...
		OPEN TableCursor 
			...
			BEGIN 
				...
				WHERE NOT EXISTS
				(
					SELECT * FROM [map].[tblEntityProductGroupToSite]
	...
	BEGIN CATCH
		...
		BEGIN
			...
			+ 'Procedure Name: usp_CreateProductGroupToSiteMapping' + CHAR(13)+CHAR(10)
