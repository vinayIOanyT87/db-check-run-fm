...
/*
	EXEC [map].[usp_GetTransactionAliasToSiteByRecordGuid] '6FC6619C-EEF5-436C-8D68-0AB7CC7B0B6E', NULL, 1
	EXEC [map].[usp_GetTransactionAliasToSiteByRecordGuid] '6FC6619C-EEF5-436C-8D68-0AB7CC7B0B6E', 'B7BD440B-674F-46F6-977A-CEFC540B1A90', 1
*/

CREATE PROCEDURE [map].[usp_GetTransactionAliasToSiteByRecordGuid]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_GetTransactionAliasToSiteByRecordGuid]
	-- Author: Hansraj Bapoo
	-- Version/Date: 1.0.003 / 2012-07-13 14:21:10.4470770 -04:00
	-- Purpose: Retrieves the TransactionAliasToSite assignments for a given TransactionAlias RecordGuid to a given site/sitegroup, and optionally to children site/sitegroup of the given sitegroup as well. 	
	-- Notes:
	-- 1. @EntityRecordGuid: Guid of the TransactionAlias record for which the mapping hierarchy is to be retrieved. It can be either the index of the specific child record version being examined, or the index of the master record version.
	...
	-- 7. This stored procedure replaces the EntityTositeMapClass.SelectSQL() inline SQL for the case where entityType is either TRANSACTION_ALIAS or TRANSACTION_ALIAS_NAME and bInTransaction is false.
	------------------------------------------------------------------------------------------------------
	SET NOCOUNT ON;
	BEGIN TRY
		...
		SELECT @EntityMasterRecordGuid = _MasterRecordGuid FROM tblTransactionAliases WHERE TransactionAliasGuid = @EntityRecordGuid

		IF (@AssignedToSiteGuid IS NULL)
		BEGIN
			SELECT @AssignedToSiteGuid = SiteGuid FROM tblTransactionAliases
			WHERE TransactionAliasGuid = @EntityMasterRecordGuid
		...
		SELECT a.TransactionAliasToSiteGuid MappingGuid, a.TransactionAliasGuid EntityRecordGuid, d.AliasName EntityId, a.SiteGuid AssignedToSiteGuid, b.SiteGroupId AssignedToSiteId, b.HierarchyLevel, a.CreatedDate, a.CreatedBy, a.UpdatedDate, a.UpdatedBy, a._RowVersion, a.AssignedFromSiteGuid , c.ID AssignedFromSiteId 
		...
		ON d._MasterRecordGuid = a.TransactionAliasGuid
		AND d.TransactionAliasGuid = [erv].[udf_GetFirstParentRecordVersionGuid] ('Transaction_Alias', a.TransactionAliasGuid, a.SiteGuid)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: map.usp_GetTransactionAliasToSiteByRecordGuid' + CHAR(13)+CHAR(10)                  
