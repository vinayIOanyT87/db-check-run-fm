...
/*
	...
	SET @dt = GETDATE()
	--EXEC [map].[usp_CreateCompanyToSiteMapping] '886AA683-C97D-461C-AFB6-AD9A4579E51D', '00000000-0000-0000-0000-000000000001', 'F4761A16-AB2F-41EE-B6FA-D17658DF2602', @dt, 'HB'
	--EXEC [map].[usp_CreateCompanyToSiteMapping] 'b44649ad-877a-4a41-93b1-9b0e048be377', 'f4761a16-ab2f-41ee-b6fa-d17658df2602', '46426312-e408-4af8-85fd-338b622b32bf', @dt, 'HB'

	--EXEC [map].[usp_CreateCompanyToSiteMapping] '5dd680be-f07c-42f6-858e-6c908f1bb87b', '00000000-0000-0000-0000-000000000001', '00000000-0000-0000-0000-000000000001', @dt, 'HB'

	--EXEC [map].[usp_CreateCompanyToSiteMapping] '1bb8c558-5277-47a5-90ae-2461bbd1eff7', '46426312-e408-4af8-85fd-338b622b32bf', 'aeba18e3-e97b-479e-8b2d-0bcd69c1c421', @dt, 'HB'

	EXEC [map].[usp_CreateCompanyToSiteMapping] '9cd18f89-0799-4e96-8b28-f59ebdbd90cd', '00000000-0000-0000-0000-000000000001', '00000000-0000-0000-0000-000000000001', @dt, 'HB'

*/


CREATE PROCEDURE [map].[usp_CreateCompanyToSiteMapping]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored Procedure: [map].[usp_CreateCompanyToSiteMapping] 
	-- Author: Hansraj Bapoo
	-- Version/Date: 1.0.003 / 2012-07-13 14:21:10.4470770 -04:00
	-- Purpose: Creates a new Company To Site mapping record, and creates a child record version as necessary.
	-- Notes:
	...
	BEGIN TRY	
		...
		SELECT @EntityMasterRecGuid = _MasterRecordGuid FROM tblCompanies
		...
		BEGIN
		  ...
		  (CompanyGuid, SiteGuid, AssignedFromSiteGuid, CreatedDate, CreatedBy)
		...
		OPEN TableCursor 
			...
			BEGIN 
				...
				-- Add CompanyToUserGroup Mapping based upon mapping at @AssignedFromSite and Company Entity Assigned to the Site
				...
				SELECT a.CompanyGuid, a.GroupGuid, @childSiteGuid, a.ID, @CreatedDate, @CreatedBy, @CreatedDate, @CreatedBy 
				FROM [map].[tblCompanyCompanyToUserGroup] a
				INNER JOIN [map].[tblEntityUserGroupToSite] b ON b.GroupGuid = a.GroupGuid AND b.SiteGuid = @childSiteGuid
				WHERE a.[SiteGuid] = @parentSiteGuid AND a.CompanyGuid = @EntityRecordGuid
				AND NOT EXISTS
				(
					 SELECT * FROM [map].[tblCompanyCompanyToUserGroup] c
					 WHERE c.CompanyGuid = a.CompanyGuid
					 AND c.GroupGuid = a.GroupGuid
					 AND c.SiteGuid = @childSiteGuid
				)

				-- Add CompanyToRole Mapping based upon mapping at @AssignedFromSite and Company Entity Assigned to the Site
				...
				SELECT a.CompanyGuid, a.LookupCompanyRoleIndex, @childSiteGuid, @CreatedDate, @CreatedBy, @CreatedDate, @CreatedBy 
				FROM [map].[tblCompanyToRole] a
				WHERE a.[SiteGuid] = @parentSiteGuid AND a.CompanyGuid = @EntityRecordGuid
				AND NOT EXISTS
				(
						SELECT * FROM [map].[tblCompanyToRole] c
						WHERE c.CompanyGuid = a.CompanyGuid
						AND c.LookupCompanyRoleIndex = a.LookupCompanyRoleIndex
						AND c.SiteGuid = @childSiteGuid
				)
				...
				EXEC [erv].[usp_IsRecordVersioningOnForEntity] 'Company', @EntityMasterRecGuid, @AssignedFromSiteGuid, @IsRecVerOn OUTPUT
				IF ((@IsRecVerOn IS NOT NULL) AND (@IsRecVerOn = 1))
				BEGIN
					DECLARE @parentEntityGuid uniqueidentifier
					SELECT @parentEntityGuid = [erv].[udf_GetFirstParentRecordVersionGuid] ('Company', @EntityMasterRecGuid, @parentSiteGuid)
					IF (@parentEntityGuid IS NULL)
					BEGIN
						RAISERROR('Cannot locate the parent record version for the assignment.',16,1); 
						RETURN;
					END
					EXEC [erv].[usp_CreateCompanyChildRecordVersion] @parentEntityGuid, @childSiteGuid, @CreatedDate, @CreatedBy
				END
		...
		COMMIT TRANSACTION --MapCompany		
	END TRY
	BEGIN CATCH  
		IF ((@@TRANCOUNT > 0) AND (XACT_STATE() <> 0) AND (@BeginTran = 1))
				ROLLBACK TRANSACTION --MapCompany
		DECLARE	@_ErrMessage NVARCHAR(2048)      
				, @_ErrNumber INT           
				, @_ErrProcName NVARCHAR(126)           
				, @_ErrLineNumber INT;            
		SET @_ErrMessage = ERROR_MESSAGE();        
		SET @_ErrNumber = ERROR_NUMBER(); 		IF(@_ErrNumber = 547 AND CHARINDEX('Uniqueness',@_ErrMessage,0) <> 0)
		...
		BEGIN
			...
			SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
							...
							+ 'Procedure Name: [map].usp_CreateCompanyToSiteMapping' + CHAR(13)+CHAR(10)                  
