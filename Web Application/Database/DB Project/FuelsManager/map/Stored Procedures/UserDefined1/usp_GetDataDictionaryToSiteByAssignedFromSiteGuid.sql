...
/*
	EXEC [map].[usp_GetDataDictionaryToSiteByAssignedFromSiteGuid] '00000000-0000-0000-0000-000000000001'
*/

CREATE PROCEDURE [map].[usp_GetDataDictionaryToSiteByAssignedFromSiteGuid]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_GetDataDictionaryToSiteByAssignedFromSiteGuid]
	-- Author: Hansraj Bapoo
	-- Version/Date: 1.0.003 / 2012-07-13 14:21:10.4470770 -04:00
	-- Purpose: Retrieves the DataDictionaryToSite assignments from a given sitegroup. 	
	-- Notes:
	-- 1. @AssignedFromSiteGuid: Guid of the AssignedFrom sitegroup from which the mapping is to be queried.	
	-- 2. DataDictionaryToSite mappings are not applied/managed on individual DataDictionary records, but across all DataDictionary records for a given sitegroup (@EntityRecordGuid)
	-- 3. This operation helps verify if a DataDictionary mapping to a given sitegroup is valid. If a DataDictionary mapping, for a given EntityRecordGuid/OwnerSiteGuid (e.g. SGZ) 
	--    has already been assigned from a given sitegroup (e.g. SGZ), then assigning a DataDictionary mapping to the same sitegroup from a higher level sitegroup (e.g. SGY) will 
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: map.usp_GetDataDictionaryToSiteByAssignedFromSiteGuid' + CHAR(13)+CHAR(10)                  
