/*
	 DROP PROCEDURE [map].[usp_CreateFuelCardLimitToSiteMapping]

*/  

CREATE PROCEDURE [map].[usp_CreateFuelCardLimitToSiteMapping]
(
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_CreateFuelCardLimitToSiteMapping]
	-- Author: Vivian Kiarie /  Hansraj Bapoo
	-- Version/Date: 1.0.002 / 2013-12-30 14:44:52.4574275 -05:00
	-- Purpose: Insert into table [map].[tblEntityFuelCardLimitToSite]
	-- Notes:
	...
	BEGIN TRY
		...
		INSERT INTO [map].[tblEntityFuelCardLimitToSite]
		(
		  [FuelCardLimitGuid]
		  ,	[SiteGuid]
		  ,	[CreatedDate]
		...
		OPEN TableCursor 
			...
			BEGIN 
				...
				WHERE NOT EXISTS
				(
					SELECT * FROM [map].[tblEntityFuelCardLimitToSite]
	...
	BEGIN CATCH
		...
		BEGIN
			...
			+ 'Procedure Name: usp_CreateFuelCardLimitToSiteMapping' + CHAR(13)+CHAR(10)
