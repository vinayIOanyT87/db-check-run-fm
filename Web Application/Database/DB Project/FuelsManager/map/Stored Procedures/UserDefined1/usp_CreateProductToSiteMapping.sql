/*
	DROP PROCEDURE [map].[usp_CreateProductToSiteMapping]

	DECLARE @dt DateTimeOffset(7)
	SET @dt = GETDATE()
	EXEC [map].[usp_CreateProductToSiteMapping] '5BEDDA5A-6ACB-41C6-B03D-8789C42BA2CE', '3DB9DF78-DE73-4D19-8E9F-56CF8BEF6470', '02F8FF74-F608-4D8F-A853-000BBBAAC67D', @dt, 'HB'
	--EXEC [map].[usp_CreateProductToSiteMapping] 'b44649ad-877a-4a41-93b1-9b0e048be377', 'f4761a16-ab2f-41ee-b6fa-d17658df2602', '46426312-e408-4af8-85fd-338b622b32bf', @dt, 'HB'

	--EXEC [map].[usp_CreateProductToSiteMapping] '5dd680be-f07c-42f6-858e-6c908f1bb87b', '00000000-0000-0000-0000-000000000001', '00000000-0000-0000-0000-000000000001', @dt, 'HB'

	--EXEC [map].[usp_CreateProductToSiteMapping] '1bb8c558-5277-47a5-90ae-2461bbd1eff7', '46426312-e408-4af8-85fd-338b622b32bf', 'aeba18e3-e97b-479e-8b2d-0bcd69c1c421', @dt, 'HB'

	EXEC [map].[usp_CreateProductToSiteMapping] '9cd18f89-0799-4e96-8b28-f59ebdbd90cd', '00000000-0000-0000-0000-000000000001', '00000000-0000-0000-0000-000000000001', @dt, 'HB'

*/


CREATE PROCEDURE [map].[usp_CreateProductToSiteMapping]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored Procedure: [map].[usp_CreateProductToSiteMapping] 
	-- Author: Hansraj Bapoo
	-- Version/Date: 1.0.003 / 2012-07-13 14:21:10.4470770 -04:00
	-- Purpose: Creates a new Product To Site mapping record, and creates a child record version as necessary.
	-- Notes:
	...
	BEGIN TRY	
		...
		SELECT @EntityMasterRecGuid = _MasterRecordGuid FROM tblProducts
		...
		BEGIN
			...
			(ProductGuid, SiteGuid, AssignedFromSiteGuid, CreatedDate, CreatedBy)
		...
		OPEN TableCursor 
			...
			BEGIN 
				...
				EXEC [erv].[usp_IsRecordVersioningOnForEntity] 'Product', @EntityMasterRecGuid, @AssignedFromSiteGuid, @IsRecVerOn OUTPUT
				IF ((@IsRecVerOn IS NOT NULL) AND (@IsRecVerOn = 1))
				BEGIN
					DECLARE @parentEntityGuid uniqueidentifier
					SELECT @parentEntityGuid = [erv].[udf_GetFirstParentRecordVersionGuid] ('Product', @EntityMasterRecGuid, @parentSiteGuid)
					IF (@parentEntityGuid IS NULL)
					BEGIN
						RAISERROR('Cannot locate the parent record version for the assignment.',16,1); 
						RETURN;
					END
					EXEC [erv].[usp_CreateProductChildRecordVersion] @parentEntityGuid, @childSiteGuid, @CreatedDate, @CreatedBy
				END
		...
		COMMIT TRANSACTION --MapProduct		
	END TRY
	BEGIN CATCH  
		IF ((@@TRANCOUNT > 0) AND (XACT_STATE() <> 0) AND (@BeginTran = 1))
				ROLLBACK TRANSACTION --MapProduct
		...
		BEGIN
			...
			SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
							...
							+ 'Procedure Name: [map].usp_CreateProductToSiteMapping' + CHAR(13)+CHAR(10)                  
