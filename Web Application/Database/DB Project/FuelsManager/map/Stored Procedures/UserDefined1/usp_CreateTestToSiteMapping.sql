/*
	DROP PROCEDURE [map].[usp_CreateTestToSiteMapping]

*/ 

CREATE PROCEDURE [map].[usp_CreateTestToSiteMapping]
(
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_CreateTestToSiteMapping]
	-- Author: Vivian Kiarie /  Hansraj Bapoo
	-- Version/Date: 1.0.002 / 2013-12-30 14:44:52.5304421 -05:00
	-- Purpose: Insert into table [map].[tblEntityTestToSite]
		-- Notes:
	...
	BEGIN TRY
		...
		BEGIN
		  ...
		  INSERT INTO [map].[tblEntityTestToSite]
		  (
				[TestDefinitionGuid]
				,	[SiteGuid]
				,	[CreatedDate]
		...
		OPEN TableCursor 
			...
			BEGIN 
				...
				WHERE NOT EXISTS
				(
					SELECT * FROM [map].[tblEntityTestToSite]
	...
	BEGIN CATCH
		...
		BEGIN
			...
			+ 'Procedure Name: usp_CreateTestToSiteMapping' + CHAR(13)+CHAR(10)
