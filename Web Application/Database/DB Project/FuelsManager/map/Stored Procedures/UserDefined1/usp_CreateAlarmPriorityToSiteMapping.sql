/*
	DROP PROCEDURE [map].[usp_CreateAlarmPriorityToSiteMapping]

*/
CREATE PROCEDURE [map].[usp_CreateAlarmPriorityToSiteMapping]
(
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_CreateAlarmPriorityToSiteMapping]
	-- Author: Vivian Kiarie /  Hansraj Bapoo
	-- Version/Date: 1.0.002 / 2013-12-30 14:44:52.4504261 -05:00
	-- Purpose: Insert into table [map].[usp_CreateAlarmPriorityToSiteMapping]
	-- Notes:
	...
	BEGIN
		...
		OPEN TableCursor 
			...
			BEGIN 
				...
				WHERE NOT EXISTS
				(
					SELECT * FROM [map].[tblEntityAlarmPriorityToSite]
	...
	BEGIN CATCH
		...
		BEGIN
			...
			+ 'Procedure Name: usp_CreateAlarmPriorityToSiteMapping' + CHAR(13)+CHAR(10)
