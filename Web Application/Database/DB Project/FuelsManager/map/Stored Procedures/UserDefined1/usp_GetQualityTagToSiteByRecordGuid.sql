...
/*
	EXEC [map].[usp_GetQualityTagToSiteByRecordGuid] '0EC22D85-95F3-495E-92A9-F193CDD069E7', NULL
	EXEC [map].[usp_GetQualityTagToSiteByRecordGuid] '05C7D1E0-745D-488D-AF05-00DF1A70D05D', 'F4761A16-AB2F-41EE-B6FA-D17658DF2602', 1
*/


CREATE PROCEDURE [map].[usp_GetQualityTagToSiteByRecordGuid]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_GetQualityTagToSiteByRecordGuid]
	-- Author: Hansraj Bapoo
	-- Version/Date: 1.0.003 / 2012-07-13 14:21:10.4470770 -04:00
	-- Purpose: Retrieves the QualityTagToSite assignments for a given QualityTag RecordGuid to a given site/sitegroup, and optionally to children site/sitegroup of the given sitegroup as well. 	
	-- Notes:
	-- 1. @EntityRecordGuid: Guid of the QualityTag record for which the mapping hierarchy is to be retrieved.
	...
	-- 7. This stored procedure replaces the EntityTositeMapClass.SelectSQL() inline SQL for the case where entityType is QUALITY_TAG and bInTransaction is false.
	------------------------------------------------------------------------------------------------------
	SET NOCOUNT ON;
	BEGIN TRY
		...
		BEGIN
			SELECT @AssignedToSiteGuid = SiteGuid FROM tblQualityTags
			WHERE QualityTagGuid = @EntityRecordGuid
		END;
		...
		SELECT a.QualityTagToSiteGuid MappingGuid, a.QualityTagGuid EntityRecordGuid, d.Name EntityId, a.SiteGuid AssignedToSiteGuid, b.SiteGroupId AssignedToSiteId, b.HierarchyLevel, a.CreatedDate, a.CreatedBy, a.UpdatedDate, a.UpdatedBy, a._RowVersion, a.AssignedFromSiteGuid , c.ID AssignedFromSiteId 
		...
		INNER JOIN tblQualityTags d
		ON d.QualityTagGuid = a.QualityTagGuid
		WHERE a.QualityTagGuid = @EntityRecordGuid
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: map.usp_GetQualityTagToSiteByRecordGuid' + CHAR(13)+CHAR(10)                  
