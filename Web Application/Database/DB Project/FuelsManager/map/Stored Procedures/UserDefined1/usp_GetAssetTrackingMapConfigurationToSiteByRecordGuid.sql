CREATE PROCEDURE [map].[usp_GetAssetTrackingMapConfigurationToSiteByRecordGuid]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_GetAssetTrackingMapConfigurationToSiteByRecordGuid]
	-- Author: Richard R. Panachida
	-- Version/Date: 1.0.003 / 2016-04-26 14:21:10.4470770 -04:00
	-- Purpose: Retrieves the AssetTrackingMapConfigurationToSite assignments for a given AssetTrackingMapConfiguration RecordGuid to a given site/sitegroup, and optionally to children site/sitegroup of the given sitegroup as well. 	
	-- Notes:
	-- 1. @EntityRecordGuid: Guid of the AssetTrackingMapConfiguration record for which the mapping hierarchy is to be retrieved.
	...
	-- 7. This stored procedure replaces the EntityTositeMapClass.SelectSQL() inline SQL for the case where entityType is ASSET_TRACKING_MAP_CONFIGURATION and bInTransaction is false.
	------------------------------------------------------------------------------------------------------
	SET NOCOUNT ON;
	BEGIN TRY
		...
		BEGIN
			SELECT @AssignedToSiteGuid = SiteGuid FROM tblAssetTrackingMapConfiguration
			WHERE AssetTrackingMapConfigurationGuid = @EntityRecordGuid
		END;
		...
		SELECT	a.AssetTrackingMapConfigurationToSiteGuid MappingGuid, 
				a.AssetTrackingMapConfigurationGuid EntityRecordGuid, 
				d.MapName EntityId, 
		...
		FROM map.tblEntityAssetTrackingMapConfigurationToSite a
		     INNER JOIN @tblSiteGroupTree b ON b.SiteGroupGuid = a.SiteGuid
		     INNER JOIN tblSites c ON c.SiteGuid = a.AssignedFromSiteGuid
		     INNER JOIN tblAssetTrackingMapConfiguration d ON d.AssetTrackingMapConfigurationGuid = a.AssetTrackingMapConfigurationGuid
		WHERE a.AssetTrackingMapConfigurationGuid = @EntityRecordGuid
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: map.usp_GetAssetTrackingMapConfigurationToSiteByRecordGuid' + CHAR(13)+CHAR(10)                  
