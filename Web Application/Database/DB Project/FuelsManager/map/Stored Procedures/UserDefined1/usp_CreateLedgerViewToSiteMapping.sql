/*
	DROP PROCEDURE [map].[usp_CreateLedgerViewToSiteMapping]

*/  

CREATE PROCEDURE [map].[usp_CreateLedgerViewToSiteMapping]
(
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_CreateLedgerViewToSiteMapping]
	-- Author: Vivian Kiarie /  Hansraj Bapoo
	-- Version/Date: 1.0.002 / 2013-12-30 14:44:52.5124385 -05:00
	-- Purpose: Insert into table [map].[tblEntityLedgerViewToSite]
	-- Notes:
	...
	BEGIN TRY
		...
		INSERT INTO [map].[tblEntityLedgerViewToSite]
		(
		  [ListViewGuid]
		  ,	[SiteGuid]
		  ,	[CreatedDate]
	...
	BEGIN CATCH
		...
		BEGIN
			...
			+ 'Procedure Name: usp_CreateLedgerViewToSiteMapping' + CHAR(13)+CHAR(10)
