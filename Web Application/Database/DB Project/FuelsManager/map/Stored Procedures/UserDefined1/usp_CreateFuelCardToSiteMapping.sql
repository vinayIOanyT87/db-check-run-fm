/*
	 DROP PROCEDURE [map].[usp_CreateFuelCardToSiteMapping]

*/  

CREATE PROCEDURE [map].[usp_CreateFuelCardToSiteMapping]
(
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_CreateFuelCardToSiteMapping]
	-- Author: Vivian Kiarie /  Hansraj Bapoo
	-- Version/Date: 1.0.002 / 2013-12-30 14:44:52.4574275 -05:00
	-- Purpose: Insert into table [map].[tblEntityFuelCardToSite]
	-- Notes:
	...
	BEGIN TRY
		...
		INSERT INTO [map].[tblEntityFuelCardToSite]
		(
		  [FuelCardGuid]
		  ,	[SiteGuid]
		  ,	[CreatedDate]
		...
		OPEN TableCursor 
			...
			BEGIN 
				...
				WHERE NOT EXISTS
				(
					SELECT * FROM [map].[tblEntityFuelCardToSite]
	...
	BEGIN CATCH
		...
		BEGIN
			...
			+ 'Procedure Name: usp_CreateFuelCardToSiteMapping' + CHAR(13)+CHAR(10)
