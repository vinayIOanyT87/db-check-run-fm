/*
	DROP PROCEDURE [map].[usp_CreateStandingOfferToSiteMapping]

*/   

CREATE PROCEDURE [map].[usp_CreateStandingOfferToSiteMapping]
(
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_CreateStandingOfferToSiteMapping]
	-- Author: Vivian Kiarie /  Hansraj Bapoo
	-- Version/Date: 1.0.002 / 2013-12-30 14:44:52.5284417 -05:00
	-- Purpose: Insert into table [map].[tblEntityStandingOfferToSite]
		-- Notes:
	...
	BEGIN TRY
		...
		INSERT INTO [map].[tblEntityStandingOfferToSite]
		(
		  [StandingOfferGuid]
		  ,	[SiteGuid]
		  ,	[CreatedDate]
 	...
 	-- Cascading Assignments do not apply to the base entity assignment (assignment of the entity record with its owner site guid).
		...
		OPEN TableCursor 
			...
			BEGIN 
				...
				WHERE NOT EXISTS
				(
					SELECT * FROM [map].[tblEntityStandingOfferToSite]
	...
	BEGIN CATCH
		...
		BEGIN
			...
			+ 'Procedure Name: usp_CreateStandingOfferToSiteMapping' + CHAR(13)+CHAR(10)
