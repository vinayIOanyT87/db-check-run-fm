...
/*
	...
	EXEC [map].[usp_GetProductToCompanyMapByAdditiveProfile] '38B24944-C577-4193-B9FC-554205CB39D7', '00000000-0000-0000-0000-000000000001'


*/

CREATE PROCEDURE [map].[usp_GetProductToCompanyMapByAdditiveProfile]
(
	@AdditiveProfileGuid uniqueidentifier, @TargetSiteGuid uniqueidentifier
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored Procedure: [map].[usp_GetProductToCompanyMapByAdditiveProfile] 
	-- Author: Hansraj Bapoo
	-- Version/Date: 1.0.003 / 2013-04-04 14:21:10.4470770 -10:00
	-- Purpose: Retrieves the Product-To-Company mappings for a given AdditiveProfileGuid and Site/Sitegroup.
	-- Notes:
	-- 1. @AdditiveProfileGuid: AdditiveProfileGuid for which to fetch the mapping record. IF NULL then all records are returned.
	-- 2. @TargetSiteGuid: Site/Sitegroup for which to retrieve the mappings.
	-- 3. This stored procedure replaces the ProductMapClass.EnumerateByAdditiveProfileGuidSQL() inline SQL, for the case where Type = PRODUCT_MAP_TYPE.PRODUCT_COMPANY_MAP.
	...
	BEGIN TRY	
		...
		INNER JOIN [erv].[udf_GetCompanyRecordVersions] (@TargetSiteGuid) h	ON h.CompanyGuid = a.AssignedToCompanyGuid
		INNER JOIN tblCompanies f ON f.CompanyGuid = h.CompanyGuid		
		LEFT OUTER JOIN map.tblProductToCompanyGroup g ON g.ProductGuid = a.ProductGuid
		WHERE (a.AdditiveProfileGuid = @AdditiveProfileGuid OR g.AdditiveProfileGuid = @AdditiveProfileGuid)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: [map].usp_GetProductToCompanyMapByAdditiveProfile' + CHAR(13)+CHAR(10)                  
