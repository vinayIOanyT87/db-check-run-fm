/*
	 DROP PROCEDURE [map].[usp_CreateFuelCardTypeToSiteMapping]

*/  

CREATE PROCEDURE [map].[usp_CreateFuelCardTypeToSiteMapping]
(
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_CreateFuelCardTypeToSiteMapping]
	-- Author: Vivian Kiarie /  Hansraj Bapoo
	-- Version/Date: 1.0.002 / 2013-12-30 14:44:52.4574275 -05:00
	-- Purpose: Insert into table [map].[tblEntityFuelCardTypeToSite]
	-- Notes:
	...
	BEGIN TRY
		...
		INSERT INTO [map].[tblEntityFuelCardTypeToSite]
		(		
		...
		OPEN TableCursor 
			...
			BEGIN 
				...
				WHERE NOT EXISTS
				(
					SELECT * FROM [map].[tblEntityFuelCardTypeToSite]
	...
	BEGIN CATCH
		...
		BEGIN
			...
			+ 'Procedure Name: usp_CreateFuelCardTypeToSiteMapping' + CHAR(13)+CHAR(10)
