...
CREATE PROCEDURE [map].[usp_GetFootnoteToShipToMapsByShipTo]
(
	@TargetSiteGuid uniqueidentifier, @ShipToGuid uniqueidentifier
)
AS
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored Procedure: [dbo].[usp_GetFootnoteToShipToMapsByShipTo] 
	-- Author: Hansraj Bapoo
	-- Version/Date: 1.0.003 / 2012-07-13 14:21:10.4470770 -04:00
	-- Purpose: Retrieve the map.tblApplicationStringToFootNoteShipTo records for a given Ship To Company and a given Site/SiteGroup.
	-- Notes:
	-- 1. @TargetSiteGuid: Limit results to Companies that have been assigned to this site/sitegroup only
	-- 2. @ShipToGuid: Company for which the Footnote mappings are to be retrieved.
	-- 3. This stored procedure replaces the ApplicationStringMapClass.EnumerateByAssignedToGuidAndTypeSQL SQL inline SQL for the case where Type = STRING_MAP_TYPE.FOOT_NOTE_SHIPTO.
	...
	BEGIN TRY	
		...
		WHERE 
		( 
			((@ShipToGuid IS NOT NULL) AND (a.CompanyGuid = @ShipToGuid ))
			OR 
			((@ShipToGuid IS NULL) AND (a.CompanyGuid IS NULL))
		)
		ORDER BY a.Sequence
	...
	BEGIN CATCH  
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: [map].usp_GetFootnoteToShipToMapsByShipTo' + CHAR(13)+CHAR(10)                  
