/*
	DROP PROCEDURE [map].[usp_CreateProductMessageToSiteMapping]

*/  

CREATE PROCEDURE [map].[usp_CreateProductMessageToSiteMapping]
(
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_CreateProductMessageToSiteMapping]
	-- Author: Vivian Kiarie /  Hansraj Bapoo
	-- Version/Date: 1.0.002 / 2013-12-30 14:44:52.5234407 -05:00
	-- Purpose: Insert into table [map].[tblEntityProductMessageToSite]
		-- Notes:
	...
	BEGIN TRY
		...
		INSERT INTO [map].[tblEntityProductMessageToSite]
		(		
		...
		OPEN TableCursor 
			...
			BEGIN 
				...
				WHERE NOT EXISTS
				(
					SELECT * FROM [map].[tblEntityProductMessageToSite]
	...
	BEGIN CATCH
		...
		BEGIN
			...
			+ 'Procedure Name: usp_CreateProductMessageToSiteMapping' + CHAR(13)+CHAR(10)
