...
/*
	EXEC [map].[usp_GetAutoDistributionRuleToSiteByRecordGuid] '3E6A6E38-2DF0-438E-9A45-B6DD5E6DDB34', NULL
	EXEC [map].[usp_GetAutoDistributionRuleToSiteByRecordGuid] '05C7D1E0-745D-488D-AF05-00DF1A70D05D', 'F4761A16-AB2F-41EE-B6FA-D17658DF2602', 1
*/

CREATE PROCEDURE [map].[usp_GetAutoDistributionRuleToSiteByRecordGuid]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_GetAutoDistributionRuleToSiteByRecordGuid]
	-- Author: Hansraj Bapoo
	-- Version/Date: 1.0.003 / 2012-07-13 14:21:10.4470770 -04:00
	-- Purpose: Retrieves the AutoDistributionRuleToSite assignments for a given AutoDistributionRule RecordGuid to a given site/sitegroup, and optionally to children site/sitegroup of the given sitegroup as well. 	
	-- Notes:
	-- 1. @EntityRecordGuid: Guid of the AutoDistributionRule record for which the mapping hierarchy is to be retrieved.
	...
	-- 7. This stored procedure replaces the EntityTositeMapClass.SelectSQL() inline SQL for the case where entityType is AUTODISTRIBUTION_RULE and bInTransaction is false.
	------------------------------------------------------------------------------------------------------
	SET NOCOUNT ON;
	BEGIN TRY
		...
		BEGIN
			SELECT @AssignedToSiteGuid = SiteGuid FROM tblAutoDistributionRule
			WHERE AutoDistributionRuleGuid = @EntityRecordGuid
		END;
		...
		SELECT a.AutoDistributionRuleToSiteGuid MappingGuid, a.AutoDistributionRuleGuid EntityRecordGuid, d.RuleID EntityId, a.SiteGuid AssignedToSiteGuid, b.SiteGroupId AssignedToSiteId, b.HierarchyLevel, a.CreatedDate, a.CreatedBy, a.UpdatedDate, a.UpdatedBy, a._RowVersion, a.AssignedFromSiteGuid , c.ID AssignedFromSiteId 
		...
		INNER JOIN tblAutoDistributionRule d
		ON d.AutoDistributionRuleGuid = a.AutoDistributionRuleGuid
		WHERE a.AutoDistributionRuleGuid = @EntityRecordGuid
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: map.usp_GetAutoDistributionRuleToSiteByRecordGuid' + CHAR(13)+CHAR(10)                  
