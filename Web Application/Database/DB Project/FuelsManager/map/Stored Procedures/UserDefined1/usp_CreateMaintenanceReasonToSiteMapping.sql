/*
	DROP PROCEDURE [map].[usp_CreateMaintenanceReasonToSiteMapping]

*/ 

CREATE PROCEDURE [map].[usp_CreateMaintenanceReasonToSiteMapping]
(
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_CreateMaintenanceReasonToSiteMapping]
	-- Author: Vivian Kiarie /  Hansraj Bapoo
	-- Version/Date: 1.0.002 / 2013-12-30 14:44:52.4864333 -05:00
	-- Purpose: Insert into table [map].[tblEntityMaintenanceReasonToSite]
		-- Notes:
	...
	BEGIN TRY
		...
		INSERT INTO [map].[tblEntityMaintenanceReasonToSite]
		(
		  [SiteGuid]
	...
	-- Cascading Assignments do not apply to the base entity assignment (assignment of the entity record with its owner site guid).
		...
		OPEN TableCursor 
			...
			BEGIN 
				...
				WHERE NOT EXISTS
				(
					SELECT * FROM [map].[tblEntityMaintenanceReasonToSite]
	...
	BEGIN CATCH
		...
		BEGIN
			...
			+ 'Procedure Name: usp_CreateMaintenanceReasonToSiteMapping' + CHAR(13)+CHAR(10)
