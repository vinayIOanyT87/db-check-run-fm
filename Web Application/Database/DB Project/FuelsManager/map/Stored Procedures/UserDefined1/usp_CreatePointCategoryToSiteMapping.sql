/*
	DROP PROCEDURE [map].[usp_CreatePointCategoryToSiteMapping]

*/ 

CREATE PROCEDURE [map].[usp_CreatePointCategoryToSiteMapping]
(
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_CreatePointCategoryToSiteMapping]
	-- Author: Vivian Kiarie /  Hansraj Bapoo
	-- Version/Date: 1.0.002 / 2013-12-30 14:44:52.4924345 -05:00
	-- Purpose: Insert into table [map].[tblEntityPointCategoryToSite]
		-- Notes:
	...
	BEGIN TRY
		...
		INSERT INTO [map].[tblEntityPointCategoryToSite]
		(		
		...
		OPEN TableCursor 
			...
			BEGIN 
				...
				WHERE NOT EXISTS
				(
					SELECT * FROM [map].[tblEntityPointCategoryToSite]
	...
	BEGIN CATCH
		...
		BEGIN
			...
			+ 'Procedure Name: usp_CreatePointCategoryToSiteMapping' + CHAR(13)+CHAR(10)
