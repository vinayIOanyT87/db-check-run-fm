/*
	DROP PROCEDURE [map].[usp_CreateAssetTrackingDeviceToSiteMapping]

*/ 
CREATE PROCEDURE [map].[usp_CreateAssetTrackingDeviceToSiteMapping]
(
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_CreateAssetTrackingDeviceToSiteMapping]
	-- Author: Vivian Kiarie /  Hansraj Bapoo
	-- Version/Date: 1.0.002 / 2016-04-26 14:44:52.5104381 -05:00
	-- Purpose: Insert into table [map].[tblEntityAssetTrackingDeviceToSite]
		-- Notes:
	...
	BEGIN TRY
		...
		INSERT INTO [map].[tblEntityAssetTrackingDeviceToSite]
		(
		...
		OPEN TableCursor 
			...
			BEGIN 			
				...
				WHERE NOT EXISTS
				(
					SELECT * FROM [map].[tblEntityAssetTrackingDeviceToSite]
	...
	BEGIN CATCH
		...
		BEGIN
			...
			+ 'Procedure Name: usp_CreateAssetTrackingDeviceToSiteMapping' + CHAR(13)+CHAR(10)
