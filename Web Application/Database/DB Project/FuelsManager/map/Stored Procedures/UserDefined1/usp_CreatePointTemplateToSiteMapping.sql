/*
	DROP PROCEDURE [map].[usp_CreatePointTemplateToSiteMapping]

*/  

CREATE PROCEDURE [map].[usp_CreatePointTemplateToSiteMapping]
(
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_CreatePointTemplateToSiteMapping]
	-- Author: Vivian Kiarie /  Hansraj Bapoo
	-- Version/Date: 1.0.002 / 2013-12-30 14:44:52.5244409 -05:00
	-- Purpose: Insert into table [map].[tblEntityPointTemplateToSite]
		-- Notes:
	...
	BEGIN TRY
		...
		INSERT INTO [map].[tblEntityPointTemplateToSite]
		(
		  [PointTemplateGuid]
		  ,	[SiteGuid]
		  ,	[CreatedDate]
		...
		OPEN TableCursor 
			...
			BEGIN 
				...
				WHERE NOT EXISTS
				(
					SELECT * FROM [map].[tblEntityPointTemplateToSite]
	...
	BEGIN CATCH
		...
		BEGIN
			...
			+ 'Procedure Name: usp_CreatePointTemplateToSiteMapping' + CHAR(13)+CHAR(10)
