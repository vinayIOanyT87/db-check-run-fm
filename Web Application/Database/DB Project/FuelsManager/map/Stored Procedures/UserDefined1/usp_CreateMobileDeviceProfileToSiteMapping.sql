/*
	DROP PROCEDURE [map].[usp_CreateMobileDeviceProfileToSiteMapping]

*/

CREATE PROCEDURE [map].[usp_CreateMobileDeviceProfileToSiteMapping]
(
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_CreateMobileDeviceProfileToSiteMapping]
	-- Author: Vivian Kiarie /  Hansraj Bapoo
	-- Version/Date: 1.0.002 / 2013-12-30 14:44:52.5154391 -05:00
	-- Purpose: Insert into table [map].[tblEntityMobileDeviceProfileToSite]
	-- Notes:
	...
	BEGIN TRY
		...
		INSERT INTO [map].[tblEntityMobileDeviceProfileToSite]
		(
	...
	-- Cascading Assignments do not apply to the base entity assignment (assignment of the entity record with its owner site guid).
		...
		OPEN TableCursor 
			...
			BEGIN 
				...
				WHERE NOT EXISTS
				(
					SELECT * FROM [map].[tblEntityMobileDeviceProfileToSite]
	...
	BEGIN CATCH
		...
		BEGIN
			...
			+ 'Procedure Name: usp_CreateMobileDeviceProfileToSiteMapping' + CHAR(13)+CHAR(10)
