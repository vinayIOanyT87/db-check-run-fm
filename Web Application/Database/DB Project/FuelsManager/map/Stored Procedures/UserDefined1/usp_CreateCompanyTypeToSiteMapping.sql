/*
	DROP PROCEDURE [map].[usp_CreateCompanyTypeToSiteMapping]

*/  
CREATE PROCEDURE [map].[usp_CreateCompanyTypeToSiteMapping]
(
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[gsp_EntityCompanyTypeToSiteInsertByPK]
	-- Author: Vivian Kiarie /  Hansraj Bapoo
	-- Version/Date: 1.0.002 / 2013-12-30 14:44:52.4924345 -05:00
	-- Purpose: Insert into table [map].[tblEntityCompanyTypeToSite]
		-- Notes:
	...
	BEGIN TRY
		...
		INSERT INTO [map].[tblEntityCompanyTypeToSite]
		(
		...
		OPEN TableCursor 
			...
			BEGIN 
				...
				WHERE NOT EXISTS
				(
					SELECT * FROM [map].[tblEntityCompanyTypeToSite]
	...
	BEGIN CATCH
		...
		BEGIN
			...
			+ 'Procedure Name: usp_CreateCompanyTypeToSiteMapping' + CHAR(13)+CHAR(10)
