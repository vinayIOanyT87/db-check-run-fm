...
/*
	EXEC [map].[usp_GetMobileDeviceProfileToSiteByRecordGuid] 'C0DA2E5F-711C-4245-B4DA-7085831674F1', NULL
	EXEC [map].[usp_GetMobileDeviceProfileToSiteByRecordGuid] '05C7D1E0-745D-488D-AF05-00DF1A70D05D', 'F4761A16-AB2F-41EE-B6FA-D17658DF2602', 1
*/

CREATE PROCEDURE [map].[usp_GetMobileDeviceProfileToSiteByRecordGuid]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_GetMobileDeviceProfileToSiteByRecordGuid]
	-- Author: Hansraj Bapoo
	-- Version/Date: 1.0.003 / 2012-07-13 14:21:10.4470770 -04:00
	-- Purpose: Retrieves the MobileDeviceProfileToSite assignments for a given MobileDeviceProfile RecordGuid to a given site/sitegroup, and optionally to children site/sitegroup of the given sitegroup as well. 	
	-- Notes:
	-- 1. @EntityRecordGuid: Guid of the MobileDeviceProfile record for which the mapping hierarchy is to be retrieved.
	...
	-- 7. This stored procedure replaces the EntityTositeMapClass.SelectSQL() inline SQL for the case where entityType is MOBILE_DEVICE_PROFILE and bInTransaction is false.
	------------------------------------------------------------------------------------------------------
	SET NOCOUNT ON;
	BEGIN TRY
		...
		BEGIN
			SELECT @AssignedToSiteGuid = SiteGuid FROM tblMobileDeviceProfile
			WHERE MobileDeviceProfileGuid = @EntityRecordGuid
		END;
		...
		SELECT a.MobileDeviceProfileToSiteGuid MappingGuid, a.MobileDeviceProfileGuid EntityRecordGuid, d.ProfileID EntityId, a.SiteGuid AssignedToSiteGuid, b.SiteGroupId AssignedToSiteId, b.HierarchyLevel, a.CreatedDate, a.CreatedBy, a.UpdatedDate, a.UpdatedBy, a._RowVersion, a.AssignedFromSiteGuid , c.ID AssignedFromSiteId 
		...
		INNER JOIN tblMobileDeviceProfile d
		ON d.MobileDeviceProfileGuid = a.MobileDeviceProfileGuid
		WHERE a.MobileDeviceProfileGuid = @EntityRecordGuid
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: map.usp_GetMobileDeviceProfileToSiteByRecordGuid' + CHAR(13)+CHAR(10)                  
