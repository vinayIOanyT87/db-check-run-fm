/*
	DROP PROCEDURE [map].[usp_CreateQualityTagToSiteMapping]

*/   
CREATE PROCEDURE [map].[usp_CreateQualityTagToSiteMapping]
(
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_CreateQualityTagToSiteMapping]
	-- Author: Vivian Kiarie /  Hansraj Bapoo
	-- Version/Date: 1.0.002 / 2013-12-30 14:44:52.5254411 -05:00
	-- Purpose: Insert into table [map].[tblEntityQualityTagToSite]
		-- Notes:
	...
	BEGIN TRY
		...
		INSERT INTO [map].[tblEntityQualityTagToSite]
		(
		  [QualityTagGuid]
		  ,	[SiteGuid]
		  ,	[CreatedDate]
		...
		OPEN TableCursor 
			...
			BEGIN 
				...
				WHERE NOT EXISTS
				(
					SELECT * FROM [map].[tblEntityQualityTagToSite]
	...
	BEGIN CATCH
		...
		BEGIN
			...
			+ 'Procedure Name: usp_CreateQualityTagToSiteMapping' + CHAR(13)+CHAR(10)
