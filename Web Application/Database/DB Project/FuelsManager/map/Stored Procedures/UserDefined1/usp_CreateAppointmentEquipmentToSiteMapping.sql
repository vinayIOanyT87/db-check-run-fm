/*
	DROP PROCEDURE [map].[usp_CreateAppointmentEquipmentToSiteMapping]

*/

CREATE PROCEDURE [map].[usp_CreateAppointmentEquipmentToSiteMapping]
(
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_CreateAppointmentEquipmentToSiteMapping]
	-- Author: Vivian Kiarie /  Hansraj Bapoo
	-- Version/Date: 1.0.002 / 2013-12-30 14:44:52.4664293 -05:00
	-- Purpose: Insert into table [map].[tblEntityAppointmentEquipmentToSite]
	-- Notes:
	...
	IF (@AssignedToSiteGuid = @AssignedFromSiteGuid)
		...
		INSERT INTO [map].[tblEntityAppointmentEquipmentToSite]
		(
		...
		OPEN TableCursor 
			...
			BEGIN 
				...
				WHERE NOT EXISTS
				(
					SELECT * FROM [map].[tblEntityAppointmentEquipmentToSite]
	...
	BEGIN CATCH
		...
		BEGIN
			...
			+ 'Procedure Name: usp_CreateAppointmentEquipmentToSiteMapping' + CHAR(13)+CHAR(10)
