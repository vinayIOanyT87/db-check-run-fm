/*
	DROP PROCEDURE [map].[usp_CreateAssetTrackingMapConfigurationToSiteMapping]

*/ 
CREATE PROCEDURE [map].[usp_CreateAssetTrackingMapConfigurationToSiteMapping]
(
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_CreateAssetTrackingMapConfigurationToSiteMapping]
	-- Author: Vivian Kiarie /  Hansraj Bapoo
	-- Version/Date: 1.0.002 / 2016-04-26 14:44:52.5104381 -05:00
	-- Purpose: Insert into table [map].[tblEntityAssetTrackingMapConfigurationToSite]
		-- Notes:
	...
	BEGIN TRY
		...
		INSERT INTO [map].[tblEntityAssetTrackingMapConfigurationToSite]
		(
 	...
 	-- Cascading Assignments do not apply to the base entity assignment (assignment of the entity record with its owner site guid).
		...
		OPEN TableCursor 
			...
			BEGIN 
				...
				WHERE NOT EXISTS
				(
					SELECT * FROM [map].[tblEntityAssetTrackingMapConfigurationToSite]
	...
	BEGIN CATCH
		...
		BEGIN
			...
			+ 'Procedure Name: usp_CreateAssetTrackingMapConfigurationToSiteMapping' + CHAR(13)+CHAR(10)
