...
/*
	EXEC [map].[usp_GetTestSetToSiteByRecordGuid] '50482B67-AD93-4020-BBC1-F22B52B211CA', NULL
	EXEC [map].[usp_GetTestSetToSiteByRecordGuid] '05C7D1E0-745D-488D-AF05-00DF1A70D05D', 'F4761A16-AB2F-41EE-B6FA-D17658DF2602', 1
*/

CREATE PROCEDURE [map].[usp_GetTestSetToSiteByRecordGuid]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_GetTestSetToSiteByRecordGuid]
	-- Author: Hansraj Bapoo
	-- Version/Date: 1.0.003 / 2012-07-13 14:21:10.4470770 -04:00
	-- Purpose: Retrieves the TestSetToSite assignments for a given TestSet RecordGuid to a given site/sitegroup, and optionally to children site/sitegroup of the given sitegroup as well. 	
	-- Notes:
	-- 1. @EntityRecordGuid: Guid of the TestSet record for which the mapping hierarchy is to be retrieved.
	...
	-- 7. This stored procedure replaces the EntityTositeMapClass.SelectSQL() inline SQL for the case where entityType is TEST_SET and bInTransaction is false.
	------------------------------------------------------------------------------------------------------
	SET NOCOUNT ON;
	BEGIN TRY
		...
		BEGIN
			SELECT @AssignedToSiteGuid = OwnerSiteGuid FROM tblTestSetDefinitions
			WHERE TestSetDefinitionGuid = @EntityRecordGuid
		END;
		...
		SELECT a.TestSetToSiteGuid MappingGuid, a.TestSetDefinitionGuid EntityRecordGuid, d.TestSetName EntityId, a.SiteGuid AssignedToSiteGuid, b.SiteGroupId AssignedToSiteId, b.HierarchyLevel, a.CreatedDate, a.CreatedBy, a.UpdatedDate, a.UpdatedBy, a._RowVersion, a.AssignedFromSiteGuid , c.ID AssignedFromSiteId 
		...
		INNER JOIN tblTestSetDefinitions d
		ON d.TestSetDefinitionGuid = a.TestSetDefinitionGuid
		WHERE a.TestSetDefinitionGuid = @EntityRecordGuid
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: map.usp_GetTestSetToSiteByRecordGuid' + CHAR(13)+CHAR(10)                  
