/*
	DROP PROCEDURE [map].[usp_CreateIATACodeToSiteMapping]

*/ 

CREATE PROCEDURE [map].[usp_CreateIATACodeToSiteMapping]
(	
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_CreateIATACodeToSiteMapping]
	-- Author: Vivian Kiarie /  Hansraj Bapoo
	-- Version/Date: 1.0.002 / 2013-12-30 14:44:52.5104381 -05:00
	-- Purpose: Insert into table [map].[tblEntityIATACodeToSite]
		-- Notes:
	...
	BEGIN TRY
		...
		INSERT INTO [map].[tblEntityIATACodeToSite]
		(
		  [IATAGuid]
		  ,	[SiteGuid]
		  ,	[CreatedDate]
		...
		OPEN TableCursor 
			...
			BEGIN 			
				...
				WHERE NOT EXISTS
				(
					SELECT * FROM [map].[tblEntityIATACodeToSite]
	...
	BEGIN CATCH
		...
		BEGIN
			...
			+ 'Procedure Name: usp_CreateIATACodeToSiteMapping' + CHAR(13)+CHAR(10)
