/*
	DROP PROCEDURE [map].[usp_CreateAdditiveProfileToSiteMapping]

*/

CREATE PROCEDURE [map].[usp_CreateAdditiveProfileToSiteMapping]
(
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_CreateAdditiveProfileToSiteMapping]
	-- Author: Vivian Kiarie /  Hansraj Bapoo
	-- Version/Date: 1.0.002 / 2013-12-30 14:44:52.4234207 -05:00
	-- Purpose: Insert into table [map].[tblEntityAdditiveProfileToSite]
		-- Notes:
	...
	BEGIN TRY
		...
		INSERT INTO [map].[tblEntityAdditiveProfileToSite]
		(
			 [AdditiveProfileGuid]
			 ,	[SiteGuid]
			 ,	[CreatedDate]
		...
		OPEN TableCursor 
			...
			BEGIN 
				...
				WHERE NOT EXISTS
				(
					SELECT * FROM [map].[tblEntityAdditiveProfileToSite]
	...
	BEGIN CATCH
		...
		BEGIN
			...
			+ 'Procedure Name: usp_CreateAdditiveProfileToSiteMapping' + CHAR(13)+CHAR(10)
