...
/*
	DROP PROCEDURE [map].[usp_GetProdToUnavailableInventoryComMapByProdGuid]

	EXEC [map].[usp_GetProdToUnavailableInventoryComMapByProdGuid] '38B24944-C577-4193-B9FC-554205CB39D7', '00000000-0000-0000-0000-000000000001'

*/

CREATE PROCEDURE [map].[usp_GetProdToUnavailableInventoryComMapByProdGuid]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored Procedure: [map].[usp_GetProdToUnavailableInventoryComMapByProdGuid] 
	-- Author: Hansraj Bapoo
	-- Version/Date: 1.0.003 / 2013-04-04 14:21:10.4470770 -10:00
	-- Purpose: Retrieves the map.tblProductToUnavailableInventoryCompany mappings for a given ProductGuid and Site/Sitegroup.
	-- Notes:
	-- 1. @ProductGuid: Product for which to fetch the mapping records.
	-- 2. @TargetSiteGuid: Site/Sitegroup for which to retrieve the mappings.
	-- 3. This stored procedure replaces the ProductMapClass.EnumerateByAssignedGuidAndTypeSQL() inline SQL, for the case where Type = PRODUCT_MAP_TYPE.UNAVAILABLE_INVENTORY_COMPANY_MAP.
	...
	BEGIN TRY	
		...
		SELECT @targetRecGuid = [erv].[udf_GetFirstParentRecordVersionGuid] ('Product', @masterRecGuid, @TargetSiteGuid)

		SELECT @firstAvailableOwnerSite = a.SiteGuid
		FROM tblProducts a
		WHERE ProductGuid = @targetRecGuid 
		...
		WHERE a.ProductGuid = @targetRecGuid
		ORDER BY AssignedID
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: [map].usp_GetProdToUnavailableInventoryComMapByProdGuid' + CHAR(13)+CHAR(10)                  
