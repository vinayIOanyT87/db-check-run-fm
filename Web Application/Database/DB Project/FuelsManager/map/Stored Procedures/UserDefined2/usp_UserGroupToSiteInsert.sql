CREATE PROCEDURE [map].[usp_UserGroupToSiteInsert]
(
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_UserGroupToSiteInsert]
	-- Author: DBA - WCGray
	-- Version/Date: 1.0.002 / 2013-12-30 14:44:52.5344429 -05:00
	-- Purpose: Insert into table [map].[tblEntityUserGroupToSite]
	-- Notes:
	...
	BEGIN TRY
		...
		OPEN TableCursor 
			...
			BEGIN 
				...
				-- Add CompanyToUserGroup Mapping based upon mapping at @AssignedFromSite and UserGroup Entity Assigned to the Site
				...
				INNER JOIN [map].[tblEntityCompanyToSite] b ON b.CompanyGuid = a.CompanyGuid AND b.SiteGuid = @childSiteGuid
				WHERE a.[SiteGuid] = @parentSiteGuid AND a.GroupGuid = @EntityRecordGuid
				AND NOT EXISTS
				(
	...
	BEGIN CATCH
		...
		BEGIN
			...
			+ 'Procedure Name: usp_UserGroupToSiteInsert' + CHAR(13)+CHAR(10)
