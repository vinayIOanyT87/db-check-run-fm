...
/*
	...
	EXEC [map].[usp_ProductToSiteDeleteAll] '1bb8c558-5277-47a5-90ae-2461bbd1eff7'

*/

CREATE PROCEDURE [map].[usp_ProductToSiteDeleteAll]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored Procedure: [map].[usp_ProductToSiteDeleteAll] 
	-- Author: Hansraj Bapoo
	-- Version/Date: 1.0.003 / 2012-07-13 14:21:10.4470770 -04:00
	-- Purpose: Cascade deletes all the Product-to-site mappings and the associated record versions for all the Product-to-site mappings between two sites.
	-- Notes:
	-- 1. @AssignedFromSiteGroupGuid: Guid of the AssignedFrom sitegroup for which the Product-to-site assignments are to be deleted.
	-- 1. @AssignedToSiteGuid: Guid of the AssignedTo site/sitegroup for which the Product-to-site assignments are to be deleted.
	...
	BEGIN TRY	
		...
		SELECT a.ProductGuid, a.AssignedFromSiteGuid, a.SiteGuid, 0, 0 FROM map.tblEntityProductToSite a
		...
		BEGIN
			...
			INSERT INTO @tblEntityToSiteMappings
			(MasterRecGuid, AssignedFromSiteGuid, AssignedToSiteGuid, MappingLevel, Processed)
			SELECT a.ProductGuid, a.AssignedFromSiteGuid, a.SiteGuid, @runningMappingLevel, 0 FROM map.tblEntityProductToSite a
			INNER JOIN @tblEntityToSiteMappings b
			ON b.MasterRecGuid = a.ProductGuid
		...
		WHERE a.ProductGuid <> b._MasterRecordGuid
		...
		WHERE a.ProductGuid <> a._MasterRecordGuid
	...
	BEGIN CATCH  
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: [map].[usp_ProductToSiteDeleteAll]' + CHAR(13)+CHAR(10)                  
