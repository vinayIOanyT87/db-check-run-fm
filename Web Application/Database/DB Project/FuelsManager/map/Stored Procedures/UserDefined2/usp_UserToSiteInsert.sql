CREATE PROCEDURE [map].[usp_UserToSiteInsert]
(
...
BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[usp_UserToSiteInsert]
	-- Author: DBA - WCGray
	-- Version/Date: 1.0.002 / 2013-12-30 14:44:52.5354431 -05:00
	-- Purpose: Insert into table [map].[tblEntityUserToSite]
	-- Notes:
	 ...
	 BEGIN TRY
		...
		OPEN TableCursor 
			...
			BEGIN 
				...
				-- Add UserToGroup Mapping based upon mapping at @AssignedFromSite and User Entity Assigned to the Site
				...
				SELECT a.UserGuid, a.GroupGuid, @childSiteGuid, @CreatedDate, @CreatedBy, @CreatedDate, @CreatedBy 
				...
				WHERE a.[SiteGuid] = @parentSiteGuid AND a.UserGuid = @EntityRecordGuid
				AND NOT EXISTS
				(
	...
	BEGIN CATCH
		...
		BEGIN
			...
			+ 'Procedure Name: usp_UserToSiteInsert' + CHAR(13)+CHAR(10)
