CREATE PROCEDURE [map].[gsp_SiteToSiteUpdateByPK]
(
	...
	,	@NullOverrideParentSiteGuid BIT=0 
	,	@NullOverrideChildSiteGuid BIT=0 
	,	@NullOverrideUpdatedDate BIT=0 
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[gsp_SiteToSiteUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:30.1384610 -05:00
	-- Purpose: Update table [map].[tblSiteToSite]
	-- Notes:
	-- 1. @SiteToSiteGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [map].[tblSiteToSite] WHERE SiteToSiteGuid=@SiteToSiteGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [map].[tblSiteToSite] SET
			[ParentSiteGuid]=(CASE ISNULL(@NullOverrideParentSiteGuid,0) WHEN 1 THEN @ParentSiteGuid ELSE ISNULL(@ParentSiteGuid,[ParentSiteGuid]) END)
		,	[ChildSiteGuid]=(CASE ISNULL(@NullOverrideChildSiteGuid,0) WHEN 1 THEN @ChildSiteGuid ELSE ISNULL(@ChildSiteGuid,[ChildSiteGuid]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_SiteToSiteUpdateByPK' + CHAR(13)+CHAR(10)                  
