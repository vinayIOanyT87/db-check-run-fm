CREATE PROCEDURE [map].[gsp_GroupToReportDetailUpdateByPK]
(
		@GroupToReportDetailGuid uniqueidentifier
	...
	,	@NullOverrideReportDetailGuid BIT=0 
	,	@NullOverrideUpdatedDate BIT=0 
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[gsp_GroupToReportDetailUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:30.0294501 -05:00
	-- Purpose: Update table [map].[tblGroupToReportDetail]
	-- Notes:
	-- 1. @GroupToReportDetailGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [map].[tblGroupToReportDetail] WHERE GroupToReportDetailGuid=@GroupToReportDetailGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [map].[tblGroupToReportDetail] SET
			[GroupGuid]=(CASE ISNULL(@NullOverrideGroupGuid,0) WHEN 1 THEN @GroupGuid ELSE ISNULL(@GroupGuid,[GroupGuid]) END)
		,	[ReportDetailGuid]=(CASE ISNULL(@NullOverrideReportDetailGuid,0) WHEN 1 THEN @ReportDetailGuid ELSE ISNULL(@ReportDetailGuid,[ReportDetailGuid]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_GroupToReportDetailUpdateByPK' + CHAR(13)+CHAR(10)                  
