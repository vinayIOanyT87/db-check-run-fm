...
CREATE PROCEDURE [map].[gsp_EntityUserToSiteUpdateByPK]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[gsp_EntityUserToSiteUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2013-12-30 14:44:41.2071453 -05:00
	-- Purpose: Update table [map].[tblEntityUserToSite]
	-- Notes:
	-- 1. @UserToSiteGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [map].[tblEntityUserToSite] WHERE UserToSiteGuid=@UserToSiteGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [map].[tblEntityUserToSite] SET
	...
	BEGIN CATCH        
		...
		BEGIN
			...
			SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
							...
							+ 'Procedure Name: gsp_EntityUserToSiteUpdateByPK' + CHAR(13)+CHAR(10)                  
