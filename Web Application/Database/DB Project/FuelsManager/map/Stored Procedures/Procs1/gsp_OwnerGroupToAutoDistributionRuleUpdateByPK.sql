CREATE PROCEDURE [map].[gsp_OwnerGroupToAutoDistributionRuleUpdateByPK]
(
	...
	,	@NullOverrideOwnerGroupGuid BIT=0 
	,	@NullOverrideUpdatedDate BIT=0 
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[gsp_OwnerGroupToAutoDistributionRuleUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:30.0464518 -05:00
	-- Purpose: Update table [map].[tblOwnerGroupToAutoDistributionRule]
	-- Notes:
	-- 1. @OwnerGroupToAutoDistributionRuleGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [map].[tblOwnerGroupToAutoDistributionRule] WHERE OwnerGroupToAutoDistributionRuleGuid=@OwnerGroupToAutoDistributionRuleGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [map].[tblOwnerGroupToAutoDistributionRule] SET
		...
		,	[OwnerGroupGuid]=(CASE ISNULL(@NullOverrideOwnerGroupGuid,0) WHEN 1 THEN @OwnerGroupGuid ELSE ISNULL(@OwnerGroupGuid,[OwnerGroupGuid]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_OwnerGroupToAutoDistributionRuleUpdateByPK' + CHAR(13)+CHAR(10)                  
