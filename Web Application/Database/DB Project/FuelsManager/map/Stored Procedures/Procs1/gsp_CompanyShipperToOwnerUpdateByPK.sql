CREATE PROCEDURE [map].[gsp_CompanyShipperToOwnerUpdateByPK]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[gsp_CompanyShipperToOwnerUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:30.0134485 -05:00
	-- Purpose: Update table [map].[tblCompanyShipperToOwner]
	-- Notes:
	-- 1. @CompanyShipperToOwnerGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [map].[tblCompanyShipperToOwner] WHERE CompanyShipperToOwnerGuid=@CompanyShipperToOwnerGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [map].[tblCompanyShipperToOwner] SET
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_CompanyShipperToOwnerUpdateByPK' + CHAR(13)+CHAR(10)                  
