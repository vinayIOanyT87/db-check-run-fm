CREATE PROCEDURE [map].[gsp_PersonnelToRoleUpdateByPK]
(
	...
	,	@NullOverrideLookupPersonnelRoleIndex BIT=0 
	,	@NullOverrideUpdatedDate BIT=0 
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[gsp_PersonnelToRoleUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:30.0504522 -05:00
	-- Purpose: Update table [map].[tblPersonnelToRole]
	-- Notes:
	-- 1. @PersonnelToRoleGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [map].[tblPersonnelToRole] WHERE PersonnelToRoleGuid=@PersonnelToRoleGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [map].[tblPersonnelToRole] SET
		...
		,	[LookupPersonnelRoleIndex]=(CASE ISNULL(@NullOverrideLookupPersonnelRoleIndex,0) WHEN 1 THEN @LookupPersonnelRoleIndex ELSE ISNULL(@LookupPersonnelRoleIndex,[LookupPersonnelRoleIndex]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_PersonnelToRoleUpdateByPK' + CHAR(13)+CHAR(10)                  
