CREATE PROCEDURE [map].[gsp_EntityUserDataToSiteDeleteByMapToSiteGuid](@MapToSiteGuid uniqueidentifier,@DetachOnly BIT=NULL,@SwapToGuid uniqueidentifier=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [map].[tblEntityUserDataToSite] WHERE [MapToSiteGuid] = @MapToSiteGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		BEGIN
			...
			ELSE
			BEGIN
				DELETE [map].[tblEntityUserDataToSite] WHERE [MapToSiteGuid] = @MapToSiteGuid; 
			END
		END
	END TRY
	BEGIN CATCH
