CREATE PROCEDURE [map].[gsp_SiteToSiteDeleteByRowGuid](@SiteToSiteGuid UNIQUEIDENTIFIER,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [map].[tblSiteToSite] WHERE [SiteToSiteGuid] = @SiteToSiteGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
				 RAISERROR('Attempted to delete a stale version of SiteToSite.',18,1); 
				 RETURN; 
			END 
		END 
		DELETE [map].[tblSiteToSite] WHERE [SiteToSiteGuid] = @SiteToSiteGuid; 
	END TRY
	BEGIN CATCH
