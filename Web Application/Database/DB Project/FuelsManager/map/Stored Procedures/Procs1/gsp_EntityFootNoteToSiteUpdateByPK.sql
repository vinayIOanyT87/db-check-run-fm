...
CREATE PROCEDURE [map].[gsp_EntityFootNoteToSiteUpdateByPK]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[gsp_EntityFootNoteToSiteUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2013-12-30 14:44:41.1591453 -05:00
	-- Purpose: Update table [map].[tblEntityFootNoteToSite]
	-- Notes:
	-- 1. @FootNoteToSiteGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [map].[tblEntityFootNoteToSite] WHERE FootNoteToSiteGuid=@FootNoteToSiteGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [map].[tblEntityFootNoteToSite] SET
	...
	BEGIN CATCH        
		...
		BEGIN
			...
			SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
							...
							+ 'Procedure Name: gsp_EntityFootNoteToSiteUpdateByPK' + CHAR(13)+CHAR(10)                  
