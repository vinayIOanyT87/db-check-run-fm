CREATE PROCEDURE [map].[gsp_FMAECompanyIDUpdateByPK]
(
	...
	,	@NullOverrideFMAECompanyID BIT=0 
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[gsp_FMAECompanyIDUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:30.0234495 -05:00
	-- Purpose: Update table [map].[tblFMAECompanyID]
	-- Notes:
	-- 1. @FMAECompanyIDMapGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [map].[tblFMAECompanyID] WHERE FMAECompanyIDMapGuid=@FMAECompanyIDMapGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [map].[tblFMAECompanyID] SET
			[FMAECompanyID]=(CASE ISNULL(@NullOverrideFMAECompanyID,0) WHEN 1 THEN @FMAECompanyID ELSE ISNULL(@FMAECompanyID,[FMAECompanyID]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_FMAECompanyIDUpdateByPK' + CHAR(13)+CHAR(10)                  
