CREATE PROCEDURE [map].[gsp_CompanySupplierToOwnerUpdateByPK]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[gsp_CompanySupplierToOwnerUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:30.0164488 -05:00
	-- Purpose: Update table [map].[tblCompanySupplierToOwner]
	-- Notes:
	-- 1. @CompanySupplierToOwnerGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [map].[tblCompanySupplierToOwner] WHERE CompanySupplierToOwnerGuid=@CompanySupplierToOwnerGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [map].[tblCompanySupplierToOwner] SET
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_CompanySupplierToOwnerUpdateByPK' + CHAR(13)+CHAR(10)                  
