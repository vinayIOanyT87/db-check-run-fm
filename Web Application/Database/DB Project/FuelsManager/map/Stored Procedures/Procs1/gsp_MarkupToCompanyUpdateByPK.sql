CREATE PROCEDURE [map].[gsp_MarkupToCompanyUpdateByPK]
(
		@MarkupToCompanyGuid uniqueidentifier
	,	@MarkupGuid uniqueidentifier=NULL
	,	@CompanyGuid uniqueidentifier=NULL
	,	@UpdatedDate datetimeoffset(7)=NULL
	,	@UpdatedBy udtUserID=NULL
	,	@_RowVersion timestamp=NULL OUTPUT
	,	@NullOverrideMarkupGuid BIT=0 
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[gsp_MarkupToCompanyUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:30.0414513 -05:00
	-- Purpose: Update table [map].[tblMarkupToCompany]
	-- Notes:
	-- 1. @MarkupToCompanyGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [map].[tblMarkupToCompany] WHERE MarkupToCompanyGuid=@MarkupToCompanyGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [map].[tblMarkupToCompany] SET
			[MarkupGuid]=(CASE ISNULL(@NullOverrideMarkupGuid,0) WHEN 1 THEN @MarkupGuid ELSE ISNULL(@MarkupGuid,[MarkupGuid]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_MarkupToCompanyUpdateByPK' + CHAR(13)+CHAR(10)                  
