CREATE PROCEDURE [map].[gsp_TrendPenToDetailTrendUpdateByPK]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [dbo].[gsp_TrendPenToDetailTrendUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-12-26 08:11:57.2297415 -05:00
	-- Purpose: Update table [map].[tblTrendPenToDetailTrend]
	-- Notes:
	-- 1. @TrendPenToDetailTrendGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [map].[tblTrendPenToDetailTrend] WHERE TrendPenToDetailTrendGuid=@TrendPenToDetailTrendGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [map].[tblTrendPenToDetailTrend] SET
			[PointTemplateTagGuid]=(CASE ISNULL(@NullOverridePointTemplateTagGuid,0) WHEN 1 THEN @PointTemplateTagGuid ELSE ISNULL(@PointTemplateTagGuid,[PointTemplateTagGuid]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_TrendPenToDetailTrendUpdateByPK' + CHAR(13)+CHAR(10)                  
