CREATE PROCEDURE [map].[gsp_MobileDeviceProfileToMobileDeviceUpdateByPK]
(
	...
	,	@NullOverrideAssignedToMobileDeviceGuid BIT=0 
	,	@NullOverrideUpdatedDate BIT=0 
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[gsp_MobileDeviceProfileToMobileDeviceUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:30.0454517 -05:00
	-- Purpose: Update table [map].[tblMobileDeviceProfileToMobileDevice]
	-- Notes:
	-- 1. @MobileDeviceProfileToMobileDeviceGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [map].[tblMobileDeviceProfileToMobileDevice] WHERE MobileDeviceProfileToMobileDeviceGuid=@MobileDeviceProfileToMobileDeviceGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [map].[tblMobileDeviceProfileToMobileDevice] SET
		...
		,	[AssignedToMobileDeviceGuid]=(CASE ISNULL(@NullOverrideAssignedToMobileDeviceGuid,0) WHEN 1 THEN @AssignedToMobileDeviceGuid ELSE ISNULL(@AssignedToMobileDeviceGuid,[AssignedToMobileDeviceGuid]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_MobileDeviceProfileToMobileDeviceUpdateByPK' + CHAR(13)+CHAR(10)                  
