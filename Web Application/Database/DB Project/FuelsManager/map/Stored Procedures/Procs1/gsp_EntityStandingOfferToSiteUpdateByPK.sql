...
CREATE PROCEDURE [map].[gsp_EntityStandingOfferToSiteUpdateByPK]
(
	...
	,	@NullOverrideStandingOfferGuid BIT=0 
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[gsp_EntityStandingOfferToSiteUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2013-12-30 14:44:41.1941453 -05:00
	-- Purpose: Update table [map].[tblEntityStandingOfferToSite]
	-- Notes:
	-- 1. @StandingOfferToSiteGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [map].[tblEntityStandingOfferToSite] WHERE StandingOfferToSiteGuid=@StandingOfferToSiteGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [map].[tblEntityStandingOfferToSite] SET
			[StandingOfferGuid]=(CASE ISNULL(@NullOverrideStandingOfferGuid,0) WHEN 1 THEN @StandingOfferGuid ELSE ISNULL(@StandingOfferGuid,[StandingOfferGuid]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_EntityStandingOfferToSiteUpdateByPK' + CHAR(13)+CHAR(10)                  
