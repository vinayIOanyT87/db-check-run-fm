CREATE PROCEDURE [map].[gsp_CompanyLoadOwnerToManagerUpdateByPK]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[gsp_CompanyLoadOwnerToManagerUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:30.0044476 -05:00
	-- Purpose: Update table [map].[tblCompanyLoadOwnerToManager]
	-- Notes:
	-- 1. @CompanyLoadOwnerToManagerGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [map].[tblCompanyLoadOwnerToManager] WHERE CompanyLoadOwnerToManagerGuid=@CompanyLoadOwnerToManagerGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [map].[tblCompanyLoadOwnerToManager] SET
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_CompanyLoadOwnerToManagerUpdateByPK' + CHAR(13)+CHAR(10)                  
