CREATE PROCEDURE [map].[gsp_MarkupToCompanyDeleteByCompanyGuid](@CompanyGuid uniqueidentifier,@DetachOnly BIT=NULL,@SwapToGuid uniqueidentifier=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [map].[tblMarkupToCompany] WHERE [CompanyGuid] = @CompanyGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		BEGIN
			...
			ELSE
			BEGIN
				DELETE [map].[tblMarkupToCompany] WHERE [CompanyGuid] = @CompanyGuid; 
			END
		END
	END TRY
	BEGIN CATCH
