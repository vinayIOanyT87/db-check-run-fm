CREATE PROCEDURE [map].[gsp_GroupToReportDetailDeleteByRowGuid](@GroupToReportDetailGuid UNIQUEIDENTIFIER,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [map].[tblGroupToReportDetail] WHERE [GroupToReportDetailGuid] = @GroupToReportDetailGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
				 RAISERROR('Attempted to delete a stale version of GroupToReportDetail.',18,1); 
				 RETURN; 
			END 
		END 
		DELETE [map].[tblGroupToReportDetail] WHERE [GroupToReportDetailGuid] = @GroupToReportDetailGuid; 
	END TRY
	BEGIN CATCH
