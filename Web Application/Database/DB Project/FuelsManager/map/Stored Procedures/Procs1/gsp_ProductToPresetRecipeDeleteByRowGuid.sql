CREATE PROCEDURE [map].[gsp_ProductToPresetRecipeDeleteByRowGuid](@ProductToPresetRecipeGuid UNIQUEIDENTIFIER,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [map].[tblProductToPresetRecipe] WHERE [ProductToPresetRecipeGuid] = @ProductToPresetRecipeGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
				 RAISERROR('Attempted to delete a stale version of ProductToPresetRecipe.',18,1); 
				 RETURN; 
			END 
		END 
		DELETE [map].[tblProductToPresetRecipe] WHERE [ProductToPresetRecipeGuid] = @ProductToPresetRecipeGuid; 
	END TRY
	BEGIN CATCH
