CREATE PROCEDURE [map].[gsp_CompanyShipToToBillToUpdateByPK]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[gsp_CompanyShipToToBillToUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:30.0144486 -05:00
	-- Purpose: Update table [map].[tblCompanyShipToToBillTo]
	-- Notes:
	-- 1. @CompanyShipToToBillToGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [map].[tblCompanyShipToToBillTo] WHERE CompanyShipToToBillToGuid=@CompanyShipToToBillToGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [map].[tblCompanyShipToToBillTo] SET
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_CompanyShipToToBillToUpdateByPK' + CHAR(13)+CHAR(10)                  
