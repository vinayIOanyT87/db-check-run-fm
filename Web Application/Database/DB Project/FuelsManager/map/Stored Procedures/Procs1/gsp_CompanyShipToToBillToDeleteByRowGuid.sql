CREATE PROCEDURE [map].[gsp_CompanyShipToToBillToDeleteByRowGuid](@CompanyShipToToBillToGuid UNIQUEIDENTIFIER,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [map].[tblCompanyShipToToBillTo] WHERE [CompanyShipToToBillToGuid] = @CompanyShipToToBillToGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
				 RAISERROR('Attempted to delete a stale version of CompanyShipToToBillTo.',18,1); 
				 RETURN; 
			END 
		END 
		DELETE [map].[tblCompanyShipToToBillTo] WHERE [CompanyShipToToBillToGuid] = @CompanyShipToToBillToGuid; 
	END TRY
	BEGIN CATCH
