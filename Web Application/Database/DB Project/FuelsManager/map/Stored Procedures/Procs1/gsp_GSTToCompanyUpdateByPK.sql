CREATE PROCEDURE [map].[gsp_GSTToCompanyUpdateByPK]
(
		@GSTToCompanyGuid uniqueidentifier
	,	@GSTGuid uniqueidentifier=NULL
	,	@CompanyGuid uniqueidentifier=NULL
	,	@UpdatedDate datetimeoffset(7)=NULL
	,	@UpdatedBy udtUserID=NULL
	,	@_RowVersion timestamp=NULL OUTPUT
	,	@NullOverrideGSTGuid BIT=0 
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[gsp_GSTToCompanyUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:30.0344506 -05:00
	-- Purpose: Update table [map].[tblGSTToCompany]
	-- Notes:
	-- 1. @GSTToCompanyGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [map].[tblGSTToCompany] WHERE GSTToCompanyGuid=@GSTToCompanyGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [map].[tblGSTToCompany] SET
			[GSTGuid]=(CASE ISNULL(@NullOverrideGSTGuid,0) WHEN 1 THEN @GSTGuid ELSE ISNULL(@GSTGuid,[GSTGuid]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_GSTToCompanyUpdateByPK' + CHAR(13)+CHAR(10)                  
