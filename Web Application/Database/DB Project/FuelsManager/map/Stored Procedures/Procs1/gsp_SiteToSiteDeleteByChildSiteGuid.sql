CREATE PROCEDURE [map].[gsp_SiteToSiteDeleteByChildSiteGuid](@ChildSiteGuid uniqueidentifier,@DetachOnly BIT=NULL,@SwapToGuid uniqueidentifier=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [map].[tblSiteToSite] WHERE [ChildSiteGuid] = @ChildSiteGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		END 
		--
		-- REPLACE ChildSiteGuid PER @SwapToGuid IF @SwapToGuid IS NOT NULL
		--
		IF NOT @SwapToGuid IS NULL
		BEGIN
			UPDATE [map].[tblSiteToSite]
			SET [ChildSiteGuid]=@SwapToGuid
		...
		BEGIN
			...
			BEGIN
				UPDATE [map].[tblSiteToSite]
				SET [ChildSiteGuid]=NULL
				WHERE [ChildSiteGuid] = @ChildSiteGuid;
			END
			ELSE
			BEGIN
				DELETE [map].[tblSiteToSite] WHERE [ChildSiteGuid] = @ChildSiteGuid; 
			END
		END
	END TRY
	BEGIN CATCH
