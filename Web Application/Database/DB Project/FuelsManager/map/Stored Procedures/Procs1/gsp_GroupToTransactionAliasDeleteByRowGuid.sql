CREATE PROCEDURE [map].[gsp_GroupToTransactionAliasDeleteByRowGuid](@GroupToTransactionAliasGuid UNIQUEIDENTIFIER,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [map].[tblGroupToTransactionAlias] WHERE [GroupToTransactionAliasGuid] = @GroupToTransactionAliasGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
				 RAISERROR('Attempted to delete a stale version of GroupToTransactionAlias.',18,1); 
				 RETURN; 
			END 
		END 
		DELETE [map].[tblGroupToTransactionAlias] WHERE [GroupToTransactionAliasGuid] = @GroupToTransactionAliasGuid; 
	END TRY
	BEGIN CATCH
