CREATE PROCEDURE [map].[gsp_EntityListViewToSiteDeleteByRowGuid](@ListViewToSiteGuid UNIQUEIDENTIFIER,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [map].[tblEntityListViewToSite] WHERE [ListViewToSiteGuid] = @ListViewToSiteGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
				 RAISERROR('Attempted to delete a stale version of EntityListViewToSite.',16,1); 
				 RETURN; 
			END 
		END 
		DELETE [map].[tblEntityListViewToSite] WHERE [ListViewToSiteGuid] = @ListViewToSiteGuid; 
	END TRY
	BEGIN CATCH
