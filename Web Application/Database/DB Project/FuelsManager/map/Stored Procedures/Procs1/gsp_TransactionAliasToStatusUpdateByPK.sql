CREATE PROCEDURE [map].[gsp_TransactionAliasToStatusUpdateByPK]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[gsp_TransactionAliasToStatusUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:30.1484620 -05:00
	-- Purpose: Update table [map].[tblTransactionAliasToStatus]
	-- Notes:
	-- 1. @TransactionAliasToStatusGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [map].[tblTransactionAliasToStatus] WHERE TransactionAliasToStatusGuid=@TransactionAliasToStatusGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [map].[tblTransactionAliasToStatus] SET
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_TransactionAliasToStatusUpdateByPK' + CHAR(13)+CHAR(10)                  
