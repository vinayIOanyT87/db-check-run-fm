CREATE PROCEDURE [map].[gsp_SessionToSQLProcessUpdateByPK]
(
		@SessionToSQLProcessIndex bigint
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[gsp_SessionToSQLProcessUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:30.1374609 -05:00
	-- Purpose: Update table [map].[tblSessionToSQLProcess]
	-- Notes:
	-- 1. @SessionToSQLProcessIndex and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [map].[tblSessionToSQLProcess] WHERE SessionToSQLProcessIndex=@SessionToSQLProcessIndex AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [map].[tblSessionToSQLProcess] SET
		...
		WHERE	SessionToSQLProcessIndex=@SessionToSQLProcessIndex;
 
		SELECT @_RowVersion=_RowVersion        
		FROM [map].[tblSessionToSQLProcess]           
		WHERE SessionToSQLProcessIndex=@SessionToSQLProcessIndex;
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_SessionToSQLProcessUpdateByPK' + CHAR(13)+CHAR(10)                  
