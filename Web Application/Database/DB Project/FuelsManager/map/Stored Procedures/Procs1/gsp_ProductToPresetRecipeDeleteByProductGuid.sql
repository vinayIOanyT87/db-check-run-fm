CREATE PROCEDURE [map].[gsp_ProductToPresetRecipeDeleteByProductGuid](@ProductGuid uniqueidentifier,@DetachOnly BIT=NULL,@SwapToGuid uniqueidentifier=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [map].[tblProductToPresetRecipe] WHERE [ProductGuid] = @ProductGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		BEGIN
			...
			ELSE
			BEGIN
				DELETE [map].[tblProductToPresetRecipe] WHERE [ProductGuid] = @ProductGuid; 
			END
		END
	END TRY
	BEGIN CATCH
