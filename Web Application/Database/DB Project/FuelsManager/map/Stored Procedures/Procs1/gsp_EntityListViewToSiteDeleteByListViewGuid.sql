CREATE PROCEDURE [map].[gsp_EntityListViewToSiteDeleteByListViewGuid](@ListViewGuid uniqueidentifier,@DetachOnly BIT=NULL,@SwapToGuid uniqueidentifier=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [map].[tblEntityListViewToSite] WHERE [ListViewGuid] = @ListViewGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		BEGIN
			...
			ELSE
			BEGIN
				DELETE [map].[tblEntityListViewToSite] WHERE [ListViewGuid] = @ListViewGuid; 
			END
		END
	END TRY
	BEGIN CATCH
