CREATE PROCEDURE [map].[gsp_QueryStorageToGroupUpdateByPK]
(
		@QueryStorageToGroupGuid uniqueidentifier
	,	@QueryStorageGuid uniqueidentifier=NULL
	,	@GroupGuid uniqueidentifier=NULL
	,	@UpdatedDate datetimeoffset(7)=NULL
	,	@UpdatedBy udtUserID=NULL
	,	@_RowVersion timestamp=NULL OUTPUT
	,	@NullOverrideQueryStorageGuid BIT=0 
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[gsp_QueryStorageToGroupUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:30.1344606 -05:00
	-- Purpose: Update table [map].[tblQueryStorageToGroup]
	-- Notes:
	-- 1. @QueryStorageToGroupGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [map].[tblQueryStorageToGroup] WHERE QueryStorageToGroupGuid=@QueryStorageToGroupGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [map].[tblQueryStorageToGroup] SET
			[QueryStorageGuid]=(CASE ISNULL(@NullOverrideQueryStorageGuid,0) WHEN 1 THEN @QueryStorageGuid ELSE ISNULL(@QueryStorageGuid,[QueryStorageGuid]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_QueryStorageToGroupUpdateByPK' + CHAR(13)+CHAR(10)                  
