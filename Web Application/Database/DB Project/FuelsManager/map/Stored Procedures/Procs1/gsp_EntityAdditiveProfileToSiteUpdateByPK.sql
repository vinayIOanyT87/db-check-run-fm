...
CREATE PROCEDURE [map].[gsp_EntityAdditiveProfileToSiteUpdateByPK]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[gsp_EntityAdditiveProfileToSiteUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2013-12-30 14:44:41.1121453 -05:00
	-- Purpose: Update table [map].[tblEntityAdditiveProfileToSite]
	-- Notes:
	-- 1. @AdditiveProfileToSiteGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [map].[tblEntityAdditiveProfileToSite] WHERE AdditiveProfileToSiteGuid=@AdditiveProfileToSiteGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [map].[tblEntityAdditiveProfileToSite] SET
	...
	BEGIN CATCH        
		...
		BEGIN
			...
			SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
							...
							+ 'Procedure Name: gsp_EntityAdditiveProfileToSiteUpdateByPK' + CHAR(13)+CHAR(10)                  
