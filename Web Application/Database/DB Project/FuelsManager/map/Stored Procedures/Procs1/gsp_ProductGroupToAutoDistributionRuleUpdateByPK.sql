CREATE PROCEDURE [map].[gsp_ProductGroupToAutoDistributionRuleUpdateByPK]
(
	...
	,	@NullOverrideProductGroupGuid BIT=0 
	,	@NullOverrideUpdatedDate BIT=0 
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[gsp_ProductGroupToAutoDistributionRuleUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:30.0544526 -05:00
	-- Purpose: Update table [map].[tblProductGroupToAutoDistributionRule]
	-- Notes:
	-- 1. @ProductGroupToAutoDistributionRuleGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [map].[tblProductGroupToAutoDistributionRule] WHERE ProductGroupToAutoDistributionRuleGuid=@ProductGroupToAutoDistributionRuleGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [map].[tblProductGroupToAutoDistributionRule] SET
		...
		,	[ProductGroupGuid]=(CASE ISNULL(@NullOverrideProductGroupGuid,0) WHEN 1 THEN @ProductGroupGuid ELSE ISNULL(@ProductGroupGuid,[ProductGroupGuid]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_ProductGroupToAutoDistributionRuleUpdateByPK' + CHAR(13)+CHAR(10)                  
