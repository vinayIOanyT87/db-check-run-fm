...
CREATE PROCEDURE [map].[gsp_EntityQuerySettingToSiteUpdateByPK]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[gsp_EntityQuerySettingToSiteUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2013-12-30 14:44:41.1901453 -05:00
	-- Purpose: Update table [map].[tblEntityQuerySettingToSite]
	-- Notes:
	-- 1. @QuerySettingToSiteGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [map].[tblEntityQuerySettingToSite] WHERE QuerySettingToSiteGuid=@QuerySettingToSiteGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [map].[tblEntityQuerySettingToSite] SET
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_EntityQuerySettingToSiteUpdateByPK' + CHAR(13)+CHAR(10)                  
