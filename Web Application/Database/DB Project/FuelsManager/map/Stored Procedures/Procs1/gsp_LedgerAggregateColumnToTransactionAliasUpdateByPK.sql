CREATE PROCEDURE [map].[gsp_LedgerAggregateColumnToTransactionAliasUpdateByPK]
(
	...
	,	@NullOverrideSymbol BIT=0 
	,	@NullOverrideUpdatedDate BIT=0 
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[gsp_LedgerAggregateColumnToTransactionAliasUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:30.0354507 -05:00
	-- Purpose: Update table [map].[tblLedgerAggregateColumnToTransactionAlias]
	-- Notes:
	-- 1. @LedgerAggregateColumnToTransactionAliasGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [map].[tblLedgerAggregateColumnToTransactionAlias] WHERE LedgerAggregateColumnToTransactionAliasGuid=@LedgerAggregateColumnToTransactionAliasGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [map].[tblLedgerAggregateColumnToTransactionAlias] SET
		...
		,	[Symbol]=(CASE ISNULL(@NullOverrideSymbol,0) WHEN 1 THEN @Symbol ELSE ISNULL(@Symbol,[Symbol]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_LedgerAggregateColumnToTransactionAliasUpdateByPK' + CHAR(13)+CHAR(10)                  
