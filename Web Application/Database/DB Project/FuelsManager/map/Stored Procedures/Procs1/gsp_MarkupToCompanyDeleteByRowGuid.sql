CREATE PROCEDURE [map].[gsp_MarkupToCompanyDeleteByRowGuid](@MarkupToCompanyGuid UNIQUEIDENTIFIER,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [map].[tblMarkupToCompany] WHERE [MarkupToCompanyGuid] = @MarkupToCompanyGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
				 RAISERROR('Attempted to delete a stale version of MarkupToCompany.',18,1); 
				 RETURN; 
			END 
		END 
		DELETE [map].[tblMarkupToCompany] WHERE [MarkupToCompanyGuid] = @MarkupToCompanyGuid; 
	END TRY
	BEGIN CATCH
