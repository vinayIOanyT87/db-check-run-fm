CREATE PROCEDURE [map].[gsp_ProductToLedgerViewDeleteByProductGuid](@ProductGuid uniqueidentifier,@DetachOnly BIT=NULL,@SwapToGuid uniqueidentifier=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [map].[tblProductToLedgerView] WHERE [ProductGuid] = @ProductGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		BEGIN
			...
			ELSE
			BEGIN
				DELETE [map].[tblProductToLedgerView] WHERE [ProductGuid] = @ProductGuid; 
			END
		END
	END TRY
	BEGIN CATCH
