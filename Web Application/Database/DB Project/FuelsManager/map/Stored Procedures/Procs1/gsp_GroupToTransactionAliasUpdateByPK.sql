CREATE PROCEDURE [map].[gsp_GroupToTransactionAliasUpdateByPK]
(
	...
	,	@NullOverrideLookupRightIndex BIT=0 
	,	@NullOverrideUpdatedDate BIT=0 
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[gsp_GroupToTransactionAliasUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:30.0324504 -05:00
	-- Purpose: Update table [map].[tblGroupToTransactionAlias]
	-- Notes:
	-- 1. @GroupToTransactionAliasGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [map].[tblGroupToTransactionAlias] WHERE GroupToTransactionAliasGuid=@GroupToTransactionAliasGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [map].[tblGroupToTransactionAlias] SET
		...
		,	[LookupRightIndex]=(CASE ISNULL(@NullOverrideLookupRightIndex,0) WHEN 1 THEN @LookupRightIndex ELSE ISNULL(@LookupRightIndex,[LookupRightIndex]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_GroupToTransactionAliasUpdateByPK' + CHAR(13)+CHAR(10)                  
