CREATE PROCEDURE [map].[gsp_CompanyOffLoadOwnerToManagerUpdateByPK]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[gsp_CompanyOffLoadOwnerToManagerUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:30.0064478 -05:00
	-- Purpose: Update table [map].[tblCompanyOffLoadOwnerToManager]
	-- Notes:
	-- 1. @CompanyOffLoadOwnerToManagerGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [map].[tblCompanyOffLoadOwnerToManager] WHERE CompanyOffLoadOwnerToManagerGuid=@CompanyOffLoadOwnerToManagerGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [map].[tblCompanyOffLoadOwnerToManager] SET
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_CompanyOffLoadOwnerToManagerUpdateByPK' + CHAR(13)+CHAR(10)                  
