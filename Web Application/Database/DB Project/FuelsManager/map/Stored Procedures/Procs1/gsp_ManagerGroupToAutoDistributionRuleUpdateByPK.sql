CREATE PROCEDURE [map].[gsp_ManagerGroupToAutoDistributionRuleUpdateByPK]
(
	...
	,	@NullOverrideManagerGroupGuid BIT=0 
	,	@NullOverrideUpdatedDate BIT=0 
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[gsp_ManagerGroupToAutoDistributionRuleUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:30.0374509 -05:00
	-- Purpose: Update table [map].[tblManagerGroupToAutoDistributionRule]
	-- Notes:
	-- 1. @ManagerGroupToAutoDistributionRuleGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [map].[tblManagerGroupToAutoDistributionRule] WHERE ManagerGroupToAutoDistributionRuleGuid=@ManagerGroupToAutoDistributionRuleGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [map].[tblManagerGroupToAutoDistributionRule] SET
		...
		,	[ManagerGroupGuid]=(CASE ISNULL(@NullOverrideManagerGroupGuid,0) WHEN 1 THEN @ManagerGroupGuid ELSE ISNULL(@ManagerGroupGuid,[ManagerGroupGuid]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_ManagerGroupToAutoDistributionRuleUpdateByPK' + CHAR(13)+CHAR(10)                  
