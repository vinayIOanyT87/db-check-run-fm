CREATE PROCEDURE [map].[gsp_TankToTankGroupUpdateByPK]
(
	...
	,	@NullOverrideAssignedToTankGroupGuid BIT=0 
	,	@NullOverrideUpdatedDate BIT=0 
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[gsp_TankToTankGroupUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:30.1434615 -05:00
	-- Purpose: Update table [map].[tblTankToTankGroup]
	-- Notes:
	-- 1. @TankToTankGroupGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [map].[tblTankToTankGroup] WHERE TankToTankGroupGuid=@TankToTankGroupGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [map].[tblTankToTankGroup] SET
			[TankGuid]=(CASE ISNULL(@NullOverrideTankGuid,0) WHEN 1 THEN @TankGuid ELSE ISNULL(@TankGuid,[TankGuid]) END)
		,	[AssignedToTankGroupGuid]=(CASE ISNULL(@NullOverrideAssignedToTankGroupGuid,0) WHEN 1 THEN @AssignedToTankGroupGuid ELSE ISNULL(@AssignedToTankGroupGuid,[AssignedToTankGroupGuid]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_TankToTankGroupUpdateByPK' + CHAR(13)+CHAR(10)                  
