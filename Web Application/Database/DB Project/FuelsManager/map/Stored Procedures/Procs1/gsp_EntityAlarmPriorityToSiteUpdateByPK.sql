...
CREATE PROCEDURE [map].[gsp_EntityAlarmPriorityToSiteUpdateByPK]
(
	...
	,	@NullOverrideAlarmPriorityGuid BIT=0 
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[gsp_EntityAlarmPriorityToSiteUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2013-12-30 14:44:41.1181453 -05:00
	-- Purpose: Update table [map].[tblEntityAlarmPriorityToSite]
	-- Notes:
	-- 1. @AlarmPriorityToSiteGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [map].[tblEntityAlarmPriorityToSite] WHERE AlarmPriorityToSiteGuid=@AlarmPriorityToSiteGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [map].[tblEntityAlarmPriorityToSite] SET
			[AlarmPriorityGuid]=(CASE ISNULL(@NullOverrideAlarmPriorityGuid,0) WHEN 1 THEN @AlarmPriorityGuid ELSE ISNULL(@AlarmPriorityGuid,[AlarmPriorityGuid]) END)
	...
	BEGIN CATCH        
		...
		BEGIN
			...
			SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
							...
							+ 'Procedure Name: gsp_EntityAlarmPriorityToSiteUpdateByPK' + CHAR(13)+CHAR(10)                  
