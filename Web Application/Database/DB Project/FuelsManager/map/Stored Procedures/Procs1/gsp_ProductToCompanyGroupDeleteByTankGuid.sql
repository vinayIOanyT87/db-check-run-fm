CREATE PROCEDURE [map].[gsp_ProductToCompanyGroupDeleteByTankGuid](@TankGuid uniqueidentifier,@DetachOnly BIT=NULL,@SwapToGuid uniqueidentifier=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [map].[tblProductToCompanyGroup] WHERE [TankGuid] = @TankGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		BEGIN
			...
			ELSE
			BEGIN
				DELETE [map].[tblProductToCompanyGroup] WHERE [TankGuid] = @TankGuid; 
			END
		END
	END TRY
	BEGIN CATCH
