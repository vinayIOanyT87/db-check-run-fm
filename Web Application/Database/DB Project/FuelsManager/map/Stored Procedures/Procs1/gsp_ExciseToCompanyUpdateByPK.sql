CREATE PROCEDURE [map].[gsp_ExciseToCompanyUpdateByPK]
(
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[gsp_ExciseToCompanyUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:30.0224494 -05:00
	-- Purpose: Update table [map].[tblExciseToCompany]
	-- Notes:
	-- 1. @ExciseToCompanyGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [map].[tblExciseToCompany] WHERE ExciseToCompanyGuid=@ExciseToCompanyGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [map].[tblExciseToCompany] SET
			[ExciseGuid]=(CASE ISNULL(@NullOverrideExciseGuid,0) WHEN 1 THEN @ExciseGuid ELSE ISNULL(@ExciseGuid,[ExciseGuid]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_ExciseToCompanyUpdateByPK' + CHAR(13)+CHAR(10)                  
