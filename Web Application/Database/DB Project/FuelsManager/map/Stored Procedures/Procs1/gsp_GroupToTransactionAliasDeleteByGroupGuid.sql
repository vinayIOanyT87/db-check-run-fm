CREATE PROCEDURE [map].[gsp_GroupToTransactionAliasDeleteByGroupGuid](@GroupGuid uniqueidentifier,@DetachOnly BIT=NULL,@SwapToGuid uniqueidentifier=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [map].[tblGroupToTransactionAlias] WHERE [GroupGuid] = @GroupGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		BEGIN
			...
			ELSE
			BEGIN
				DELETE [map].[tblGroupToTransactionAlias] WHERE [GroupGuid] = @GroupGuid; 
			END
		END
	END TRY
	BEGIN CATCH
