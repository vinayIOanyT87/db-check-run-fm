CREATE PROCEDURE [map].[gsp_EntityPointTemplateTypeToSiteDeleteBySiteGuid](@SiteGuid uniqueidentifier,@DetachOnly BIT=NULL,@SwapToGuid uniqueidentifier=NULL,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [map].[tblEntityPointTemplateTypeToSite] WHERE [SiteGuid] = @SiteGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
		...
		BEGIN
			...
			ELSE
			BEGIN
				DELETE [map].[tblEntityPointTemplateTypeToSite] WHERE [SiteGuid] = @SiteGuid; 
			END
		END
	END TRY
	BEGIN CATCH
