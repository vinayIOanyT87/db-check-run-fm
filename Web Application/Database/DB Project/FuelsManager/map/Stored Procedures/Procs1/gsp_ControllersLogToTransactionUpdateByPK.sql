CREATE PROCEDURE [map].[gsp_ControllersLogToTransactionUpdateByPK]
(
	...
	,	@NullOverrideControllersLogGuid BIT=0 
	...
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[gsp_ControllersLogToTransactionUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:30.0204492 -05:00
	-- Purpose: Update table [map].[tblControllersLogToTransaction]
	-- Notes:
	-- 1. @ControllersLogToTransactionGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [map].[tblControllersLogToTransaction] WHERE ControllersLogToTransactionGuid=@ControllersLogToTransactionGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [map].[tblControllersLogToTransaction] SET
			[ControllersLogGuid]=(CASE ISNULL(@NullOverrideControllersLogGuid,0) WHEN 1 THEN @ControllersLogGuid ELSE ISNULL(@ControllersLogGuid,[ControllersLogGuid]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_ControllersLogToTransactionUpdateByPK' + CHAR(13)+CHAR(10)                  
