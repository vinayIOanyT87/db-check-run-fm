CREATE PROCEDURE [map].[gsp_PersonnelToRoleDeleteByRowGuid](@PersonnelToRoleGuid UNIQUEIDENTIFIER,@_RowVersion BINARY(8)=NULL)
AS
BEGIN 
	BEGIN TRY
		SET NOCOUNT ON; 
		...
		BEGIN 
			 IF NOT EXISTS(SELECT 1 FROM [map].[tblPersonnelToRole] WHERE [PersonnelToRoleGuid] = @PersonnelToRoleGuid AND [_RowVersion]=@_RowVersion) 
			 BEGIN 
				 RAISERROR('Attempted to delete a stale version of PersonnelToRole.',18,1); 
				 RETURN; 
			END 
		END 
		DELETE [map].[tblPersonnelToRole] WHERE [PersonnelToRoleGuid] = @PersonnelToRoleGuid; 
	END TRY
	BEGIN CATCH
