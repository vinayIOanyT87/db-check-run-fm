CREATE PROCEDURE [map].[gsp_ManagerToAutoDistributionRuleUpdateByPK]
(
	...
	,	@NullOverrideManagerGuid BIT=0 
	,	@NullOverrideUpdatedDate BIT=0 
)
	AS
	BEGIN
	------------------------------------------------------------------------------------------------------
	-- Stored procedure: [map].[gsp_ManagerToAutoDistributionRuleUpdateByPK] 
	-- Author: DBA - Auto generated
	-- Version/Date: 1.0.003 / 2014-02-05 16:52:30.0394511 -05:00
	-- Purpose: Update table [map].[tblManagerToAutoDistributionRule]
	-- Notes:
	-- 1. @ManagerToAutoDistributionRuleGuid and @UpdatedBy are required parameter.
	...
	SET NOCOUNT ON;
	BEGIN TRY
		IF @_RowVersion IS NOT NULL AND NOT EXISTS(SELECT 1 FROM [map].[tblManagerToAutoDistributionRule] WHERE ManagerToAutoDistributionRuleGuid=@ManagerToAutoDistributionRuleGuid AND _RowVersion=@_RowVersion)
		BEGIN
			RAISERROR('Attempted to modify a stale copy of the record',16,1);
			RETURN;
		END
 
		UPDATE [map].[tblManagerToAutoDistributionRule] SET
		...
		,	[ManagerGuid]=(CASE ISNULL(@NullOverrideManagerGuid,0) WHEN 1 THEN @ManagerGuid ELSE ISNULL(@ManagerGuid,[ManagerGuid]) END)
	...
	BEGIN CATCH        
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: gsp_ManagerToAutoDistributionRuleUpdateByPK' + CHAR(13)+CHAR(10)                  
