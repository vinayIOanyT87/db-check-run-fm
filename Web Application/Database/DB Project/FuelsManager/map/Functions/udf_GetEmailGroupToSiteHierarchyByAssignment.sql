...
/*
	SELECT * FROM [map].[udf_GetEmailGroupToSiteHierarchyByAssignment] ( 'C0DA2E5F-711C-4245-B4DA-7085831674F1', '00000000-0000-0000-0000-000000000001', NULL) ORDER BY HierarchyLevel, SiteId	
	SELECT * FROM [map].[udf_GetEmailGroupToSiteHierarchyByAssignment] ( 'C0DA2E5F-711C-4245-B4DA-7085831674F1', '00000000-0000-0000-0000-000000000001', 'F4761A16-AB2F-41EE-B6FA-D17658DF2602') ORDER BY HierarchyLevel, SiteId	
	SELECT * FROM [map].[udf_GetEmailGroupToSiteHierarchyByAssignment] ( 'C0DA2E5F-711C-4245-B4DA-7085831674F1', NULL, NULL) ORDER BY HierarchyLevel, SiteId	

*/

CREATE FUNCTION [map].[udf_GetEmailGroupToSiteHierarchyByAssignment]
(
...
BEGIN
	/*
	------------------------------------------------------------------------------------------------------
	-- Function: [map].[udf_GetEmailGroupToSiteHierarchyByAssignment] 
	-- Author: Hansraj Bapoo
	-- Version/Date: 1.0.003 / 2012-07-13 14:21:10.4470770 -04:00
	-- Purpose: Retrieves the EmailGroupToSite assignment hierarchy below a given EmailGroup assignment. 	
	-- Notes:
	...
	*/
	...
	BEGIN
		SELECT @AssignedFromSiteGuid = SiteGuid FROM tblEmailGroups WHERE EmailGroupGuid = @EntityRecordGuid
		SET @AssignedToSiteGuid = @AssignedFromSiteGuid
	END;

	IF (@AssignedFromSiteGuid IS NULL)
	BEGIN
		SELECT @AssignedFromSiteGuid = AssignedFromSiteGuid FROM map.tblEntityEmailGroupToSite WHERE EmailGroupGuid = @EntityRecordGuid AND SiteGuid = @AssignedToSiteGuid
	END;
	...
	(
		...
		SELECT EmailGroupToSiteGuid, AssignedFromSiteGuid, SiteGuid, 0
		...
		-- Recursive Call
		UNION ALL
		SELECT a.EmailGroupToSiteGuid, a.AssignedFromSiteGuid, a.SiteGuid, lvl + 1
		FROM map.tblEntityEmailGroupToSite a 
		INNER JOIN AssignmentHierarchy b
		ON a.EmailGroupGuid = @EntityRecordGuid
