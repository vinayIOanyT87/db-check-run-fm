CREATE FUNCTION [map].[udf_GetAssetTrackingMapConfigurationToSiteHierarchyByAssignment]
(
...
BEGIN
	/*
	------------------------------------------------------------------------------------------------------
	-- Function: [map].[udf_GetAssetTrackingMapConfigurationToSiteHierarchyByAssignment] 
	-- Author: Richard R. Panachida
	-- Version/Date: 1.0.003 / 2016-04-26 14:21:10.4470770 -04:00
	-- Purpose: Retrieves the AssetTrackingMapConfigurationToSite assignment hierarchy below a given AssetTrackingMapConfiguration assignment. 	
	-- Notes:
	...
	*/
	...
	BEGIN
		SELECT @AssignedFromSiteGuid = SiteGuid FROM tblAssetTrackingMapConfiguration WHERE AssetTrackingMapConfigurationGuid = @EntityRecordGuid
		SET @AssignedToSiteGuid = @AssignedFromSiteGuid
	END;

	IF (@AssignedFromSiteGuid IS NULL)
	BEGIN
		SELECT @AssignedFromSiteGuid = AssignedFromSiteGuid FROM map.tblEntityAssetTrackingMapConfigurationToSite WHERE AssetTrackingMapConfigurationGuid = @EntityRecordGuid AND SiteGuid = @AssignedToSiteGuid
	END;
	...
	(
		...
		SELECT AssetTrackingMapConfigurationToSiteGuid, AssignedFromSiteGuid, SiteGuid, 0
		FROM map.tblEntityAssetTrackingMapConfigurationToSite
		...
		-- Recursive Call
		UNION ALL
		SELECT a.AssetTrackingMapConfigurationToSiteGuid, a.AssignedFromSiteGuid, a.SiteGuid, lvl + 1
		FROM map.tblEntityAssetTrackingMapConfigurationToSite a 
			 INNER JOIN AssignmentHierarchy b ON a.AssetTrackingMapConfigurationGuid = @EntityRecordGuid
