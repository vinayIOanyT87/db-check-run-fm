...
/*
	SELECT * FROM [map].[udf_GetCompanyCertificateAndPermitToSiteHierarchyByAssignment] ( 'C0DA2E5F-711C-4245-B4DA-7085831674F1', '00000000-0000-0000-0000-000000000001', NULL) ORDER BY HierarchyLevel, SiteId	
	SELECT * FROM [map].[udf_GetCompanyCertificateAndPermitToSiteHierarchyByAssignment] ( 'C0DA2E5F-711C-4245-B4DA-7085831674F1', '00000000-0000-0000-0000-000000000001', 'F4761A16-AB2F-41EE-B6FA-D17658DF2602') ORDER BY HierarchyLevel, SiteId	
	SELECT * FROM [map].[udf_GetCompanyCertificateAndPermitToSiteHierarchyByAssignment] ( 'C0DA2E5F-711C-4245-B4DA-7085831674F1', NULL, NULL) ORDER BY HierarchyLevel, SiteId	

*/

CREATE FUNCTION [map].[udf_GetCompanyCertificateAndPermitToSiteHierarchyByAssignment]
(
...
BEGIN
	/*
	------------------------------------------------------------------------------------------------------
	-- Function: [map].[udf_GetCompanyCertificateAndPermitToSiteHierarchyByAssignment] 
	-- Author: Hansraj Bapoo
	-- Version/Date: 1.0.003 / 2012-07-13 14:21:10.4470770 -04:00
	-- Purpose: Retrieves the CompanyCertificateAndPermitToSite assignment hierarchy below a given CompanyCertificateAndPermit assignment. 	
	-- Notes:
	-- 1. @EntityRecordGuid: Guid of the CompanyCertificateAndPermit record for which the mapping hierarchy is to be retrieved.
	...
	*/
	...
	BEGIN
		SELECT @AssignedFromSiteGuid = SiteGuid FROM tblQualifications WHERE QualificationGuid = @EntityRecordGuid
		SET @AssignedToSiteGuid = @AssignedFromSiteGuid
	END;

	IF (@AssignedFromSiteGuid IS NULL)
	BEGIN
		SELECT @AssignedFromSiteGuid = AssignedFromSiteGuid FROM map.tblEntityCompanyCertificateAndPermitToSite WHERE QualificationGuid = @EntityRecordGuid AND SiteGuid = @AssignedToSiteGuid
	END;
	...
	(
		...
		SELECT CompanyCertificateAndPermitToSiteGuid, AssignedFromSiteGuid, SiteGuid, 0
		...
		-- Recursive Call
		UNION ALL
		SELECT a.CompanyCertificateAndPermitToSiteGuid, a.AssignedFromSiteGuid, a.SiteGuid, lvl + 1
		FROM map.tblEntityCompanyCertificateAndPermitToSite a 
		INNER JOIN AssignmentHierarchy b
		ON a.QualificationGuid = @EntityRecordGuid
