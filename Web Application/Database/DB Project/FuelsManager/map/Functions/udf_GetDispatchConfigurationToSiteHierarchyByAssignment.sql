...
/*
	SELECT * FROM [map].[udf_GetDispatchConfigurationToSiteHierarchyByAssignment] ( 'C0DA2E5F-711C-4245-B4DA-7085831674F1', '00000000-0000-0000-0000-000000000001', NULL) ORDER BY HierarchyLevel, SiteId	
	SELECT * FROM [map].[udf_GetDispatchConfigurationToSiteHierarchyByAssignment] ( 'C0DA2E5F-711C-4245-B4DA-7085831674F1', '00000000-0000-0000-0000-000000000001', 'F4761A16-AB2F-41EE-B6FA-D17658DF2602') ORDER BY HierarchyLevel, SiteId	
	SELECT * FROM [map].[udf_GetDispatchConfigurationToSiteHierarchyByAssignment] ( 'C0DA2E5F-711C-4245-B4DA-7085831674F1', NULL, NULL) ORDER BY HierarchyLevel, SiteId	

*/

CREATE FUNCTION [map].[udf_GetDispatchConfigurationToSiteHierarchyByAssignment]
(
...
BEGIN
	/*
	------------------------------------------------------------------------------------------------------
	-- Function: [map].[udf_GetDispatchConfigurationToSiteHierarchyByAssignment] 
	-- Author: Hansraj Bapoo
	-- Version/Date: 1.0.003 / 2012-07-13 14:21:10.4470770 -04:00
	-- Purpose: Retrieves the DispatchConfigurationToSite assignment hierarchy below a given DispatchConfiguration assignment. 	
	-- Notes:
	...
	*/
	...
	BEGIN
		SELECT @AssignedFromSiteGuid = SiteGuid FROM tblDispatchConfiguration WHERE DispatchConfigurationGuid = @EntityRecordGuid
		SET @AssignedToSiteGuid = @AssignedFromSiteGuid
	END;

	IF (@AssignedFromSiteGuid IS NULL)
	BEGIN
		SELECT @AssignedFromSiteGuid = AssignedFromSiteGuid FROM map.tblEntityDispatchConfigurationToSite WHERE DispatchConfigurationGuid = @EntityRecordGuid AND SiteGuid = @AssignedToSiteGuid
	END;
	...
	(
		...
		SELECT DispatchConfigurationToSiteGuid, AssignedFromSiteGuid, SiteGuid, 0
		...
		-- Recursive Call
		UNION ALL
		SELECT a.DispatchConfigurationToSiteGuid, a.AssignedFromSiteGuid, a.SiteGuid, lvl + 1
		FROM map.tblEntityDispatchConfigurationToSite a 
		INNER JOIN AssignmentHierarchy b
		ON a.DispatchConfigurationGuid = @EntityRecordGuid
