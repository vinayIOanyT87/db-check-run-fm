CREATE FUNCTION [map].[udf_GetAssetTrackingDeviceToSiteHierarchyByAssignment]
(
...
BEGIN
	/*
	------------------------------------------------------------------------------------------------------
	-- Function: [map].[udf_GetAssetTrackingDeviceToSiteHierarchyByAssignment] 
	-- Author: Richard R. Panachida
	-- Version/Date: 1.0.003 / 2016-04-26 14:21:10.4470770 -04:00
	-- Purpose: Retrieves the AssetTrackingDeviceToSite assignment hierarchy below a given AssetTrackingDevice assignment. 	
	-- Notes:
	-- 1. @EntityRecordGuid: Guid of the AssetTrackingDevice record for which the mapping hierarchy is to be retrieved.
	...
	*/
	...
	BEGIN
		SELECT @AssignedFromSiteGuid = SiteGuid FROM tblAssetTrackingDevice WHERE AssetTrackingDeviceGuid = @EntityRecordGuid
		SET @AssignedToSiteGuid = @AssignedFromSiteGuid
	END;

	IF (@AssignedFromSiteGuid IS NULL)
	BEGIN
		SELECT @AssignedFromSiteGuid = AssignedFromSiteGuid FROM map.tblEntityAssetTrackingDeviceToSite WHERE AssetTrackingDeviceGuid = @EntityRecordGuid AND SiteGuid = @AssignedToSiteGuid
	END;
	...
	(
		...
		SELECT AssetTrackingDeviceToSiteGuid, AssignedFromSiteGuid, SiteGuid, 0
		FROM map.tblEntityAssetTrackingDeviceToSite
		WHERE AssetTrackingDeviceGuid = @EntityRecordGuid
		...
		-- Recursive Call
		UNION ALL
		SELECT a.AssetTrackingDeviceToSiteGuid, a.AssignedFromSiteGuid, a.SiteGuid, lvl + 1
		FROM map.tblEntityAssetTrackingDeviceToSite a 
			 INNER JOIN AssignmentHierarchy b ON a.AssetTrackingDeviceGuid = @EntityRecordGuid
