...
/*
	SELECT * FROM [map].[udf_GetCompanyGroupToSiteHierarchyByAssignment] ( 'C0DA2E5F-711C-4245-B4DA-7085831674F1', '00000000-0000-0000-0000-000000000001', NULL) ORDER BY HierarchyLevel, SiteId	
	SELECT * FROM [map].[udf_GetCompanyGroupToSiteHierarchyByAssignment] ( 'C0DA2E5F-711C-4245-B4DA-7085831674F1', '00000000-0000-0000-0000-000000000001', 'F4761A16-AB2F-41EE-B6FA-D17658DF2602') ORDER BY HierarchyLevel, SiteId	
	SELECT * FROM [map].[udf_GetCompanyGroupToSiteHierarchyByAssignment] ( 'C0DA2E5F-711C-4245-B4DA-7085831674F1', NULL, NULL) ORDER BY HierarchyLevel, SiteId	

*/

CREATE FUNCTION [map].[udf_GetCompanyGroupToSiteHierarchyByAssignment]
(
...
BEGIN
	/*
	------------------------------------------------------------------------------------------------------
	-- Function: [map].[udf_GetCompanyGroupToSiteHierarchyByAssignment] 
	-- Author: Hansraj Bapoo
	-- Version/Date: 1.0.003 / 2012-07-13 14:21:10.4470770 -04:00
	-- Purpose: Retrieves the CompanyGroupToSite assignment hierarchy below a given CompanyGroup assignment. 	
	-- Notes:
	-- 1. @EntityRecordGuid: Guid of the CompanyGroup record for which the mapping hierarchy is to be retrieved.
	...
	-- 7. If both the AssignedToSiteGuid and the AssignedFromSiteGuid are provided, then the AssignedToSiteGuid is used. The AssignedFromSiteGuid is ignored since an entity record cannot be assigned to a site/sitegroup from more than one sitegroup.)
	-- 8. If both the AssignedToSiteGuid and the AssignedFromSiteGuid are null, then the first root (top-level) sitegroup node is used as the AssignedFromSiteGuid
	...
	*/
	...
	BEGIN
		SELECT @AssignedFromSiteGuid = SiteGuid FROM tblApplicationString WHERE ApplicationStringGuid = @EntityRecordGuid
		SET @AssignedToSiteGuid = @AssignedFromSiteGuid
	END;

	IF (@AssignedFromSiteGuid IS NULL)
	BEGIN
		SELECT @AssignedFromSiteGuid = AssignedFromSiteGuid FROM map.tblEntityCompanyGroupToSite WHERE ApplicationStringGuid = @EntityRecordGuid AND SiteGuid = @AssignedToSiteGuid
	END;
	...
	(
		...
		SELECT CompanyGroupToSiteGuid, AssignedFromSiteGuid, SiteGuid, 0
		FROM map.tblEntityCompanyGroupToSite
		WHERE ApplicationStringGuid = @EntityRecordGuid
		...
		-- Recursive Call
		UNION ALL
		SELECT a.CompanyGroupToSiteGuid, a.AssignedFromSiteGuid, a.SiteGuid, lvl + 1
		FROM map.tblEntityCompanyGroupToSite a 
		INNER JOIN AssignmentHierarchy b
		ON a.ApplicationStringGuid = @EntityRecordGuid
