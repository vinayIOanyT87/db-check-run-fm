...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerApplyIncrementalUpdates_tblOPCConnections]
...
@sync_supported_columns_tblOPCConnections varchar(8000)
AS
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM [track].[tblOPCConnections] CT
    ...
    BEGIN
        ;   WITH existingData AS (
                SELECT [dbo].[tblOPCConnections].[URL],[dbo].[tblOPCConnections].[ProgID],[dbo].[tblOPCConnections].[CreatedDate],[dbo].[tblOPCConnections].[CreatedBy],[dbo].[tblOPCConnections].[OPCConnectionGuid]
            ...
            USING (SELECT @URL,@ProgID,@CreatedDate,@CreatedBy,@OPCConnectionGuid
            ...
            WHEN MATCHED AND (@sync_force_write = 1 
                ...
                THEN
                UPDATE SET [URL] = CASE (SELECT PATINDEX([sync].[udf_GetColumnSearchPattern]('URL'), @sync_supported_columns_tblOPCConnections)) WHEN 0 THEN existingData.[URL] ELSE remoteChanges.[URL] END
                       ,[ProgID] = CASE (SELECT PATINDEX([sync].[udf_GetColumnSearchPattern]('ProgID'), @sync_supported_columns_tblOPCConnections)) WHEN 0 THEN existingData.[ProgID] ELSE remoteChanges.[ProgID] END
                       ,[CreatedDate] = CASE (SELECT PATINDEX([sync].[udf_GetColumnSearchPattern]('CreatedDate'), @sync_supported_columns_tblOPCConnections)) WHEN 0 THEN existingData.[CreatedDate] ELSE remoteChanges.[CreatedDate] END
                       ,[CreatedBy] = CASE (SELECT PATINDEX([sync].[udf_GetColumnSearchPattern]('CreatedBy'), @sync_supported_columns_tblOPCConnections)) WHEN 0 THEN existingData.[CreatedBy] ELSE remoteChanges.[CreatedBy] END
