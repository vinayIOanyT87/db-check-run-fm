...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalUpdates_tblEntityCompanyToSite]
...
BEGIN
    ...
    END
        ...
        (
            SELECT data.[CompanyToSiteGuid],data.[TrackedCompanyToSiteGuid],data.[IncludeChangeTrackingFlag],CT.UpdatedRowVersion AS 'UpdatedRowVersion' 
                FROM [map].[tblEntityCompanyToSite]
        ...
        SELECT TOP(@sync_batch_size_tblEntityCompanyToSite) WITH TIES [map].[tblEntityCompanyToSite].[CompanyToSiteGuid],[map].[tblEntityCompanyToSite].[CompanyGuid],[map].[tblEntityCompanyToSite].[SiteGuid],[map].[tblEntityCompanyToSite].[CreatedDate],[map].[tblEntityCompanyToSite].[CreatedBy],[map].[tblEntityCompanyToSite].[UpdatedDate],[map].[tblEntityCompanyToSite].[UpdatedBy],[map].[tblEntityCompanyToSite].[AssignedFromSiteGuid], data.UpdatedRowVersion AS '_RowVersion'
            FROM [map].[tblEntityCompanyToSite]
                INNER JOIN (SELECT data.[CompanyToSiteGuid], data1.[UpdatedRowVersion]
