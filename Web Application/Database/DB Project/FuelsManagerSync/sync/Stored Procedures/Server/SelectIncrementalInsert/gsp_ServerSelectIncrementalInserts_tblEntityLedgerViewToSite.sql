...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblEntityLedgerViewToSite]
...
@sync_batch_size_tblEntityLedgerViewToSite int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblEntityLedgerViewToSite int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblEntityLedgerViewToSite IS NULL OR 
        (@sync_batch_size_tblEntityLedgerViewToSite IS NOT NULL AND @sync_batch_size_tblEntityLedgerViewToSite = 0))
    BEGIN
        SET @sync_batch_size_tblEntityLedgerViewToSite = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
    SELECT TOP(@sync_batch_size_tblEntityLedgerViewToSite) WITH TIES [map].[tblEntityLedgerViewToSite].[LedgerViewToSiteGuid],[map].[tblEntityLedgerViewToSite].[ListViewGuid],[map].[tblEntityLedgerViewToSite].[SiteGuid],[map].[tblEntityLedgerViewToSite].[CreatedDate],[map].[tblEntityLedgerViewToSite].[CreatedBy],[map].[tblEntityLedgerViewToSite].[UpdatedDate],[map].[tblEntityLedgerViewToSite].[UpdatedBy],[map].[tblEntityLedgerViewToSite].[AssignedFromSiteGuid],CT.InsertedRowVersion AS '_RowVersion'
        FROM [map].[tblEntityLedgerViewToSite]
