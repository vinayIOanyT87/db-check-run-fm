...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblQualificationPersonTrainingToStation]
...
@sync_batch_size_tblQualificationPersonTrainingToStation int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblQualificationPersonTrainingToStation int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblQualificationPersonTrainingToStation IS NULL OR 
        (@sync_batch_size_tblQualificationPersonTrainingToStation IS NOT NULL AND @sync_batch_size_tblQualificationPersonTrainingToStation = 0))
    BEGIN
        SET @sync_batch_size_tblQualificationPersonTrainingToStation = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
        SELECT TOP(@sync_batch_size_tblQualificationPersonTrainingToStation) WITH TIES [QualificationPersonTrainingToStationGuid],[QualificationGuid],[StationGuid],[Sequence],[Instructor],[DateCompleted],[DateDue],[ExpirationDate],[ID],[Rating],[HistoricalRecord],[CreatedDate],[CreatedBy],[UpdatedDate],[UpdatedBy],_RowVersion
        FROM (
            SELECT TOP(@sync_batch_size_tblQualificationPersonTrainingToStation) WITH TIES [map].[tblQualificationPersonTrainingToStation].[QualificationPersonTrainingToStationGuid],[map].[tblQualificationPersonTrainingToStation].[QualificationGuid],[map].[tblQualificationPersonTrainingToStation].[StationGuid],[map].[tblQualificationPersonTrainingToStation].[Sequence],[map].[tblQualificationPersonTrainingToStation].[Instructor],[map].[tblQualificationPersonTrainingToStation].[DateCompleted],[map].[tblQualificationPersonTrainingToStation].[DateDue],[map].[tblQualificationPersonTrainingToStation].[ExpirationDate],[map].[tblQualificationPersonTrainingToStation].[ID],[map].[tblQualificationPersonTrainingToStation].[Rating],[map].[tblQualificationPersonTrainingToStation].[HistoricalRecord],[map].[tblQualificationPersonTrainingToStation].[CreatedDate],[map].[tblQualificationPersonTrainingToStation].[CreatedBy],[map].[tblQualificationPersonTrainingToStation].[UpdatedDate],[map].[tblQualificationPersonTrainingToStation].[UpdatedBy],sync.udf_GetMaxRowVersion(CT.InsertedRowVersion,MAPCT.InsertedRowVersion,MAPCT2.InsertedRowVersion) AS '_RowVersion'
                FROM [map].[tblQualificationPersonTrainingToStation]
                    INNER JOIN (SELECT [PersonnelTrainingToSiteGuid],[QualificationGuid],[OwnerSiteGuid],[AssignedToSiteGuid] FROM [dbo].[udf_GetAssignedQualificationPersonnelTrainingListForSite](@sync_context_site_guid)) data
                        ON [map].[tblQualificationPersonTrainingToStation].[QualificationGuid] = data.[QualificationGuid]
                    INNER JOIN (SELECT [StationGuid],[OwnerSiteGuid] FROM [dbo].[udf_GetAssociatedStationListForSite](@sync_context_site_guid)) data1
                        ON [map].[tblQualificationPersonTrainingToStation].[StationGuid] = data1.[StationGuid]
