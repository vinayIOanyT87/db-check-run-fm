...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblPointTemplateProperty]
...
@sync_batch_size_tblPointTemplateProperty int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblPointTemplateProperty int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblPointTemplateProperty IS NULL OR 
        (@sync_batch_size_tblPointTemplateProperty IS NOT NULL AND @sync_batch_size_tblPointTemplateProperty = 0))
    BEGIN
        SET @sync_batch_size_tblPointTemplateProperty = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
        SELECT TOP(@sync_batch_size_tblPointTemplateProperty) WITH TIES [ID],[ValueType],[Value],[CreatedDate],[CreatedBy],[UpdatedDate],[UpdatedBy],[PointTemplatePropertyGuid],[PointTemplateGuid],_RowVersion
        FROM (
            SELECT TOP(@sync_batch_size_tblPointTemplateProperty) WITH TIES [dbo].[tblPointTemplateProperty].[ID],[dbo].[tblPointTemplateProperty].[ValueType],[dbo].[tblPointTemplateProperty].[Value],[dbo].[tblPointTemplateProperty].[CreatedDate],[dbo].[tblPointTemplateProperty].[CreatedBy],[dbo].[tblPointTemplateProperty].[UpdatedDate],[dbo].[tblPointTemplateProperty].[UpdatedBy],[dbo].[tblPointTemplateProperty].[PointTemplatePropertyGuid],[dbo].[tblPointTemplateProperty].[PointTemplateGuid],sync.udf_GetMaxRowVersion(CT.InsertedRowVersion,MAPCT.InsertedRowVersion,NULL) AS '_RowVersion'
                FROM [dbo].[tblPointTemplateProperty]
                    INNER JOIN (SELECT [PointTemplateToSiteGuid],[PointTemplateGuid],[OwnerSiteGuid],[AssignedToSiteGuid] FROM [dbo].[udf_GetAssignedPointTemplateForSite](@sync_context_site_guid)) data
                        ON [dbo].[tblPointTemplateProperty].[PointTemplateGuid] = data.[PointTemplateGuid]
