...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblTransactionLinks]
...
@sync_first_time_sync_option_tblTransactionLinks int
AS
BEGIN
    ...
    IF ((@sync_request_type = 4 AND (@sync_first_time_sync_option_tblTransactionLinks IS NOT NULL AND @sync_first_time_sync_option_tblTransactionLinks = 1))
    ...
    END
        ...
        -- We only do this on tblTransactionLinks so we know what which tblTransactionLinks records are being processed.
        -- Synchronization will only synchronize other dependent records that are associated with the tblTransactionLinks records
        -- that were included.
