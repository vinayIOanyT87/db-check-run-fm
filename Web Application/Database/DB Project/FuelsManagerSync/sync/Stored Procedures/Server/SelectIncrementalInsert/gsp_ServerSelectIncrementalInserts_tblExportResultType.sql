...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblExportResultType]
...
@sync_batch_size_tblExportResultType int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblExportResultType int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblExportResultType IS NULL OR 
        (@sync_batch_size_tblExportResultType IS NOT NULL AND @sync_batch_size_tblExportResultType = 0))
    BEGIN
        SET @sync_batch_size_tblExportResultType = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
    SELECT TOP(@sync_batch_size_tblExportResultType) WITH TIES [lookup].[tblExportResultType].[ExportResultTypeIndex],[lookup].[tblExportResultType].[ExportResultTypeCode],[lookup].[tblExportResultType].[ExportResultTypeName],[lookup].[tblExportResultType].[ExportResultTypeGuid],[lookup].[tblExportResultType].[CreatedDate],[lookup].[tblExportResultType].[CreatedBy],[lookup].[tblExportResultType].[UpdatedDate],[lookup].[tblExportResultType].[UpdatedBy],CT.InsertedRowVersion AS '_RowVersion'
        FROM [lookup].[tblExportResultType]
