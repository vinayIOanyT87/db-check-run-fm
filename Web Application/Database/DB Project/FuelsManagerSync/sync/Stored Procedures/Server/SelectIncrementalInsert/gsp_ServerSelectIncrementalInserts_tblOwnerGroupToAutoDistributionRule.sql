...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblOwnerGroupToAutoDistributionRule]
...
@sync_batch_size_tblOwnerGroupToAutoDistributionRule int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblOwnerGroupToAutoDistributionRule int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblOwnerGroupToAutoDistributionRule IS NULL OR 
        (@sync_batch_size_tblOwnerGroupToAutoDistributionRule IS NOT NULL AND @sync_batch_size_tblOwnerGroupToAutoDistributionRule = 0))
    BEGIN
        SET @sync_batch_size_tblOwnerGroupToAutoDistributionRule = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
        SELECT TOP(@sync_batch_size_tblOwnerGroupToAutoDistributionRule) WITH TIES [OwnerGroupToAutoDistributionRuleGuid],[AutoDistributionRuleGuid],[OwnerGroupGuid],[CreatedDate],[CreatedBy],[UpdatedDate],[UpdatedBy],_RowVersion
        FROM (
            SELECT TOP(@sync_batch_size_tblOwnerGroupToAutoDistributionRule) WITH TIES [map].[tblOwnerGroupToAutoDistributionRule].[OwnerGroupToAutoDistributionRuleGuid],[map].[tblOwnerGroupToAutoDistributionRule].[AutoDistributionRuleGuid],[map].[tblOwnerGroupToAutoDistributionRule].[OwnerGroupGuid],[map].[tblOwnerGroupToAutoDistributionRule].[CreatedDate],[map].[tblOwnerGroupToAutoDistributionRule].[CreatedBy],[map].[tblOwnerGroupToAutoDistributionRule].[UpdatedDate],[map].[tblOwnerGroupToAutoDistributionRule].[UpdatedBy],sync.udf_GetMaxRowVersion(CT.InsertedRowVersion,MAPCT.InsertedRowVersion,NULL) AS '_RowVersion'
                FROM [map].[tblOwnerGroupToAutoDistributionRule]
                    INNER JOIN (SELECT [CompanyGroupToSiteGuid],[ApplicationStringGuid],[OwnerSiteGuid],[AssignedToSiteGuid] FROM [dbo].[udf_GetAssignedApplicationStringCompanyGroupListForSite](@sync_context_site_guid)) data
                        ON [map].[tblOwnerGroupToAutoDistributionRule].[OwnerGroupGuid] = data.[ApplicationStringGuid]
