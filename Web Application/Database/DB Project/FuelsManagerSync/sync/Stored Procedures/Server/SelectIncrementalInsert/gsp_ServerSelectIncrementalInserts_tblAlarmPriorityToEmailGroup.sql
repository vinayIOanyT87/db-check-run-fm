...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblAlarmPriorityToEmailGroup]
...
@sync_batch_size_tblAlarmPriorityToEmailGroup int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblAlarmPriorityToEmailGroup int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblAlarmPriorityToEmailGroup IS NULL OR 
        (@sync_batch_size_tblAlarmPriorityToEmailGroup IS NOT NULL AND @sync_batch_size_tblAlarmPriorityToEmailGroup = 0))
    BEGIN
        SET @sync_batch_size_tblAlarmPriorityToEmailGroup = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
        SELECT TOP(@sync_batch_size_tblAlarmPriorityToEmailGroup) WITH TIES [AlarmPriorityEmailGroupGuid],[AlarmPriorityGuid],[EmailGroupGuid],[CreatedDate],[CreatedBy],[UpdatedDate],[UpdatedBy],_RowVersion
        FROM (
            SELECT TOP(@sync_batch_size_tblAlarmPriorityToEmailGroup) WITH TIES [map].[tblAlarmPriorityToEmailGroup].[AlarmPriorityEmailGroupGuid],[map].[tblAlarmPriorityToEmailGroup].[AlarmPriorityGuid],[map].[tblAlarmPriorityToEmailGroup].[EmailGroupGuid],[map].[tblAlarmPriorityToEmailGroup].[CreatedDate],[map].[tblAlarmPriorityToEmailGroup].[CreatedBy],[map].[tblAlarmPriorityToEmailGroup].[UpdatedDate],[map].[tblAlarmPriorityToEmailGroup].[UpdatedBy],sync.udf_GetMaxRowVersion(CT.InsertedRowVersion,MAPCT.InsertedRowVersion,MAPCT2.InsertedRowVersion) AS '_RowVersion'
                FROM [map].[tblAlarmPriorityToEmailGroup]
                    INNER JOIN (SELECT [AlarmPriorityToSiteGuid],[AlarmPriorityGuid],[OwnerSiteGuid],[AssignedToSiteGuid] FROM [dbo].[udf_GetAssignedAlarmPriorityListForSite](@sync_context_site_guid)) data
                        ON [map].[tblAlarmPriorityToEmailGroup].[AlarmPriorityGuid] = data.[AlarmPriorityGuid]
                    INNER JOIN (SELECT [EmailGroupToSiteGuid],[EmailGroupGuid],[OwnerSiteGuid],[AssignedToSiteGuid] FROM [dbo].[udf_GetAssignedEmailGroupListForSite](@sync_context_site_guid)) data1
                        ON [map].[tblAlarmPriorityToEmailGroup].[EmailGroupGuid] = data1.[EmailGroupGuid]
