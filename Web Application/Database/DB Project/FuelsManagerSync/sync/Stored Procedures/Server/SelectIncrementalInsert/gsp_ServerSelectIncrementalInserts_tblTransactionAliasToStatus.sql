...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblTransactionAliasToStatus]
...
@sync_batch_size_tblTransactionAliasToStatus int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblTransactionAliasToStatus int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblTransactionAliasToStatus IS NULL OR 
        (@sync_batch_size_tblTransactionAliasToStatus IS NOT NULL AND @sync_batch_size_tblTransactionAliasToStatus = 0))
    BEGIN
        SET @sync_batch_size_tblTransactionAliasToStatus = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
        SELECT TOP(@sync_batch_size_tblTransactionAliasToStatus) WITH TIES [TransactionAliasToStatusGuid],[TransactionAliasGuid],[LookupTransactionStatusIndex],[CreatedDate],[CreatedBy],[UpdatedDate],[UpdatedBy],_RowVersion
        FROM (
            SELECT TOP(@sync_batch_size_tblTransactionAliasToStatus) WITH TIES [map].[tblTransactionAliasToStatus].[TransactionAliasToStatusGuid],[map].[tblTransactionAliasToStatus].[TransactionAliasGuid],[map].[tblTransactionAliasToStatus].[LookupTransactionStatusIndex],[map].[tblTransactionAliasToStatus].[CreatedDate],[map].[tblTransactionAliasToStatus].[CreatedBy],[map].[tblTransactionAliasToStatus].[UpdatedDate],[map].[tblTransactionAliasToStatus].[UpdatedBy],sync.udf_GetMaxRowVersion(CT.InsertedRowVersion,MAPCT.InsertedRowVersion,NULL) AS '_RowVersion'
                FROM [map].[tblTransactionAliasToStatus]
                    INNER JOIN (SELECT [TransactionAliasToSiteGuid],[TransactionAliasGuid],[OwnerSiteGuid],[AssignedToSiteGuid] FROM [dbo].[udf_GetAssignedTransactionAliasListForSite](@sync_context_site_guid)) data
                        ON [map].[tblTransactionAliasToStatus].[TransactionAliasGuid] = data.[TransactionAliasGuid]
