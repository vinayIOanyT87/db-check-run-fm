...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblApplicationStringToEntryMessage]
...
@sync_batch_size_tblApplicationStringToEntryMessage int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblApplicationStringToEntryMessage int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblApplicationStringToEntryMessage IS NULL OR 
        (@sync_batch_size_tblApplicationStringToEntryMessage IS NOT NULL AND @sync_batch_size_tblApplicationStringToEntryMessage = 0))
    BEGIN
        SET @sync_batch_size_tblApplicationStringToEntryMessage = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
        SELECT TOP(@sync_batch_size_tblApplicationStringToEntryMessage) WITH TIES [ApplicationStringToEntryMessageGuid],[ApplicationStringGuid],[ProductGroupApplicationStringGuid],[Sequence],[CreatedDate],[CreatedBy],[UpdatedDate],[UpdatedBy],_RowVersion
        FROM (
            SELECT TOP(@sync_batch_size_tblApplicationStringToEntryMessage) WITH TIES [map].[tblApplicationStringToEntryMessage].[ApplicationStringToEntryMessageGuid],[map].[tblApplicationStringToEntryMessage].[ApplicationStringGuid],[map].[tblApplicationStringToEntryMessage].[ProductGroupApplicationStringGuid],[map].[tblApplicationStringToEntryMessage].[Sequence],[map].[tblApplicationStringToEntryMessage].[CreatedDate],[map].[tblApplicationStringToEntryMessage].[CreatedBy],[map].[tblApplicationStringToEntryMessage].[UpdatedDate],[map].[tblApplicationStringToEntryMessage].[UpdatedBy],sync.udf_GetMaxRowVersion(CT.InsertedRowVersion,MAPCT.InsertedRowVersion,MAPCT2.InsertedRowVersion) AS '_RowVersion'
                FROM [map].[tblApplicationStringToEntryMessage]
                    INNER JOIN (SELECT [EntryMessageToSiteGuid],[ApplicationStringGuid],[OwnerSiteGuid],[AssignedToSiteGuid] FROM [dbo].[udf_GetAssignedApplicationStringEntryMessageListForSite](@sync_context_site_guid)) data
                        ON [map].[tblApplicationStringToEntryMessage].[ApplicationStringGuid] = data.[ApplicationStringGuid]
                    INNER JOIN (SELECT [ProductGroupToSiteGuid],[ApplicationStringGuid],[OwnerSiteGuid],[AssignedToSiteGuid] FROM [dbo].[udf_GetAssignedApplicationStringProductGroupListForSite](@sync_context_site_guid)) data1
                        ON [map].[tblApplicationStringToEntryMessage].[ProductGroupApplicationStringGuid] = data1.[ApplicationStringGuid]
                    ...
                    INNER JOIN [track].[tblEntityEntryMessageToSite] MAPCT
                        ON MAPCT.PK_EntryMessageToSiteGuid = data.[EntryMessageToSiteGuid]
