...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblPointHistory]
...
@sync_batch_size_tblPointHistory int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblPointHistory int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblPointHistory IS NULL OR 
        (@sync_batch_size_tblPointHistory IS NOT NULL AND @sync_batch_size_tblPointHistory = 0))
    BEGIN
        SET @sync_batch_size_tblPointHistory = 2147483647;
    END


    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
    SELECT TOP(@sync_batch_size_tblPointHistory) WITH TIES [dbo].[tblPointHistory].[PointHistoryGuid],[dbo].[tblPointHistory].[UserGuid],[dbo].[tblPointHistory].[SiteGuid],[dbo].[tblPointHistory].[StartDate],[dbo].[tblPointHistory].[IntervalQuantity],[dbo].[tblPointHistory].[IntervalType],[dbo].[tblPointHistory].[RangeQuantity],[dbo].[tblPointHistory].[RangeType],[dbo].[tblPointHistory].[ColumnsDefinition],[dbo].[tblPointHistory].[CreatedDate],[dbo].[tblPointHistory].[CreatedBy],[dbo].[tblPointHistory].[UpdatedDate],[dbo].[tblPointHistory].[UpdatedBy],CT.InsertedRowVersion AS '_RowVersion'
        FROM [dbo].[tblPointHistory]
            INNER JOIN [track].[tblPointHistory] CT
                ON CT.PK_PointHistoryGuid = [dbo].[tblPointHistory].[PointHistoryGuid] 
        WHERE ( [dbo].[tblPointHistory].[SiteGuid] = @sync_context_site_guid )
