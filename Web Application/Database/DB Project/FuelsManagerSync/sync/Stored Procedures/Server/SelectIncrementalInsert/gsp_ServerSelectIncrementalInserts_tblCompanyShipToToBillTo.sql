...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblCompanyShipToToBillTo]
...
@sync_batch_size_tblCompanyShipToToBillTo int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblCompanyShipToToBillTo int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblCompanyShipToToBillTo IS NULL OR 
        (@sync_batch_size_tblCompanyShipToToBillTo IS NOT NULL AND @sync_batch_size_tblCompanyShipToToBillTo = 0))
    BEGIN
        SET @sync_batch_size_tblCompanyShipToToBillTo = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
    SELECT TOP(@sync_batch_size_tblCompanyShipToToBillTo) WITH TIES [map].[tblCompanyShipToToBillTo].[CompanyShipToToBillToGuid],[map].[tblCompanyShipToToBillTo].[CompanyGuid],[map].[tblCompanyShipToToBillTo].[CompanyBillToToShipperGuid],[map].[tblCompanyShipToToBillTo].[SiteGuid],[map].[tblCompanyShipToToBillTo].[ID],[map].[tblCompanyShipToToBillTo].[CreatedDate],[map].[tblCompanyShipToToBillTo].[CreatedBy],[map].[tblCompanyShipToToBillTo].[UpdatedDate],[map].[tblCompanyShipToToBillTo].[UpdatedBy],CT.InsertedRowVersion AS '_RowVersion'
        FROM [map].[tblCompanyShipToToBillTo]
        ...
        WHERE ( [map].[tblCompanyShipToToBillTo].[SiteGuid] = @sync_context_site_guid )
