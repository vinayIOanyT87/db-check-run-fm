...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblCompanyPersonnelToShipToBillTo]
...
@sync_batch_size_tblCompanyPersonnelToShipToBillTo int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblCompanyPersonnelToShipToBillTo int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblCompanyPersonnelToShipToBillTo IS NULL OR 
        (@sync_batch_size_tblCompanyPersonnelToShipToBillTo IS NOT NULL AND @sync_batch_size_tblCompanyPersonnelToShipToBillTo = 0))
    BEGIN
        SET @sync_batch_size_tblCompanyPersonnelToShipToBillTo = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
    SELECT TOP(@sync_batch_size_tblCompanyPersonnelToShipToBillTo) WITH TIES [map].[tblCompanyPersonnelToShipToBillTo].[CompanyPersonnelToShipToBillToGuid],[map].[tblCompanyPersonnelToShipToBillTo].[PersonnelGuid],[map].[tblCompanyPersonnelToShipToBillTo].[CompanyShipToToBillToGuid],[map].[tblCompanyPersonnelToShipToBillTo].[SiteGuid],[map].[tblCompanyPersonnelToShipToBillTo].[ID],[map].[tblCompanyPersonnelToShipToBillTo].[CreatedDate],[map].[tblCompanyPersonnelToShipToBillTo].[CreatedBy],[map].[tblCompanyPersonnelToShipToBillTo].[UpdatedDate],[map].[tblCompanyPersonnelToShipToBillTo].[UpdatedBy],CT.InsertedRowVersion AS '_RowVersion'
        FROM [map].[tblCompanyPersonnelToShipToBillTo]
        ...
        WHERE ( [map].[tblCompanyPersonnelToShipToBillTo].[SiteGuid] = @sync_context_site_guid )
