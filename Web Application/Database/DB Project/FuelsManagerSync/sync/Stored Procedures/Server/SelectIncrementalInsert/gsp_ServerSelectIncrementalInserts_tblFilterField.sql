...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblFilterField]
...
@sync_batch_size_tblFilterField int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblFilterField int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblFilterField IS NULL OR 
        (@sync_batch_size_tblFilterField IS NOT NULL AND @sync_batch_size_tblFilterField = 0))
    BEGIN
        SET @sync_batch_size_tblFilterField = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
    SELECT TOP(@sync_batch_size_tblFilterField) WITH TIES [lookup].[tblFilterField].[FilterFieldIndex],[lookup].[tblFilterField].[FilterFieldCode],[lookup].[tblFilterField].[FilterFieldName],[lookup].[tblFilterField].[FilterFieldGuid],[lookup].[tblFilterField].[CreatedDate],[lookup].[tblFilterField].[CreatedBy],[lookup].[tblFilterField].[UpdatedDate],[lookup].[tblFilterField].[UpdatedBy],CT.InsertedRowVersion AS '_RowVersion'
        FROM [lookup].[tblFilterField]
