...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblEntityProductToSite]
...
@sync_first_time_sync_option_tblEntityProductToSite int
AS
BEGIN
    ...
    IF ((@sync_request_type = 4 AND (@sync_first_time_sync_option_tblEntityProductToSite IS NOT NULL AND @sync_first_time_sync_option_tblEntityProductToSite = 1))
    ...
    (
        SELECT data.[ProductToSiteGuid],data.[TrackedProductToSiteGuid],data.[IncludeChangeTrackingFlag],CT.InsertedRowVersion AS 'InsertedRowVersion' 
            FROM [map].[tblEntityProductToSite]
    ...
    SELECT TOP(@sync_batch_size_tblEntityProductToSite) WITH TIES [map].[tblEntityProductToSite].[ProductToSiteGuid],[map].[tblEntityProductToSite].[ProductGuid],[map].[tblEntityProductToSite].[SiteGuid],[map].[tblEntityProductToSite].[CreatedDate],[map].[tblEntityProductToSite].[CreatedBy],[map].[tblEntityProductToSite].[UpdatedDate],[map].[tblEntityProductToSite].[UpdatedBy],[map].[tblEntityProductToSite].[AssignedFromSiteGuid], data.InsertedRowVersion AS '_RowVersion'
        FROM [map].[tblEntityProductToSite]
            INNER JOIN (SELECT data.[ProductToSiteGuid], data1.[InsertedRowVersion]
