...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblAnimationToDrawing]
...
@sync_batch_size_tblAnimationToDrawing int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblAnimationToDrawing int
AS
BEGIN
    ...
    IF ((@sync_request_type = 4 AND (@sync_first_time_sync_option_tblAnimationToDrawing IS NOT NULL AND @sync_first_time_sync_option_tblAnimationToDrawing = 1))
        OR (@sync_bypass_insert_update_extraction IS NOT NULL AND @sync_bypass_insert_update_extraction = 1))
    BEGIN
        SELECT [map].[tblAnimationToDrawing].[AnimationToDrawingGuid],[map].[tblAnimationToDrawing].[AnimationGuid],[map].[tblAnimationToDrawing].[DrawingGuid],[map].[tblAnimationToDrawing].[CreatedBy],[map].[tblAnimationToDrawing].[CreatedDate],[map].[tblAnimationToDrawing].[UpdatedBy],[map].[tblAnimationToDrawing].[UpdatedDate], [map].[tblAnimationToDrawing].[_RowVersion]
    ...
    IF (@sync_batch_size_tblAnimationToDrawing IS NULL OR 
        (@sync_batch_size_tblAnimationToDrawing IS NOT NULL AND @sync_batch_size_tblAnimationToDrawing = 0))
    BEGIN
        SET @sync_batch_size_tblAnimationToDrawing = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
    SELECT TOP(@sync_batch_size_tblAnimationToDrawing) WITH TIES [AnimationToDrawingGuid],[AnimationGuid],[DrawingGuid],[CreatedBy],[CreatedDate],[UpdatedBy],[UpdatedDate],_RowVersion
    FROM (
        SELECT [AnimationToDrawingGuid],[AnimationGuid],[DrawingGuid],[CreatedBy],[CreatedDate],[UpdatedBy],[UpdatedDate],_RowVersion
        FROM (
            SELECT TOP(@sync_batch_size_tblAnimationToDrawing) WITH TIES [map].[tblAnimationToDrawing].[AnimationToDrawingGuid],[map].[tblAnimationToDrawing].[AnimationGuid],[map].[tblAnimationToDrawing].[DrawingGuid],[map].[tblAnimationToDrawing].[CreatedBy],[map].[tblAnimationToDrawing].[CreatedDate],[map].[tblAnimationToDrawing].[UpdatedBy],[map].[tblAnimationToDrawing].[UpdatedDate],sync.udf_GetMaxRowVersion(CT.InsertedRowVersion,MAPCT.InsertedRowVersion,NULL) AS '_RowVersion'
                FROM [map].[tblAnimationToDrawing]
                    INNER JOIN (SELECT [DrawingGuid],[PointTemplateToSiteGuid] FROM [dbo].[udf_GetAssignedPointDetailListPerSite](@sync_context_site_guid)) data
                        ON [map].[tblAnimationToDrawing].[DrawingGuid] = data.[DrawingGuid]
            ...
            SELECT TOP(@sync_batch_size_tblAnimationToDrawing) WITH TIES [map].[tblAnimationToDrawing].[AnimationToDrawingGuid],[map].[tblAnimationToDrawing].[AnimationGuid],[map].[tblAnimationToDrawing].[DrawingGuid],[map].[tblAnimationToDrawing].[CreatedBy],[map].[tblAnimationToDrawing].[CreatedDate],[map].[tblAnimationToDrawing].[UpdatedBy],[map].[tblAnimationToDrawing].[UpdatedDate],sync.udf_GetMaxRowVersion(CT.InsertedRowVersion,NULL,NULL) AS '_RowVersion'
                FROM [map].[tblAnimationToDrawing]
                    INNER JOIN (SELECT [DrawingGuid] FROM [dbo].[udf_GetAssignedDrawingListPerSite](@sync_context_site_guid)) data1
                        ON [map].[tblAnimationToDrawing].[DrawingGuid] = data1.[DrawingGuid]
                    INNER JOIN [track].[tblAnimationToDrawing] CT
                        ON CT.PK_AnimationToDrawingGuid = [map].[tblAnimationToDrawing].[AnimationToDrawingGuid] 
                WHERE ((CT.InsertedRowVersion > @sync_last_received_anchor_varbinary)
                        AND (CT.InsertedRowVersion <= @sync_new_received_anchor_varbinary)
