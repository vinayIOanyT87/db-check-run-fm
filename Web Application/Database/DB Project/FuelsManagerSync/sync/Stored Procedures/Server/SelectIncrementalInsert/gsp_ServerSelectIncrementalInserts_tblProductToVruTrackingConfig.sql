...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblProductToVruTrackingConfig]
...
@sync_batch_size_tblProductToVruTrackingConfig int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblProductToVruTrackingConfig int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblProductToVruTrackingConfig IS NULL OR 
        (@sync_batch_size_tblProductToVruTrackingConfig IS NOT NULL AND @sync_batch_size_tblProductToVruTrackingConfig = 0))
    BEGIN
        SET @sync_batch_size_tblProductToVruTrackingConfig = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
        SELECT TOP(@sync_batch_size_tblProductToVruTrackingConfig) WITH TIES [ProductToVruTrackingConfigGuid],[ProductGuid],[AssignedToSiteGuid],[Sequence],[BlendPercentage],[AdditiveRate],[Ratio],[AdditiveCycleVolume],[Tolerance],[PresetNumber],[AdditiveProfileGuid],[TankGuid],[MeterID],[ShipToProductID],[ShipToProductCode],[ShipToLoadRackDisplayText],[UnavailableInventoryGross],[UnavailableInventoryNet],[CreatedDate],[CreatedBy],[UpdatedDate],[UpdatedBy],[SpecialInstructionNote],_RowVersion
        FROM (
            SELECT TOP(@sync_batch_size_tblProductToVruTrackingConfig) WITH TIES [map].[tblProductToVruTrackingConfig].[ProductToVruTrackingConfigGuid],[map].[tblProductToVruTrackingConfig].[ProductGuid],[map].[tblProductToVruTrackingConfig].[AssignedToSiteGuid],[map].[tblProductToVruTrackingConfig].[Sequence],[map].[tblProductToVruTrackingConfig].[BlendPercentage],[map].[tblProductToVruTrackingConfig].[AdditiveRate],[map].[tblProductToVruTrackingConfig].[Ratio],[map].[tblProductToVruTrackingConfig].[AdditiveCycleVolume],[map].[tblProductToVruTrackingConfig].[Tolerance],[map].[tblProductToVruTrackingConfig].[PresetNumber],[map].[tblProductToVruTrackingConfig].[AdditiveProfileGuid],[map].[tblProductToVruTrackingConfig].[TankGuid],[map].[tblProductToVruTrackingConfig].[MeterID],[map].[tblProductToVruTrackingConfig].[ShipToProductID],[map].[tblProductToVruTrackingConfig].[ShipToProductCode],[map].[tblProductToVruTrackingConfig].[ShipToLoadRackDisplayText],[map].[tblProductToVruTrackingConfig].[UnavailableInventoryGross],[map].[tblProductToVruTrackingConfig].[UnavailableInventoryNet],[map].[tblProductToVruTrackingConfig].[CreatedDate],[map].[tblProductToVruTrackingConfig].[CreatedBy],[map].[tblProductToVruTrackingConfig].[UpdatedDate],[map].[tblProductToVruTrackingConfig].[UpdatedBy],[map].[tblProductToVruTrackingConfig].[SpecialInstructionNote],sync.udf_GetMaxRowVersion(CT.InsertedRowVersion,MAPCT.InsertedRowVersion,MAPCT2.InsertedRowVersion) AS '_RowVersion'
                FROM [map].[tblProductToVruTrackingConfig]
                    INNER JOIN (SELECT [SiteGuid] FROM [dbo].[tblSites] WHERE SiteGuid = @sync_context_site_guid) data
                        ON [map].[tblProductToVruTrackingConfig].[AssignedToSiteGuid] = data.[SiteGuid]
                    INNER JOIN (SELECT [ProductToSiteGuid],[ProductGuid],[OwnerSiteGuid],[AssignedToSiteGuid] FROM [dbo].[udf_GetAssignedProductListForSite](@sync_context_site_guid)) data1
                        ON [map].[tblProductToVruTrackingConfig].[ProductGuid] = data1.[ProductGuid]
                    ...
                    INNER JOIN [track].[tblSites] MAPCT
                        ON MAPCT.PK_SiteGuid = data.[SiteGuid]
