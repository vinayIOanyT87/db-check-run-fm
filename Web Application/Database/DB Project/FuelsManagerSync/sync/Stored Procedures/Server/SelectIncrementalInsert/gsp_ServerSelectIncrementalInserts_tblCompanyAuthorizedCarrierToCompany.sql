...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblCompanyAuthorizedCarrierToCompany]
...
@sync_batch_size_tblCompanyAuthorizedCarrierToCompany int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblCompanyAuthorizedCarrierToCompany int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblCompanyAuthorizedCarrierToCompany IS NULL OR 
        (@sync_batch_size_tblCompanyAuthorizedCarrierToCompany IS NOT NULL AND @sync_batch_size_tblCompanyAuthorizedCarrierToCompany = 0))
    BEGIN
        SET @sync_batch_size_tblCompanyAuthorizedCarrierToCompany = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
        SELECT TOP(@sync_batch_size_tblCompanyAuthorizedCarrierToCompany) WITH TIES [CompanyAuthorizedCarrierToCompanyGuid],[CompanyGuid],[AssignedToCompanyGuid],[SiteGuid],[ID],[CreatedDate],[CreatedBy],[UpdatedDate],[UpdatedBy],_RowVersion
        FROM (
            SELECT TOP(@sync_batch_size_tblCompanyAuthorizedCarrierToCompany) WITH TIES [map].[tblCompanyAuthorizedCarrierToCompany].[CompanyAuthorizedCarrierToCompanyGuid],[map].[tblCompanyAuthorizedCarrierToCompany].[CompanyGuid],[map].[tblCompanyAuthorizedCarrierToCompany].[AssignedToCompanyGuid],[map].[tblCompanyAuthorizedCarrierToCompany].[SiteGuid],[map].[tblCompanyAuthorizedCarrierToCompany].[ID],[map].[tblCompanyAuthorizedCarrierToCompany].[CreatedDate],[map].[tblCompanyAuthorizedCarrierToCompany].[CreatedBy],[map].[tblCompanyAuthorizedCarrierToCompany].[UpdatedDate],[map].[tblCompanyAuthorizedCarrierToCompany].[UpdatedBy],sync.udf_GetMaxRowVersion(CT.InsertedRowVersion,MAPCT.InsertedRowVersion,MAPCT2.InsertedRowVersion) AS '_RowVersion'
                FROM [map].[tblCompanyAuthorizedCarrierToCompany]
                    INNER JOIN (SELECT [CompanyToSiteGuid],[CompanyGuid] FROM [dbo].[udf_GetAssignedCompanyListForSite](@sync_context_site_guid)) data
                        ON [map].[tblCompanyAuthorizedCarrierToCompany].[CompanyGuid] = data.[CompanyGuid]
                    INNER JOIN (SELECT [CompanyToSiteGuid],[CompanyGuid] FROM [dbo].[udf_GetAssignedCompanyListForSite](@sync_context_site_guid)) data1
                        ON [map].[tblCompanyAuthorizedCarrierToCompany].[AssignedToCompanyGuid] = data1.[CompanyGuid]
