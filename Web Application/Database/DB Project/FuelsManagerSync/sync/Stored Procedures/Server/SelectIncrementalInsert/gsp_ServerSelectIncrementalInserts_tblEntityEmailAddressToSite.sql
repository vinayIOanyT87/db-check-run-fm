...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblEntityEmailAddressToSite]
...
@sync_batch_size_tblEntityEmailAddressToSite int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblEntityEmailAddressToSite int
AS
BEGIN
    ...
    IF ((@sync_request_type = 4 AND (@sync_first_time_sync_option_tblEntityEmailAddressToSite IS NOT NULL AND @sync_first_time_sync_option_tblEntityEmailAddressToSite = 1))
    ...
    IF (@sync_batch_size_tblEntityEmailAddressToSite IS NULL OR 
        (@sync_batch_size_tblEntityEmailAddressToSite IS NOT NULL AND @sync_batch_size_tblEntityEmailAddressToSite = 0))
    BEGIN
        SET @sync_batch_size_tblEntityEmailAddressToSite = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
    SELECT TOP(@sync_batch_size_tblEntityEmailAddressToSite) WITH TIES [map].[tblEntityEmailAddressToSite].[EmailAddressToSiteGuid],[map].[tblEntityEmailAddressToSite].[ApplicationStringGuid],[map].[tblEntityEmailAddressToSite].[SiteGuid],[map].[tblEntityEmailAddressToSite].[CreatedDate],[map].[tblEntityEmailAddressToSite].[CreatedBy],[map].[tblEntityEmailAddressToSite].[UpdatedDate],[map].[tblEntityEmailAddressToSite].[UpdatedBy],[map].[tblEntityEmailAddressToSite].[AssignedFromSiteGuid],CT.InsertedRowVersion AS '_RowVersion'
        FROM [map].[tblEntityEmailAddressToSite]
             INNER JOIN [dbo].[tblApplicationString] ON [map].[tblEntityEmailAddressToSite].[ApplicationStringGuid] = [dbo].[tblApplicationString].[ApplicationStringGuid] 
