...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblEntityAutoDistributionRuleToSite]
...
@sync_batch_size_tblEntityAutoDistributionRuleToSite int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblEntityAutoDistributionRuleToSite int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblEntityAutoDistributionRuleToSite IS NULL OR 
        (@sync_batch_size_tblEntityAutoDistributionRuleToSite IS NOT NULL AND @sync_batch_size_tblEntityAutoDistributionRuleToSite = 0))
    BEGIN
        SET @sync_batch_size_tblEntityAutoDistributionRuleToSite = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
    SELECT TOP(@sync_batch_size_tblEntityAutoDistributionRuleToSite) WITH TIES [map].[tblEntityAutoDistributionRuleToSite].[AutoDistributionRuleToSiteGuid],[map].[tblEntityAutoDistributionRuleToSite].[SiteGuid],[map].[tblEntityAutoDistributionRuleToSite].[AutoDistributionRuleGuid],[map].[tblEntityAutoDistributionRuleToSite].[CreatedDate],[map].[tblEntityAutoDistributionRuleToSite].[CreatedBy],[map].[tblEntityAutoDistributionRuleToSite].[UpdatedDate],[map].[tblEntityAutoDistributionRuleToSite].[UpdatedBy],[map].[tblEntityAutoDistributionRuleToSite].[AssignedFromSiteGuid],CT.InsertedRowVersion AS '_RowVersion'
        FROM [map].[tblEntityAutoDistributionRuleToSite]
