...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblDispatchGridColumnType]
...
@sync_batch_size_tblDispatchGridColumnType int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblDispatchGridColumnType int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblDispatchGridColumnType IS NULL OR 
        (@sync_batch_size_tblDispatchGridColumnType IS NOT NULL AND @sync_batch_size_tblDispatchGridColumnType = 0))
    BEGIN
        SET @sync_batch_size_tblDispatchGridColumnType = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
    SELECT TOP(@sync_batch_size_tblDispatchGridColumnType) WITH TIES [lookup].[tblDispatchGridColumnType].[DispatchGridColumnTypeIndex],[lookup].[tblDispatchGridColumnType].[LookupDispatchGridTypeIndex],[lookup].[tblDispatchGridColumnType].[DispatchGridColumnTypeGuid],[lookup].[tblDispatchGridColumnType].[CreatedDate],[lookup].[tblDispatchGridColumnType].[CreatedBy],[lookup].[tblDispatchGridColumnType].[UpdatedDate],[lookup].[tblDispatchGridColumnType].[UpdatedBy],[lookup].[tblDispatchGridColumnType].[ID],[lookup].[tblDispatchGridColumnType].[DisplayName],[lookup].[tblDispatchGridColumnType].[DataField],[lookup].[tblDispatchGridColumnType].[Width],[lookup].[tblDispatchGridColumnType].[DefaultColumnOrder],CT.InsertedRowVersion AS '_RowVersion'
        FROM [lookup].[tblDispatchGridColumnType]
