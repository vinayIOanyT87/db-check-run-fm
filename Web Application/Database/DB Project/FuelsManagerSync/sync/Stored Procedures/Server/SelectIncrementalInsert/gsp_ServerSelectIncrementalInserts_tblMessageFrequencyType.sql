...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblMessageFrequencyType]
...
@sync_batch_size_tblMessageFrequencyType int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblMessageFrequencyType int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblMessageFrequencyType IS NULL OR 
        (@sync_batch_size_tblMessageFrequencyType IS NOT NULL AND @sync_batch_size_tblMessageFrequencyType = 0))
    BEGIN
        SET @sync_batch_size_tblMessageFrequencyType = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
    SELECT TOP(@sync_batch_size_tblMessageFrequencyType) WITH TIES [lookup].[tblMessageFrequencyType].[MessageFrequencyTypeIndex],[lookup].[tblMessageFrequencyType].[MessageFrequencyTypeCode],[lookup].[tblMessageFrequencyType].[MessageFrequencyTypeName],[lookup].[tblMessageFrequencyType].[MessageFrequencyTypeGuid],[lookup].[tblMessageFrequencyType].[CreatedDate],[lookup].[tblMessageFrequencyType].[CreatedBy],[lookup].[tblMessageFrequencyType].[UpdatedDate],[lookup].[tblMessageFrequencyType].[UpdatedBy],CT.InsertedRowVersion AS '_RowVersion'
        FROM [lookup].[tblMessageFrequencyType]
