...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblApplicationStringToFootNoteShipToState]
...
@sync_batch_size_tblApplicationStringToFootNoteShipToState int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblApplicationStringToFootNoteShipToState int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblApplicationStringToFootNoteShipToState IS NULL OR 
        (@sync_batch_size_tblApplicationStringToFootNoteShipToState IS NOT NULL AND @sync_batch_size_tblApplicationStringToFootNoteShipToState = 0))
    BEGIN
        SET @sync_batch_size_tblApplicationStringToFootNoteShipToState = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
        SELECT [ApplicationStringToFootNoteShipToStateGuid],[ApplicationStringGuid],[AssignedToApplicationStringGuid],[Sequence],[CreatedDate],[CreatedBy],[UpdatedDate],[UpdatedBy],_RowVersion
        FROM (
            SELECT TOP(@sync_batch_size_tblApplicationStringToFootNoteShipToState) WITH TIES [map].[tblApplicationStringToFootNoteShipToState].[ApplicationStringToFootNoteShipToStateGuid],[map].[tblApplicationStringToFootNoteShipToState].[ApplicationStringGuid],[map].[tblApplicationStringToFootNoteShipToState].[AssignedToApplicationStringGuid],[map].[tblApplicationStringToFootNoteShipToState].[Sequence],[map].[tblApplicationStringToFootNoteShipToState].[CreatedDate],[map].[tblApplicationStringToFootNoteShipToState].[CreatedBy],[map].[tblApplicationStringToFootNoteShipToState].[UpdatedDate],[map].[tblApplicationStringToFootNoteShipToState].[UpdatedBy],sync.udf_GetMaxRowVersion(CT.InsertedRowVersion,MAPCT.InsertedRowVersion,NULL) AS '_RowVersion'
                FROM [map].[tblApplicationStringToFootNoteShipToState]
                    INNER JOIN (SELECT [FootNoteToSiteGuid],[ApplicationStringGuid],[OwnerSiteGuid],[AssignedToSiteGuid] FROM [dbo].[udf_GetAssignedApplicationStringFootNoteListForSite](@sync_context_site_guid)) data
                        ON [map].[tblApplicationStringToFootNoteShipToState].[ApplicationStringGuid] = data.[ApplicationStringGuid]
