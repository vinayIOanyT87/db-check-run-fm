...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblEntityMobileDeviceProfileToSite]
...
@sync_batch_size_tblEntityMobileDeviceProfileToSite int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblEntityMobileDeviceProfileToSite int
AS
BEGIN
    ...
    IF ((@sync_request_type = 4 AND (@sync_first_time_sync_option_tblEntityMobileDeviceProfileToSite IS NOT NULL AND @sync_first_time_sync_option_tblEntityMobileDeviceProfileToSite = 1))
    ...
    IF (@sync_batch_size_tblEntityMobileDeviceProfileToSite IS NULL OR 
        (@sync_batch_size_tblEntityMobileDeviceProfileToSite IS NOT NULL AND @sync_batch_size_tblEntityMobileDeviceProfileToSite = 0))
    BEGIN
        SET @sync_batch_size_tblEntityMobileDeviceProfileToSite = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
    SELECT TOP(@sync_batch_size_tblEntityMobileDeviceProfileToSite) WITH TIES [map].[tblEntityMobileDeviceProfileToSite].[MobileDeviceProfileToSiteGuid],[map].[tblEntityMobileDeviceProfileToSite].[MobileDeviceProfileGuid],[map].[tblEntityMobileDeviceProfileToSite].[SiteGuid],[map].[tblEntityMobileDeviceProfileToSite].[CreatedDate],[map].[tblEntityMobileDeviceProfileToSite].[CreatedBy],[map].[tblEntityMobileDeviceProfileToSite].[UpdatedDate],[map].[tblEntityMobileDeviceProfileToSite].[UpdatedBy],[map].[tblEntityMobileDeviceProfileToSite].[AssignedFromSiteGuid],CT.InsertedRowVersion AS '_RowVersion'
        FROM [map].[tblEntityMobileDeviceProfileToSite]
             INNER JOIN [dbo].[tblMobileDeviceProfile] ON [map].[tblEntityMobileDeviceProfileToSite].[MobileDeviceProfileGuid] = [dbo].[tblMobileDeviceProfile].[MobileDeviceProfileGuid] 
