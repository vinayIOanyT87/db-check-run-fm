...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblAirplaneTankLocation]
...
@sync_batch_size_tblAirplaneTankLocation int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblAirplaneTankLocation int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblAirplaneTankLocation IS NULL OR 
        (@sync_batch_size_tblAirplaneTankLocation IS NOT NULL AND @sync_batch_size_tblAirplaneTankLocation = 0))
    BEGIN
        SET @sync_batch_size_tblAirplaneTankLocation = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
    SELECT TOP(@sync_batch_size_tblAirplaneTankLocation) WITH TIES [lookup].[tblAirplaneTankLocation].[TankLocationIndex],[lookup].[tblAirplaneTankLocation].[TankLocationCode],[lookup].[tblAirplaneTankLocation].[TankLocationName],[lookup].[tblAirplaneTankLocation].[TankLocationGuid],[lookup].[tblAirplaneTankLocation].[CreatedDate],[lookup].[tblAirplaneTankLocation].[CreatedBy],[lookup].[tblAirplaneTankLocation].[UpdatedDate],[lookup].[tblAirplaneTankLocation].[UpdatedBy],CT.InsertedRowVersion AS '_RowVersion'
        FROM [lookup].[tblAirplaneTankLocation]
