...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblEntityAppointmentEquipmentToSite]
...
@sync_batch_size_tblEntityAppointmentEquipmentToSite int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblEntityAppointmentEquipmentToSite int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblEntityAppointmentEquipmentToSite IS NULL OR 
        (@sync_batch_size_tblEntityAppointmentEquipmentToSite IS NOT NULL AND @sync_batch_size_tblEntityAppointmentEquipmentToSite = 0))
    BEGIN
        SET @sync_batch_size_tblEntityAppointmentEquipmentToSite = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
    SELECT TOP(@sync_batch_size_tblEntityAppointmentEquipmentToSite) WITH TIES [map].[tblEntityAppointmentEquipmentToSite].[AppointmentEquipmentToSiteGuid],[map].[tblEntityAppointmentEquipmentToSite].[AppointmentEquipmentGuid],[map].[tblEntityAppointmentEquipmentToSite].[SiteGuid],[map].[tblEntityAppointmentEquipmentToSite].[CreatedDate],[map].[tblEntityAppointmentEquipmentToSite].[CreatedBy],[map].[tblEntityAppointmentEquipmentToSite].[UpdatedDate],[map].[tblEntityAppointmentEquipmentToSite].[UpdatedBy],[map].[tblEntityAppointmentEquipmentToSite].[AssignedFromSiteGuid],CT.InsertedRowVersion AS '_RowVersion'
        FROM [map].[tblEntityAppointmentEquipmentToSite]
