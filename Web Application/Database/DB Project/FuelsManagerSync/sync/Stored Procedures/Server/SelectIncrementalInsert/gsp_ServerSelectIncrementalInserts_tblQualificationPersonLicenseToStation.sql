...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblQualificationPersonLicenseToStation]
...
@sync_batch_size_tblQualificationPersonLicenseToStation int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblQualificationPersonLicenseToStation int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblQualificationPersonLicenseToStation IS NULL OR 
        (@sync_batch_size_tblQualificationPersonLicenseToStation IS NOT NULL AND @sync_batch_size_tblQualificationPersonLicenseToStation = 0))
    BEGIN
        SET @sync_batch_size_tblQualificationPersonLicenseToStation = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
        SELECT TOP(@sync_batch_size_tblQualificationPersonLicenseToStation) WITH TIES [QualificationPersonLicenseToStationGuid],[QualificationGuid],[StationGuid],[Sequence],[Instructor],[DateCompleted],[DateDue],[ExpirationDate],[ID],[Rating],[HistoricalRecord],[CreatedDate],[CreatedBy],[UpdatedDate],[UpdatedBy],_RowVersion
        FROM (
            SELECT TOP(@sync_batch_size_tblQualificationPersonLicenseToStation) WITH TIES [map].[tblQualificationPersonLicenseToStation].[QualificationPersonLicenseToStationGuid],[map].[tblQualificationPersonLicenseToStation].[QualificationGuid],[map].[tblQualificationPersonLicenseToStation].[StationGuid],[map].[tblQualificationPersonLicenseToStation].[Sequence],[map].[tblQualificationPersonLicenseToStation].[Instructor],[map].[tblQualificationPersonLicenseToStation].[DateCompleted],[map].[tblQualificationPersonLicenseToStation].[DateDue],[map].[tblQualificationPersonLicenseToStation].[ExpirationDate],[map].[tblQualificationPersonLicenseToStation].[ID],[map].[tblQualificationPersonLicenseToStation].[Rating],[map].[tblQualificationPersonLicenseToStation].[HistoricalRecord],[map].[tblQualificationPersonLicenseToStation].[CreatedDate],[map].[tblQualificationPersonLicenseToStation].[CreatedBy],[map].[tblQualificationPersonLicenseToStation].[UpdatedDate],[map].[tblQualificationPersonLicenseToStation].[UpdatedBy],sync.udf_GetMaxRowVersion(CT.InsertedRowVersion,MAPCT.InsertedRowVersion,MAPCT2.InsertedRowVersion) AS '_RowVersion'
                FROM [map].[tblQualificationPersonLicenseToStation]
                    INNER JOIN (SELECT [PersonnelLicenseToSiteGuid],[QualificationGuid],[OwnerSiteGuid],[AssignedToSiteGuid] FROM [dbo].[udf_GetAssignedQualificationPersonnelLicenseListForSite](@sync_context_site_guid)) data
                        ON [map].[tblQualificationPersonLicenseToStation].[QualificationGuid] = data.[QualificationGuid]
                    INNER JOIN (SELECT [StationGuid],[OwnerSiteGuid] FROM [dbo].[udf_GetAssociatedStationListForSite](@sync_context_site_guid)) data1
                        ON [map].[tblQualificationPersonLicenseToStation].[StationGuid] = data1.[StationGuid]
                    ...
                    INNER JOIN [track].[tblEntityPersonnelLicenseToSite] MAPCT
                        ON MAPCT.PK_PersonnelLicenseToSiteGuid = data.[PersonnelLicenseToSiteGuid]
