...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblModuleToPointTemplate]
...
@sync_batch_size_tblModuleToPointTemplate int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblModuleToPointTemplate int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblModuleToPointTemplate IS NULL OR 
        (@sync_batch_size_tblModuleToPointTemplate IS NOT NULL AND @sync_batch_size_tblModuleToPointTemplate = 0))
    BEGIN
        SET @sync_batch_size_tblModuleToPointTemplate = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
        SELECT TOP(@sync_batch_size_tblModuleToPointTemplate) WITH TIES [ID],[Order],[ModuleToPointTemplateData],[CreatedDate],[CreatedBy],[UpdatedDate],[UpdatedBy],[ModuleToPointTemplateGuid],[PointTemplateGuid],[ModuleGuid],_RowVersion
        FROM (
            SELECT TOP(@sync_batch_size_tblModuleToPointTemplate) WITH TIES [map].[tblModuleToPointTemplate].[ID],[map].[tblModuleToPointTemplate].[Order],[map].[tblModuleToPointTemplate].[ModuleToPointTemplateData],[map].[tblModuleToPointTemplate].[CreatedDate],[map].[tblModuleToPointTemplate].[CreatedBy],[map].[tblModuleToPointTemplate].[UpdatedDate],[map].[tblModuleToPointTemplate].[UpdatedBy],[map].[tblModuleToPointTemplate].[ModuleToPointTemplateGuid],[map].[tblModuleToPointTemplate].[PointTemplateGuid],[map].[tblModuleToPointTemplate].[ModuleGuid],sync.udf_GetMaxRowVersion(CT.InsertedRowVersion,MAPCT.InsertedRowVersion,NULL) AS '_RowVersion'
                FROM [map].[tblModuleToPointTemplate]
                    INNER JOIN (SELECT [PointTemplateToSiteGuid],[PointTemplateGuid],[OwnerSiteGuid],[AssignedToSiteGuid] FROM [dbo].[udf_GetAssignedPointTemplateForSite](@sync_context_site_guid)) data
                        ON [map].[tblModuleToPointTemplate].[PointTemplateGuid] = data.[PointTemplateGuid]
