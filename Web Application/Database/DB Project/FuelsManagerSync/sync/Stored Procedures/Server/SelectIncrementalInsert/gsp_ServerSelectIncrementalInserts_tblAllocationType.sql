...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblAllocationType]
...
@sync_batch_size_tblAllocationType int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblAllocationType int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblAllocationType IS NULL OR 
        (@sync_batch_size_tblAllocationType IS NOT NULL AND @sync_batch_size_tblAllocationType = 0))
    BEGIN
        SET @sync_batch_size_tblAllocationType = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
    SELECT TOP(@sync_batch_size_tblAllocationType) WITH TIES [lookup].[tblAllocationType].[AllocationTypeIndex],[lookup].[tblAllocationType].[AllocationTypeCode],[lookup].[tblAllocationType].[AllocationTypeName],[lookup].[tblAllocationType].[AllocationTypeGuid],[lookup].[tblAllocationType].[CreatedDate],[lookup].[tblAllocationType].[CreatedBy],[lookup].[tblAllocationType].[UpdatedDate],[lookup].[tblAllocationType].[UpdatedBy],CT.InsertedRowVersion AS '_RowVersion'
        FROM [lookup].[tblAllocationType]
