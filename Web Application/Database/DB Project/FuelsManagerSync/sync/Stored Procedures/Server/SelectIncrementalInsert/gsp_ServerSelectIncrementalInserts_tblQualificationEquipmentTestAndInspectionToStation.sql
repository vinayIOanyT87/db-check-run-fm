...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblQualificationEquipmentTestAndInspectionToStation]
...
@sync_batch_size_tblQualificationEquipmentTestAndInspectionToStation int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblQualificationEquipmentTestAndInspectionToStation int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblQualificationEquipmentTestAndInspectionToStation IS NULL OR 
        (@sync_batch_size_tblQualificationEquipmentTestAndInspectionToStation IS NOT NULL AND @sync_batch_size_tblQualificationEquipmentTestAndInspectionToStation = 0))
    BEGIN
        SET @sync_batch_size_tblQualificationEquipmentTestAndInspectionToStation = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
        SELECT TOP(@sync_batch_size_tblQualificationEquipmentTestAndInspectionToStation) WITH TIES [QualificationEquipmentTestAndInspectionToStationGuid],[QualificationGuid],[StationGuid],[Sequence],[Instructor],[DateCompleted],[DateDue],[ExpirationDate],[ID],[Rating],[HistoricalRecord],[CreatedDate],[CreatedBy],[UpdatedDate],[UpdatedBy],_RowVersion
        FROM (
            SELECT TOP(@sync_batch_size_tblQualificationEquipmentTestAndInspectionToStation) WITH TIES [map].[tblQualificationEquipmentTestAndInspectionToStation].[QualificationEquipmentTestAndInspectionToStationGuid],[map].[tblQualificationEquipmentTestAndInspectionToStation].[QualificationGuid],[map].[tblQualificationEquipmentTestAndInspectionToStation].[StationGuid],[map].[tblQualificationEquipmentTestAndInspectionToStation].[Sequence],[map].[tblQualificationEquipmentTestAndInspectionToStation].[Instructor],[map].[tblQualificationEquipmentTestAndInspectionToStation].[DateCompleted],[map].[tblQualificationEquipmentTestAndInspectionToStation].[DateDue],[map].[tblQualificationEquipmentTestAndInspectionToStation].[ExpirationDate],[map].[tblQualificationEquipmentTestAndInspectionToStation].[ID],[map].[tblQualificationEquipmentTestAndInspectionToStation].[Rating],[map].[tblQualificationEquipmentTestAndInspectionToStation].[HistoricalRecord],[map].[tblQualificationEquipmentTestAndInspectionToStation].[CreatedDate],[map].[tblQualificationEquipmentTestAndInspectionToStation].[CreatedBy],[map].[tblQualificationEquipmentTestAndInspectionToStation].[UpdatedDate],[map].[tblQualificationEquipmentTestAndInspectionToStation].[UpdatedBy],sync.udf_GetMaxRowVersion(CT.InsertedRowVersion,MAPCT.InsertedRowVersion,MAPCT2.InsertedRowVersion) AS '_RowVersion'
                FROM [map].[tblQualificationEquipmentTestAndInspectionToStation]
                    INNER JOIN (SELECT [EquipmentTestAndInspectionToSiteGuid],[QualificationGuid],[OwnerSiteGuid],[AssignedToSiteGuid] FROM [dbo].[udf_GetAssignedQualificationEquipmentTestAndInspectionListForSite](@sync_context_site_guid)) data
                        ON [map].[tblQualificationEquipmentTestAndInspectionToStation].[QualificationGuid] = data.[QualificationGuid]
                    INNER JOIN (SELECT [StationGuid],[OwnerSiteGuid] FROM [dbo].[udf_GetAssociatedStationListForSite](@sync_context_site_guid)) data1
                        ON [map].[tblQualificationEquipmentTestAndInspectionToStation].[StationGuid] = data1.[StationGuid]
