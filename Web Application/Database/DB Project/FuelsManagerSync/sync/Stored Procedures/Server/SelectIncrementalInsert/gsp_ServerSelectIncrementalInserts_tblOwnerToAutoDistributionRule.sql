...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblOwnerToAutoDistributionRule]
...
@sync_batch_size_tblOwnerToAutoDistributionRule int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblOwnerToAutoDistributionRule int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblOwnerToAutoDistributionRule IS NULL OR 
        (@sync_batch_size_tblOwnerToAutoDistributionRule IS NOT NULL AND @sync_batch_size_tblOwnerToAutoDistributionRule = 0))
    BEGIN
        SET @sync_batch_size_tblOwnerToAutoDistributionRule = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
        SELECT TOP(@sync_batch_size_tblOwnerToAutoDistributionRule) WITH TIES [OwnerToAutoDistributionRuleGuid],[AutoDistributionRuleGuid],[OwnerGuid],[CreatedDate],[CreatedBy],[UpdatedDate],[UpdatedBy],_RowVersion
        FROM (
            SELECT TOP(@sync_batch_size_tblOwnerToAutoDistributionRule) WITH TIES [map].[tblOwnerToAutoDistributionRule].[OwnerToAutoDistributionRuleGuid],[map].[tblOwnerToAutoDistributionRule].[AutoDistributionRuleGuid],[map].[tblOwnerToAutoDistributionRule].[OwnerGuid],[map].[tblOwnerToAutoDistributionRule].[CreatedDate],[map].[tblOwnerToAutoDistributionRule].[CreatedBy],[map].[tblOwnerToAutoDistributionRule].[UpdatedDate],[map].[tblOwnerToAutoDistributionRule].[UpdatedBy],sync.udf_GetMaxRowVersion(CT.InsertedRowVersion,MAPCT.InsertedRowVersion,NULL) AS '_RowVersion'
                FROM [map].[tblOwnerToAutoDistributionRule]
                    INNER JOIN (SELECT [CompanyToSiteGuid],[CompanyGuid],[OwnerSiteGuid],[AssignedToSiteGuid] FROM [dbo].[udf_GetAssignedCompanyListForSite](@sync_context_site_guid)) data
                        ON [map].[tblOwnerToAutoDistributionRule].[OwnerGuid] = data.[CompanyGuid]
