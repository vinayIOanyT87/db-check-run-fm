...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblCompanySupplierToOwner]
...
@sync_batch_size_tblCompanySupplierToOwner int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblCompanySupplierToOwner int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblCompanySupplierToOwner IS NULL OR 
        (@sync_batch_size_tblCompanySupplierToOwner IS NOT NULL AND @sync_batch_size_tblCompanySupplierToOwner = 0))
    BEGIN
        SET @sync_batch_size_tblCompanySupplierToOwner = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
        SELECT TOP(@sync_batch_size_tblCompanySupplierToOwner) WITH TIES [CompanySupplierToOwnerGuid],[CompanyGuid],[CompanyOffLoadOwnerToManagerGuid],[SiteGuid],[ID],[CreatedDate],[CreatedBy],[UpdatedDate],[UpdatedBy],_RowVersion
        FROM (
            SELECT TOP(@sync_batch_size_tblCompanySupplierToOwner) WITH TIES [map].[tblCompanySupplierToOwner].[CompanySupplierToOwnerGuid],[map].[tblCompanySupplierToOwner].[CompanyGuid],[map].[tblCompanySupplierToOwner].[CompanyOffLoadOwnerToManagerGuid],[map].[tblCompanySupplierToOwner].[SiteGuid],[map].[tblCompanySupplierToOwner].[ID],[map].[tblCompanySupplierToOwner].[CreatedDate],[map].[tblCompanySupplierToOwner].[CreatedBy],[map].[tblCompanySupplierToOwner].[UpdatedDate],[map].[tblCompanySupplierToOwner].[UpdatedBy],sync.udf_GetMaxRowVersion(CT.InsertedRowVersion,MAPCT.InsertedRowVersion,MAPCT2.InsertedRowVersion) AS '_RowVersion'
                FROM [map].[tblCompanySupplierToOwner]
                    INNER JOIN (SELECT [CompanySupplierToOwnerGuid],[CompanyToSiteGuid],[CompanyGuid],[CompanyOffLoadOwnerToManagerGuid],[OwnerSiteGuid] FROM [map].[udf_GetAssociatedCompanySupplierToOwnerListForSite](@sync_context_site_guid)) data
                        ON [map].[tblCompanySupplierToOwner].[CompanySupplierToOwnerGuid] = data.[CompanySupplierToOwnerGuid]
                    ...
                    INNER JOIN [track].[tblCompanyOffLoadOwnerToManager] MAPCT2
                        ON MAPCT2.PK_CompanyOffLoadOwnerToManagerGuid = data.[CompanyOffLoadOwnerToManagerGuid]
