...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblProductToCompany]
...
@sync_batch_size_tblProductToCompany int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblProductToCompany int
AS
BEGIN
    ...
    IF ((@sync_request_type = 4 AND (@sync_first_time_sync_option_tblProductToCompany IS NOT NULL AND @sync_first_time_sync_option_tblProductToCompany = 1))
            ...
            SELECT TOP(@sync_batch_size_tblProductToCompany) WITH TIES [map].[tblProductToCompany].[ProductToCompanyGuid],[map].[tblProductToCompany].[ProductGuid],[map].[tblProductToCompany].[AssignedToCompanyGuid],[map].[tblProductToCompany].[Sequence],[map].[tblProductToCompany].[BlendPercentage],[map].[tblProductToCompany].[AdditiveRate],[map].[tblProductToCompany].[Ratio],[map].[tblProductToCompany].[AdditiveCycleVolume],[map].[tblProductToCompany].[Tolerance],[map].[tblProductToCompany].[PresetNumber],[map].[tblProductToCompany].[AdditiveProfileGuid],[map].[tblProductToCompany].[TankGuid],[map].[tblProductToCompany].[MeterID],[map].[tblProductToCompany].[ShipToProductID],[map].[tblProductToCompany].[ShipToProductCode],[map].[tblProductToCompany].[ShipToLoadRackDisplayText],[map].[tblProductToCompany].[UnavailableInventoryGross],[map].[tblProductToCompany].[UnavailableInventoryNet],[map].[tblProductToCompany].[CreatedDate],[map].[tblProductToCompany].[CreatedBy],[map].[tblProductToCompany].[UpdatedDate],[map].[tblProductToCompany].[UpdatedBy],[map].[tblProductToCompany].[SpecialInstructionNote],sync.udf_GetMaxRowVersion(CT.InsertedRowVersion,MAPCT.InsertedRowVersion,MAPCT2.InsertedRowVersion) AS '_RowVersion'
                FROM [map].[tblProductToCompany]
                    INNER JOIN (SELECT [ProductToSiteGuid],[ProductGuid],[OwnerSiteGuid],[AssignedToSiteGuid] FROM [dbo].[udf_GetAssignedProductListForSite](@sync_context_site_guid)) data
                        ON [map].[tblProductToCompany].[ProductGuid] = data.[ProductGuid]
                    INNER JOIN (SELECT [CompanyToSiteGuid],[CompanyGuid],[OwnerSiteGuid],[AssignedToSiteGuid] FROM [dbo].[udf_GetAssignedCompanyListForSite](@sync_context_site_guid)) data1
                        ON [map].[tblProductToCompany].[AssignedToCompanyGuid] = data1.[CompanyGuid]
