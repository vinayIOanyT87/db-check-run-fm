...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblCompanyOffLoadOwnerToManager]
...
@sync_batch_size_tblCompanyOffLoadOwnerToManager int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblCompanyOffLoadOwnerToManager int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblCompanyOffLoadOwnerToManager IS NULL OR 
        (@sync_batch_size_tblCompanyOffLoadOwnerToManager IS NOT NULL AND @sync_batch_size_tblCompanyOffLoadOwnerToManager = 0))
    BEGIN
        SET @sync_batch_size_tblCompanyOffLoadOwnerToManager = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
        SELECT TOP(@sync_batch_size_tblCompanyOffLoadOwnerToManager) WITH TIES [CompanyOffLoadOwnerToManagerGuid],[CompanyGuid],[AssignedToCompanyGuid],[SiteGuid],[ID],[CreatedDate],[CreatedBy],[UpdatedDate],[UpdatedBy],_RowVersion
        FROM (
            SELECT TOP(@sync_batch_size_tblCompanyOffLoadOwnerToManager) WITH TIES [map].[tblCompanyOffLoadOwnerToManager].[CompanyOffLoadOwnerToManagerGuid],[map].[tblCompanyOffLoadOwnerToManager].[CompanyGuid],[map].[tblCompanyOffLoadOwnerToManager].[AssignedToCompanyGuid],[map].[tblCompanyOffLoadOwnerToManager].[SiteGuid],[map].[tblCompanyOffLoadOwnerToManager].[ID],[map].[tblCompanyOffLoadOwnerToManager].[CreatedDate],[map].[tblCompanyOffLoadOwnerToManager].[CreatedBy],[map].[tblCompanyOffLoadOwnerToManager].[UpdatedDate],[map].[tblCompanyOffLoadOwnerToManager].[UpdatedBy],sync.udf_GetMaxRowVersion(CT.InsertedRowVersion,MAPCT.InsertedRowVersion,MAPCT2.InsertedRowVersion) AS '_RowVersion'
                FROM [map].[tblCompanyOffLoadOwnerToManager]
                    INNER JOIN (SELECT [CompanyOffLoadOwnerToManagerGuid],[CompanyToSiteGuid],[AssignedToCompanyToSiteGuid],[CompanyGuid],[AssignedToCompanyGuid],[OwnerSiteGuid] FROM [map].[udf_GetAssociatedCompanyOffLoadOwnerToManagerListForSite](@sync_context_site_guid)) data
                        ON [map].[tblCompanyOffLoadOwnerToManager].[CompanyOffLoadOwnerToManagerGuid] = data.[CompanyOffLoadOwnerToManagerGuid]
