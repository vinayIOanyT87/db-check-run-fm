...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblQualityTags]
...
@sync_first_time_sync_option_tblQualityTags int
AS
BEGIN
    ...
    IF ((@sync_request_type = 4 AND (@sync_first_time_sync_option_tblQualityTags IS NOT NULL AND @sync_first_time_sync_option_tblQualityTags = 1))
    ...
    -- and/or any new entity site assignments (if assignable).
        ...
        FROM (
            SELECT TOP(@sync_batch_size_tblQualityTags) WITH TIES [dbo].[tblQualityTags].[Name],[dbo].[tblQualityTags].[Severity],[dbo].[tblQualityTags].[Active],[dbo].[tblQualityTags].[CreatedDate],[dbo].[tblQualityTags].[CreatedBy],[dbo].[tblQualityTags].[UpdatedDate],[dbo].[tblQualityTags].[UpdatedBy],[dbo].[tblQualityTags].[QualityTagGuid],[dbo].[tblQualityTags].[SiteGuid],sync.udf_GetMaxRowVersion(CT.InsertedRowVersion,MAPCT.InsertedRowVersion,NULL) AS '_RowVersion'
                ...
                WHERE ( [dbo].[tblQualityTags].[SiteGuid] = @sync_context_site_guid )
