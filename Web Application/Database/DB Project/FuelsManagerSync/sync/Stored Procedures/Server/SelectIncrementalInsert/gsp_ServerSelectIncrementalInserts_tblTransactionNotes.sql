...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblTransactionNotes]
...
@sync_first_time_sync_option_tblTransactionNotes int
AS
BEGIN
    ...
    IF ((@sync_request_type = 4 AND (@sync_first_time_sync_option_tblTransactionNotes IS NOT NULL AND @sync_first_time_sync_option_tblTransactionNotes = 1))
    ...
    END
        ...
        -- We only do this on tblTransactionNotes so we know what which tblTransactionNotes records are being processed.
        -- Synchronization will only synchronize other dependent records that are associated with the tblTransactionNotes records
        -- that were included.
    ...
    -- 
    SELECT [dbo].[tblTransactionNotes].[Notes],[dbo].[tblTransactionNotes].[CreatedBy],[dbo].[tblTransactionNotes].[CreatedDate],[dbo].[tblTransactionNotes].[UpdatedBy],[dbo].[tblTransactionNotes].[UpdatedDate],[dbo].[tblTransactionNotes].[AdditionalInformation],[dbo].[tblTransactionNotes].[TransactionNoteGuid],[dbo].[tblTransactionNotes].[TransactionGuid],CT.InsertedRowVersion AS '_RowVersion'
        FROM [dbo].[tblTransactionNotes]
