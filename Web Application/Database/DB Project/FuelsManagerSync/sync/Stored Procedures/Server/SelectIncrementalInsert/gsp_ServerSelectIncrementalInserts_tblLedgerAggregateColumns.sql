...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblLedgerAggregateColumns]
...
@sync_batch_size_tblLedgerAggregateColumns int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblLedgerAggregateColumns int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblLedgerAggregateColumns IS NULL OR 
        (@sync_batch_size_tblLedgerAggregateColumns IS NOT NULL AND @sync_batch_size_tblLedgerAggregateColumns = 0))
    BEGIN
        SET @sync_batch_size_tblLedgerAggregateColumns = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
        SELECT TOP(@sync_batch_size_tblLedgerAggregateColumns) WITH TIES [ID],[CreatedDate],[CreatedBy],[UpdatedDate],[UpdatedBy],[CustomFunctionName],[LedgerAggregateColumnGuid],[SiteGuid],[LookupAggregateFieldIndex],_RowVersion
        FROM (
            SELECT TOP(@sync_batch_size_tblLedgerAggregateColumns) WITH TIES [dbo].[tblLedgerAggregateColumns].[ID],[dbo].[tblLedgerAggregateColumns].[CreatedDate],[dbo].[tblLedgerAggregateColumns].[CreatedBy],[dbo].[tblLedgerAggregateColumns].[UpdatedDate],[dbo].[tblLedgerAggregateColumns].[UpdatedBy],[dbo].[tblLedgerAggregateColumns].[CustomFunctionName],[dbo].[tblLedgerAggregateColumns].[LedgerAggregateColumnGuid],[dbo].[tblLedgerAggregateColumns].[SiteGuid],[dbo].[tblLedgerAggregateColumns].[LookupAggregateFieldIndex],sync.udf_GetMaxRowVersion(CT.InsertedRowVersion,MAPCT.InsertedRowVersion,NULL) AS '_RowVersion'
                FROM [dbo].[tblLedgerAggregateColumns]
                    INNER JOIN (SELECT [LedgerAggregateColumnToSiteGuid],[LedgerAggregateColumnGuid],[OwnerSiteGuid],[AssignedToSiteGuid] FROM [dbo].[udf_GetAssignedLedgerAggregateColumnListForSite](@sync_context_site_guid)) data
                        ON [dbo].[tblLedgerAggregateColumns].[LedgerAggregateColumnGuid] = data.[LedgerAggregateColumnGuid]
                ...
                WHERE ( [dbo].[tblLedgerAggregateColumns].[SiteGuid] = @sync_context_site_guid )
