...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblSiteCloseoutTime]
...
@sync_batch_size_tblSiteCloseoutTime int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblSiteCloseoutTime int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblSiteCloseoutTime IS NULL OR 
        (@sync_batch_size_tblSiteCloseoutTime IS NOT NULL AND @sync_batch_size_tblSiteCloseoutTime = 0))
    BEGIN
        SET @sync_batch_size_tblSiteCloseoutTime = 2147483647;
    END


    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
    SELECT TOP(@sync_batch_size_tblSiteCloseoutTime) WITH TIES [dbo].[tblSiteCloseoutTime].[SiteCloseoutTimeGuid],[dbo].[tblSiteCloseoutTime].[EffectiveDate],[dbo].[tblSiteCloseoutTime].[ExpirationDate],[dbo].[tblSiteCloseoutTime].[CloseoutTime],[dbo].[tblSiteCloseoutTime].[PointTagRefDataAsXML],[dbo].[tblSiteCloseoutTime].[CreatedDate],[dbo].[tblSiteCloseoutTime].[CreatedBy],[dbo].[tblSiteCloseoutTime].[UpdatedDate],[dbo].[tblSiteCloseoutTime].[UpdatedBy],[dbo].[tblSiteCloseoutTime].[SiteGuid],CT.InsertedRowVersion AS '_RowVersion'
        FROM [dbo].[tblSiteCloseoutTime]
            INNER JOIN [track].[tblSiteCloseoutTime] CT
                ON CT.PK_SiteCloseoutTimeGuid = [dbo].[tblSiteCloseoutTime].[SiteCloseoutTimeGuid] 
        WHERE ( dbo.tblSiteCloseoutTime.[SiteGuid] = @sync_context_site_guid )
