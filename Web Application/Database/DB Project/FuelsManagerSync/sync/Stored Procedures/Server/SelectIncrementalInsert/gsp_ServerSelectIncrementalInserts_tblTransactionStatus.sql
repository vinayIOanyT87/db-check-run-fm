...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblTransactionStatus]
...
@sync_batch_size_tblTransactionStatus int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblTransactionStatus int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblTransactionStatus IS NULL OR 
        (@sync_batch_size_tblTransactionStatus IS NOT NULL AND @sync_batch_size_tblTransactionStatus = 0))
    BEGIN
        SET @sync_batch_size_tblTransactionStatus = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
    SELECT TOP(@sync_batch_size_tblTransactionStatus) WITH TIES [lookup].[tblTransactionStatus].[TransactionStatusIndex],[lookup].[tblTransactionStatus].[TransactionStatusCode],[lookup].[tblTransactionStatus].[TransactionStatusName],[lookup].[tblTransactionStatus].[TransactionStatusGuid],[lookup].[tblTransactionStatus].[CreatedDate],[lookup].[tblTransactionStatus].[CreatedBy],[lookup].[tblTransactionStatus].[UpdatedDate],[lookup].[tblTransactionStatus].[UpdatedBy],CT.InsertedRowVersion AS '_RowVersion'
        FROM [lookup].[tblTransactionStatus]
