...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblApplicationStringToEmailAddress]
...
@sync_batch_size_tblApplicationStringToEmailAddress int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblApplicationStringToEmailAddress int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblApplicationStringToEmailAddress IS NULL OR 
        (@sync_batch_size_tblApplicationStringToEmailAddress IS NOT NULL AND @sync_batch_size_tblApplicationStringToEmailAddress = 0))
    BEGIN
        SET @sync_batch_size_tblApplicationStringToEmailAddress = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
        SELECT TOP(@sync_batch_size_tblApplicationStringToEmailAddress) WITH TIES [ApplicationStringToEmailAddressGuid],[ApplicationStringGuid],[EmailGroupGuid],[Sequence],[CreatedDate],[CreatedBy],[UpdatedDate],[UpdatedBy],_RowVersion
        FROM (
            SELECT TOP(@sync_batch_size_tblApplicationStringToEmailAddress) WITH TIES [map].[tblApplicationStringToEmailAddress].[ApplicationStringToEmailAddressGuid],[map].[tblApplicationStringToEmailAddress].[ApplicationStringGuid],[map].[tblApplicationStringToEmailAddress].[EmailGroupGuid],[map].[tblApplicationStringToEmailAddress].[Sequence],[map].[tblApplicationStringToEmailAddress].[CreatedDate],[map].[tblApplicationStringToEmailAddress].[CreatedBy],[map].[tblApplicationStringToEmailAddress].[UpdatedDate],[map].[tblApplicationStringToEmailAddress].[UpdatedBy],sync.udf_GetMaxRowVersion(CT.InsertedRowVersion,MAPCT.InsertedRowVersion,MAPCT2.InsertedRowVersion) AS '_RowVersion'
                FROM [map].[tblApplicationStringToEmailAddress]
                    INNER JOIN (SELECT [EmailAddressToSiteGuid],[ApplicationStringGuid],[OwnerSiteGuid],[AssignedToSiteGuid] FROM [dbo].[udf_GetAssignedApplicationStringEmailAddressListForSite](@sync_context_site_guid)) data
                        ON [map].[tblApplicationStringToEmailAddress].[ApplicationStringGuid] = data.[ApplicationStringGuid]
                    INNER JOIN (SELECT [EmailGroupToSiteGuid],[EmailGroupGuid],[OwnerSiteGuid],[AssignedToSiteGuid] FROM [dbo].[udf_GetAssignedEmailGroupListForSite](@sync_context_site_guid)) data1
                        ON [map].[tblApplicationStringToEmailAddress].[EmailGroupGuid] = data1.[EmailGroupGuid]
                    ...
                    INNER JOIN [track].[tblEntityEmailAddressToSite] MAPCT
                        ON MAPCT.PK_EmailAddressToSiteGuid = data.[EmailAddressToSiteGuid]
