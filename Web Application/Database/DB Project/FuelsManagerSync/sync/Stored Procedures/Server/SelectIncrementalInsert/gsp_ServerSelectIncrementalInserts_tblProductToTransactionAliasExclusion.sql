...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblProductToTransactionAliasExclusion]
...
@sync_batch_size_tblProductToTransactionAliasExclusion int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblProductToTransactionAliasExclusion int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblProductToTransactionAliasExclusion IS NULL OR 
        (@sync_batch_size_tblProductToTransactionAliasExclusion IS NOT NULL AND @sync_batch_size_tblProductToTransactionAliasExclusion = 0))
    BEGIN
        SET @sync_batch_size_tblProductToTransactionAliasExclusion = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
        SELECT TOP(@sync_batch_size_tblProductToTransactionAliasExclusion) WITH TIES [ProductToTransactionAliasExclusionGuid],[ProductGuid],[AssignedToTransactionAliasGuid],[Sequence],[BlendPercentage],[AdditiveRate],[Ratio],[AdditiveCycleVolume],[Tolerance],[PresetNumber],[AdditiveProfileGuid],[TankGuid],[MeterID],[ShipToProductID],[ShipToProductCode],[ShipToLoadRackDisplayText],[UnavailableInventoryGross],[UnavailableInventoryNet],[CreatedDate],[CreatedBy],[UpdatedDate],[UpdatedBy],_RowVersion
        FROM (
            SELECT TOP(@sync_batch_size_tblProductToTransactionAliasExclusion) WITH TIES [map].[tblProductToTransactionAliasExclusion].[ProductToTransactionAliasExclusionGuid],[map].[tblProductToTransactionAliasExclusion].[ProductGuid],[map].[tblProductToTransactionAliasExclusion].[AssignedToTransactionAliasGuid],[map].[tblProductToTransactionAliasExclusion].[Sequence],[map].[tblProductToTransactionAliasExclusion].[BlendPercentage],[map].[tblProductToTransactionAliasExclusion].[AdditiveRate],[map].[tblProductToTransactionAliasExclusion].[Ratio],[map].[tblProductToTransactionAliasExclusion].[AdditiveCycleVolume],[map].[tblProductToTransactionAliasExclusion].[Tolerance],[map].[tblProductToTransactionAliasExclusion].[PresetNumber],[map].[tblProductToTransactionAliasExclusion].[AdditiveProfileGuid],[map].[tblProductToTransactionAliasExclusion].[TankGuid],[map].[tblProductToTransactionAliasExclusion].[MeterID],[map].[tblProductToTransactionAliasExclusion].[ShipToProductID],[map].[tblProductToTransactionAliasExclusion].[ShipToProductCode],[map].[tblProductToTransactionAliasExclusion].[ShipToLoadRackDisplayText],[map].[tblProductToTransactionAliasExclusion].[UnavailableInventoryGross],[map].[tblProductToTransactionAliasExclusion].[UnavailableInventoryNet],[map].[tblProductToTransactionAliasExclusion].[CreatedDate],[map].[tblProductToTransactionAliasExclusion].[CreatedBy],[map].[tblProductToTransactionAliasExclusion].[UpdatedDate],[map].[tblProductToTransactionAliasExclusion].[UpdatedBy],sync.udf_GetMaxRowVersion(CT.InsertedRowVersion,MAPCT.InsertedRowVersion,MAPCT2.InsertedRowVersion) AS '_RowVersion'
                FROM [map].[tblProductToTransactionAliasExclusion]
                    INNER JOIN (SELECT [ProductToSiteGuid],[ProductGuid],[OwnerSiteGuid],[AssignedToSiteGuid] FROM [dbo].[udf_GetAssignedProductListForSite](@sync_context_site_guid)) data
                        ON [map].[tblProductToTransactionAliasExclusion].[ProductGuid] = data.[ProductGuid]
                    INNER JOIN (SELECT [TransactionAliasToSiteGuid],[TransactionAliasGuid],[OwnerSiteGuid],[AssignedToSiteGuid] FROM [dbo].[udf_GetAssignedTransactionAliasListForSite](@sync_context_site_guid)) data1
                        ON [map].[tblProductToTransactionAliasExclusion].[AssignedToTransactionAliasGuid] = data1.[TransactionAliasGuid]
