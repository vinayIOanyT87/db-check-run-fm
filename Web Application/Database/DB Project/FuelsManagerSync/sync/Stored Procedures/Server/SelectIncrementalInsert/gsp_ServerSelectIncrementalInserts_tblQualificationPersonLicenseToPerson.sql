...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblQualificationPersonLicenseToPerson]
...
@sync_batch_size_tblQualificationPersonLicenseToPerson int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblQualificationPersonLicenseToPerson int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblQualificationPersonLicenseToPerson IS NULL OR 
        (@sync_batch_size_tblQualificationPersonLicenseToPerson IS NOT NULL AND @sync_batch_size_tblQualificationPersonLicenseToPerson = 0))
    BEGIN
        SET @sync_batch_size_tblQualificationPersonLicenseToPerson = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
        SELECT TOP(@sync_batch_size_tblQualificationPersonLicenseToPerson) WITH TIES [QualificationPersonLicenseToPersonGuid],[QualificationGuid],[PersonnelGuid],[Sequence],[Instructor],[DateCompleted],[DateDue],[ExpirationDate],[ID],[Rating],[HistoricalRecord],[CreatedDate],[CreatedBy],[UpdatedDate],[UpdatedBy],_RowVersion
        FROM (
            SELECT TOP(@sync_batch_size_tblQualificationPersonLicenseToPerson) WITH TIES [map].[tblQualificationPersonLicenseToPerson].[QualificationPersonLicenseToPersonGuid],[map].[tblQualificationPersonLicenseToPerson].[QualificationGuid],[map].[tblQualificationPersonLicenseToPerson].[PersonnelGuid],[map].[tblQualificationPersonLicenseToPerson].[Sequence],[map].[tblQualificationPersonLicenseToPerson].[Instructor],[map].[tblQualificationPersonLicenseToPerson].[DateCompleted],[map].[tblQualificationPersonLicenseToPerson].[DateDue],[map].[tblQualificationPersonLicenseToPerson].[ExpirationDate],[map].[tblQualificationPersonLicenseToPerson].[ID],[map].[tblQualificationPersonLicenseToPerson].[Rating],[map].[tblQualificationPersonLicenseToPerson].[HistoricalRecord],[map].[tblQualificationPersonLicenseToPerson].[CreatedDate],[map].[tblQualificationPersonLicenseToPerson].[CreatedBy],[map].[tblQualificationPersonLicenseToPerson].[UpdatedDate],[map].[tblQualificationPersonLicenseToPerson].[UpdatedBy],sync.udf_GetMaxRowVersion(CT.InsertedRowVersion,MAPCT.InsertedRowVersion,MAPCT2.InsertedRowVersion) AS '_RowVersion'
                FROM [map].[tblQualificationPersonLicenseToPerson]
                    INNER JOIN (SELECT [PersonnelLicenseToSiteGuid],[QualificationGuid],[OwnerSiteGuid],[AssignedToSiteGuid] FROM [dbo].[udf_GetAssignedQualificationPersonnelLicenseListForSite](@sync_context_site_guid)) data
                        ON [map].[tblQualificationPersonLicenseToPerson].[QualificationGuid] = data.[QualificationGuid]
                    INNER JOIN (SELECT [PersonnelToSiteGuid],[PersonnelGuid],[OwnerSiteGuid],[AssignedToSiteGuid] FROM [dbo].[udf_GetAssignedPersonnelListForSite](@sync_context_site_guid)) data1
                        ON [map].[tblQualificationPersonLicenseToPerson].[PersonnelGuid] = data1.[PersonnelGuid]
