...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblMajorCorrectionType]
...
@sync_batch_size_tblMajorCorrectionType int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblMajorCorrectionType int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblMajorCorrectionType IS NULL OR 
        (@sync_batch_size_tblMajorCorrectionType IS NOT NULL AND @sync_batch_size_tblMajorCorrectionType = 0))
    BEGIN
        SET @sync_batch_size_tblMajorCorrectionType = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
    SELECT TOP(@sync_batch_size_tblMajorCorrectionType) WITH TIES [lookup].[tblMajorCorrectionType].[MajorCorrectionTypeIndex],[lookup].[tblMajorCorrectionType].[MajorCorrectionTypeCode],[lookup].[tblMajorCorrectionType].[MajorCorrectionTypeName],[lookup].[tblMajorCorrectionType].[MajorCorrectionTypeGuid],[lookup].[tblMajorCorrectionType].[CreatedDate],[lookup].[tblMajorCorrectionType].[CreatedBy],[lookup].[tblMajorCorrectionType].[UpdatedDate],[lookup].[tblMajorCorrectionType].[UpdatedBy],CT.InsertedRowVersion AS '_RowVersion'
        FROM [lookup].[tblMajorCorrectionType]
