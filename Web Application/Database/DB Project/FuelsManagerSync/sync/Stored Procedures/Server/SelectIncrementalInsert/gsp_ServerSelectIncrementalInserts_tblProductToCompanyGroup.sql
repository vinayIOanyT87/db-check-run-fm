...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblProductToCompanyGroup]
...
@sync_batch_size_tblProductToCompanyGroup int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblProductToCompanyGroup int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblProductToCompanyGroup IS NULL OR 
        (@sync_batch_size_tblProductToCompanyGroup IS NOT NULL AND @sync_batch_size_tblProductToCompanyGroup = 0))
    BEGIN
        SET @sync_batch_size_tblProductToCompanyGroup = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
        SELECT TOP(@sync_batch_size_tblProductToCompanyGroup) WITH TIES [ProductToCompanyGroupGuid],[ProductGuid],[AssignedToApplicationStringGuid],[Sequence],[BlendPercentage],[AdditiveRate],[Ratio],[AdditiveCycleVolume],[Tolerance],[PresetNumber],[AdditiveProfileGuid],[TankGuid],[MeterID],[ShipToProductID],[ShipToProductCode],[ShipToLoadRackDisplayText],[UnavailableInventoryGross],[UnavailableInventoryNet],[CreatedDate],[CreatedBy],[UpdatedDate],[UpdatedBy],[SpecialInstructionNote],_RowVersion
        FROM (
            SELECT TOP(@sync_batch_size_tblProductToCompanyGroup) WITH TIES [map].[tblProductToCompanyGroup].[ProductToCompanyGroupGuid],[map].[tblProductToCompanyGroup].[ProductGuid],[map].[tblProductToCompanyGroup].[AssignedToApplicationStringGuid],[map].[tblProductToCompanyGroup].[Sequence],[map].[tblProductToCompanyGroup].[BlendPercentage],[map].[tblProductToCompanyGroup].[AdditiveRate],[map].[tblProductToCompanyGroup].[Ratio],[map].[tblProductToCompanyGroup].[AdditiveCycleVolume],[map].[tblProductToCompanyGroup].[Tolerance],[map].[tblProductToCompanyGroup].[PresetNumber],[map].[tblProductToCompanyGroup].[AdditiveProfileGuid],[map].[tblProductToCompanyGroup].[TankGuid],[map].[tblProductToCompanyGroup].[MeterID],[map].[tblProductToCompanyGroup].[ShipToProductID],[map].[tblProductToCompanyGroup].[ShipToProductCode],[map].[tblProductToCompanyGroup].[ShipToLoadRackDisplayText],[map].[tblProductToCompanyGroup].[UnavailableInventoryGross],[map].[tblProductToCompanyGroup].[UnavailableInventoryNet],[map].[tblProductToCompanyGroup].[CreatedDate],[map].[tblProductToCompanyGroup].[CreatedBy],[map].[tblProductToCompanyGroup].[UpdatedDate],[map].[tblProductToCompanyGroup].[UpdatedBy],[map].[tblProductToCompanyGroup].[SpecialInstructionNote],sync.udf_GetMaxRowVersion(CT.InsertedRowVersion,MAPCT.InsertedRowVersion,MAPCT2.InsertedRowVersion) AS '_RowVersion'
                FROM [map].[tblProductToCompanyGroup]
                    INNER JOIN (SELECT [ProductToSiteGuid],[ProductGuid],[OwnerSiteGuid],[AssignedToSiteGuid] FROM [dbo].[udf_GetAssignedProductListForSite](@sync_context_site_guid)) data
                        ON [map].[tblProductToCompanyGroup].[ProductGuid] = data.[ProductGuid]
                    INNER JOIN (SELECT [CompanyGroupToSiteGuid],[ApplicationStringGuid],[OwnerSiteGuid],[AssignedToSiteGuid] FROM [dbo].[udf_GetAssignedApplicationStringCompanyGroupListForSite](@sync_context_site_guid)) data1
                        ON [map].[tblProductToCompanyGroup].[AssignedToApplicationStringGuid] = data1.[ApplicationStringGuid]
