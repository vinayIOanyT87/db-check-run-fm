...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblManagerGroupToAutoDistributionRule]
...
@sync_batch_size_tblManagerGroupToAutoDistributionRule int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblManagerGroupToAutoDistributionRule int
AS
BEGIN
    ...
    IF ((@sync_request_type = 4 AND (@sync_first_time_sync_option_tblManagerGroupToAutoDistributionRule IS NOT NULL AND @sync_first_time_sync_option_tblManagerGroupToAutoDistributionRule = 1))
    ...
    IF (@sync_batch_size_tblManagerGroupToAutoDistributionRule IS NULL OR 
        (@sync_batch_size_tblManagerGroupToAutoDistributionRule IS NOT NULL AND @sync_batch_size_tblManagerGroupToAutoDistributionRule = 0))
    BEGIN
        SET @sync_batch_size_tblManagerGroupToAutoDistributionRule = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
        SELECT TOP(@sync_batch_size_tblManagerGroupToAutoDistributionRule) WITH TIES [ManagerGroupToAutoDistributionRuleGuid],[AutoDistributionRuleGuid],[ManagerGroupGuid],[CreatedDate],[CreatedBy],[UpdatedDate],[UpdatedBy],_RowVersion
        FROM (
            SELECT TOP(@sync_batch_size_tblManagerGroupToAutoDistributionRule) WITH TIES [map].[tblManagerGroupToAutoDistributionRule].[ManagerGroupToAutoDistributionRuleGuid],[map].[tblManagerGroupToAutoDistributionRule].[AutoDistributionRuleGuid],[map].[tblManagerGroupToAutoDistributionRule].[ManagerGroupGuid],[map].[tblManagerGroupToAutoDistributionRule].[CreatedDate],[map].[tblManagerGroupToAutoDistributionRule].[CreatedBy],[map].[tblManagerGroupToAutoDistributionRule].[UpdatedDate],[map].[tblManagerGroupToAutoDistributionRule].[UpdatedBy],sync.udf_GetMaxRowVersion(CT.InsertedRowVersion,MAPCT.InsertedRowVersion,MAPCT2.InsertedRowVersion) AS '_RowVersion'
                FROM [map].[tblManagerGroupToAutoDistributionRule]
                    INNER JOIN (SELECT [CompanyGroupToSiteGuid],[ApplicationStringGuid],[OwnerSiteGuid],[AssignedToSiteGuid] FROM [dbo].[udf_GetAssignedApplicationStringCompanyGroupListForSite](@sync_context_site_guid)) data
                        ON [map].[tblManagerGroupToAutoDistributionRule].[ManagerGroupGuid] = data.[ApplicationStringGuid]
                    INNER JOIN (SELECT [AutoDistributionRuleToSiteGuid],[AutoDistributionRuleGuid],[OwnerSiteGuid],[AssignedToSiteGuid] FROM [dbo].[udf_GetAssignedAutoDistributionRuleListForSite](@sync_context_site_guid)) data1
                        ON [map].[tblManagerGroupToAutoDistributionRule].[AutoDistributionRuleGuid] = data1.[AutoDistributionRuleGuid]
                    ...
                    INNER JOIN [track].[tblEntityCompanyGroupToSite] MAPCT
                        ON MAPCT.PK_CompanyGroupToSiteGuid = data.[CompanyGroupToSiteGuid]
