...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblGroups]
...
@sync_batch_size_tblGroups int,
@sync_bypass_insert_update_extraction bit,
@sync_request_type int,
@sync_first_time_sync_option_tblGroups int
AS
BEGIN
    ...
    IF (@sync_batch_size_tblGroups IS NULL OR 
        (@sync_batch_size_tblGroups IS NOT NULL AND @sync_batch_size_tblGroups = 0))
    BEGIN
        SET @sync_batch_size_tblGroups = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
        SELECT TOP(@sync_batch_size_tblGroups) WITH TIES [GroupID],[GroupDescription],[SessionTimeout],[CreatedDate],[CreatedBy],[UpdatedDate],[UpdatedBy],[GroupGuid],[SiteGuid],[ActiveDirectoryUserGroupGuid],_RowVersion
        FROM (
            SELECT TOP(@sync_batch_size_tblGroups) WITH TIES [dbo].[tblGroups].[GroupID],[dbo].[tblGroups].[GroupDescription],[dbo].[tblGroups].[SessionTimeout],[dbo].[tblGroups].[CreatedDate],[dbo].[tblGroups].[CreatedBy],[dbo].[tblGroups].[UpdatedDate],[dbo].[tblGroups].[UpdatedBy],[dbo].[tblGroups].[GroupGuid],[dbo].[tblGroups].[SiteGuid],[dbo].[tblGroups].[ActiveDirectoryUserGroupGuid],sync.udf_GetMaxRowVersion(CT.InsertedRowVersion,MAPCT.InsertedRowVersion,NULL) AS '_RowVersion'
                FROM [dbo].[tblGroups]
                    INNER JOIN (SELECT [UserGroupToSiteGuid],[GroupGuid],[OwnerSiteGuid],[AssignedToSiteGuid] FROM [dbo].[udf_GetAssignedGroupListForSite](@sync_context_site_guid)) data
                        ON [dbo].[tblGroups].[GroupGuid] = data.[GroupGuid]
