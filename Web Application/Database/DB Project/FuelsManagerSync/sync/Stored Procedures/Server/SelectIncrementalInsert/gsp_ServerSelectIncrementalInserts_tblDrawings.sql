...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblDrawings]
...
@sync_first_time_sync_option_tblDrawings int
AS
BEGIN
    ...
    IF ((@sync_request_type = 4 AND (@sync_first_time_sync_option_tblDrawings IS NOT NULL AND @sync_first_time_sync_option_tblDrawings = 1))
    ...
    FROM (
        ...
        SELECT [DrawingGuid],[ID],[Description],[Image],[SiteGuid],[PanelType],[PointTemplateGuid],[Published],[CreatedDate],[CreatedBy],[UpdatedDate],[UpdatedBy],_RowVersion
        FROM (
            SELECT TOP(@sync_batch_size_tblDrawings) WITH TIES [dbo].[tblDrawings].[DrawingGuid],[dbo].[tblDrawings].[ID],[dbo].[tblDrawings].[Description],[dbo].[tblDrawings].[Image],[dbo].[tblDrawings].[SiteGuid],[dbo].[tblDrawings].[PanelType],[dbo].[tblDrawings].[PointTemplateGuid],[dbo].[tblDrawings].[Published],[dbo].[tblDrawings].[CreatedDate],[dbo].[tblDrawings].[CreatedBy],[dbo].[tblDrawings].[UpdatedDate],[dbo].[tblDrawings].[UpdatedBy],CT.InsertedRowVersion AS '_RowVersion'
                FROM [dbo].[tblDrawings]
                ...
                WHERE ((CT.InsertedRowVersion > @sync_last_received_anchor_varbinary)
                        ...
                        AND (CT.InsertedContext IS NULL OR CT.InsertedContext <> @sync_client_id_binary))   -- USE THE CLIENT ID HERE SO WE DO NOT RETURN WHAT WAS JUST RECEIVED
                ORDER BY _RowVersion ASC
            ) rs2  -- DetectedSubFunctions OR IncludeEntityAssignments: False
        ) mainRs
        ORDER BY _RowVersion ASC
