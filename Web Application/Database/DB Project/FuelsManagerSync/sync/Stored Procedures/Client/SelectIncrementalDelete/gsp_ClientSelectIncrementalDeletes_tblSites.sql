...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ClientSelectIncrementalDeletes_tblSites]
...
BEGIN
    ...
    BEGIN 
        IF (@sync_batch_size_tblSites IS NULL OR 
            (@sync_batch_size_tblSites IS NOT NULL AND @sync_batch_size_tblSites = 0))
        BEGIN
            SET @sync_batch_size_tblSites = 2147483647;
        END

        SELECT TOP(@sync_batch_size_tblSites) WITH TIES CT.PK_SiteGuid 'SiteGuid', CT.DeletedRowVersion '_RowVersion'
            FROM [track].[tblSites] CT
    ...
    ELSE
    BEGIN
        SELECT CT.PK_SiteGuid 'SiteGuid', CT.DeletedRowVersion '_RowVersion'
            FROM [track].[tblSites] CT
            WHERE 1=2
    END
