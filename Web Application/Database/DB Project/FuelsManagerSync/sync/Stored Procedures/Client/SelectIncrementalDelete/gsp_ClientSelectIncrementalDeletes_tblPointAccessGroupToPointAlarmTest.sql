...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ClientSelectIncrementalDeletes_tblPointAccessGroupToPointAlarmTest]
...
BEGIN
    ...
    BEGIN 
		...
		-- Deletions associated with Points
			...
			BEGIN
			  ...
			  SELECT CT.PK_PointAccessGroupToPointAlarmTestGuid 'PointAccessGroupToPointAlarmTestGuid', CT.[FK_ParentPK] 'AlarmTestGuid', CT.DeletedRowVersion '_RowVersion'
					FROM [track].[tblPointAccessGroupToPointAlarmTest] CT
		...
		ELSE
		BEGIN
			  SELECT TOP(@sync_batch_size_tblPointAccessGroupToPointAlarmTest) WITH TIES CT.PK_PointAccessGroupToPointAlarmTestGuid 'PointAccessGroupToPointAlarmTestGuid', CT.[FK_ParentPK] 'AlarmTestGuid', CT.DeletedRowVersion '_RowVersion'
					FROM [track].[tblPointAccessGroupToPointAlarmTest] CT
					INNER JOIN [track].tblAlarmTest AT ON AT.PK_AlarmTestGuid = CT.FK_ParentPK
					INNER JOIN [track].tblAlarm A ON A.PK_AlarmGuid = AT.FK_ParentPK
    ...
    ELSE
    BEGIN
        SELECT CT.PK_PointAccessGroupToPointAlarmTestGuid 'PointAccessGroupToPointAlarmTestGuid', CT.DeletedRowVersion '_RowVersion'
            FROM [track].[tblPointAccessGroupToPointAlarmTest] CT
            WHERE 1=2
    END
