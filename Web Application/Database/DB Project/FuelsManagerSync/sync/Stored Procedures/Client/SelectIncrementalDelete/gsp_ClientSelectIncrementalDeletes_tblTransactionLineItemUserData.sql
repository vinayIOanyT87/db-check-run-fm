...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ClientSelectIncrementalDeletes_tblTransactionLineItemUserData]
...
BEGIN
    ...
    BEGIN 
        ...
        SELECT TOP(@sync_batch_size_tblTransactionLineItemUserData) WITH TIES CT.PK_TransactionLineItemUserDataGuid 'TransactionLineItemUserDataGuid', CT.[FK_ParentPK] 'TransactionLineItemGuid', CT.DeletedRowVersion '_RowVersion'
            FROM [track].[tblTransactionLineItemUserData] CT
                INNER JOIN [track].[tblTransactionLineItems]
                    ON [track].[tblTransactionLineItems].[PK_TransactionLineItemGuid] = CT.[FK_ParentPK]
                INNER JOIN #SyncTable 
                    ON #SyncTable.PK = [track].[tblTransactionLineItems].[FK_ParentPK]
    ...
    ELSE
    BEGIN
        SELECT CT.PK_TransactionLineItemUserDataGuid 'TransactionLineItemUserDataGuid', CT.DeletedRowVersion '_RowVersion'
            FROM [track].[tblTransactionLineItemUserData] CT
            WHERE 1=2
    END
