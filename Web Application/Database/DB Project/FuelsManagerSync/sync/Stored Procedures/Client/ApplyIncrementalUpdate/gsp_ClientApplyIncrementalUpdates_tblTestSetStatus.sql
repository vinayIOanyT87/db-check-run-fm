...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ClientApplyIncrementalUpdates_tblTestSetStatus]
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM [track].[tblTestSetStatus] CT
    ...
    BEGIN
        ;   WITH existingData AS (
                SELECT [lookup].[tblTestSetStatus].[TestSetStatusIndex],[lookup].[tblTestSetStatus].[TestSetStatusCode],[lookup].[tblTestSetStatus].[TestSetStatusName],[lookup].[tblTestSetStatus].[TestSetStatusGuid],[lookup].[tblTestSetStatus].[CreatedDate],[lookup].[tblTestSetStatus].[CreatedBy],[lookup].[tblTestSetStatus].[UpdatedDate],[lookup].[tblTestSetStatus].[UpdatedBy]
            ...
            USING (SELECT @TestSetStatusIndex,@TestSetStatusCode,@TestSetStatusName,@TestSetStatusGuid,@CreatedDate,@CreatedBy,@UpdatedDate,@UpdatedBy
            ...
            WHEN MATCHED AND (@sync_force_write = 1 
                ...
                THEN
                UPDATE SET [TestSetStatusCode] = remoteChanges.[TestSetStatusCode]
                       ,[TestSetStatusName] = remoteChanges.[TestSetStatusName]
                       ,[TestSetStatusGuid] = remoteChanges.[TestSetStatusGuid]
            ...
            WHEN NOT MATCHED THEN
                INSERT ([TestSetStatusIndex],[TestSetStatusCode],[TestSetStatusName],[TestSetStatusGuid],[CreatedDate],[CreatedBy],[UpdatedDate],[UpdatedBy])
                    VALUES (@TestSetStatusIndex,@TestSetStatusCode,@TestSetStatusName,@TestSetStatusGuid,@CreatedDate,@CreatedBy,@UpdatedDate,@UpdatedBy)
            ;
