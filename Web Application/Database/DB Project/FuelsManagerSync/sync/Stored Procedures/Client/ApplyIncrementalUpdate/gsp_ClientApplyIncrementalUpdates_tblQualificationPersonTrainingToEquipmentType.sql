...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ClientApplyIncrementalUpdates_tblQualificationPersonTrainingToEquipmentType]
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM [track].[tblQualificationPersonTrainingToEquipmentType] CT
    ...
    BEGIN
        ;   WITH existingData AS (
            ...
            USING (SELECT @QualificationPersonTrainingToEquipmentTypeGuid,@QualificationGuid,@EquipmentTypeGuid,@Sequence,@Instructor,@DateCompleted,@DateDue,@ExpirationDate,@ID,@Rating,@HistoricalRecord,@CreatedDate,@CreatedBy,@UpdatedDate,@UpdatedBy,@SiteGuid
            ...
            WHEN NOT MATCHED THEN
                INSERT ([QualificationPersonTrainingToEquipmentTypeGuid],[QualificationGuid],[EquipmentTypeGuid],[Sequence],[Instructor],[DateCompleted],[DateDue],[ExpirationDate],[ID],[Rating],[HistoricalRecord],[CreatedDate],[CreatedBy],[UpdatedDate],[UpdatedBy],[SiteGuid])
                    VALUES (@QualificationPersonTrainingToEquipmentTypeGuid,@QualificationGuid,@EquipmentTypeGuid,@Sequence,@Instructor,@DateCompleted,@DateDue,@ExpirationDate,@ID,@Rating,@HistoricalRecord,@CreatedDate,@CreatedBy,@UpdatedDate,@UpdatedBy,@SiteGuid)
            ;
