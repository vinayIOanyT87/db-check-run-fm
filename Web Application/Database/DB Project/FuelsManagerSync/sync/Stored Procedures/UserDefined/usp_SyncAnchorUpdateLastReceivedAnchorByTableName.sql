...
/* {CheckPoint: CREATING STORED PROCEDURE: [sync].[usp_SyncAnchorUpdateLastReceivedAnchorByTableName]] } */
CREATE PROCEDURE [sync].[usp_SyncAnchorUpdateLastReceivedAnchorByTableName] (
...
BEGIN
	...
	USING (SELECT sa.SiteID, sa.TableName, sa.LastReceivedAnchor
			...
			) AS source (SiteID, TableName, LastReceivedAnchor)
	ON (target.SiteID = source.SiteID
		AND target.TableName = source.TableName)
	WHEN MATCHED 
		THEN UPDATE SET LastReceivedAnchor = @sync_anchor;
END
