...
/* {CheckPoint: CREATING STORED PROCEDURE: [sync].[usp_SyncAnchorUpdateLastReceivedAnchor2ByTableName]] } */
CREATE PROCEDURE [sync].[usp_SyncAnchorUpdateLastReceivedAnchor2ByTableName] (
...
BEGIN
	...
	-- Note, Don't try to combine this stored procedure with usp_SyncAnchorUpdateLastSentAnchorByTableName.  
	...
	USING (SELECT sa.SiteID, sa.TableName, sa.LastReceivedAnchor2
			...
			) AS source (SiteID, TableName, LastReceivedAnchor2)
	ON (target.SiteID = source.SiteID
		AND target.TableName = source.TableName)
	WHEN MATCHED 
		THEN UPDATE SET LastReceivedAnchor2 = @sync_anchor;
END
