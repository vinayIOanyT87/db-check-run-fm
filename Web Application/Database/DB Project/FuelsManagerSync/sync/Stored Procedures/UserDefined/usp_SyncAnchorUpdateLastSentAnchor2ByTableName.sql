...
/* {CheckPoint: CREATING STORED PROCEDURE: [sync].[usp_SyncAnchorUpdateLastSentAnchor2ByTableName]] } */
CREATE PROCEDURE [sync].[usp_SyncAnchorUpdateLastSentAnchor2ByTableName] (
...
BEGIN
	...
	USING (SELECT sa.SiteID, sa.TableName, sa.LastSentAnchor2 
			...
			) AS source (SiteID, TableName, LastSentAnchor2)
	ON (target.SiteID = source.SiteID 
		AND target.TableName = source.TableName)
	WHEN MATCHED 
		THEN UPDATE SET LastSentAnchor2 = @sync_anchor;
END
