...
/* {CheckPoint: CREATING STORED PROCEDURE: [sync].[usp_SyncAnchorSelectLastReceivedAnchor2ByTableName] } */
CREATE PROCEDURE [sync].[usp_SyncAnchorSelectLastReceivedAnchor2ByTableName] (
...
BEGIN
	...
	SELECT @sync_table_received_anchor = sa.LastReceivedAnchor2
		FROM [sync].[tblSyncAnchor] sa WITH (NOLOCK)
			WHERE sa.TableName = @sync_current_table
					AND (@sync_context_site_id IS NOT NULL AND sa.SiteID = @sync_context_site_id)

	IF (@sync_table_received_anchor IS NULL)
		SET @sync_table_received_anchor = 1
END
