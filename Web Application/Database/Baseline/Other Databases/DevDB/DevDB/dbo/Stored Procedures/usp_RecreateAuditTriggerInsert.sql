...
CREATE PROCEDURE [dbo].[usp_RecreateAuditTriggerInsert]
AS
BEGIN
	...
	BEGIN
		...
		PRINT 'CREATE TRIGGER ['+@Schema+'].['+@Trigger+'] ON ['+@Schema+'].['+@Table+'] AFTER INSERT '
		PRINT 'AS'
		PRINT 'BEGIN'
		...
		PRINT '	SET @_AuditEventType= ''I'' -- For Inserts '
