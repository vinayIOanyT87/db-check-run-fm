/*
	DROP PROCEDURE [Staging].[usp_LoadTransactionLineItemUserData]
 
	EXEC [staging].[usp_LoadTransactionLineItemUserData]
 
*/
CREATE PROCEDURE [staging].[usp_LoadTransactionLineItemUserData]
AS
BEGIN
------------------------------------------------------------------------------------------------------
-- Stored procedure: [staging].[usp_LoadTransactionLineItemUserData]
-- Author: Hansraj Bapoo
-- Version/Date: 1.0.003 / 2012-07-13 14:21:10.4470770 -04:00
-- Purpose: Loads all the TransactionLineItemUserData records from staging into the tblTransactionLineItemUserData table in the Archive database.
-- Notes:
-- 1. This operation assumes that no referential integrity or dependencies are enforced on the tblTransactionLineItemUserData table.
------------------------------------------------------------------------------------------------------
	SET NOCOUNT ON;
	BEGIN TRY
 
		IF (
			(	SELECT COUNT(*) FROM staging.tblTransactionLineItemUserData
				WHERE IsProcessed = 0
			) = 0)
		BEGIN
			RETURN
	...
	(
		...
		SELECT src.[TransactionLineItemUserDataGuid], src.[SourceClusterIdx], src.[TransactionGuid]
		FROM staging.tblTransactionLineItemUserData src
		WHERE src.TransactionLineItemUserDataGuid IS NOT NULL
		AND src.IsProcessed = 0
		AND src.IgnoreRecord = 0
		AND EXISTS
		(
			SELECT * FROM [dbo].[tblTransactionLineItemUserData] tgt
			WHERE tgt.TransactionLineItemUserDataGuid = src.TransactionLineItemUserDataGuid
		)
		...
		, src.[UpdatedBy]
		, src.[UpdatedDate]
		, src.[TransactionLineItemUserDataGuid]
		...
		ON b.RecordGuid = src.TransactionLineItemUserDataGuid
		WHERE src.TransactionLineItemUserDataGuid IS NOT NULL
		AND src.IgnoreRecord = 0
		AND src.IsProcessed = 0
		...
		, tgt.[TransactionLineItemUserDataGuid] = src.[TransactionLineItemUserDataGuid]
		...
		FROM [dbo].[tblTransactionLineItemUserData] tgt
		INNER JOIN staging.tblTransactionLineItemUserData src
		ON src.TransactionLineItemUserDataGuid = tgt.TransactionLineItemUserDataGuid
		...
		UPDATE staging.tblTransactionLineItemUserData SET IsProcessed = 1
 
		INSERT INTO staging.tblInsertedRecords
		(TargetTableName, RecordGuid, RecordIndex, ParentRecordGuid)
		SELECT '[dbo].[tblTransactionLineItemUserData]', RecordGuid, RecordIndex, ParentRecordGuid
	...
	BEGIN CATCH
		...
		+ 'Procedure Name: [staging].[usp_LoadTransactionLineItemUserData]' + CHAR(13) + CHAR(10)
