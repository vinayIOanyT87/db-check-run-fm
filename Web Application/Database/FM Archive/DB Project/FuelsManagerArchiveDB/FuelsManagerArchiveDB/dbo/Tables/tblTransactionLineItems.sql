/*
	...
	DROP TABLE [dbo].[tblTransactionLineItems]
...
CREATE TABLE [dbo].[tblTransactionLineItems] (
    ...
    CONSTRAINT [PK_tblTransactionLineItems_GUID] PRIMARY KEY NONCLUSTERED ([InventoryDateKey] ASC, [TransactionLineItemGuid] ASC) ON [AnnualPS]([InventoryDateKey])
...
CREATE UNIQUE CLUSTERED INDEX [IX_tblTransactionLineItems_ClusterIdx] 
	ON [dbo].[tblTransactionLineItems]([InventoryDateKey], [_ClusterIdx])
	ON [AnnualPS]([InventoryDateKey]);
GO
...
CREATE UNIQUE NONCLUSTERED INDEX [IXU_tblTransactionLineItems_CoveringAssociatedTransactionQueries]
    ON [dbo].[tblTransactionLineItems]([InventoryDateKey], [TransactionLineItemGuid] ASC)
    INCLUDE([TransactionGuid], [GrossQuantity], [Product], [ProductPrice], [Tax1], [Tax2], [Tax3], [LookupTransactionStatusIndex], [LookupQualityIndex])
	ON [AnnualPS]([InventoryDateKey]);
GO
