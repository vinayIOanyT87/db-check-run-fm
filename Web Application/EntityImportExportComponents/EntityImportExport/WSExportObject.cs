...
namespace EntityImportExport
{
    public class WSExportObject : WorksheetBaseExport
    {
           ...
           EntityImportExportAttribute[] importExportAttributes)
        {
            ...
            bool rootAttributeFound = false;
            foreach (EntityImportExportAttribute attribute in importExportAttributes)
            {
                if (attribute.ColumnName == rootName)
                {
                    this.RootAttributeList.Add(attribute);
                    rootAttributeFound = true;
                    break;
                }
            }

            if (!rootAttributeFound)
            {
                throw new Exception("Root Attribute " + rootName + " Not Found in Import Export Attributes");
            }
            ...
            foreach (EntityImportExportAttribute attribute in importExportAttributes)
            {
                rootAttributeFound = false;

                // Skip the rootAttributes for this worksheet as it was already added
                foreach (var rootattribute in this.RootAttributeList)
                {
                    if (attribute.ColumnName == rootattribute.ColumnName)
                    {
                        rootAttributeFound = true;
                        break;
                    }
   ...
   EntityImportExportAttribute[] importExportAttributes)
        ...
        public void CreateRowData(
           object o, bool includeStrapTables)
        {
            XmlNode headerRow;
            XmlNode cellNode;
            XmlNode dataNode;
            ...
            member = memberHashTable[this.RootAttributeList[this.RootAttributeList.Count - 1].MemberName.ToUpper()] as MemberInfo;
            if (member == null)
                throw new Exception("Object Member not found - " + this.RootAttributeList[this.RootAttributeList.Count - 1].MemberName.ToUpper());
            ...
            this.RootAttributeList[this.RootAttributeList.Count - 1].Value = value as string;
            ...
            foreach (var rootAttribute in this.RootAttributeList)
            {
                ...
                dataNode.InnerText = rootAttribute.Value;
                cellNode.AppendChild(dataNode);
                headerRow.AppendChild(cellNode);

               if (includeStrapTables == false && rootAttribute.Value.ToUpper() == "STRAP TABLE") return;
            }

            // Addd Remaining Attributes
            foreach (EntityImportExportAttribute attribute in importExportAttributes)
            {
                bool rootAttributeFound = false;
