...
STDMETHODIMP OPCGroupBase::UpdateClients()
{
   ...
   HRESULT hr = S_OK;
	...
	if(g_pDeviceManager->m_bTagUpdatePending)
		return hr;

	g_pDeviceManager->BeginOPCClientUpdates();
	hr=DoUpdateClients();
	g_pDeviceManager->EndOPCClientUpdates();
	return hr;
}

STDMETHODIMP OPCGroupBase::DoUpdateClients()
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState())
   HRESULT hr = S_OK;
   if( !m_dataWaiting )
