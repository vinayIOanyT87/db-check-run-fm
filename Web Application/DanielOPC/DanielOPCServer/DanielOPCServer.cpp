// DanielOPCServer.cpp : Implementation of DLL Exports.
//
...
class CDanielOPCServerModule : public CAtlDllModuleT< CDanielOPCServerModule >
{
public :
	DECLARE_LIBID(LIBID_DanielOPCServerLib)
	DECLARE_REGISTRY_APPID_RESOURCEID(IDR_DANIELOPCSERVER, "{0E52D2D9-E9BB-4C85-89F5-0DC51713A62D}")
};

CDanielOPCServerModule _AtlModule;

#define SERVICE_NAME _T("DanielOPC Server")
...
BEGIN_MESSAGE_MAP(CDanielOPCServerApp, CWinApp)
END_MESSAGE_MAP()

CDanielOPCServerApp theApp;

// Logging functions
void CDanielOPCServerApp::WriteToEventLog(	WORD wEventType,
											DWORD dwEventID,
											USHORT nNumStrings,
											LPCTSTR* pszStrings,
											ULONG nDataSize,
...
void CDanielOPCServerApp::WriteTextToEventLog(WORD wEventType, LPCTSTR pszMsg) const
{
...
void CDanielOPCServerApp::LogInfo(LPCTSTR pszFormat, ...) const
{
	TCHAR		chMsg[1024];
	va_list	pArg;
...
void CDanielOPCServerApp::LogError(LPCTSTR pszFormat, ...) const
{
	TCHAR		chMsg[1024];
	va_list	pArg;
...
void CDanielOPCServerApp::LogWarning(LPCTSTR pszFormat, ...) const
{
	TCHAR		chMsg[1024];
	va_list	pArg;
...
BOOL CDanielOPCServerApp::InitInstance()
{
...
int CDanielOPCServerApp::ExitInstance()
{
	 if(g_pDeviceManager)
...
void	CDanielOPCServerApp::GetProviderString()
{
...
void	CDanielOPCServerApp::GetConnectionString()
{
	// ODBC
	// TCHAR szString[512]={_T("Provider=MSDASQL;DSN=DanielOPC;")};

	// Jet OLEDB - Note: does not support transactions
	// TCHAR szString[512]={_T("Provider=Microsoft.Jet.OLEDB.4.0; Data Source=c:\\Program Files\\DanielOPC\\DanielOPC\\DanielOPC.mdb;")};

	// SQL Server OLEDB
	TCHAR szString[512]={_T("provider=SQLOLEDB;Data Source=127.0.0.1;Initial Catalog=DanielOPC;Integrated Security=SSPI;")};

	CRegKey RegKey;

	if(ERROR_SUCCESS == RegKey.Create(HKEY_LOCAL_MACHINE,_T("SOFTWARE\\Varec\\DanielOPC")))
	{
