/******************************************************************************
	...
	PURPOSE:			Implementation of the OPCGroup
    ...
    HRESULT				* pErrors)
{
   ...
   for( DWORD index=0; index < dwNumItems; index++ )
   {
      if( pErrors[index] == S_OK )
      {
         ...
         CWeightScaleItem* pItem = (CWeightScaleItem*) ppItems[index];
         if(!pItem)
				continue;
...
BOOL COPCGroup::DoUpdateGroup()
{
   ...
   while( pos )
   {
      m_itemMap.GetNextAssoc( pos, key, (COPCItem*&)pItem );
		...
		if(pIO != pItem->m_pTag->m_pIO)
		{
			if(pIO)
				LeaveCriticalSection(&pIO->m_cs);

			pIO=pItem->m_pTag->m_pIO;
			if(pIO)
				EnterCriticalSection(&pIO->m_cs);
		}
    ...
    HRESULT        * pErrors)
{
   ...
   for(DWORD i=0; i < dwNumItems; i++ )
   {
      ...
      CString itemName( pItemArray[i].szItemID );
		CTag*	pTag=g_pDeviceManager->FindTag( itemName.GetBuffer(0) );
      if( pTag == NULL
		|| !pTag->m_bLeaf )
      {
      ...
      {
         // Create the OPC item
         CWeightScaleItem* pItem = new CWeightScaleItem(pTag,itemName.GetBuffer(0));
         if( pItem )
         {
            pErrors[i] = S_OK;
