namespace FMBusinessObjects.PIDXTransactions
{
	...
	public class AuthorizationGrantedCO : AuthorizationGrantedBase
	{
		...
		#region Properties
		public ArrayList ProductGroupList => this.productGroupList;
		...
		public Hashtable ProductLiftAmountList => this.productLiftAmountList;
	    ...
	    protected override void ParseProductAuthorizations(string productAuthorizations)
	    {
		...
		private void ParseProductGroups ( string response )
		{
			...
			int prodGrpEnd = response.IndexOf ( "R?", StringComparison.Ordinal ) - 1;
			...
			for (int nextProd = 0; nextProd < 12; nextProd++)
			{
				...
				if (( productID.Length > 0 ) && ( liftAmount.Length > 0 ))
				{
					if (this.productLiftAmountList.Contains ( productID ) == false)
					{
						try
						{
							double prodLiftAmount = Convert.ToDouble ( liftAmount );
