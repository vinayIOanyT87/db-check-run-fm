...
function checkForCrossFrameScriptingMenuBar() {
	if (window.cfsCheckedMenuBar) {
		return;
	}
	var cfsDetectedMenuBar = true;
	...
	try {
		...
		if (topLocation === selfLocation) {
			...
			if (p1.length > 1 && p2.length > 1 && p1[1] === p2[1]) {
				window.cfsCheckedMenuBar = true;
				cfsDetectedMenuBar = false;
			}
		}
	}
	catch (e) {; }

	if (cfsDetectedMenuBar) {
		try {
...
window.addEventListener("load", function (e) {
	checkForCrossFrameScriptingMenuBar();
});
