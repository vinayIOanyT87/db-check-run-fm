...
PolygonDrawingTool.prototype.finishShape = function () {
  ...
  if (shape !== null && this.archetypePartData !== null) {
    ...
    if (tempgeo.figures.first().segments.count >= (this.isPolygon ? 2 : 1)) {
      ...
      if (this.isPolygon) {
          diagram.model.setDataProperty(d, "category", "polygon");
      }
      else
      {
          diagram.model.setDataProperty(d, "category", "polyline");
      }
      ...
      if ( geo.figures.first().segments.count === 1 )
      {
      	part.selectionAdorned = false;
      	part.rotatable = false;
      	part.resizable = false;
	      //diagram.model.setDataProperty(d, "resizable", "false");
	      //diagram.model.setDataProperty(d, "rotatable", "false");
      }
      this.transactionResult = this.name;
      diagram.select(part);

      if (SetColorObject === true && d.color !== null && typeof (d.color) === 'object') {
          
          var patternNumber = parseInt(d.patternImageName);
          var patternTagId = 'canvasPalatte-propertiesMenu-FILLPATTERNPALETTE-' + d.patternImageName;
