/** IMPORTANT: This add-in has been customized for FuelsManager.*/
...
; (function ($, window) {
    ...
    (function (p) {
        ...
        p.setElementReferences = function () {
            ...
            stc.$downScrollArrow = $tabsContainer.find('.scrtabs-js-tab-scroll-arrow-down');
            stc.$scrollArrows = stc.$downScrollArrow.add( stc.$leftScrollArrow.add(stc.$rightScrollArrow) );
        ...
        p.setElementWidths = function () {
            ...
            stc.scrollArrowsCombinedWidth = stc.$leftScrollArrow.outerWidth() + stc.$rightScrollArrow.outerWidth() + stc.$downScrollArrow.outerWidth();
        ...
        p.setEventListeners = function () {
            ...
            stc.$downScrollArrow.off('.scrtabs').on({
                'click.scrtabs': function (e) { evh.handleClickOnDownScrollArrow.call(evh, e); }
            });
        ...
        p.setFixedContainerWidth = function () {
            var ehd = this,
                stc = ehd.stc;
            // changed to getBoundingClientRect to avoid rounding issues
            stc.$fixedContainer.width(stc.fixedContainerWidth = stc.$tabsContainer[0].getBoundingClientRect().width);
        };
        ...
        p.setFixedContainerWidthForJustHiddenScrollArrows = function () {
            ...
            stc.$fixedContainer.width(stc.fixedContainerWidth / 1);
        };
    ...
    (function (p) {
        ...
        p.handleClickOnDownScrollArrow = function (e, obj) {
            var evh = this;
            var stc = evh.stc;
            var tabList = $(e.target).closest('.scrtabs-tab-container').find('.nav-tabs li');
            // get width of the scrollable tab container
            var tabContainerWidth = $(e.target).closest('.scrtabs-tab-container').find('.scrtabs-tabs-fixed-container').width();
            var tabContainerLeft = $(e.target).closest('.scrtabs-tab-container').find('.scrtabs-tabs-fixed-container').offset().left;

            //look for the tabs that are hidden
            var hiddenTabs = [];
            tabList.each(function (index, tab) {
                // if hidden on the left or the right in the scrollable container
                if ($(tab).offset().left + ( $(tab).width() / 2 ) < tabContainerLeft ||
                    $(tab).offset().left >= tabContainerWidth) { 

                    var selectedTab = $(tab).find('a'); // the tab is actually the 'a' tag
                    if (!selectedTab.hasClass('tab-add-group')) {
                        hiddenTabs.push({
                            ...
                            action: function() {
                                ...
                                $(selectedTab).click();
//                                window.setTimeout(function() { stc.scrollMovement.scrollToActiveTab({ isOnWindowResize: true, isOnTabsRefresh: false }); }, 500);
                            }
                        });
                    }
                }
            ...
            //use the contextmenu plugin to display the hidden tabs
            $(e.target).contextPopup({
                items: hiddenTabs
            });

            // trigger the right button event since it's the on used by default so the list appears
            var event = new jQuery.Event("contextmenu");
            event.pageX = e.pageX;
            event.pageY = e.pageY;
            $(e.target).trigger(event);

            // disable the right click event that gets bind it when creating the menu
            $(e.target).unbind("contextmenu");

            // change menu size and make it closer to the design
            $("#fmContextMenu").css({ left: e.pageX - 90, top: e.pageY + 20, "max-height": $(window).height() - e.pageY - 100, 'overflow-x': 'hidden', 'overflow-y': 'auto' });
            $("#fmContextMenu .gutterLine").addClass('hidden');
            $("#fmContextMenu a").css({ 'padding-left': '10px' });
        };
    ...
    (function (p) {
        ...
        p.scrollToActiveTab = function (options) {
            ...
            rightArrowLeftPos = stc.$downScrollArrow.offset().left;
            ...
            //FJM: check if we need to scroll to the right instead
            if (activeTabLeftPos < stc.$fixedContainer.offset().left) {
                stc.movableContainerLeftPos = stc.$movableContainer.offset().left - activeTabLeftPos;
                smv.slideMovableContainerToLeftPos();
            }
    ...
    var tabElements = (function () {
        ...
        function getNewElScrollerElementWrappingNavTabsInstance($navTabsInstance) {
            var $tabsContainer = $('<div class="scrtabs-tab-container"></div>'),
                $leftArrow = $('<div class="scrtabs-tab-scroll-arrow scrtabs-js-tab-scroll-arrow-left"><span class="glyphicon glyphicon-triangle-left"></span></div>'),
                $rightArrow = $('<div class="scrtabs-tab-scroll-arrow scrtabs-js-tab-scroll-arrow-right"><span class="glyphicon glyphicon-triangle-right"></span></div>'),
                $downArrow = $('<div class="scrtabs-tab-scroll-arrow scrtabs-js-tab-scroll-arrow-down"><span class="glyphicon glyphicon-triangle-bottom"></span></div>'),
