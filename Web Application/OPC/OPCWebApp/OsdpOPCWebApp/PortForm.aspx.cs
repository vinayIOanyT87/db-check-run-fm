		...
		protected void Page_Load(object sender, EventArgs e)
		{
			try
			{
				if (!this.Page.IsPostBack) 
				{
					...
					for(OSDP_BAUD baud=OSDP_BAUD.OSDP_BAUD_1200;baud < OSDP_BAUD.MAX_OSDP_BAUD;baud++)
					{
					...
					for(OSDP_DATA_BITS dataBits=OSDP_DATA_BITS.DATA_BITS_7;dataBits < OSDP_DATA_BITS.MAX_OSDP_DATA_BITS;dataBits++)
					{
					...
					for(OSDP_PARITY parity=OSDP_PARITY.OSDP_PARITY_NONE;parity < OSDP_PARITY.MAX_OSDP_PARITY;parity++)
					{
						ListItem newItem=new ListItem("Osdp|"+port.ParityID(parity),((int) parity).ToString());
					...
					for(OSDP_STOP_BITS stopBits=OSDP_STOP_BITS.STOP_BITS_1;stopBits < OSDP_STOP_BITS.MAX_OSDP_STOP_BITS;stopBits++)
					{
		...
		private void OK_Command(object sender, CommandEventArgs e)
		{
			try
			{
				...
				if(this.BaudDropDownList.SelectedIndex != -1)
					port.Baud=(OSDP_BAUD) Convert.ToInt32(this.BaudDropDownList.SelectedValue);
				if(this.DataBitsDropDownList.SelectedIndex != -1)
					port.DataBits=(OSDP_DATA_BITS) Convert.ToInt32(this.DataBitsDropDownList.SelectedValue);
				if(this.ParityDropDownList.SelectedIndex != -1)
					port.Parity=(OSDP_PARITY) Convert.ToInt32(this.ParityDropDownList.SelectedValue);
				if(this.StopBitsDropDownList.SelectedIndex != -1)
					port.StopBits=(OSDP_STOP_BITS) Convert.ToInt32(this.StopBitsDropDownList.SelectedValue);
