/******************************************************************************
	...
	COMMENTS:
		...
		This file shall not be copied or reproduced in any form without
				the expressed written consent of Leidos.
		...
		protected void Page_Load(object sender, EventArgs e)
		{
			try
			{
				if (!this.Page.IsPostBack) 
				{
					...
					for(CONTREC_BAUD baud=CONTREC_BAUD.CONTREC_BAUD_1200;baud < CONTREC_BAUD.MAX_CONTREC_BAUD;baud++)
					{
					...
					for(CONTREC_DATA_BITS dataBits=CONTREC_DATA_BITS.DATA_BITS_7;dataBits < CONTREC_DATA_BITS.MAX_CONTREC_DATA_BITS;dataBits++)
					{
					...
					for(CONTREC_PARITY parity=CONTREC_PARITY.CONTREC_PARITY_NONE;parity < CONTREC_PARITY.MAX_CONTREC_PARITY;parity++)
					{
						ListItem newItem=new ListItem( "Contrec|" + port.ParityID(parity),((int) parity).ToString());
					...
					for(CONTREC_STOP_BITS stopBits=CONTREC_STOP_BITS.STOP_BITS_1;stopBits < CONTREC_STOP_BITS.MAX_CONTREC_STOP_BITS;stopBits++)
					{
		...
		private void OK_Command(object sender, CommandEventArgs e)
		{
			try
			{
				...
				if(this.BaudDropDownList.SelectedIndex != -1)
					port.Baud=(CONTREC_BAUD) Convert.ToInt32(this.BaudDropDownList.SelectedValue);
				if(this.DataBitsDropDownList.SelectedIndex != -1)
					port.DataBits=(CONTREC_DATA_BITS) Convert.ToInt32(this.DataBitsDropDownList.SelectedValue);
				if(this.ParityDropDownList.SelectedIndex != -1)
					port.Parity=(CONTREC_PARITY) Convert.ToInt32(this.ParityDropDownList.SelectedValue);
				if(this.StopBitsDropDownList.SelectedIndex != -1)
					port.StopBits=(CONTREC_STOP_BITS) Convert.ToInt32(this.StopBitsDropDownList.SelectedValue);
