...
using System.Text;
	...
	class InventoryDailyAliasDO
	{
		...
		private enum IgnoreQuantityRounding { IGNORE_ROUNDING, USE_ROUNDING };
		#endregion
		...
		/// </summary>
		public InventoryDailyAliasDO(double volumeConversionFactor, double massConversionFactor, double currencyFactor,
									int volumeDecimalPlaces, int massDecimalPlaces, int currencyDecimalPlaces,
		...
		{
			this.grossQuantity += this.GetVolumeQuantity(gross, IgnoreQuantityRounding.USE_ROUNDING);
		}
		...
		{
			double convertedNet=this.GetVolumeQuantity(net,IgnoreQuantityRounding.USE_ROUNDING);
			this.netQuantity += convertedNet;
			if(!loadByWeight && volumePackageSize != 0)
				this.packageQuantity += convertedNet / volumePackageSize;
		}
		...
		{
			double convertedMass=this.GetMassQuantity(mass,IgnoreQuantityRounding.USE_ROUNDING);
			this.massQuantity += convertedMass;
			if(loadByWeight
			&& massPackageSize != 0)
				this.packageQuantity+=convertedMass/massPackageSize;
		}
		...
		public void SumGrossPrice(double price, double gross)
		{
			...
			this.grossPrice += (newPrice * this.GetVolumeQuantity(gross, IgnoreQuantityRounding.IGNORE_ROUNDING));
		}
		...
		public void SumNetPrice(double price, double net)
		{
			...
			// Sum the prices weighted by the quantity. Use the quantity that is not rounded.
			this.netPrice += (newPrice * this.GetVolumeQuantity(net, IgnoreQuantityRounding.IGNORE_ROUNDING));
		}
		...
		public void SumMassPrice(double price,double mass)
		{
			...
			// Sum the prices weighted by the quantity. Use the quantity that is not rounded.
			this.massPrice += (newPrice * this.GetMassQuantity(mass,IgnoreQuantityRounding.IGNORE_ROUNDING));
		}
		...
		private double GetVolumeQuantity(double quantity, IgnoreQuantityRounding quantityRounding)
		{
			...
			{
				if (reversalType == "R" || reversalType == "RU")
				{
					if (quantity > 0)
					{
						newQuantity = 0.0;
			...
			if (IgnoreQuantityRounding.USE_ROUNDING == quantityRounding)
			{
				 newQuantity = Math.Round(newQuantity, this.volumeDecimalPlaces, MidpointRounding.AwayFromZero);
			}
