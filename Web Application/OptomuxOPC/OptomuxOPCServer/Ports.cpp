...
STDMETHODIMP CPorts::raw_Modify(IDispatch* pPort)
{
	...
	try
	{
		...
		g_OptomuxControllerManager.ModifyPort(oPort);
...
STDMETHODIMP CPorts::raw_Purge(LONG lIndex)
{
	...
	try
	{
		...
		for(LONG lItem=0;lItem < oOptomuxControllerCollection->Count;lItem++)
		{
			...
			if(oOptomuxController->PortIndex == oPort->Index)
			{
				oOptomuxController->PortIndex=0;
				oOptomuxControllers->Modify(oOptomuxController);
			}
		}
