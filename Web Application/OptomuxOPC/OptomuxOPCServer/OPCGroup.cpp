    ...
    HRESULT				* pErrors)
{
   ...
   for( DWORD index=0; index < dwNumItems; index++ )
   {
      if( pErrors[index] == S_OK )
      {
         ...
         COptomuxControllerItem* pItem = (COptomuxControllerItem*) ppItems[index];
         ASSERT( pItem );
...
BOOL COPCGroup::DoUpdateGroup()
{
   ...
   COptomuxControllerItem* pItem = NULL;
   LPVOID key = 0;
   POSITION pos = m_itemMap.GetStartPosition();
   while( pos )
   {
    ...
    HRESULT        * pErrors)
{
   ...
   for(DWORD i=0; i < dwNumItems; i++ )
   {
      ...
      CString itemName( pItemArray[i].szItemID );
		CTag*	pTag=g_OptomuxControllerManager.FindTag( itemName.GetBuffer(0) );
      if( pTag == NULL
		|| !pTag->m_bLeaf )
      {
      ...
      {
         // Create the OPC item
         COptomuxControllerItem* pItem = new COptomuxControllerItem(pTag,itemName.GetBuffer(0));
         if( pItem )
         {
            pErrors[i] = S_OK;
