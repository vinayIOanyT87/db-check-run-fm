...
HRESULT COPCServer::FinalConstruct()
{
	...
	try
	{
		return g_OptomuxControllerManager.AddServer(this);
	}
	catch (_com_error& e)
	{
...
void COPCServer::FinalRelease() 
{
   ...
   m_groupMap.RemoveAll();
	...
	g_OptomuxControllerManager.RemoveServer(this);
}
...
HRESULT COPCServer::DoQueryNumProperties(
                              ...
                              LPVOID    * ppVoid)
{
   CTag*	pTag = g_OptomuxControllerManager.FindTag( szItemID );
   if( pTag 
	&& pTag->m_bLeaf)
   {
      *ppVoid = (LPVOID) pTag;
                              ...
                              VARTYPE   * pvtDataTypes)
{
   USES_CONVERSION;
	...
	CSLock	Lock(&g_OptomuxControllerManager.m_cs);
	CTag*	pTag=g_OptomuxControllerManager.FindTag(szItemID);
   if( pTag == NULL
	&& pTag->m_bLeaf )
      return OPC_E_UNKNOWNITEMID;
                              ...
                              LPCWSTR           szString)
{
   ...
   if( dwBrowseDirection == OPC_BROWSE_UP )
	...
	{
		m_pCurrentTag=g_OptomuxControllerManager.m_pRoot;
                              ...
                              LPWSTR *    szItemID)
{
   ...
   CString name( szItemDataID );
	...
	if(name.GetLength())
	{
		...
		CTag* pTag=g_OptomuxControllerManager.FindTag(path.GetBuffer(0));
...
//*******************************************************************
// Note: No protection agains an OptomuxController being deleted during
