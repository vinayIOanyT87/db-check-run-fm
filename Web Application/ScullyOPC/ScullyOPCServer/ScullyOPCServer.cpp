// ScullyOPCServer.cpp : Implementation of DLL Exports.
//
...
class CScullyOPCServerModule : public CAtlDllModuleT< CScullyOPCServerModule >
{
public :
	DECLARE_LIBID(LIBID_ScullyOPCServerLib)
	DECLARE_REGISTRY_APPID_RESOURCEID(IDR_SCULLYOPCSERVER, "{1E19AABF-8C67-4124-8705-3D280408EDD4}")
};

CScullyOPCServerModule _AtlModule;

#define SERVICE_NAME _T("ScullyOPC Server")
...
BEGIN_MESSAGE_MAP(CScullyOPCServerApp, CWinApp)
END_MESSAGE_MAP()

CScullyOPCServerApp theApp;

// Logging functions
void CScullyOPCServerApp::WriteToEventLog(	WORD wEventType,
											DWORD dwEventID,
											USHORT nNumStrings,
											LPCTSTR* pszStrings,
											ULONG nDataSize,
...
void CScullyOPCServerApp::WriteTextToEventLog(WORD wEventType, LPCTSTR pszMsg) const
{
...
void CScullyOPCServerApp::LogInfo(LPCTSTR pszFormat, ...) const
{
	TCHAR		chMsg[1024];
	va_list	pArg;
...
void CScullyOPCServerApp::LogError(LPCTSTR pszFormat, ...) const
{
	TCHAR		chMsg[1024];
	va_list	pArg;
...
void CScullyOPCServerApp::LogWarning(LPCTSTR pszFormat, ...) const
{
	TCHAR		chMsg[1024];
	va_list	pArg;
...
BOOL CScullyOPCServerApp::InitInstance()
{
...
int CScullyOPCServerApp::ExitInstance()
{
	 if(g_pDeviceManager)
...
void	CScullyOPCServerApp::GetProviderString()
{
...
void	CScullyOPCServerApp::GetConnectionString()
{
	// ODBC
	// TCHAR szString[512]={_T("Provider=MSDASQL;DSN=ScullyOPC;")};

	// Jet OLEDB - Note: does not support transactions
	// TCHAR szString[512]={_T("Provider=Microsoft.Jet.OLEDB.4.0; Data Source=c:\\Program Files\\ScullyOPC\\ScullyOPC\\ScullyOPC.mdb;")};

	// SQL Server OLEDB
	TCHAR szString[512]={_T("provider=SQLOLEDB;Data Source=127.0.0.1;Initial Catalog=ScullyOPC;Integrated Security=SSPI;")};

	CRegKey RegKey;

	if(ERROR_SUCCESS == RegKey.Create(HKEY_LOCAL_MACHINE,_T("SOFTWARE\\Varec\\ScullyOPC")))
	{
