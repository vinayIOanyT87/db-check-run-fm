CREATE PROCEDURE [dbo].[usp_GasboyFleetEnumerate]
...
BEGIN
	...
	BEGIN TRY
		...
		DECLARE @FleetNamePattern NVARCHAR(50);
		SET @FleetNamePattern = @FleetName

		IF (LEFT(@FleetNamePattern, 1) <> '%')
		BEGIN
			SET @FleetNamePattern = '%' + @FleetNamePattern;
		END

		IF (RIGHT(@FleetNamePattern, 1) <> '%')
		BEGIN
			SET @FleetNamePattern = @FleetNamePattern + '%';
		END

		-- Retrieve the Gasboy Fleet record(s) that contain the specified FleetName
		...
		WHERE [map].[tblEntityGasboyFleetToSite].[SiteGuid] = @SiteGuid 
				AND (@FleetNamePattern IS NULL OR (@FleetNamePattern IS NOT NULL AND [dbo].[tblGasboyFleet].[FleetName] LIKE @FleetNamePattern))
	...
	BEGIN CATCH
		...
		SET @_ErrMessage = 'Error: ' + @_ErrMessage + CHAR(13)+CHAR(10)                 
						...
						+ 'Procedure Name: usp_GasboyFleetEnumerate' + CHAR(13)+CHAR(10)                  
