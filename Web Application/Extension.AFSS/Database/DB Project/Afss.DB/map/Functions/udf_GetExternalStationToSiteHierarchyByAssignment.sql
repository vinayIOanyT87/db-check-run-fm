CREATE FUNCTION [map].[udf_GetExternalStationToSiteHierarchyByAssignment]
(
...
BEGIN
	/*
	------------------------------------------------------------------------------------------------------
	-- Function: [map].[udf_GetExternalStationToSiteHierarchyByAssignment] 
	-- Author: Ryan Hill
	-- Purpose: Retrieves the ExternalStationToSite assignment hierarchy below a given ExternalStation assignment. 	
	-- Notes:
	-- 1. @EntityRecordGuid: Guid of the ExternalStation record for which the mapping hierarchy is to be retrieved.
	...
	*/
	...
	BEGIN
		...
		WHERE ExternalStationGuid = @EntityRecordGuid AND SiteGuid = @AssignedToSiteGuid
	END;
	...
	(
		...
		SELECT ExternalStationToSiteGuid, 
		...
		-- Recursive Call
		UNION ALL
		SELECT a.ExternalStationToSiteGuid, 
			a.AssignedFromSiteGuid, 
			a.SiteGuid, 
			lvl + 1
		FROM map.tblEntityExternalStationToSite a 
		INNER JOIN AssignmentHierarchy b ON a.ExternalStationGuid = @EntityRecordGuid
