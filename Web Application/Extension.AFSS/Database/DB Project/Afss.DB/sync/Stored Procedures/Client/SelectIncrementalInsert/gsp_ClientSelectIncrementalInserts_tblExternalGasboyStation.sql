-- ********************** CREATE SYNCHRONIZATION METHODS FOR tblExternalGasboyStation **********************
-- 
-- 
-- ========================================================================================
...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ClientSelectIncrementalInserts_tblExternalGasboyStation]
...
@sync_batch_size_tblExternalGasboyStation int,
...
BEGIN
    ...
    IF (@sync_batch_size_tblExternalGasboyStation IS NULL OR 
        (@sync_batch_size_tblExternalGasboyStation IS NOT NULL AND @sync_batch_size_tblExternalGasboyStation = 0))
    BEGIN
        SET @sync_batch_size_tblExternalGasboyStation = 2147483647;
    END

        -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
        -- and/or any new entity site assignments (if assignable).
            SELECT [ExternalStationGuid],[SiteCode],[IPAddress],[UserName],[Password],[CreatedBy],[CreatedDate],[UpdatedBy],[UpdatedDate],_RowVersion
            FROM (
                SELECT TOP(@sync_batch_size_tblExternalGasboyStation) [dbo].[tblExternalGasboyStation].[ExternalStationGuid],[dbo].[tblExternalGasboyStation].[SiteCode],[dbo].[tblExternalGasboyStation].[IPAddress],[dbo].[tblExternalGasboyStation].[UserName],[dbo].[tblExternalGasboyStation].[Password],[dbo].[tblExternalGasboyStation].[CreatedBy],[dbo].[tblExternalGasboyStation].[CreatedDate],[dbo].[tblExternalGasboyStation].[UpdatedBy],[dbo].[tblExternalGasboyStation].[UpdatedDate],sync.udf_GetMaxRowVersion(CT.InsertedRowVersion,MAPCT.InsertedRowVersion,NULL) AS '_RowVersion'
                    FROM [dbo].[tblExternalGasboyStation]
                        INNER JOIN (SELECT [ExternalStationToSiteGuid],[ExternalStationGuid],[OwnerSiteGuid],[AssignedToSiteGuid] FROM [dbo].[udf_GetAssignedExternalStationListForSite](@sync_context_site_guid)) data
                            ON [dbo].[tblExternalGasboyStation].[ExternalStationGuid] = data.[ExternalStationGuid]
