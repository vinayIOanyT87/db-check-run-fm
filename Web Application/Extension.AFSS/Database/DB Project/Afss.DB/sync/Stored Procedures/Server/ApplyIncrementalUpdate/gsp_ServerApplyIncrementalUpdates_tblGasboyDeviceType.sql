...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerApplyIncrementalUpdates_tblGasboyDeviceType]
...
@sync_supported_columns_tblGasboyDeviceType varchar(8000)
AS
BEGIN
    DECLARE @wasDeleted int
    SET @wasDeleted = 0
    
    IF EXISTS (SELECT 1 FROM [lookup].[tblGasboyDeviceType]
    ...
    BEGIN
        ;   WITH existingData AS (
            ...
            USING (SELECT @GasboyDeviceTypeIndex,@GasboyDeviceTypeCode,@GasboyDeviceTypeName,@GasboyDeviceTypeGuid,@CreatedBy,@CreatedDate,@UpdatedBy,@UpdatedDate
            ...
            WHEN MATCHED AND (@sync_force_write = 1 
                ...
                THEN
                UPDATE SET [GasboyDeviceTypeCode] = CASE (SELECT PATINDEX([sync].[udf_GetColumnSearchPattern]('GasboyDeviceTypeCode'), @sync_supported_columns_tblGasboyDeviceType)) WHEN 0 THEN existingData.[GasboyDeviceTypeCode] ELSE remoteChanges.[GasboyDeviceTypeCode] END
                       ,[GasboyDeviceTypeName] = CASE (SELECT PATINDEX([sync].[udf_GetColumnSearchPattern]('GasboyDeviceTypeName'), @sync_supported_columns_tblGasboyDeviceType)) WHEN 0 THEN existingData.[GasboyDeviceTypeName] ELSE remoteChanges.[GasboyDeviceTypeName] END
                       ,[GasboyDeviceTypeGuid] = CASE (SELECT PATINDEX([sync].[udf_GetColumnSearchPattern]('GasboyDeviceTypeGuid'), @sync_supported_columns_tblGasboyDeviceType)) WHEN 0 THEN existingData.[GasboyDeviceTypeGuid] ELSE remoteChanges.[GasboyDeviceTypeGuid] END
                       ,[CreatedBy] = CASE (SELECT PATINDEX([sync].[udf_GetColumnSearchPattern]('CreatedBy'), @sync_supported_columns_tblGasboyDeviceType)) WHEN 0 THEN existingData.[CreatedBy] ELSE remoteChanges.[CreatedBy] END
                       ,[CreatedDate] = CASE (SELECT PATINDEX([sync].[udf_GetColumnSearchPattern]('CreatedDate'), @sync_supported_columns_tblGasboyDeviceType)) WHEN 0 THEN existingData.[CreatedDate] ELSE remoteChanges.[CreatedDate] END
                       ,[UpdatedBy] = CASE (SELECT PATINDEX([sync].[udf_GetColumnSearchPattern]('UpdatedBy'), @sync_supported_columns_tblGasboyDeviceType)) WHEN 0 THEN existingData.[UpdatedBy] ELSE remoteChanges.[UpdatedBy] END
                       ,[UpdatedDate] = CASE (SELECT PATINDEX([sync].[udf_GetColumnSearchPattern]('UpdatedDate'), @sync_supported_columns_tblGasboyDeviceType)) WHEN 0 THEN existingData.[UpdatedDate] ELSE remoteChanges.[UpdatedDate] END
