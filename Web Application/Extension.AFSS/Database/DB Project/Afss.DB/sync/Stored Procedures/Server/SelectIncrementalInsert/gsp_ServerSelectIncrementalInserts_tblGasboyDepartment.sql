-- ********************** CREATE SYNCHRONIZATION METHODS FOR tblGasboyDepartment **********************
-- 
-- 
-- ========================================================================================
...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblGasboyDepartment]
...
@sync_batch_size_tblGasboyDepartment int,
...
BEGIN
    ...
    IF (@sync_batch_size_tblGasboyDepartment IS NULL OR 
        (@sync_batch_size_tblGasboyDepartment IS NOT NULL AND @sync_batch_size_tblGasboyDepartment = 0))
    BEGIN
        SET @sync_batch_size_tblGasboyDepartment = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
        SELECT [GasboyDepartmentGuid],[SiteGuid],[DepartmentID],[DepartmentCode],[DepartmentName],[GroupRuleName],[PriceListName],[LookupGasboyRecordStatusIndex],[UsePINCodeFlag],[PINCode],[AuthPINFrom],[PromptForVehiclePlateFlag],[LookupGasboyVehiclePlateCheckTypeIndex],[AlwaysPromptForAdditionalValidationFlag],[CreatedBy],[CreatedDate],[UpdatedBy],[UpdatedDate],_RowVersion
        FROM (
            SELECT TOP(@sync_batch_size_tblGasboyDepartment) [dbo].[tblGasboyDepartment].[GasboyDepartmentGuid],[dbo].[tblGasboyDepartment].[SiteGuid],[dbo].[tblGasboyDepartment].[DepartmentID],[dbo].[tblGasboyDepartment].[DepartmentCode],[dbo].[tblGasboyDepartment].[DepartmentName],[dbo].[tblGasboyDepartment].[GroupRuleName],[dbo].[tblGasboyDepartment].[PriceListName],[dbo].[tblGasboyDepartment].[LookupGasboyRecordStatusIndex],[dbo].[tblGasboyDepartment].[UsePINCodeFlag],[dbo].[tblGasboyDepartment].[PINCode],[dbo].[tblGasboyDepartment].[AuthPINFrom],[dbo].[tblGasboyDepartment].[PromptForVehiclePlateFlag],[dbo].[tblGasboyDepartment].[LookupGasboyVehiclePlateCheckTypeIndex],[dbo].[tblGasboyDepartment].[AlwaysPromptForAdditionalValidationFlag],[dbo].[tblGasboyDepartment].[CreatedBy],[dbo].[tblGasboyDepartment].[CreatedDate],[dbo].[tblGasboyDepartment].[UpdatedBy],[dbo].[tblGasboyDepartment].[UpdatedDate],sync.udf_GetMaxRowVersion(CT.InsertedRowVersion,MAPCT.InsertedRowVersion,NULL) AS '_RowVersion'
                FROM [dbo].[tblGasboyDepartment]
                    INNER JOIN (SELECT [GasboyDepartmentToSiteGuid],[GasboyDepartmentGuid],[OwnerSiteGuid],[AssignedToSiteGuid] FROM [dbo].[udf_GetAssignedGasboyDepartmentListForSite](@sync_context_site_guid)) data
                        ON [dbo].[tblGasboyDepartment].[GasboyDepartmentGuid] = data.[GasboyDepartmentGuid]
                    ...
                    INNER JOIN [track].[tblEntityGasboyDepartmentToSite] MAPCT
                        ON MAPCT.PK_GasboyDepartmentToSiteGuid = data.[GasboyDepartmentGuid] 
