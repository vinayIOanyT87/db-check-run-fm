-- ********************** CREATE SYNCHRONIZATION METHODS FOR tblGasboyFleet **********************
-- 
-- 
-- ========================================================================================
...
-- ========================================================================================
CREATE PROCEDURE [sync].[gsp_ServerSelectIncrementalInserts_tblGasboyFleet]
...
@sync_batch_size_tblGasboyFleet int,
...
BEGIN
    ...
    IF (@sync_batch_size_tblGasboyFleet IS NULL OR 
        (@sync_batch_size_tblGasboyFleet IS NOT NULL AND @sync_batch_size_tblGasboyFleet = 0))
    BEGIN
        SET @sync_batch_size_tblGasboyFleet = 2147483647;
    END

    -- Get a list of the Owned/Assigned Entities and locate any newly inserted entities
    -- and/or any new entity site assignments (if assignable).
        SELECT [GasboyFleetGuid],[SiteGuid],[FleetID],[FleetCode],[FleetName],[GroupRuleName],[PriceListName],[LookupGasboyRecordStatusIndex],[UsePINCodeFlag],[PINCode],[AuthPINFrom],[PromptForVehiclePlateFlag],[LookupGasboyVehiclePlateCheckTypeIndex],[AlwaysPromptForAdditionalValidationFlag],[CreatedBy],[CreatedDate],[UpdatedBy],[UpdatedDate],_RowVersion
        FROM (
            SELECT TOP(@sync_batch_size_tblGasboyFleet) [dbo].[tblGasboyFleet].[GasboyFleetGuid],[dbo].[tblGasboyFleet].[SiteGuid],[dbo].[tblGasboyFleet].[FleetID],[dbo].[tblGasboyFleet].[FleetCode],[dbo].[tblGasboyFleet].[FleetName],[dbo].[tblGasboyFleet].[GroupRuleName],[dbo].[tblGasboyFleet].[PriceListName],[dbo].[tblGasboyFleet].[LookupGasboyRecordStatusIndex],[dbo].[tblGasboyFleet].[UsePINCodeFlag],[dbo].[tblGasboyFleet].[PINCode],[dbo].[tblGasboyFleet].[AuthPINFrom],[dbo].[tblGasboyFleet].[PromptForVehiclePlateFlag],[dbo].[tblGasboyFleet].[LookupGasboyVehiclePlateCheckTypeIndex],[dbo].[tblGasboyFleet].[AlwaysPromptForAdditionalValidationFlag],[dbo].[tblGasboyFleet].[CreatedBy],[dbo].[tblGasboyFleet].[CreatedDate],[dbo].[tblGasboyFleet].[UpdatedBy],[dbo].[tblGasboyFleet].[UpdatedDate],sync.udf_GetMaxRowVersion(CT.InsertedRowVersion,MAPCT.InsertedRowVersion,NULL) AS '_RowVersion'
                FROM [dbo].[tblGasboyFleet]
                    INNER JOIN (SELECT [GasboyFleetToSiteGuid],[GasboyFleetGuid],[OwnerSiteGuid],[AssignedToSiteGuid] FROM [dbo].[udf_GetAssignedGasboyFleetListForSite](@sync_context_site_guid)) data
                        ON [dbo].[tblGasboyFleet].[GasboyFleetGuid] = data.[GasboyFleetGuid]
                    INNER JOIN [track].[tblGasboyFleet] CT
                        ON CT.PK_GasboyFleetGuid = [dbo].[tblGasboyFleet].[GasboyFleetGuid] 
                    INNER JOIN [track].[tblEntityGasboyFleetToSite] MAPCT
                        ON MAPCT.PK_GasboyFleetToSiteGuid = data.[GasboyFleetGuid] 
