...
namespace FMSynchronizationService
{
	...
	public class SyncControllerFM : IDisposable
	{
		...
		/// The event log.
		/// </summary>
		private static EventLog _EventLog = new EventLog("Application", ".", "SynchControllerFM");
		...
		private SYNCSESSIONSTATUS Synchronize(string syncProfileId, SyncClientConfigurationDO clientSyncConfig)
		{
			...
			catch (Exception eX)
			{
				...
				UpdateSessionStatus(SYNCSESSIONSTATUS.FAILED);
			}
			finally
			{
						...
						case SYNCSCOPETYPE.GLOBAL:
							{
								...
								while (!this.SyncStopped && FMChannelHelper.MakeCall<ISyncControllerProcessor, bool>(
									x =>
										{
			...
			foreach (SiteClass sc in siteList)
			{
				...
				while (!this.SyncStopped && FMChannelHelper.MakeCall<ISyncControllerProcessor, bool>(
					x =>
						{
							((IClientChannel) x).OperationTimeout = new TimeSpan(0, 10, 0);
							return x.SynchronizeScope(this._ClientSyncConfig, syncContext, this._SyncSessionLog, syncScope);
						}));
		...
		{
			FMChannelHelper.MakeCall<ISyncSessionScopeLogs>(x => x.Modify(this._SyncContext.Security, syncSessionLogDetailDo));
		}
